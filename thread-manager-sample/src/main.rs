...
use anyhow::bail;
use clap::Parser;
use ody_core_api::AbsolutePathBuf;
use ody_core_api::AltScreenMode;
use ody_core_api::ApprovalsReviewer;
use ody_core_api::Arg0DispatchPaths;
use ody_core_api::AskForApproval;
use ody_core_api::AuthCredentialsStoreMode;
use ody_core_api::AuthManager;
use ody_core_api::AutoCompactTokenLimitScope;
use ody_core_api::OdyHomeUserInstructionsProvider;
use ody_core_api::OdyThread;
use ody_core_api::Config;
use ody_core_api::ConfigLayerStack;
use ody_core_api::Constrained;
use ody_core_api::EnvironmentManager;
use ody_core_api::EventMsg;
use ody_core_api::ExecServerRuntimePaths;
use ody_core_api::Features;
use ody_core_api::GhostSnapshotConfig;
use ody_core_api::History;
use ody_core_api::MemoriesConfig;
use ody_core_api::ModelAvailabilityNuxConfig;
use ody_core_api::MultiAgentV2Config;
use ody_core_api::NewThread;
use ody_core_api::Notice;
use ody_core_api::OAuthCredentialsStoreMode;
use ody_core_api::OPENAI_PROVIDER_ID;
use ody_core_api::Op;
use ody_core_api::OtelConfig;
use ody_core_api::PermissionProfile;
use ody_core_api::Permissions;
use ody_core_api::ProjectConfig;
use ody_core_api::RealtimeAudioConfig;
use ody_core_api::RealtimeConfig;
use ody_core_api::SessionPickerViewMode;
use ody_core_api::SessionSource;
use ody_core_api::TerminalResizeReflowConfig;
use ody_core_api::ThreadManager;
use ody_core_api::ThreadStoreConfig;
use ody_core_api::ToolSuggestConfig;
use ody_core_api::TuiKeymap;
use ody_core_api::TuiNotificationSettings;
use ody_core_api::TuiPetAnchor;
use ody_core_api::UriBasedFileOpener;
use ody_core_api::UserInput;
use ody_core_api::WebSearchMode;
use ody_core_api::arg0_dispatch_or_else;
use ody_core_api::built_in_model_providers;
use ody_core_api::empty_extension_registry;
use ody_core_api::find_ody_home;
use ody_core_api::init_state_db;
use ody_core_api::item_event_to_server_notification;
use ody_core_api::resolve_installation_id;
use ody_core_api::set_default_originator;
use ody_core_api::thread_store_from_config;

#[derive(Debug, Parser)]
#[command(
    name = "ody-thread-manager-sample",
    about = "Run one Ody turn through ThreadManager and print mapped notifications as newline-delimited JSON."
)]
struct Args {
...
async fn run_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("ody_thread_manager_sample".to_string()) {
...
fn new_config(model: Option<String>, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<Config> {
    ...
    let mut config = Config {
        ...
        sqlite_home: ody_home.to_path_buf(),
        log_dir: ody_home.join("log").to_path_buf(),
        config_lock_export_dir: None,
        config_lock_allow_ody_version_mismatch: false,
...
async fn run_turn(thread: &OdyThread, thread_id: &str, prompt: String) -> anyhow::Result<()> {
    thread
        .submit(Op::UserInput {
    ...
    loop {
        let event = thread.next_event().await.context("read Ody event")?;

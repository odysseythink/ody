...
use ody_rollout::RolloutConfig;
use ody_rollout::RolloutRecorder;
use ody_rollout::find_thread_names_by_ids;
use ody_rollout::parse_cursor;
...
pub(super) async fn list_threads(
    ...
    let sort_key = match params.sort_key {
        ThreadSortKey::CreatedAt => ody_rollout::ThreadSortKey::CreatedAt,
        ThreadSortKey::UpdatedAt => ody_rollout::ThreadSortKey::UpdatedAt,
        ThreadSortKey::RecencyAt => ody_rollout::ThreadSortKey::RecencyAt,
    };
    let sort_direction = match params.sort_direction {
        SortDirection::Asc => ody_rollout::SortDirection::Asc,
        SortDirection::Desc => ody_rollout::SortDirection::Desc,
    };
    let state_db = store.state_db().await;
    let rollout_config = RolloutConfig {
        ody_home: store.config.ody_home.clone(),
        sqlite_home: store.config.sqlite_home.clone(),
        cwd: store.config.ody_home.clone(),
    ...
    if names.len() < thread_ids.len()
        && let Ok(legacy_names) =
            find_thread_names_by_ids(store.config.ody_home.as_path(), &thread_ids).await
    {
...
pub(super) async fn list_rollout_threads(
    state_db: Option<ody_rollout::StateDbHandle>,
    config: &RolloutConfig,
    default_model_provider_id: &str,
    params: &ListThreadsParams,
    cursor: Option<&ody_rollout::Cursor>,
    sort_key: ody_rollout::ThreadSortKey,
    sort_direction: ody_rollout::SortDirection,
) -> ThreadStoreResult<ody_rollout::ThreadsPage> {
    if let Some(parent_thread_id) = params.parent_thread_id {
        let page = ody_rollout::state_db::list_threads_db(
            state_db.as_deref(),
        ...
        let mut page: ody_rollout::ThreadsPage = page.into();

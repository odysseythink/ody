  ...
  "plugins": [
    ...
    assert_eq!(
        ...
        ResolvedMarketplacePlugin {
            plugin_id: PluginId::new("local-plugin".to_string(), "ody-curated".to_string())
                .unwrap(),
  ...
  "plugins": [
    {
      ...
      "source": {
        "source": "git-subdir",
        "url": "odysseythink/joey_marketplace3",
    ...
    assert_eq!(
        resolved,
        ResolvedMarketplacePlugin {
            plugin_id: PluginId::new("remote-plugin".to_string(), "ody-curated".to_string())
                .unwrap(),
            source: MarketplacePluginSource::Git {
                url: "https://github.com/odysseythink/joey_marketplace3.git".to_string(),
  ...
  "plugins": [
    ...
    assert_eq!(
        ...
        MarketplacePluginSource::Git {
            url: "https://github.com/odysseythink/toolkit.git".to_string(),
            path: Some("plugins/toolkit".to_string()),
            ref_name: None,
            sha: None,
        }
  ...
  "plugins": [
    {{
      ...
      "source": {{
        ...
        assert_eq!(
            err.to_string(),
            "plugin `remote-plugin` was not found in marketplace `ody-curated`"
        );
    }
}
...
fn find_marketplace_plugin_reports_missing_plugin() {
    ...
    assert_eq!(
        err.to_string(),
        "plugin `missing` was not found in marketplace `ody-curated`"
    );
}
...
fn list_marketplaces_supports_repo_root_local_plugin_sources() {
    for path in [".", "./"] {
        ...
        fs::create_dir_all(repo_root.join(".git")).unwrap();
        fs::create_dir_all(repo_root.join(".agents/plugins")).unwrap();
        fs::create_dir_all(repo_root.join(".ody-plugin")).unwrap();
        fs::write(
            repo_root.join(".agents/plugins/marketplace.json"),
            format!(
                r#"{{
  ...
  "plugins": [
    {{
      ...
      "source": {{
        ...
        .unwrap();
        fs::write(
            repo_root.join(".ody-plugin/plugin.json"),
            r#"{
  "name":"repo-root-plugin",
  "interface": {
  ...
  "plugins": [
    {{
      ...
      "source": {{
        ...
        assert_eq!(
            err.to_string(),
            "plugin `local-plugin` was not found in marketplace `ody-curated`"
        );
    }
}
...
fn find_marketplace_plugin_allows_missing_products_field() {
  ...
  "plugins": [
    ...
    assert_eq!(resolved.plugin_id.as_key(), "default-plugin@ody-curated");
}

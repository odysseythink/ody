...
pub async fn fetch_remote_featured_plugin_ids(
    ...
    let mut request = client
        ...
        .query(&[(
            "platform",
            product.unwrap_or(Product::Ody).to_app_platform(),
        )])
        .timeout(REMOTE_FEATURED_PLUGIN_FETCH_TIMEOUT);

    if let Some(auth) = auth.filter(|auth| auth.uses_ody_backend()) {
        request =
            request.headers(ody_model_provider::auth_provider_from_auth(auth).to_auth_headers());
    }
...
fn ensure_ody_backend_auth(
    auth: Option<&OdyAuth>,
) -> Result<&OdyAuth, RemotePluginMutationError> {
...
async fn post_remote_plugin_mutation(
    ...
    let auth = ensure_ody_backend_auth(auth)?;

...
pub async fn add_marketplace(
    ...
    tokio::task::spawn_blocking(move || add_marketplace_sync(ody_home.as_path(), request))
        .await
...
fn add_marketplace_sync(
    ...
    add_marketplace_sync_with_cloner(ody_home, request, clone_git_source)
}
    ...
    F: Fn(&str, Option<&str>, &[String], &Path) -> Result<(), MarketplaceAddError>,
{
    ...
    let install_root = marketplace_install_root(ody_home);
    ...
    if let Some(existing_root) =
        installed_marketplace_root_for_source(ody_home, &install_root, &install_metadata)?
    {
    ...
    if let MarketplaceSource::Local { path } = &source {
        ...
        if is_odysseythink_curated_marketplace_name(&marketplace_name) {
        ...
        if find_marketplace_root_by_name(ody_home, &install_root, &marketplace_name)?.is_some() {
            return Err(MarketplaceAddError::InvalidRequest(format!(
...
mod tests {
    ...
    fn add_marketplace_sync_installs_marketplace_and_updates_config() -> Result<()> {
        ...
        let config = fs::read_to_string(ody_home.path().join(ody_config::CONFIG_TOML_FILE))?;
    ...
    fn add_marketplace_sync_treats_existing_local_directory_source_as_already_added() -> Result<()>
    {
        ...
        let first_result = add_marketplace_sync_with_cloner(ody_home.path(), request.clone(), {
            |_url, _ref_name, _sparse_paths, _destination| {
                panic!("git cloner should not be called for local marketplace sources")
            }
        })?;
        let second_result = add_marketplace_sync_with_cloner(ody_home.path(), request, {
            |_url, _ref_name, _sparse_paths, _destination| {
                panic!("git cloner should not be called for local marketplace sources")
            }
        })?;
    ...
    fn write_marketplace_source(source: &Path, marker: &str) -> std::io::Result<()> {
        ...
        fs::create_dir_all(source.join("plugins/sample/.ody-plugin"))?;
        fs::write(
  ...
  "plugins": [
    {
      ...
      "source": {
        ...
        fs::write(
            source.join("plugins/sample/.ody-plugin/plugin.json"),
            r#"{"name":"sample"}"#,
        )?;

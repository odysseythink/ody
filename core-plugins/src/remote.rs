...
use ody_app_server_protocol::SkillInterface;
...
pub const REMOTE_GLOBAL_MARKETPLACE_NAME: &str = "odysseythink-curated-remote";
    ...
    for source in sources {
        match source {
            RemoteMarketplaceSource::Global => {
                ...
                if let (Some(ody_home), Some(directory_plugins)) =
...
pub async fn fetch_and_cache_global_remote_plugin_catalog(
    ...
    catalog_cache::write_cached_global_directory_plugins(ody_home, config, auth, &plugins);
    Ok(())
}
...
pub fn has_cached_global_remote_plugin_catalog(
    ...
    catalog_cache::load_cached_global_directory_plugins(ody_home, config, auth).is_some()
}
...
pub fn cached_global_remote_discoverable_plugins(
    ...
    catalog_cache::load_cached_global_directory_plugins(ody_home, config, auth)
        .unwrap_or_default()
        .into_iter()
...
pub async fn fetch_odysseythink_curated_remote_collection_marketplace(
    config: &RemotePluginServiceConfig,
    auth: Option<&OdyAuth>,
...
pub async fn uninstall_remote_plugin(
    ...
    tokio::task::spawn_blocking(move || {
        remove_remote_plugin_cache(ody_home, marketplace_name, plugin_name, legacy_plugin_id)
    })
    .await
    .map_err(|err| {
...
fn remove_remote_plugin_cache(
    ...
    let store = PluginStore::try_new(ody_home.clone())
    ...
    let legacy_remote_plugin_cache_root = ody_home
...
fn ensure_chatgpt_auth(auth: Option<&OdyAuth>) -> Result<&OdyAuth, RemotePluginCatalogError> {

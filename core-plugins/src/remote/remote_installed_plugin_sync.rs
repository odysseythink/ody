...
pub(crate) fn maybe_start_remote_installed_plugin_bundle_sync(
    ...
    let key = RemoteInstalledPluginBundleSyncKey {
        plugin_cache_root: remote_plugin_cache_root(&ody_home),
    };
    ...
    tokio::spawn(async move {
        let result =
            sync_remote_installed_plugin_bundles_once(ody_home, &config, Some(&auth)).await;
        match result {
            Ok(outcome) => {
...
fn remove_stale_remote_plugin_caches(
    ...
    for marketplace_name in [
        ...
        let marketplace_root = ody_home.join(PLUGINS_CACHE_DIR).join(marketplace_name);
        ...
        for entry in fs::read_dir(&marketplace_root).map_err(|err| {
            ...
            if is_remote_plugin_cache_mutation_in_flight(ody_home, marketplace_name, &plugin_name)
            {
                continue;
            }
...
fn remote_plugin_cache_root(ody_home: &Path) -> PathBuf {
    ody_home.join(PLUGINS_CACHE_DIR)
}
...
mod tests {
    ...
    fn remote_installed_plugin_sync_in_flight_dedupes_by_cache_root() {
        ...
        let key = RemoteInstalledPluginBundleSyncKey {
            plugin_cache_root: remote_plugin_cache_root(ody_home.path()),
        };
    ...
    async fn sync_backfills_remote_plugin_install_metadata_for_current_bundle() {
        ...
        let metadata_path = PluginStore::new(ody_home.path().to_path_buf())
            .plugin_base_root(&plugin_id)
            .join(".ody-remote-plugin-install.json");
        assert_eq!(
    ...
    fn stale_remote_plugin_cleanup_skips_cache_mutations_in_progress() {
        ...
        assert_eq!(removed, vec!["linear@odysseythink-curated-remote".to_string()]);
    ...
    fn stale_remote_plugin_cleanup_removes_stale_marketplace_caches_and_keeps_canonical_cache() {
        let ody_home = tempfile::tempdir().expect("create ody home");
        let created_by_me_cached_manifest = ody_home
            .path()
            .join(PLUGINS_CACHE_DIR)
        ...
        std::fs::write(&cached_manifest, r#"{"name":"private-plugin"}"#)
            .expect("write cached plugin manifest");
        let canonical_cached_manifest = ody_home
            .path()
            .join(PLUGINS_CACHE_DIR)

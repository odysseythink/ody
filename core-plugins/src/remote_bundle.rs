...
pub async fn download_and_install_remote_plugin_bundle(
    ...
    tokio::task::spawn_blocking(move || {
        install_remote_plugin_bundle(ody_home, bundle, bundle_bytes)
    })
    .await
    .map_err(|err| {
...
fn install_remote_plugin_bundle(
    ...
    let staging_root = ody_home.join(REMOTE_PLUGIN_INSTALL_STAGING_DIR);
    ...
    let store = PluginStore::try_new(ody_home)?;
...
mod tests {
    ...
    fn validate_remote_plugin_bundle_uses_detail_name_for_local_plugin_id() {
        ...
        assert_eq!(bundle.plugin_id.marketplace_name, "odysseythink-curated-remote");
    ...
    fn install_persists_remote_plugin_install_metadata() {
        ...
        let store = PluginStore::new(ody_home.path().to_path_buf());
    ...
    fn find_extracted_plugin_root_uses_local_manifest_discovery() {
        ...
        std::fs::create_dir_all(extraction_root.path().join(".ody-plugin"))
            .expect("create manifest dir");
        std::fs::write(
            extraction_root.path().join(".ody-plugin/plugin.json"),
            r#"{"name":"linear"}"#,
        )
    ...
    fn find_extracted_plugin_root_rejects_nested_plugin_root() {
        ...
        std::fs::create_dir_all(plugin_root.join(".ody-plugin")).expect("create manifest dir");
        std::fs::write(

...
use ody_config::RemoveMarketplaceConfigOutcome;
use ody_config::remove_user_marketplace_config;
...
pub async fn remove_marketplace(
    ...
    tokio::task::spawn_blocking(move || remove_marketplace_sync(ody_home.as_path(), request))
        .await
        .map_err(|err| {
...
fn remove_marketplace_sync(
    ...
    let destination = marketplace_install_root(ody_home).join(&marketplace_name);
    let config_outcome =
        remove_user_marketplace_config(ody_home, &marketplace_name).map_err(|err| {
...
mod tests {
    ...
    fn remove_marketplace_sync_removes_config_and_installed_root() {
        ...
        let config =
            fs::read_to_string(ody_home.path().join(ody_config::CONFIG_TOML_FILE)).unwrap();

...
const REMOTE_PLUGIN_INSTALL_METADATA_FILE: &str = ".ody-remote-plugin-install.json";
...
impl PluginStore {
    pub fn new(ody_home: PathBuf) -> Self {
        Self::try_new(ody_home)
            .unwrap_or_else(|err| panic!("plugin cache root should be absolute: {err}"))
    }

    pub fn try_new(ody_home: PathBuf) -> Result<Self, PluginStoreError> {
        let root = AbsolutePathBuf::from_absolute_path_checked(ody_home.join(PLUGINS_CACHE_DIR))
            .map_err(|err| PluginStoreError::io("failed to resolve plugin cache root", err))?;
        let data_root =
            AbsolutePathBuf::from_absolute_path_checked(ody_home.join(PLUGINS_DATA_DIR))
...
fn plugin_manifest_for_source(
    ...
    match manifest {
        ...
        InstallManifest::Fallback(contents) => parse_plugin_manifest(
            source_path,
            &source_path.join(".ody-plugin/plugin.json"),
            contents,
        )
...
fn replace_plugin_root_atomically(
    ...
    if let InstallManifest::Fallback(contents) = manifest {
        ...
        let manifest_path = staged_version_root.join(".ody-plugin/plugin.json");

...
use ody_otel::CURATED_PLUGINS_STARTUP_SYNC_FINAL_METRIC;
use ody_otel::CURATED_PLUGINS_STARTUP_SYNC_METRIC;
use reqwest::Client;
...
const OPENAI_PLUGINS_OWNER: &str = "odysseythink";
const OPENAI_PLUGINS_REPO: &str = "plugins";
const OPENAI_PLUGINS_GIT_URL: &str = "https://github.com/odysseythink/plugins.git";
...
// Keep this comfortably above a normal sync attempt so we do not race another Ody process.
...
pub fn curated_plugins_repo_path(ody_home: &Path) -> PathBuf {
    ody_home.join(CURATED_PLUGINS_RELATIVE_DIR)
}
...
pub fn read_curated_plugins_sha(ody_home: &Path) -> Option<String> {
    read_sha_file(curated_plugins_sha_path(ody_home).as_path())
}

fn curated_plugins_sha_path(ody_home: &Path) -> PathBuf {
    ody_home.join(CURATED_PLUGINS_SHA_FILE)
}

pub fn sync_odysseythink_plugins_repo(ody_home: &Path) -> Result<String, String> {
...
fn sync_odysseythink_plugins_repo_with_transport_overrides(
    ody_home: &Path,
    git_binary: &str,
    api_base_url: &str,
    backup_archive_api_url: &str,
) -> Result<String, String> {
    let _file_guard = lock_curated_plugins_startup_sync(ody_home)?;

    match sync_odysseythink_plugins_repo_via_git(ody_home, git_binary) {
        Ok(remote_sha) => {
        ...
        Err(err) => {
            ...
            match sync_odysseythink_plugins_repo_via_http(ody_home, api_base_url) {
                Ok(remote_sha) => {
                ...
                Err(http_err) => {
                    ...
                    if has_local_curated_plugins_snapshot(ody_home) {
                        ...
                        let result = sync_odysseythink_plugins_repo_via_backup_archive(
                            ody_home,
                            backup_archive_api_url,
                        );
...
fn lock_curated_plugins_startup_sync(ody_home: &Path) -> Result<File, String> {
...
fn sync_odysseythink_plugins_repo_via_git(ody_home: &Path, git_binary: &str) -> Result<String, String> {
...
fn sync_odysseythink_plugins_repo_via_http(
    ody_home: &Path,
    api_base_url: &str,
) -> Result<String, String> {
    let repo_path = curated_plugins_repo_path(ody_home);
...
fn sync_odysseythink_plugins_repo_via_backup_archive(
    ody_home: &Path,
    backup_archive_api_url: &str,
) -> Result<String, String> {
    let repo_path = curated_plugins_repo_path(ody_home);
    let sha_path = curated_plugins_sha_path(ody_home);
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .map_err(|err| format!("failed to create curated plugins sync runtime: {err}"))?;
...
pub fn has_local_curated_plugins_snapshot(ody_home: &Path) -> bool {
    curated_plugins_repo_path(ody_home)
        .join(".agents/plugins/marketplace.json")
        .is_file()
        && ody_home.join(CURATED_PLUGINS_SHA_FILE).is_file()
}
...
fn git_ls_remote_head_sha(git_binary: &str) -> Result<String, String> {
    let output = run_git_command_with_timeout(
        Command::new(git_binary)
            .env("GIT_OPTIONAL_LOCKS", "0")
            .arg("ls-remote")
            .arg("https://github.com/odysseythink/plugins.git")
            .arg("HEAD"),

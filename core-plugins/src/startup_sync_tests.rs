...
fn write_odysseythink_curated_marketplace(root: &Path, plugin_names: &[&str]) {
...
fn write_curated_plugin_sha(ody_home: &Path) {
    write_file(
...
fn has_plugins_clone_dirs(ody_home: &Path) -> bool {
    let Ok(entries) = std::fs::read_dir(ody_home.join(".tmp")) else {
        return false;
    };
...
async fn mount_github_repo_and_ref(server: &MockServer, sha: &str) {
    Mock::given(method("GET"))
        .and(path("/repos/odysseythink/plugins"))
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"{"default_branch":"main"}"#))
        .mount(server)
        .await;
    Mock::given(method("GET"))
        .and(path("/repos/odysseythink/plugins/git/ref/heads/main"))
        .respond_with(
...
async fn mount_github_zipball(server: &MockServer, sha: &str, bytes: Vec<u8>) {
    Mock::given(method("GET"))
        .and(path(format!("/repos/odysseythink/plugins/zipball/{sha}")))
        .respond_with(
            ResponseTemplate::new(200)
...
async fn run_sync_with_transport_overrides(
    ...
    tokio::task::spawn_blocking(move || {
        sync_odysseythink_plugins_repo_with_transport_overrides(
            ody_home.as_path(),
            &git_binary,
            &api_base_url,
...
async fn run_http_sync(
    ...
    let api_base_url = api_base_url.into();
    tokio::task::spawn_blocking(move || {
        sync_odysseythink_plugins_repo_via_http(ody_home.as_path(), &api_base_url)
    })
    .await
    .expect("sync task should join")
}
...
fn assert_curated_gmail_repo(repo_path: &Path) {
    ...
    assert!(
        repo_path
            .join("plugins/gmail/.ody-plugin/plugin.json")
            .is_file()
    );
}

#[test]
fn curated_plugins_repo_path_uses_ody_home_tmp_dir() {
...
if [ "$1" = "-C" ] && [ "$3" = "reset" ]; then
  mkdir -p "$2/.agents/plugins" "$2/plugins/gmail/.ody-plugin"
  cat > "$2/.agents/plugins/marketplace.json" <<'EOF'
{{"name":"odysseythink-curated","plugins":[{{"name":"gmail","source":{{"source":"local","path":"./plugins/gmail"}}}}]}}
EOF
  printf '%s\n' '{{"name":"gmail"}}' > "$2/plugins/gmail/.ody-plugin/plugin.json"
  exit 0
fi
if [ "$1" = "-C" ] && [ "$3" = "clean" ]; then
  exit 0
...
#[cfg(unix)]
#[test]
fn sync_odysseythink_plugins_repo_via_git_succeeds_with_local_rewritten_remote() {
    let tmp = tempdir().expect("tempdir");
    ...
    let remote_repo = repo_root.path().join("remotes/odysseythink/plugins.git");
    std::fs::create_dir_all(work_repo.join(".agents/plugins")).expect("create marketplace dir");
    std::fs::create_dir_all(work_repo.join("plugins/gmail/.ody-plugin"))
        .expect("create plugin dir");
    std::fs::write(
        work_repo.join(".agents/plugins/marketplace.json"),
        r#"{"name":"odysseythink-curated","plugins":[{"name":"gmail","source":{"source":"local","path":"./plugins/gmail"}}]}"#,
    )
    .expect("write marketplace");
    std::fs::write(
        work_repo.join("plugins/gmail/.ody-plugin/plugin.json"),
        r#"{"name":"gmail"}"#,
    )
    ...
    let synced_sha =
        sync_odysseythink_plugins_repo_via_git(tmp.path(), git_wrapper.to_str().expect("utf8 path"))
    ...
    write_odysseythink_curated_marketplace(&work_repo, &["gmail", "linear"]);
    run_git(&work_repo, &["add", "."]);
    run_git(
        &work_repo,
        &[
    ...
    assert!(
        curated_plugins_repo_path(tmp.path())
            .join("plugins/linear/.ody-plugin/plugin.json")
            .is_file()
    );
    assert_eq!(
        read_curated_plugins_sha(tmp.path()).as_deref(),
    ...
    assert!(incremental_sync_invocations.iter().any(|invocation| {
        invocation.starts_with(&format!("-C {} fetch ", curated_repo_path.display()))
            && invocation.contains(" https://github.com/odysseythink/plugins.git ")
...
#[tokio::test]
async fn sync_odysseythink_plugins_repo_falls_back_to_http_when_git_is_unavailable() {
    let tmp = tempdir().expect("tempdir");
...
#[cfg(unix)]
#[tokio::test]
async fn sync_odysseythink_plugins_repo_falls_back_to_http_when_git_sync_fails() {
    let tmp = tempdir().expect("tempdir");
    let bin_dir = tempfile::Builder::new()
...
#[cfg(unix)]
#[test]
fn sync_odysseythink_plugins_repo_via_git_cleans_up_staged_dir_on_fetch_failure() {
    let tmp = tempdir().expect("tempdir");
    let bin_dir = tempfile::Builder::new()
...
"#
    ...
    let err = sync_odysseythink_plugins_repo_via_git(tmp.path(), git_path.to_str().expect("utf8 path"))
...
#[cfg(unix)]
#[test]
fn sync_odysseythink_plugins_repo_via_git_preserves_existing_snapshot_on_validation_failure() {
    let tmp = tempdir().expect("tempdir");
    let repo_path = curated_plugins_repo_path(tmp.path());
    write_odysseythink_curated_marketplace(&repo_path, &["gmail"]);
...
if [ "$1" = "-C" ] && [ "$3" = "reset" ]; then
  mkdir -p "$2/plugins/linear/.ody-plugin"
  printf '%s\n' '{{"name":"linear"}}' > "$2/plugins/linear/.ody-plugin/plugin.json"
  exit 0
fi
if [ "$1" = "-C" ] && [ "$3" = "clean" ]; then
  exit 0
...
#[tokio::test]
async fn sync_odysseythink_plugins_repo_via_http_cleans_up_staged_dir_on_extract_failure() {
    let tmp = tempdir().expect("tempdir");
    let server = MockServer::start().await;
    let sha = "0123456789abcdef0123456789abcdef01234567";
...
#[tokio::test]
async fn sync_odysseythink_plugins_repo_falls_back_to_export_archive_when_no_snapshot_exists() {
    let tmp = tempdir().expect("tempdir");
    let server = MockServer::start().await;
...
#[tokio::test]
async fn sync_odysseythink_plugins_repo_skips_export_archive_when_snapshot_exists() {
    let tmp = tempdir().expect("tempdir");
    let curated_root = curated_plugins_repo_path(tmp.path());
    write_odysseythink_curated_marketplace(&curated_root, &["linear"]);
    write_curated_plugin_sha(tmp.path());

    let plugin_manifest_path = curated_root.join("plugins/linear/.ody-plugin/plugin.json");
...
fn curated_repo_zipball_bytes(sha: &str) -> Vec<u8> {
    ...
    let root = format!("odysseythink-plugins-{sha}");
    writer
  ...
  "plugins": [
    ...
    writer
        .start_file(
            format!("{root}/plugins/gmail/.ody-plugin/plugin.json"),
            options,
        )
  ...
  "plugins": [
    {
      ...
      "source": {
        ...
        .expect("write marketplace");
    writer
        .start_file("plugins/plugins/gmail/.ody-plugin/plugin.json", options)
        .expect("start plugin manifest entry");
    writer
        .write_all(br#"{"name":"gmail"}"#)
        .expect("write plugin manifest");

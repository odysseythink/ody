...
pub(crate) fn write_odysseythink_curated_marketplace(root: &Path, plugin_names: &[&str]) {
...
pub(crate) fn write_odysseythink_api_curated_marketplace(root: &Path, plugin_names: &[&str]) {
    write_curated_marketplace(
        root,
...
pub(crate) fn write_curated_plugin_sha_with(ody_home: &Path, sha: &str) {
    write_file(&ody_home.join(".tmp/plugins.sha"), &format!("{sha}\n"));
}

pub(crate) async fn load_plugins_config(ody_home: &Path, cwd: &Path) -> PluginsConfigInput {
    let ody_home = AbsolutePathBuf::try_from(ody_home).expect("ody home should be absolute");

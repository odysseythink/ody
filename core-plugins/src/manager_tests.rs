...
use ody_config::McpServerOAuthConfig;
...
fn write_auth_projection_plugin(ody_home: &Path, name: &str, include_app: bool) {
  ...
  "mcpServers": {{
    ...
    if include_app {
        write_auth_projection_app(ody_home, name, name);
    }
}

fn write_auth_projection_app(ody_home: &Path, plugin_name: &str, app_name: &str) {
    let plugin_root = ody_home
        .join("plugins/cache")
        .join("test")
        .join(plugin_name)
...
async fn auth_projection_config(ody_home: &Path) -> PluginsConfigInput {
...
"#
    .to_string();
    write_file(&ody_home.join(CONFIG_TOML_FILE), &config_toml);
    load_config(ody_home, ody_home).await
}
...
fn write_plugin_with_version(
    ...
    fs::create_dir_all(plugin_root.join(".ody-plugin")).unwrap();
...
fn init_git_repo(repo: &Path) {
    run_git(repo, &["init"]);
    run_git(repo, &["config", "user.email", "ody-test@example.com"]);
    run_git(repo, &["config", "user.name", "Ody Test"]);
    run_git(repo, &["add", "."]);
...
async fn load_plugins_from_config(
    ...
    write_file(&ody_home.join(CONFIG_TOML_FILE), config_toml);
    let config = load_config(ody_home, ody_home).await;
    PluginsManager::new_with_options(ody_home.to_path_buf(), Some(Product::Ody), auth_mode)
        .plugins_for_config(&config)
        .await
}

async fn load_config(ody_home: &Path, cwd: &Path) -> PluginsConfigInput {
...
async fn installed_plugin_telemetry_metadata_accepts_authoritative_remote_identity() {
    ...
    let manager = PluginsManager::new(ody_home.path().to_path_buf());
    let plugin_id =
        PluginId::parse("linear@odysseythink-curated-remote").expect("plugin id should parse");
...
fn capability_summary_telemetry_metadata_uses_local_identity() {
    ...
    let summary = PluginCapabilitySummary {
        config_name: "linear@odysseythink-curated-remote".to_string(),
    ...
    assert_eq!(
        ...
        Some(PluginTelemetryMetadata {
            plugin_id: PluginId::parse("linear@odysseythink-curated-remote")
...
fn capability_index_filters_inactive_and_zero_capability_plugins() {
    ...
    let plugin = |config_name: &str, dir_name: &str, manifest_name: &str| LoadedPlugin {
        ...
        manifest_description: None,
        root: AbsolutePathBuf::try_from(ody_home.path().join(dir_name)).unwrap(),
        enabled: true,
        skill_roots: Vec::new(),
        disabled_skill_paths: HashSet::new(),
        has_enabled_skills: false,
    ...
    let outcome = PluginLoadOutcome::from_plugins(vec![
        LoadedPlugin {
            skill_roots: vec![ody_home.path().join("skills-plugin/skills").abs()],
            has_enabled_skills: true,
        ...
        LoadedPlugin {
            enabled: false,
            skill_roots: vec![ody_home.path().join("disabled-plugin/skills").abs()],
...
async fn load_plugins_returns_empty_when_feature_disabled() {
    ...
    let config = load_config(ody_home.path(), ody_home.path()).await;
    let outcome = PluginsManager::new(ody_home.path().to_path_buf())
        .plugins_for_config(&config)
        .await;
...
async fn plugin_cache_ignores_unrelated_session_overrides() {
    ...
    let plugin_root = ody_home
        ...
        .join("test/sample/local");
    write_plugin(
        ody_home.path().join("plugins/cache/test").as_path(),
        "sample/local",
        "sample",
    );
    write_file(
  ...
  "mcpServers": {
    ...
    let user_file = ody_home.path().join(CONFIG_TOML_FILE).abs();
...
async fn skills_service_reuses_skills_parsed_during_plugin_load() {
    let ody_home = TempDir::new().unwrap();
    let ody_home_abs = ody_home.path().to_path_buf().abs();
    let plugin_root = ody_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    ...
    let skills_service = SkillsService::new(ody_home_abs, /*bundled_skills_enabled*/ false);
...
async fn install_plugin_writes_marketplace_manifest_fallback_when_missing_plugin_json() {
  ...
  "plugins": [
    ...
    assert!(!plugin_root.join(".ody-plugin/plugin.json").exists());
    assert!(
        !tmp.path()
    ...
    let fallback_json: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(installed_path.join(".ody-plugin/plugin.json")).unwrap(),
    )
    .unwrap();
    assert_eq!(
...
async fn install_plugin_supports_git_subdir_marketplace_sources() {
  ...
  "plugins": [
    ...
    assert!(installed_path.join(".ody-plugin/plugin.json").is_file());
}
...
"#,
    ...
    let listed_plugin = manager
        .list_marketplaces_for_config(
            ...
            /*include_odysseythink_curated*/ false,
        )
        .unwrap()
        .marketplaces
        .into_iter()
...
async fn read_plugin_for_config_does_not_fallback_from_invalid_plugin_manifest() {
  ...
  "plugins": [
    ...
    write_file(&plugin_root.join(".ody-plugin/plugin.json"), "{");
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
plugins = true
...
"#,
    ...
    write_file(
        &repo_root.join(".ody/config.toml"),
        r#"[[skills.config]]
...
#[tokio::test]
async fn list_marketplaces_can_skip_odysseythink_curated_before_loading() {
    let tmp = tempfile::tempdir().unwrap();
    let curated_root = curated_plugins_repo_path(tmp.path());
...
"#,
    ...
    let config = load_config(tmp.path(), tmp.path()).await;
    let outcome = PluginsManager::new(tmp.path().to_path_buf())
        .list_marketplaces_for_config(&config, &[], /*include_odysseythink_curated*/ false)
        .unwrap();
...
"#,
  ...
  "plugins": [
    ...
    assert_eq!(
        curated_marketplace,
        ConfiguredMarketplace {
            name: "odysseythink-api-curated".to_string(),
            path: AbsolutePathBuf::try_from(
                curated_root.join(".agents/plugins/api_marketplace.json")
            )
            .unwrap(),
            ...
            plugins: vec![ConfiguredMarketplacePlugin {
                id: "api-plugin@odysseythink-api-curated".to_string(),
...
async fn configured_marketplace_upgrade_invalidates_cached_tool_suggest_metadata() {
  ...
  "plugins": [
    ...
    write_file(
        &remote_repo.join("plugins/sample/.ody-plugin/plugin.json"),
...
fn refresh_curated_plugin_cache_replaces_existing_local_version_with_short_sha_version() {
    ...
    .unwrap();
    write_plugin(
        &tmp.path().join("plugins/cache/odysseythink-curated"),
        "slack/local",
        "slack",
    );
    ...
    assert!(
        !tmp.path()
            .join("plugins/cache/odysseythink-curated/slack/local")
            .exists()
    );
    assert!(
        tmp.path()
...
fn refresh_curated_plugin_cache_reinstalls_missing_api_curated_plugin() {
    let tmp = tempfile::tempdir().unwrap();
    let curated_root = curated_plugins_repo_path(tmp.path());
    write_odysseythink_curated_marketplace(&curated_root, &[]);
    write_odysseythink_api_curated_marketplace(&curated_root, &["api-only"]);
    write_curated_plugin_sha(tmp.path(), TEST_CURATED_PLUGIN_SHA);
    let plugin_id = PluginId::new(
        "api-only".to_string(),
    ...
    assert!(
        tmp.path()
            .join(format!(
                "plugins/cache/odysseythink-api-curated/api-only/{TEST_CURATED_PLUGIN_CACHE_VERSION}"
            ))
            .is_dir()
    );
}
...
fn refresh_curated_plugin_cache_leaves_api_curated_plugin_when_api_manifest_missing() {
    ...
    assert!(
        tmp.path()
            .join("plugins/cache/odysseythink-api-curated/api-only/local")
            .is_dir()
    );
}
...
fn refresh_curated_plugin_cache_removes_cache_for_plugin_removed_from_marketplace() {
    ...
    let plugin_cache_root = tmp
        .path()
        .join("plugins/cache/odysseythink-curated/google-sheets");
    write_plugin(
        &tmp.path().join("plugins/cache/odysseythink-curated"),
...
#[test]
fn curated_plugin_ids_from_config_keys_reads_latest_ody_home_user_config() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
[plugins."slack@odysseythink-curated"]
enabled = true

[plugins."api-only@odysseythink-api-curated"]
enabled = true
...
"#,
    ...
    assert_eq!(
        configured_curated_plugin_ids_from_ody_home(tmp.path())
            .into_iter()
            .map(|plugin_id| plugin_id.as_key())
            .collect::<Vec<_>>(),
        vec![
            "api-only@odysseythink-api-curated".to_string(),
...
fn refresh_curated_plugin_cache_migrates_full_sha_cache_version_to_short_version() {
    ...
    assert!(
        !tmp.path()
            .join(format!(
                "plugins/cache/odysseythink-curated/slack/{TEST_CURATED_PLUGIN_SHA}"
            ))
            .exists()
    );
    assert!(
...
async fn load_plugins_ignores_project_config_files() {
    let ody_home = TempDir::new().unwrap();
    let project_root = ody_home.path().join("project");
    let plugin_root = ody_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    ...
    write_file(
        &project_root.join(".ody/config.toml"),
        &plugin_config_toml(/*enabled*/ true, /*plugins_feature_enabled*/ true),
    );
    ...
    let stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_ody_folder: AbsolutePathBuf::try_from(project_root.join(".ody")).unwrap(),
            },
    ...
    let plugins = load_plugins_from_layer_stack(
        ...
        &PluginStore::new(ody_home.path().to_path_buf()),
        /*plugin_skill_snapshots*/ None,
        Some(Product::Ody),

...
use crate::is_odysseythink_curated_marketplace_name;
...
use ody_core_skills::config_rules::resolve_disabled_skill_paths;
...
use ody_mcp::PluginMcpServerPlacement;
use ody_mcp::parse_plugin_mcp_config;
use ody_plugin::AppConnectorId;
use ody_plugin::AppDeclaration;
use ody_plugin::LoadedPlugin;
...
use ody_utils_plugins::find_plugin_manifest_path;
...
fn merge_configured_plugins_with_remote_installed(
    ...
    for plugin_key in configured_plugins.keys() {
        ...
        if !is_odysseythink_curated_marketplace_name(&plugin_id.marketplace_name)
...
pub fn refresh_curated_plugin_cache(
    ...
    let store = PluginStore::try_new(ody_home.to_path_buf()).map_err(|err| err.to_string())?;
    let curated_marketplace_paths = curated_marketplace_paths_for_cache_refresh(ody_home)?;
...
fn curated_marketplace_paths_for_cache_refresh(
    ...
    let api_marketplace_path = ody_home
        .join(".tmp/plugins")
...
fn refresh_non_curated_plugin_cache_with_mode(
    ...
    let configured_non_curated_plugin_ids =
        non_curated_plugin_ids_from_config_keys(configured_plugins_from_ody_home(
            ody_home,
    ...
    for plugin_id in configured_non_curated_plugin_ids {
        ...
        let materialized =
            materialize_marketplace_plugin_source(ody_home, &source).map_err(|err| {
...
fn configured_plugins_from_ody_home(
    ody_home: &Path,
...
fn curated_plugin_ids_from_config_keys(
    ...
    .into_iter()
    .filter(|plugin_id| is_odysseythink_curated_marketplace_name(&plugin_id.marketplace_name))
    .collect::<Vec<_>>();
...
pub fn configured_curated_plugin_ids_from_ody_home(ody_home: &Path) -> Vec<PluginId> {
    curated_plugin_ids_from_config_keys(configured_plugins_from_ody_home(
        ody_home,
    ...
    match &manifest_paths.hooks {
        ...
        Some(PluginManifestHooks::Inline(hooks_files)) => {
            let manifest_path = find_plugin_manifest_path(plugin_root.as_path())
                ...
                .unwrap_or_else(|| plugin_root.join(".ody-plugin/plugin.json"));
...
pub fn materialize_marketplace_plugin_source(
    ...
    match source {
        ...
        MarketplacePluginSource::Git {
            ...
            let staging_root = ody_home.join("plugins/.marketplace-plugin-source-staging");

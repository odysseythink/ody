...
#[test]
fn test_personal_access_token_uses_chatgpt_ody_base_url() {
    let api_provider = ModelProviderInfo::create_odysseythink_provider(/*base_url*/ None)
        .to_api_provider(Some(AuthMode::PersonalAccessToken))
        .expect("OpenAI provider should build API provider");

    assert_eq!(api_provider.base_url, CHATGPT_ODY_BASE_URL);
}
...
fn test_create_amazon_bedrock_provider() {
    assert_eq!(
        ...
        ModelProviderInfo {
            ...
            base_url: Some("https://bedrock-mantle.us-east-1.api.aws/odysseythink/v1".to_string()),
            env_key: None,
            env_key_instructions: None,
            experimental_bearer_token: None,
            auth: None,
...
fn test_merge_configured_model_providers_allows_amazon_bedrock_default_fields() {
    ...
    assert_eq!(
        ...
        Ok(built_in_model_providers(/*odysseythink_base_url*/ None))
    );
}
...
fn test_validate_provider_aws_rejects_conflicting_auth() {
    ...
    assert_eq!(
        provider.validate(),
        Err("provider aws cannot be combined with env_key, requires_odysseythink_auth".to_string())
    );
}

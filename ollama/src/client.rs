...
use crate::url::is_odysseythink_compatible_base_url;
...
impl OllamaClient {
    ...
    pub(crate) async fn try_from_provider(provider: &ModelProviderInfo) -> io::Result<Self> {
        ...
        let uses_odysseythink_compat = is_odysseythink_compatible_base_url(base_url);
    ...
    async fn probe_server(&self) -> io::Result<()> {
        let url = if self.uses_odysseythink_compat {
    ...
    fn from_host_root(host_root: impl Into<String>) -> Self {
        ...
        Self {
            ...
            uses_odysseythink_compat: false,
        }
    }
}
...
mod tests {
    ...
    async fn test_fetch_models_happy_path() {
        if std::env::var(ody_core::spawn::ODY_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
            tracing::info!(
    ...
    #[tokio::test]
    async fn test_probe_server_happy_path_odysseythink_compat_and_native() {
        if std::env::var(ody_core::spawn::ODY_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
            tracing::info!(
                "{} set; skipping test_probe_server_happy_path_odysseythink_compat_and_native",
                ody_core::spawn::ODY_SANDBOX_NETWORK_DISABLED_ENV_VAR
            );
            return;
        }

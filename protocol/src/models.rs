...
use ody_utils_image::ImageProcessingError;
...
pub enum SandboxEnforcement {
    /// Ody owns sandbox construction for this profile.
    #[default]
    Managed,
...
/// Filesystem permissions for profiles where Ody owns sandbox construction.
...
fn local_image_error_placeholder(
    ...
    ContentItem::InputText {
        text: format!(
            "Ody could not read the local image at `{}`: {}",
            path.display(),
            error
        ),
    }
...
fn unsupported_image_error_placeholder(path: &std::path::Path, mime: &str) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "Ody cannot attach image at `{}`: unsupported image `{}`.",
            path.display(),
            mime
        ),
    }
            ...
            Ok(McpContent::Image {
                ...
                FunctionCallOutputContentItem::InputImage {
                    ...
                    detail: meta
                        ...
                        .and_then(|meta| meta.get(ODY_IMAGE_DETAIL_META_KEY))
...
// (Moved event mapping logic into ody-core to avoid coupling protocol to UI-facing events.)
...
mod tests {
    ...
    fn function_call_deserializes_optional_namespace() {
        let item: ResponseItem = serde_json::from_value(serde_json::json!({
            "type": "function_call",
            "name": "mcp__ody_apps__gmail_get_recent_emails",
            "namespace": "mcp__ody_apps__gmail",
            "arguments": "{\"top_k\":5}",
            "call_id": "call-1",
        }))
        ...
        assert_eq!(
            ...
            ResponseItem::FunctionCall {
                id: None,
                name: "mcp__ody_apps__gmail_get_recent_emails".to_string(),
                namespace: Some("mcp__ody_apps__gmail".to_string()),
    ...
    fn tool_search_output_roundtrips() -> Result<()> {
        let input = ResponseInputItem::ToolSearchOutput {
            ...
            tools: vec![serde_json::json!({
                "type": "function",
                "name": "mcp__ody_apps__calendar_create_event",
...
<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"></svg>"#,
        ...
        match item {
            ResponseInputItem::Message { content, .. } => {
                assert_eq!(content.len(), 1);
                let expected = format!(
                    "Ody cannot attach image at `{}`: unsupported image `image/svg+xml`.",
                    svg_path.display()
                );
                match &content[0] {

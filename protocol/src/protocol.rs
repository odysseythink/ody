//! Defines the protocol for a Ody session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Ody:";
...
pub struct ConversationStartParams {
    /// Whether Ody response handoffs are managed through explicit client append calls.
    pub client_managed_handoffs: bool,
    /// Sends automatic Ody responses as realtime conversation items instead of handoff appends.
    pub ody_responses_as_items: bool,
    /// Optional prefix added to automatic Ody response items when `ody_responses_as_items` is set.
    pub ody_response_item_prefix: Option<String>,
    /// Optional prefix added to automatic V1 Ody commentary sent with
    /// `conversation.handoff.append` when `ody_responses_as_items` is not set. Final answers are
    /// sent without the prefix.
    pub ody_response_handoff_prefix: Option<String>,
    /// Overrides the configured realtime model for this session only.
    pub model: Option<String>,
    /// Selects whether the realtime session should produce text or audio output.
    pub output_modality: RealtimeOutputModality,
    /// Whether to append Ody's startup context to the realtime backend prompt.
...
pub enum Op {
    ...
    /// Request Ody to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down ody instance.
    Shutdown,
...
/// running the command proposed by Ody.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.ody`, `.git`, notably `.git/hooks`) under
...
/// Ody errors that we expose to clients.
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "snake_case")]
#[ts(rename_all = "snake_case")]
pub enum OdyErrorInfo {
...
pub struct ErrorEvent {
    pub message: String,
    #[serde(default)]
    pub ody_error_info: Option<OdyErrorInfo>,
}
...
impl ErrorEvent {
    ...
    pub fn affects_turn_status(&self) -> bool {
        self.ody_error_info
            .as_ref()
            .is_none_or(OdyErrorInfo::affects_turn_status)
    }
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::Ody),
...
pub struct TurnContextItem {
    ...
    // Compatibility-only field written with a default value so older Ody
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(ody_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
...
mod tests {
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("ody".to_string()).restriction_product(),
            Some(Product::Ody)
        );
        assert_eq!(
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd = ody_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
        ...
        let expected_blocked = AbsolutePathBuf::from_absolute_path(
            ody_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
                .expect("canonicalize cwd")
                .join("blocked"),
        )
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".ody")).expect("create .ody");
        let canonical_cwd = ody_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
            .expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_ody = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".ody"))
            .expect("canonical .ody");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_ody.as_path())
        );
    }
    ...
    fn restricted_file_system_policy_treats_read_entries_as_read_only_subpaths() {
        ...
        let expected_dot_ody = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".ody"))
            .expect("canonical .ody");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
    ...
    fn generic_error_affects_turn_status() {
        let event = ErrorEvent {
            ...
            ody_error_info: Some(OdyErrorInfo::Other),
        };
        assert!(event.affects_turn_status());
    }

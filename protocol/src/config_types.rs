...
/// See https://platform.odysseythink.com/docs/guides/reasoning?api-mode=responses#reasoning-summaries
#[derive(
    Debug, Serialize, Deserialize, Default, Clone, Copy, PartialEq, Eq, Display, JsonSchema, TS,
)]
...
/// Validated plain profile-v2 name used to select `$ODY_HOME/<name>.config.toml`.
...
/// Collaboration mode for a Ody session.
...
mod tests {
    ...
    fn web_search_tool_config_merge_prefers_overlay_values() {
        let base = WebSearchToolConfig {
            ...
            allowed_domains: Some(vec!["odysseythink.com".to_string()]),

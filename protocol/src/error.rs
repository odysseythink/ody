...
use ody_utils_string::truncate_middle_chars;
use ody_utils_string::truncate_middle_with_token_budget;
...
pub type Result<T> = std::result::Result<T, OdyErr>;
...
pub enum OdyErr {
    ...
    #[error(
        "Ody ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `OdyError::Spawn` variant.
    ...
    QuotaExceeded,
    #[error(
        "To use Ody with your ChatGPT plan, upgrade to Plus: https://chatgpt.com/explore/plus."
    )]
    UsageNotIncluded,
...
impl From<CancelErr> for OdyErr {
...
impl OdyErr {
    pub fn is_retryable(&self) -> bool {
        match self {
            ...
            | OdyErr::EnvVar(_)
            | OdyErr::Fatal(_)
            | OdyErr::UsageNotIncluded
            | OdyErr::QuotaExceeded
            | OdyErr::InvalidImageRequest()
            | OdyErr::InvalidRequest(_)
            | OdyErr::RefreshTokenFailed(_)
            | OdyErr::UnsupportedOperation(_)
            | OdyErr::Sandbox(_)
            | OdyErr::LandlockSandboxExecutableNotProvided
            | OdyErr::RetryLimit(_)
            | OdyErr::ContextWindowExceeded
            | OdyErr::ThreadNotFound(_)
            | OdyErr::AgentLimitReached { .. }
            | OdyErr::Spawn
            | OdyErr::SessionConfiguredNotFirstEvent
            | OdyErr::UsageLimitReached(_)
            | OdyErr::ServerOverloaded
            | OdyErr::CyberPolicy { .. } => false,
            OdyErr::Stream(..)
            | OdyErr::Timeout
            | OdyErr::RequestTimeout
            | OdyErr::UnexpectedStatus(_)
            | OdyErr::ResponseStreamFailed(_)
            | OdyErr::ConnectionFailed(_)
            | OdyErr::InternalServerError
            | OdyErr::InternalAgentDied
            | OdyErr::Io(_)
            | OdyErr::Json(_)
            | OdyErr::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            OdyErr::LandlockRuleset(_) | OdyErr::LandlockPathFd(_) => false,
        }
    }

    /// Minimal shim so that existing `e.downcast_ref::<OdyErr>()` checks continue to compile
    ...
    pub fn to_ody_protocol_error(&self) -> OdyErrorInfo {
        match self {
            ...
            OdyErr::ConnectionFailed(_) => OdyErrorInfo::HttpConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            OdyErr::ResponseStreamFailed(_) => OdyErrorInfo::ResponseStreamConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            OdyErr::RefreshTokenFailed(_) => OdyErrorInfo::Unauthorized,
            OdyErr::SessionConfiguredNotFirstEvent
            | OdyErr::InternalServerError
            | OdyErr::InternalAgentDied => OdyErrorInfo::InternalServerError,
            OdyErr::UnsupportedOperation(_)
            | OdyErr::ThreadNotFound(_)
            | OdyErr::AgentLimitReached { .. } => OdyErrorInfo::BadRequest,
            OdyErr::Sandbox(_) => OdyErrorInfo::SandboxError,
            _ => OdyErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            ody_error_info: Some(self.to_ody_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match self {
            OdyErr::RetryLimit(err) => Some(err.status),
            OdyErr::UnexpectedStatus(err) => Some(err.status),
            OdyErr::ConnectionFailed(err) => err.source.status(),
            OdyErr::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let message = match self.plan_type.as_ref() {
            Some(PlanType::Known(KnownPlan::Plus)) => format!(
                "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/ody/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
            Some(PlanType::Known(
                KnownPlan::Team
            ...
            Some(PlanType::Known(KnownPlan::Free)) | Some(PlanType::Known(KnownPlan::Go)) => {
                format!(
                    "You've hit your usage limit. Upgrade to Plus to continue using Ody (https://chatgpt.com/explore/plus),{}",
                    retry_suffix_after_or(self.resets_at.as_ref())
                )
            }
            Some(PlanType::Known(KnownPlan::Pro | KnownPlan::ProLite)) => format!(
                "You've hit your usage limit. Visit https://chatgpt.com/ody/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
...
pub fn get_error_message_ui(e: &OdyErr) -> String {
    let message = match e {
        OdyErr::Sandbox(SandboxErr::Denied { output, .. }) => {
        ...
        OdyErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(

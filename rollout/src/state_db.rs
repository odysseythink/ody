...
pub type StateDbHandle = Arc<ody_state::StateRuntime>;
...
async fn try_init_with_roots_inner(
    ...
    let runtime =
        ody_state::StateRuntime::init(sqlite_home.clone(), default_model_provider_id.clone())
            .await
            .with_context(|| {
                format!(
    ...
    .await;
    ody_state::record_backfill_gate(
        /*telemetry*/ None,
...
async fn wait_for_backfill_gate(
    ...
    loop {
        ...
        if backfill_state.status == ody_state::BackfillStatus::Complete {
            return Ok(());
        }
        ...
        if let Some(backfill_lease_seconds) = backfill_lease_seconds {
            ...
            metadata::backfill_sessions(runtime, ody_home, default_model_provider_id).await;
        }
        let backfill_state = runtime.get_backfill_state().await.map_err(|err| {
            anyhow::anyhow!(
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    let state_path = ody_state::state_db_path(config.sqlite_home());
    if !tokio::fs::try_exists(&state_path).await.unwrap_or(false) {
        ody_state::record_fallback(
            "get_state_db",
            "db_unavailable",
    ...
    let runtime = match ody_state::StateRuntime::init(
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == ody_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<ody_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&ody_state::StateRuntime>,
    ody_home: &Path,
    page_size: usize,
    ...
    let ctx = context?;
    if ctx.ody_home() != ody_home {
        warn!(
            "state db ody_home mismatch: expected {}, got {}",
            ctx.ody_home().display(),
            ody_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => ody_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => ody_state::SortKey::UpdatedAt,
                ThreadSortKey::RecencyAt => ody_state::SortKey::RecencyAt,
            },
...
pub async fn list_threads_db(
    ...
    let filters = ody_state::ThreadFilterOptions {
        ...
        sort_direction: match sort_direction {
            SortDirection::Asc => ody_state::SortDirection::Asc,
            SortDirection::Desc => ody_state::SortDirection::Desc,
        },
        search_term,
    };

...
async fn backfill_sessions_resumes_from_watermark_and_marks_complete() {
    let dir = tempdir().expect("tempdir");
    let ody_home = dir.path().to_path_buf();
    ...
    let runtime = ody_state::StateRuntime::init(ody_home.clone(), "test-provider".to_string())
        .await
        .expect("initialize runtime");
    let first_watermark = backfill_watermark_for_path(ody_home.as_path(), first_path.as_path());
    ...
    backfill_sessions(runtime.as_ref(), ody_home.as_path(), "test-provider").await;
...
fn write_rollout_in_sessions_with_cwd(
    ...
    let sessions_dir = ody_home.join("sessions");

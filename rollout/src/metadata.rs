...
use ody_state::BackfillState;
use ody_state::BackfillStats;
use ody_state::BackfillStatus;
use ody_state::DB_ERROR_METRIC;
use ody_state::DB_METRIC_BACKFILL;
use ody_state::DB_METRIC_BACKFILL_DURATION_MS;
use ody_state::ExtractionOutcome;
use ody_state::ThreadMetadataBuilder;
use ody_state::apply_rollout_item;
use std::path::Path;
...
pub(crate) async fn backfill_sessions_with_lease(
    ...
    let metric_client = ody_otel::global();
    ...
    let sessions_root = ody_home.join(SESSIONS_SUBDIR);
    let archived_root = ody_home.join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(ody_home, &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(ody_home: &Path, path: &Path) -> String {
    path.strip_prefix(ody_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}

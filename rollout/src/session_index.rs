...
pub async fn append_thread_name(
    ...
    append_session_index_entry(ody_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(ody_home);
...
fn session_index_path(ody_home: &Path) -> PathBuf {
    ody_home.join(SESSION_INDEX_FILE)
}

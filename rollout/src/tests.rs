...
async fn insert_state_db_thread(
    ...
    let runtime = ody_state::StateRuntime::init(home.to_path_buf(), TEST_PROVIDER.to_string())
        .await
...
async fn test_model_provider_filter_selects_only_matching_sessions() -> Result<()> {
    ...
    let odysseythink_id = Uuid::from_u128(1);
    ...
    let odysseythink_ids: Vec<_> = odysseythink_sessions
        .items
        .iter()
        .filter_map(|item| item.thread_id.as_ref().map(ToString::to_string))
        .collect();
    assert!(odysseythink_ids.contains(&odysseythink_id_str));
    assert!(odysseythink_ids.contains(&none_id_str));

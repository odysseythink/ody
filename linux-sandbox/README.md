...
- a `ody-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Ody CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `ody-exec` CLI can check if its arg0 is `ody-linux-sandbox` and, if so, execute as if it were `ody-linux-sandbox`
  - this should also be true of the `ody` multitool CLI

On Linux, Ody prefers the first `bwrap` found on `PATH`
...
the helper falls back to the bundled `ody-resources/bwrap` binary shipped
with Ody.
Ody also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. Ody surfaces the same startup warning
...
it cannot create the required user namespaces, so Ody rejects sandboxed shell
...
- If `bwrap` is missing, the helper falls back to the bundled
  `ody-resources/bwrap` path.
- If `bwrap` is missing, Ody also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, Ody surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; Ody rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.ody`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface is `ody sandbox`; the host OS selects the sandbox backend.

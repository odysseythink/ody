...
fn ody_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_ody-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> ody_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<ody_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let ody_linux_sandbox_exe = Some(ody_linux_sandbox_exe());
...
fn is_bwrap_unavailable_output(output: &ody_protocol::exec_output::ExecToolCallOutput) -> bool {
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(OdyErr::Sandbox(SandboxErr::Denied { output, .. })) => {
        ...
        Err(OdyErr::Sandbox(SandboxErr::Timeout { .. })) => true,
        Err(err) => panic!("bwrap availability probe failed unexpectedly: {err:?}"),
    }
}

fn expect_denied(
    result: Result<ody_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> ody_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
        ...
        Err(OdyErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let ody_linux_sandbox_exe: Option<PathBuf> = Some(ody_linux_sandbox_exe());
...
async fn sandbox_blocks_curl() {
    assert_network_blocked(&["curl", "-I", "http://odysseythink.com"]).await;
}

#[tokio::test]
async fn sandbox_blocks_wget() {
    assert_network_blocked(&["wget", "-qO-", "http://odysseythink.com"]).await;
}
...
#[tokio::test]
async fn sandbox_blocks_git_and_ody_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_ody = tmpdir.path().join(".ody");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_ody).expect("create .ody");

    let git_target = dot_git.join("config");
    let ody_target = dot_ody.join("config.toml");
    ...
    let ody_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", ody_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
        ...
        .await,
        ".ody write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(ody_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_ody_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-ody");
    std::fs::create_dir_all(&decoy).expect("create decoy dir");

    let dot_ody = tmpdir.path().join(".ody");
    symlink(&decoy, &dot_ody).expect("create .ody symlink");
    ...
    let ody_output = expect_denied(
        ...
        .await,
        ".ody symlink replacement should be denied",
    );
    assert_ne!(ody_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_reports_ody_symlink_build_failure_without_panicking() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    {
        Err(OdyErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
        result => panic!(".ody symlink build failure should deny: {result:?}"),
    };
...
if grep -E '(^|[[:space:]])\.(git|ody|agents)(/|$)' status.before; then
  cat status.before
  exit 21
fi
"#,
    ...
    let mkdir_ody_output = expect_denied(
        run_cmd_result_with_cwd_and_writable_roots(
            &["mkdir", ".ody"],
            &subdir,
            std::slice::from_ref(&subdir),
        ...
        .await,
        "child .ody directory creation should be denied",
    );
    assert_ne!(mkdir_ody_output.exit_code, 0);
    assert!(!subdir.join(".ody").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = ody_linux_sandbox_exe()
        .parent()
...
async fn sandbox_blocks_getent() {
    assert_network_blocked(&["getent", "ahosts", "odysseythink.com"]).await;
}

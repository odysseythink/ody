...
//! - sensitive subpaths such as `.git`, `.agents`, and `.ody` remain
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, and `.ody`.
...
mod tests {
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let real_ody = temp_dir.path().join("real-ody");
        let logical_ody = logical_home.join(".ody");
        let real_memories = real_ody.join("memories");
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".ody"));
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let dot_ody = path_to_string(&temp_dir.path().join(".ody"));
        ...
        assert_empty_directory_mounted_read_only(&args.args, Path::new(&dot_ody));
        ...
        assert!(synthetic_targets.contains(&PathBuf::from(&dot_ody)));
        assert_eq!(
            protected_create_target_paths(&args),
            Vec::<PathBuf>::new(),

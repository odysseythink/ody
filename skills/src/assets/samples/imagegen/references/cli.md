...
Set a stable path to the skill CLI (default `ODY_HOME` is `~/.ody-code`):

```
export ODY_HOME="${ODY_HOME:-$HOME/.ody}"
export IMAGE_GEN="$ODY_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
Dry-run (no API call; no network required; does not require the `odysseythink` package):
...
- Network/sandbox notes for fallback CLI mode: `references/ody-network.md`
- Built-in-first transparent image workflow: `SKILL.md` and `$ODY_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`

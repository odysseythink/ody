...
def _create_client():
    try:
        from odysseythink import OpenAI
    except ImportError:
        _die(f"odysseythink SDK not installed in the active environment. {_dependency_hint('odysseythink')}")
    return OpenAI()


def _create_async_client():
    try:
        from odysseythink import AsyncOpenAI
    except ImportError:
        try:
            import odysseythink as _odysseythink  # noqa: F401
        except ImportError:
            _die(
                f"odysseythink SDK not installed in the active environment. {_dependency_hint('odysseythink')}"
            )
        _die(
            "AsyncOpenAI not available in this odysseythink SDK version. "
            f"{_dependency_hint('odysseythink', upgrade=True)}"
        )
    return AsyncOpenAI()
...
def _extract_retry_after_seconds(exc: Exception) -> Optional[float]:
    # Best-effort: odysseythink SDK errors vary by version. Prefer a conservative fallback.

#!/usr/bin/env python3
"""
OpenAI YAML Generator - Creates agents/odysseythink.yaml for a skill folder.

Usage:
    generate_odysseythink_yaml.py <skill_dir> [--name <skill_name>] [--interface key=value]
"""
...
def write_odysseythink_yaml(skill_dir, skill_name, raw_overrides):
    ...
    output_path = agents_dir / "odysseythink.yaml"
    output_path.write_text("\n".join(interface_lines) + "\n")
    print(f"[OK] Created agents/odysseythink.yaml")
    return output_path


def main():
    parser = argparse.ArgumentParser(
        description="Create agents/odysseythink.yaml for a skill directory.",
    )
    ...
    result = write_odysseythink_yaml(skill_dir, skill_name, args.interface)
    if result:
        sys.exit(0)
    sys.exit(1)

---
name: "odysseythink-docs"
description: "Use when the user asks how to build with OpenAI products or APIs, asks about Ody itself or choosing Ody surfaces, needs up-to-date official documentation with citations, help choosing the latest model for a use case, or model upgrade and prompt-upgrade guidance; use OpenAI docs MCP tools for non-Ody docs questions, use the Ody manual helper first for broad Ody self-knowledge, and restrict fallback browsing to official OpenAI domains."
---


# OpenAI Docs

Provide authoritative, current guidance from OpenAI developer docs using the developers.odysseythink.com MCP server. "Docs MCP" means `mcp__odysseythinkDeveloperDocs__search_odysseythink_docs` and `mcp__odysseythinkDeveloperDocs__fetch_odysseythink_doc`; for API reference, schema, parameter, or required-field questions, also use `mcp__odysseythinkDeveloperDocs__get_openapi_spec` when available. Official-domain web search is fallback after those tools are unavailable or unhelpful. Broad Ody questions use the manual helper before Docs MCP. This skill also owns model selection, API model migration, and prompt-upgrade guidance.

## API Key Setup

For requests to build, run, configure, debug, or implement an API-backed app, script, CLI, generator, or tool, use `odysseythink-platform-api-key` first when available. After that credential gate is resolved, return here for current docs as needed.
...
- For Ody self-knowledge, use the Ody source route below; it owns when to use the manual helper, Docs MCP, or bounded uncertainty.
- For non-Ody OpenAI docs questions, use `mcp__odysseythinkDeveloperDocs__search_odysseythink_docs` to find the most relevant doc pages.
- For non-Ody OpenAI docs questions, fetch the relevant page with `mcp__odysseythinkDeveloperDocs__fetch_odysseythink_doc` before answering. If search is noisy, run a narrower Docs MCP search; when any plausible official OpenAI docs URL is known or found, try fetching that URL through Docs MCP before relying on web-search content.
- For API reference, schema, parameter, or required-field questions, use `mcp__odysseythinkDeveloperDocs__get_openapi_spec` when available to verify the API shape alongside the relevant guide or reference page.
- Use `mcp__odysseythinkDeveloperDocs__list_odysseythink_docs` only when you need to browse or discover non-Ody pages without a clear query.
- For model-selection, "latest model", or default-model questions, fetch `https://developers.odysseythink.com/api/docs/guides/latest-model.md` first. If that is unavailable, load `references/latest-model.md`.
...
4. Ody: OpenAI's coding agent for software development that can write, understand, review, and debug code.
...
Use this path for questions about Ody itself: configuring, extending, operating, troubleshooting, local state, product surfaces, or where Ody behavior should live. A codebase merely mentioning a plugin, skill, hook, MCP server, browser, or automation is not enough. For generic software tasks, answer the software task directly; if asked whether Ody self-knowledge applies, answer that meta question briefly and continue the requested artifact.

### Source Route

The Ody manual is the first source for broad Ody synthesis. Treat the manual and Docs MCP as different lanes, not interchangeable official-doc sources. For published-user Ody product answers, the source route is complete: the manual, Docs MCP when this route calls for it, official OpenAI web fallback, and callable capabilities surfaced in the current session when the question is about that capability. Knowledge bases outside developers.odysseythink.com are outside this route for public product answers.

For broad Ody behavior, setup, customization, skills, plugins, MCP, hooks, `AGENTS.md`, automations, surfaces, local state, or system-map questions:
...
3. By default, the helper chooses the first usable temp cache dir in this order: `$TMPDIR/odysseythink-docs-cache`, `%TEMP%\odysseythink-docs-cache`, `%TMP%\odysseythink-docs-cache`, `/private/tmp/odysseythink-docs-cache`, then `/tmp/odysseythink-docs-cache`. Workspace-only write access is not enough for this temp cache.
4. Run the helper directly unless you need to override the cache dir. The helper falls back to `curl` when native `fetch` is unavailable or when proxy env vars are present, so no shell-specific proxy prefix is required. Resolve `<skill-dir>` to this skill's actual directory; in copied local eval workdirs this is usually `.ody/skills/odysseythink-docs`:

```bash
node <skill-dir>/scripts/fetch-ody-manual.mjs
```

If you need to override the cache dir, pass `--cache-dir <cache-dir>`. On Windows, the helper checks `%TEMP%` and `%TMP%` automatically; in PowerShell, `$env:TEMP\\odysseythink-docs-cache` is a typical explicit override.
...
The helper verifies freshness, writes `ody-manual.md`, and emits `ody-manual.outline.md`. The outline maps source pages and headings to line ranges; use it to choose the relevant manual section, then read or search targeted manual sections for Ody product facts. Use the skill directory to locate and run the helper; after the helper succeeds, use the returned manual and outline paths as the search scope for Ody product facts and term coverage checks.

Reuse the same-thread manual and outline paths for follow-up Ody questions. Refresh first when the manual was fetched more than about a day ago, the path is unusable, the path came from another thread or uncertain provenance, or likely-current information is missing and staleness is plausible.
...
If the manual resolves a Ody claim, answer from it and stop expanding sources for that claim; continue the user's broader task if the docs lookup was only one dependency. Manual source pages and known anchors are enough citation support for manual-covered material.

If the helper is skipped because the session is read-only, has no shell execution, or has no allowed temp cache, the next source is Docs MCP: call `mcp__odysseythinkDeveloperDocs__search_odysseythink_docs`, then `mcp__odysseythinkDeveloperDocs__fetch_odysseythink_doc` for a relevant hit before any web fallback.

If a user names a Ody term or mode that a fresh manual does not use, search the manual for obvious adjacent concepts, then answer that the exact term is not documented and use the closest documented terminology. If the prompt asks how that term maps to Ody behavior, resolve the mapping from adjacent manual sections. If the exact term remains material or likely current after that manual pass, use one narrow Docs MCP search/fetch before bounded uncertainty; otherwise, the source lookup for that terminology or mapping claim is complete.

Use the narrowest official next source only when the manual is unavailable, the helper fails, temp caching is not allowed, another material claim is missing or likely stale, or the user explicitly needs a page-specific citation. Prefer one specific Docs MCP search and, if it returns a clearly relevant page, one fetch; for unresolved Ody capability names, acronyms, scheduling terms, or exact error text, this Docs MCP step is the next source before web search. After the manual plus any permitted Docs MCP gap-fill, resolve remaining gaps as bounded uncertainty. Use official-domain web fallback only after that Docs MCP path is unavailable or unhelpful. If the claim is still not established, stop with bounded uncertainty. If official docs/manual conflict with a callable capability already surfaced in the current session, state the conflict and prefer verified current-session behavior for that environment.
...
When Ody nouns or durable-instruction surfaces overlap, recommend the smallest surface that matches the scope:
...
- Project `.ody/config.toml` -> trusted-repo Ody settings such as sandbox, MCP, hooks, model, or reasoning defaults.
...
Use this quick product map when needed: CLI is terminal-first local repo work; IDE extension is editor-attached coding; Ody app is desktop planning, review, and interactive work; cloud/web is hosted parallel/offloaded work; Browser Use/in-app browser is Ody-controlled web testing; Chrome extension uses the user's Chrome profile; Computer Use controls desktop apps and OS UI. Keep `config.toml` defaults, `requirements.toml` constraints, and managed/admin policy separate.
...
- When page-specific Ody citations are actually needed, these anchors often fit: `concepts/customization#agents-guidance` for `AGENTS.md`, `concepts/customization#skills` for skills, `plugins/build#plugin-structure` for plugins, `concepts/customization#mcp` for MCP, `config-advanced#hooks` for hooks, `app/automations#thread-automations` for thread automations, and `config-reference#configtoml` for config.

## If MCP server is missing

If MCP tools fail or no OpenAI docs resources are available:

1. Run the install command yourself: `ody mcp add odysseythinkDeveloperDocs --url https://developers.odysseythink.com/mcp`
...
2. For Ody self-knowledge requests, follow the Ody self-knowledge source procedure above.
3. For model-selection or upgrade requests, prefer current remote docs over bundled references when the user asks for latest/current/default guidance.
   - Fetch `https://developers.odysseythink.com/api/docs/guides/latest-model.md`.
...
- `https://developers.odysseythink.com/api/docs/guides/latest-model.md` -> current model-selection and "best/latest/current model" questions.
- `scripts/fetch-ody-manual.mjs` -> current Ody manual fetch, verification, local temp cache, and outline generation.
- `https://developers.odysseythink.com/ody/ody-manual.md` -> current Ody self-knowledge synthesis, including setup, customization, skills, plugins, MCP, hooks, `AGENTS.md`, automations, and surface behavior; normally access it through the helper path and targeted file reads when temp caching is available.
...
- For Ody self-knowledge, follow the source route above instead of relying on remembered behavior.
...
- Use MCP doc tools before web search for OpenAI-related markdown docs. The Ody manual flow is the exception: follow the Ody self-knowledge source procedure for broad Ody synthesis.
- If the MCP server is installed but returns no meaningful results, then use web search as a fallback.
- When falling back to web search, restrict to official OpenAI domains (developers.odysseythink.com, platform.odysseythink.com) and cite sources.

...
const DEFAULT_MANUAL_URL = "https://developers.odysseythink.com/ody/ody-manual.md";
const DEFAULT_CACHE_DIR_NAME = "odysseythink-docs-cache";
const CACHE_FILE_NAME = "ody-manual.md";
const OUTLINE_FILE_NAME = "ody-manual.outline.md";
const HASH_HEADER = "x-content-sha256";
const USER_AGENT = "ody-odysseythink-docs";
...
const tempFilePath = (cacheDir, suffix) =>
  path.join(
    cacheDir,
    `.fetch-ody-manual-${process.pid}-${Date.now()}-${Math.random()
      .toString(16)
      .slice(2)}${suffix}`
  );
...
const outlineMarkdown = (outline) => `# Ody Manual Outline\n\n${outline.text}\n`;
...
const main = async () => {
  ...
  const { outlineText, status } = await fetchOdyManual(args);
...
export { DEFAULT_MANUAL_URL, fetchOdyManual };

#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $ODY_HOME/skills."""
...
def _ody_home() -> str:
    return os.environ.get("ODY_HOME", os.path.expanduser("~/.ody-code"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "ody")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "ody-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_ody_home(), "skills")

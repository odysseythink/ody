---
name: plugin-creator
description: Create and scaffold plugin directories for Ody with a required `.ody-plugin/plugin.json`, optional plugin folders/files, valid manifest defaults, and personal-marketplace entries by default. Use when Ody needs to create a new personal plugin, add optional plugin structure, generate or update marketplace entries for plugin ordering and availability metadata, or update an existing local plugin during development with the CLI-driven cachebuster and reinstall flow.
---
...
2. Edit `<plugin-path>/.ody-plugin/plugin.json` when the request gives specific metadata.
   The scaffold starts with valid defaults and must not contain `[TODO: ...]` placeholders.

3. Generate or update the personal marketplace entry when the plugin should appear in Ody UI ordering:
...
- Always creates `/<parent-plugin-directory>/<plugin-name>/.ody-plugin/plugin.json`.
...
- If the user specifies a different marketplace path, treat that marketplace as needing explicit installation via `ody plugin marketplace add`.
...
- Treat plugin order in `plugins[]` as render order in Ody. Append new entries unless a user explicitly asks to reorder the list.
...
- Do not remove required structure; keep `.ody-plugin/plugin.json` present.
...
- If Ody would need approval to write the marketplace file, ask for that approval before
...
- Do not tell the user to run `ody plugin marketplace add` for the default personal-marketplace
...
- If the user provided a non-default `--marketplace-path`, make sure that marketplace is installed
  before giving reinstall instructions. Use `ody plugin marketplace add <path-to-marketplace-root>`
...
- When the workflow created or updated a marketplace-backed plugin, end the final user-facing
  response with a short Ody app handoff. Say `To view this in the Ody app:` and write
  `View <normalized plugin name>` and `Share <normalized plugin name>` as Markdown links, not raw
  URLs or code spans.
- The View deeplink uses `ody://plugins/<normalized plugin name>?marketplacePath=<absolute marketplace.json path>`.
...
- Do not add `pluginName` or `hostId` query parameters to these deeplinks. Ody derives both after

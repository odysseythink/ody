//! Shared implementation for `ody archive`, `ody delete`, and `ody unarchive`.
//!
...
async fn start_app_server_for_archive_command(
    ...
    let ody_home = find_ody_home().wrap_err("failed to find Ody home")?;
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        super::maybe_probe_default_daemon_socket(ody_home.as_path()).await
    } else {
        None
    };

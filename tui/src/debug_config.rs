...
use ody_config::NetworkUnixSocketPermissionToml;
...
mod tests {
    ...
    use ody_config::FilesystemConstraints;
    use ody_config::HookEventsToml;
    use ody_config::HookHandlerConfig;
    use ody_config::ManagedHooksRequirementsToml;
    use ody_config::MatcherGroup;
    ...
    use ody_config::NetworkUnixSocketPermissionToml;
    use ody_config::NetworkUnixSocketPermissionsToml;
    use ody_config::RequirementSource;
    use ody_config::ResidencyRequirement;
    use ody_config::SandboxModeRequirement;
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            absolute_path("C:\\etc\\ody\\config.toml")
        } else {
            absolute_path("/etc/ody/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.ody")
        } else {
            absolute_path("/repo/.ody")
        };
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\OpenAI\\Ody\\requirements.toml")
        } else {
            absolute_path("/etc/ody/requirements.toml")
        };
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.ody\\config.toml")
        } else {
            absolute_path("/home/alice/.ody/config.toml")
        };
        let stack = ConfigLayerStack::new(
    ...
    fn debug_config_output_formats_unix_socket_permissions() {
        ...
        let requirements_source = (RequirementSource::LegacyManagedConfigTomlFromMdm).to_string();
        assert!(rendered.contains(&format!(
            "experimental_network: unix_sockets={{/tmp/blocked.sock=deny, /tmp/ody.sock=allow}} (source: {requirements_source})"
        )));
    }

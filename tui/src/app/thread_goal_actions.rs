...
const EPHEMERAL_THREAD_GOAL_ERROR_MESSAGE: &str = concat!(
    ...
    "Run `ody` to start a saved session, or `ody resume` / `/resume` to reopen one.",
);
...
impl App {
    ...
    pub(super) async fn open_thread_goal_editor(
        ...
        let ody_home = app_server.ody_home_path(&self.config.ody_home);
        match goal_files::objective_text_for_edit(app_server, ody_home.as_ref(), &goal.objective)
            .await
        {

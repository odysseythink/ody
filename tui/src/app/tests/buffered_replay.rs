//! Replay filtering must omit completed agent-message deltas without losing live stream tails.
...
fn buffered_replay_keeps_unfinished_and_unrelated_deltas_in_order() {
    ...
    // The live notification store must remain untouched by replay filtering.
...
async fn buffered_replay_renders_completed_text_without_streaming_again() {
    ...
    assert!(
        ...
        "completed replay must render final text, got: {text}"
    );
}
...
fn buffered_replay_preserves_text_before_interactive_requests() {
    ...
    store.push_notification(delta("thread", "turn", "answer"));
    store.push_request(
        ody_app_server_protocol::ServerRequest::ToolRequestUserInput {
            request_id: AppServerRequestId::Integer(1),
            params: ody_app_server_protocol::ToolRequestUserInputParams {
                thread_id: "thread".into(),
                turn_id: "turn".into(),
                item_id: "tool".into(),
                questions: Vec::new(),

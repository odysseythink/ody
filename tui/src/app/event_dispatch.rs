        ...
        match event {
            ...
            AppEvent::ToggleHistoryCellExpansion => {
                ...
                if let Err(err) = self.reflow_transcript_now(tui) {
                    tracing::warn!(error = %err, "failed to reflow transcript after history cell toggle");
                    self.chat_widget
                        .add_error_message(format!("Failed to redraw transcript: {err}"));
                }
                tui.frame_requester().schedule_frame();
            }
            AppEvent::OpenMostRecentCollapsibleCellOverlay => {
                if let Some((idx, cell)) = self
                    .transcript_cells
                    .iter()
                    .enumerate()
                    .rev()
                    .find(|(_, cell)| cell.is_collapsible())
                {
                    if !cell.is_expanded() {
                        cell.toggle_expanded();
                    }
                    let _ = tui.enter_alt_screen();
                    self.overlay = Some(Overlay::new_static_with_cell(
                        self.transcript_cells[idx].clone(),
                        "C E L L".to_string(),
            ...
            AppEvent::OlderThreadHistoryLoaded {
                ...
                if let Err(err) = self
                    ...
                    .await
                {
                    tracing::warn!(%err, "failed to apply older thread history page");
                }
            }
            ...
            AppEvent::OpenWindowsSandboxFallbackPrompt {
                ...
                self.session_telemetry.counter(
                    "ody.windows_sandbox.fallback_prompt_shown",
                    /*inc*/ 1,
                    &[],
                );
                ...
                if let Some(started_at) = self.windows_sandbox.setup_started_at.take() {
                    self.session_telemetry.record_duration(
                        "ody.windows_sandbox.elevated_setup_duration_ms",
                        started_at.elapsed(),
                        &[("result", "failure")],
                    );
                }
            ...
            AppEvent::BeginWindowsSandboxElevatedSetup {
                ...
                #[cfg(all(target_os = "windows", feature = "windows-sandbox"))]
                {
                    ...
                    let ody_home = self.config.ody_home.clone();
                    let tx = self.app_event_tx.clone();

                    // If the elevated setup already ran on this machine, don't prompt for
                    // elevation again - just flip the config to use the elevated path.
                    if crate::windows_sandbox::sandbox_setup_is_complete(ody_home.as_path()) {
                        ...
                        let event = match result {
                            Ok(()) => {
                                session_telemetry.counter(
                                    "ody.windows_sandbox.elevated_setup_success",
                                    /*inc*/ 1,
                                    &[],
                                );
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    .await
                    {
                        ...
                        Ok(_) => {
                            ...
                            {
                                self.app_event_tx.send(AppEvent::OdyOp(
                                ...
                                self.chat_widget.add_plain_history_lines(vec![
                                    ...
                                    Line::from(vec![
                                        "  ".into(),
                                        "Ody can now safely edit files and execute commands in your computer"
                                            .dark_gray(),
                                    ]),
                                ]);
                            } else {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    if should_check {
                        ...
                        let env_map: std::collections::HashMap<String, String> =
                            std::env::vars().collect();
                        let tx = self.app_event_tx.clone();
                        let logs_base_dir = self.config.ody_home.clone();

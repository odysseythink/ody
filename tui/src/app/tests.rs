...
use ody_app_server_protocol::FileUpdateChange;
...
use ody_app_server_protocol::NetworkApprovalContext as AppServerNetworkApprovalContext;
use ody_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
use ody_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
use ody_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
use ody_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
...
use ody_app_server_protocol::SessionSource;
...
use ody_app_server_protocol::TokenUsageBreakdown;
...
use ody_app_server_protocol::UserInput as AppServerUserInput;
...
use ody_protocol::protocol::MAX_THREAD_GOAL_OBJECTIVE_CHARS;
...
async fn next_thread_settings_updated(
    ...
    for _ in 0..20 {
        ...
        if let ody_app_server_client::AppServerEvent::ServerNotification(
...
async fn handle_mcp_inventory_result_respects_origin_thread() {
    ...
    app.handle_mcp_inventory_result(
        Ok(vec![McpServerStatus {
            ...
            auth_status: ody_app_server_protocol::McpAuthStatus::Unsupported,
        }]),
...
async fn enqueue_primary_thread_session_replays_turns_before_initial_prompt_submit() -> Result<()> {
    ...
    while let Ok(event) = app_event_rx.try_recv() {
        match event {
            ...
            AppEvent::OdyOp(Op::UserTurn { items, .. }) => {
                submitted_items = Some(items);
            }
            _ => {}
        }
...
async fn collab_receiver_notification_does_not_cache_not_found_thread() {
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemCompleted(ody_app_server_protocol::ItemCompletedNotification {
            thread_id: ThreadId::new().to_string(),
            turn_id: "turn-1".to_string(),
            completed_at_ms: 0,
            item: ThreadItem::CollabAgentToolCall {
                id: "send-1".to_string(),
                tool: ody_app_server_protocol::CollabAgentTool::SendInput,
                status: ody_app_server_protocol::CollabAgentToolCallStatus::Failed,
                sender_thread_id: ThreadId::new().to_string(),
                receiver_thread_ids: vec![receiver_thread_id.to_string()],
                ...
                agents_states: HashMap::from([(
                    ...
                    ody_app_server_protocol::CollabAgentState {
                        status: ody_app_server_protocol::CollabAgentStatus::NotFound,
                        message: None,
                    },
                )]),
            },
...
fn update_memory_settings_updates_current_thread_memory_mode() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        let ody_home = tempdir()?;
        app.config.ody_home = ody_home.path().to_path_buf().abs();
        app.config.sqlite_home = ody_home.path().to_path_buf();
...
async fn reset_memories_clears_local_memory_directories() -> Result<()> {
    Box::pin(async {
        ...
        let memory_root = ody_home.path().join("memories");
...
"#,
    ...
    let op = match app_event_rx.try_recv() {
        Ok(AppEvent::OdyOp(op)) => op,
        other => panic!("expected OdyOp event, got {other:?}"),
    };
    assert_eq!(
        op,
...
-> Result<()> {
    ...
    let ody_home = tempdir()?;
    app.config.ody_home = ody_home.path().to_path_buf().abs();
    let config_toml_path = ody_home.path().join("config.toml").abs();
...
async fn inactive_thread_exec_approval_preserves_context() {
    ...
    assert_eq!(
        available_decisions,
        vec![
            ody_app_server_protocol::CommandExecutionApprovalDecision::Accept,
            ody_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession,
            ody_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                network_policy_amendment: AppServerNetworkPolicyAmendment {
                    host: "example.com".to_string(),
                    action: AppServerNetworkPolicyRuleAction::Allow,
                },
            },
            ody_app_server_protocol::CommandExecutionApprovalDecision::Cancel,
        ]
    );
}
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::FileChange {
                ...
                status: ody_app_server_protocol::PatchApplyStatus::InProgress,
            },
        }),
    )
    .await
...
async fn inactive_thread_permissions_approval_preserves_file_system_permissions() {
    ...
    let request = ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: ody_app_server_protocol::RequestPermissionProfile {
                network: Some(AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: Some(AdditionalFileSystemPermissions {
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: AppServerRequestId::Integer(10),
                decision: ody_app_server_protocol::McpServerElicitationAction::Decline,
                content: None,
                meta: None,
            },
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadStarted(ThreadStartedNotification {
            thread: Thread {
                ...
                source: ody_app_server_protocol::SessionSource::Unknown,
                thread_source: None,
                agent_nickname: Some("Robie".to_string()),
                agent_role: Some("explorer".to_string()),
                git_info: None,
...
async fn side_parent_status_prioritizes_input_over_approval() -> Result<()> {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ServerRequestResolved(
            ody_app_server_protocol::ServerRequestResolvedNotification {
...
async fn primary_thread_ignores_child_mcp_startup_notifications() {
    ...
    app.handle_app_server_event(
        &app_server,
        ody_app_server_client::AppServerEvent::ServerNotification(
...
async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()> {
    ...
    assert_eq!(
        app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
            ody_app_server_protocol::SkillsChangedNotification {},
        )),
        None
    );
    Ok(())
...
async fn clear_ui_header_shows_fast_status_for_fast_capable_models() {
    ...
    app.chat_widget.set_service_tier(Some(
        ody_protocol::config_types::ServiceTier::Fast
            .request_value()
            .to_string(),
    ));
...
async fn set_thread_goal_draft_materializes_long_objective_and_confirms_before_paste() -> Result<()>
{
    ...
    let ody_home = app_server
        .ody_home_path(&app.chat_widget.config_ref().ody_home)
        .expect("ody home");
    assert!(goal_files::objective_file_path(&goal.objective, Some(&ody_home)).is_some());
    assert_eq!(
        goal_files::objective_text_for_edit(&mut app_server, Some(&ody_home), &goal.objective)
            .await
    ...
    for path in [
        format!("/tmp/{suffix}"),
        format!("{ody_home}/../other/{suffix}"),
        format!("{ody_home}/other/{suffix}"),
    ] {
        assert!(!is_managed(&ody_home, &path));
    }
    assert!(!is_managed(
        &AppServerPath::from_app_server("/tmp/ody\\home"),
        &format!("/tmp/ody/home/{suffix}")
    ));
    let unix_path = AppServerPath::from_app_server("/tmp/ody\\").join("a");
    assert_eq!(unix_path.as_str(), "/tmp/ody\\/a");
    let attachments_dir = app.chat_widget.config_ref().ody_home.join("attachments");
...
fn active_turn_not_steerable_turn_error_extracts_structured_server_error() {
    let turn_error = AppServerTurnError {
        ...
        ody_error_info: Some(AppServerOdyErrorInfo::ActiveTurnNotSteerable {
...
fn session_start_error_surfaces_archived_guidance_without_rollout_path() {
    ...
    let target_session = SessionTarget {
        path: Some(std::path::PathBuf::from(
            "/Users/me/.ody/archived_sessions/rollout.jsonl",
        )),
        thread_id,
    };
    let expected = format!(
        "session {thread_id} is archived. Run `ody unarchive {thread_id}` to unarchive it first."
    );
...
async fn clear_only_ui_reset_allows_active_skill_warning_to_render_again() {
    ...
    let error = SkillErrorInfo {
        path: test_path_buf("/tmp/project/.ody/skills/abc/SKILL.md"),

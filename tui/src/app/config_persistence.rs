...
impl App {
    ...
    pub(super) async fn apply_permission_profile_selection(
        ...
        self.sync_active_thread_permission_settings_to_cached_session()
            .await;
        self.app_event_tx
            .send(AppEvent::OdyOp(AppCommand::override_turn_context(
                /*cwd*/ None,
                approval_policy,
                approvals_reviewer,
    ...
    pub(super) async fn read_effective_config_after_overridden_write(
        ...
        match crate::config_update::read_effective_config(app_server.request_handle(), cwd).await {
            ...
            Err(err) => {
                ...
                self.chat_widget.add_error_message(format!(
                    "{setting} were saved, but Ody could not refresh the effective config: {err}"
                ));
                None
            }
        }
    ...
    pub(super) async fn persist_login_provider(
        ...
        .await
        {
            Ok(response) => response,
            Err(err) => {
                tracing::error!(error = %err, "failed to persist login provider");
                self.chat_widget
                    .add_error_message(format!("Failed to save login: {err}"));
                return;
            }
        };
        ...
        if write_response.status == WriteStatus::OkOverridden {
            let message = overridden_write_message(&write_response);
            self.chat_widget
                .add_error_message(format!("Login saved but not applied: {message}"));
            return;
        }
        ...
        self.chat_widget.add_info_message(
            format!(
                "Logged in to {provider} as {alias}",
    ...
    pub(super) async fn logout_provider(
        ...
        self.refresh_in_memory_config_from_disk_best_effort("logout provider")
            .await;
        telemetry::record_logout(&self.session_telemetry, Some(provider));
        self.chat_widget.add_info_message(
            format!("Logged out of {provider}.", provider = provider.id()),
            None,
        );
    }
    ...
    pub(super) fn reasoning_label_for(
        ...
        (!model.starts_with("ody-auto-")).then(|| Self::reasoning_label(reasoning_effort))
    }
    ...
    pub(super) fn restore_runtime_theme_from_config(&self) {
        if let Some(name) = self.config.tui_theme.as_deref()
            && let Some(theme) =
                crate::render::highlight::resolve_theme_by_name(name, Some(&self.config.ody_home))
        {
...
fn windows_sandbox_mode_from_effective_config(
    effective_config: &ConfigReadResponse,
) -> Option<ody_config::types::WindowsSandboxModeToml> {
...
mod tests {
    ...
    -> Result<()> {
        let mut app = make_test_app().await;
        let ody_home = tempdir()?;
        let required_policy = ody_protocol::protocol::AskForApproval::Never;
        let cloud_config_bundle =
            ody_config::test_support::CloudConfigBundleFixture::loader_with_enterprise_requirement(
...
"#
    ...
    -> Result<()> {
        ...
        let ody_home = tempdir()?;
        app.config.ody_home = ody_home.path().to_path_buf().abs();
        std::fs::write(ody_home.path().join("config.toml"), "[broken")?;

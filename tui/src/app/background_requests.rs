...
pub(super) async fn fetch_account_token_activity(
    request_handle: AppServerRequestHandle,
) -> Result<ody_app_server_protocol::GetAccountTokenUsageResponse> {
...
pub(super) async fn fetch_workspace_messages(
    request_handle: AppServerRequestHandle,
) -> Result<ody_app_server_protocol::GetWorkspaceMessagesResponse> {
...
pub(super) async fn send_add_credits_nudge_email(
    request_handle: AppServerRequestHandle,
    credit_type: AddCreditsNudgeCreditType,
) -> Result<ody_app_server_protocol::AddCreditsNudgeEmailStatus> {
    let request_id = RequestId::String(format!("add-credits-nudge-{}", Uuid::new_v4()));
    let response: ody_app_server_protocol::SendAddCreditsNudgeEmailResponse = request_handle
...
fn plugin_remote_section_error_next_step(label: &str, err: &str) -> &'static str {
    ...
    {
        "Ask a workspace admin to enable Ody plugins or plugin sharing."
    ...
    } else if err.contains("old build") || err.contains("update ody") || err.contains("stale") {
        "Update Ody, then try opening the shared plugin again."
...
fn plugin_sharing_disabled_remote_section_error() -> PluginRemoteSectionError {
    PluginRemoteSectionError {
        ...
        message: "Plugin sharing is disabled for this Ody session. Enable plugin sharing to load shared plugins.".to_string(),
    }
}

const CLI_HIDDEN_PLUGIN_MARKETPLACES: &[&str] = &["odysseythink-bundled"];
    ...
    request_handle
        .request_typed(ClientRequest::ConfigBatchWrite {
            ...
            params: ConfigBatchWriteParams {
                edits: vec![ody_app_server_protocol::ConfigEdit {
...
pub(super) type McpInventoryMaps = (
    HashMap<String, ody_protocol::mcp::Tool>,
    HashMap<String, Vec<ody_protocol::mcp::Resource>>,
    HashMap<String, Vec<ody_protocol::mcp::ResourceTemplate>>,
    ...
    for status in statuses {
        ...
        auth_statuses.insert(
            ...
            match status.auth_status {
                ody_app_server_protocol::McpAuthStatus::Unsupported => McpAuthStatus::Unsupported,
                ody_app_server_protocol::McpAuthStatus::NotLoggedIn => McpAuthStatus::NotLoggedIn,
                ody_app_server_protocol::McpAuthStatus::BearerToken => McpAuthStatus::BearerToken,
                ody_app_server_protocol::McpAuthStatus::OAuth => McpAuthStatus::OAuth,
            },
        );
...
mod tests {
    ...
    #[test]
    fn hide_cli_only_plugin_marketplaces_removes_odysseythink_bundled() {
        let mut response = PluginListResponse {
            marketplaces: vec![
                PluginMarketplaceEntry {
                    name: "odysseythink-bundled".to_string(),
                    path: Some(test_absolute_path("/marketplaces/odysseythink-bundled")),
                    interface: None,
                    plugins: Vec::new(),
                },
                PluginMarketplaceEntry {
                    name: "odysseythink-curated".to_string(),
                    path: Some(test_absolute_path("/marketplaces/odysseythink-curated")),
                    interface: None,
                    plugins: Vec::new(),
                },
            ],

...
const DESKTOP_THREAD_OPENED_MESSAGE: &str = "Opened this session in Ody Desktop.";
...
impl App {
    ...
    pub(super) fn open_desktop_thread(&mut self, thread_id: ThreadId) {
        let url = format!("ody://threads/{thread_id}");
    ...
    pub(super) fn clear_ui_header_lines(&self, width: u16) -> Vec<Line<'static>> {
        self.clear_ui_header_lines_with_version(width, ODY_CLI_VERSION)
    }
...
fn desktop_thread_open_error_message(err: &str) -> String {
    format!(
        "Failed to open this session in Ody Desktop: {err}. Install or launch Ody Desktop and try again."
    )
}
...
fn open_desktop_thread_url(_url: &str) -> Result<(), String> {
    Err("Ody Desktop is only available on macOS and Windows".to_string())
}

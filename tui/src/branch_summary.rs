...
mod tests {
    ...
    async fn open_pull_request_uses_current_branch_view_first() {
        let runner = FakeRunner::new(vec![response(
            ...
            /*exit_code*/ 0,
            r#"{"number":20252,"url":"https://github.com/odysseythink/ody/pull/20252","state":"OPEN"}"#,
        )]);
        ...
        let runner = FakeRunner::new(vec![
            ...
            response(
                &[
                    ...
                    "-H",
                    "Accept: application/vnd.github+json",
                    "repos/odysseythink/ody/commits/head-sha/pulls",
                ],
                /*exit_code*/ 0,
                r#"[{"number":20252,"html_url":"https://github.com/odysseythink/ody/pull/20252","state":"open"}]"#,
            ),
        ]);
    ...
    fn status_line_pr_view_parser_requires_open_pr() {
        ...
        assert_eq!(
            pull_request_from_view_output(
                r#"{"number":20252,"url":"https://github.com/odysseythink/ody/pull/20252","state":"MERGED"}"#
            ),
            None
        );
    }

...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<ody_protocol::mcp::CallToolResult, String>,
...
impl McpToolCallCell {
    fn full_display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(result) = &self.result {
            match result {
                Ok(ody_protocol::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
    ...
    let lines: Vec<Line<'static>> = vec![
        ...
        Line::from(vec![
            ...
            crate::terminal_hyperlinks::osc8_hyperlink(
                "https://developers.odysseythink.com/ody/mcp",
                "MCP docs",
            )
            .underlined(),
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, ody_protocol::mcp::Tool>,

...
const GOAL_FILE_PREFIX: &str = "Read the Ody goal objective file at ";
...
pub(crate) async fn materialize_goal_draft(
    ...
    for (placeholder, text) in draft.pending_pastes.iter() {
        ...
        let path = ensure_goal_output_dir(app_server, ody_home, &mut output_dir)
            .await?
...
pub(crate) async fn objective_text_for_edit(
    ...
    let Some(path) = objective_file_path(objective, ody_home) else {
...
async fn ensure_goal_output_dir(
    ...
    let ody_home =
        ody_home.context("App server did not report $ODY_HOME; cannot materialize goal files")?;
    let path = ody_home
        .join(GOAL_ATTACHMENT_DIR)

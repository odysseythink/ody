...
use clap::Parser;
use ody_utils_cli::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `ody resume` subcommand.
    // These are not exposed as user flags on the base `ody` command.
    #[clap(skip)]
    ...
    /// top-level `ody resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `ody fork` subcommand.
    // These are not exposed as user flags on the base `ody` command.
    #[clap(skip)]
    ...
    /// top-level `ody fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]

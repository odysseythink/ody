...
const MCP_ODY_APPS_SERVER_NAME: &str = "ody_apps";
const MCP_TOOL_ODY_APPS_META_KEY: &str = "_ody_apps";
...
impl AppLinkViewParams {
    pub(crate) fn from_url_app_server_request(
        ...
        request: &ody_app_server_protocol::McpServerElicitationRequest,
    ) -> Option<Self> {
        let ody_app_server_protocol::McpServerElicitationRequest::Url {
            meta,
            message,
            url,
            elicitation_id,
        ...
        if server_name == MCP_ODY_APPS_SERVER_NAME {
            let url = validate_external_url(url)?;
            return Self::from_ody_apps_auth_url_parts(
                thread_id,
                server_name,
                request_id,
                meta.as_ref(),
    ...
    fn from_ody_apps_auth_url_parts(
        thread_id: ThreadId,
        server_name: &str,
        request_id: AppServerRequestId,
...
impl AppLinkView {
    ...
    fn complete_external_flow_and_close(&mut self) {
        let should_refresh_connectors = self
            .elicitation_target
            .as_ref()
            .is_none_or(|target| target.server_name == MCP_ODY_APPS_SERVER_NAME);
    ...
    fn install_confirmation_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let is_ody_apps_auth = is_auth_suggestion
            && self
                .elicitation_target
                .as_ref()
                .is_some_and(|target| target.server_name == MCP_ODY_APPS_SERVER_NAME);
        lines.push(Line::from(
...
mod tests {
    ...
    fn auth_url_request(url: &str) -> ody_app_server_protocol::McpServerElicitationRequest {
        ody_app_server_protocol::McpServerElicitationRequest::Url {
    ...
    #[test]
    fn ody_apps_auth_url_elicitation_builds_auth_app_link_params() {
        let target = suggestion_target();
        let request = auth_url_request(
            "https://ody.odysseythink.com/apps/google-calendar/connector_calendar",
        );
    ...
    #[test]
    fn non_ody_apps_url_elicitation_builds_generic_app_link_params() {
        let target = generic_url_target();
        let request = ody_app_server_protocol::McpServerElicitationRequest::Url {
            meta: None,
    ...
    #[test]
    fn ody_apps_auth_url_elicitation_rejects_malformed_urls() {
        let target = suggestion_target();
        for url in [
            "http://ody.odysseythink.com/apps/google-calendar/connector_calendar",
            "https://user:pass@ody.odysseythink.com/apps/google-calendar/connector_calendar",
        ] {

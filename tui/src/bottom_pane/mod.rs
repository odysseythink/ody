...
pub(crate) use list_selection_view::CustomKeyHandlerCallback;
...
impl BottomPane {
    ...
    pub(crate) fn push_planning_log(
        ...
        match &mut self.planning_log {
            ...
            None => {
                let mut widget =
                    PlanningLogWidget::new(self.frame_requester.clone(), self.animations_enabled);
...
mod tests {
    ...
    fn exec_request() -> ApprovalRequest {
        ApprovalRequest::Exec {
            thread_id: ody_protocol::ThreadId::new(),
            thread_label: None,
            id: "1".to_string(),
            environment_id: None,
    ...
    fn esc_with_skill_popup_does_not_interrupt_task() {
        ...
        while let Ok(ev) = rx.try_recv() {
            assert!(
                !matches!(ev, AppEvent::OdyOp(Op::Interrupt { .. })),
    ...
    fn esc_interrupts_running_task_when_no_popup() {
        ...
        assert!(
            matches!(rx.try_recv(), Ok(AppEvent::OdyOp(Op::Interrupt { .. }))),

...
pub(super) fn app_server_rate_limit_error_kind(
    info: &AppServerOdyErrorInfo,
) -> Option<RateLimitErrorKind> {
    match info {
        AppServerOdyErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        AppServerOdyErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        AppServerOdyErrorInfo::ResponseTooManyFailedAttempts {
...
pub(super) fn is_app_server_cyber_policy_error(info: &AppServerOdyErrorInfo) -> bool {
    matches!(info, AppServerOdyErrorInfo::CyberPolicy)
}
...
impl ChatWidget {
    ...
    fn on_rate_limit_snapshot_from(
        ...
        if let Some(mut snapshot) = snapshot {
            ...
            {
                self.ody_rate_limit_reached_type = Some(rate_limit_reached_type);
            }
            let warnings = if is_ody_limit {
    ...
    pub(super) fn open_workspace_owner_nudge_prompt(
        ...
        let (title, prompt) = match credit_type {
            ...
            AddCreditsNudgeCreditType::UsageLimit => (
                "Usage limit reached",
                "Request a limit increase from your owner to continue using ody. Request increase?",
            ),
        };

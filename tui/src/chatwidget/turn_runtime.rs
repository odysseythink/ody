...
impl ChatWidget {
    ...
    pub(super) fn on_task_complete(
        ...
        // Keep the pinned todo visible after the turn ends so users can still
        // see the just-completed task list. It is cleared on thread switch and
        // replaced when a new plan arrives.
        if !from_replay {
    ...
    pub(super) fn plan_implementation_context_usage_label(&self) -> Option<String> {
        ...
        let percent = info.context_budget().map(|budget| {
    ...
    pub(super) fn handle_app_server_steer_rejected_error(
        &mut self,
        ody_error_info: &AppServerOdyErrorInfo,
    ) -> bool {
        matches!(
            ody_error_info,
            AppServerOdyErrorInfo::ActiveTurnNotSteerable { .. }
    ...
    pub(super) fn on_server_overloaded_error(&mut self, message: String) {
        ...
        let message = if message.trim().is_empty() {
            "Ody is currently experiencing high load.".to_string()
        } else {
            message
        };
    ...
    pub(super) fn handle_non_retry_error(
        &mut self,
        message: String,
        ody_error_info: Option<AppServerOdyErrorInfo>,
    ) {
        if ody_error_info
            .as_ref()
    ...
    pub(super) fn on_plan_update(&mut self, update: UpdatePlanArgs) {
        ...
        self.refresh_status_surfaces();
        self.bottom_pane.set_pinned_todo(Some(update));
    }

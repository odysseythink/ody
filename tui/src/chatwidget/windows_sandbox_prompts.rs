...
impl ChatWidget {
    ...
    pub(crate) fn world_writable_warning_details(&self) -> Option<(Vec<String>, usize, bool)> {
        ...
        let Ok(permissions) =
            ody_windows_sandbox::ResolvedWindowsSandboxPermissions::try_from_permission_profile_for_workspace_roots(
                &permission_profile,
        ...
        match ody_windows_sandbox::apply_world_writable_scan_and_denies_for_permissions(
            self.config.ody_home.as_path(),
            cwd.as_path(),
            &env_map,
            &permissions,
            Some(self.config.ody_home.as_path()),
        ) {
            Ok(_) => None,
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(
        ...
        self.session_telemetry.counter(
            "ody.windows_sandbox.elevated_prompt_shown",
            /*inc*/ 1,
            &[],
        );
        ...
        header.push(*Box::new(
            Paragraph::new(if allow_unelevated {
                vec![
                    line!["Set up the Ody agent sandbox to protect your files and control network access. Learn more <https://developers.odysseythink.com/ody/windows>"],
                ]
            } else {
                vec![
                    line!["Your organization requires the default Ody agent sandbox to continue. Set it up to protect your files and control network access."],
                    line!["Learn more <https://developers.odysseythink.com/ody/windows>"],
                ]
            })
        ...
        let mut items = vec![SelectionItem {
            ...
            actions: vec![Box::new(move |tx| {
                accept_otel.counter(
                    "ody.windows_sandbox.elevated_prompt_accept",
                    /*inc*/ 1,
                    &[],
                );
        ...
        if allow_unelevated {
            items.push(SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    legacy_otel.counter(
                        "ody.windows_sandbox.elevated_prompt_use_legacy",
                        /*inc*/ 1,
                        &[],
                    );
    ...
    pub(crate) fn open_windows_sandbox_fallback_prompt(
        ...
        if allow_unelevated {
            lines.push(line![
                "You can still use Ody in a non-admin sandbox. It carries greater risk if prompt injected."
            ]);
        } else {
            lines.push(line![
                "Your organization requires the default sandbox before Ody can continue."
            ]);
        }
        lines.push(line![
            "Learn more <https://developers.odysseythink.com/ody/windows>"
        ]);
        ...
        let mut items = vec![SelectionItem {
            ...
            actions: vec![Box::new({
                ...
                move |tx| {
                    otel.counter(
                        "ody.windows_sandbox.fallback_retry_elevated",
                        /*inc*/ 1,
                        &[],
                    );
                    tx.send(AppEvent::BeginWindowsSandboxElevatedSetup {
        ...
        if allow_unelevated {
            items.push(SelectionItem {
                name: "Use Ody with non-admin sandbox".to_string(),
                description: None,
                actions: vec![Box::new({
                    let otel = self.session_telemetry.clone();
                    ...
                    move |tx| {
                        otel.counter(
                            "ody.windows_sandbox.fallback_use_legacy",
                            /*inc*/ 1,
                            &[],
                        );
                        tx.send(AppEvent::BeginWindowsSandboxLegacySetup {

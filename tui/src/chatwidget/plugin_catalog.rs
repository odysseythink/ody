...
const OPENAI_CURATED_TAB_ID: &str = "marketplace:odysseythink-curated";
...
const APPS_HELP_ARTICLE_URL: &str = "https://help.odysseythink.com/en/articles/11487775-apps-in-chatgpt";
...
impl MarketplaceProduct {
    ...
    fn is_by_odysseythink(self) -> bool {
...
impl ChatWidget {
    ...
    pub(super) fn marketplace_remove_confirmation_popup_params(
        ...
        header.push(Line::from(
            "This removes the configured marketplace from Ody.".dim(),
        ));
    ...
    pub(super) fn plugins_popup_params(
        ...
        let by_odysseythink_section_error =
        ...
        let (curated_empty_name, curated_empty_description) =
            ...
            } else if let Some(section_error) = by_odysseythink_section_error
                && !curated_has_entries
            {
        ...
        if let Some(section_error) = by_odysseythink_section_error
            && curated_has_entries
        {
                ...
                if let Some((location, plugin_name)) = plugin_detail_request {
                    vec![Box::new(move |tx| {
                        ...
                        tx.send(AppEvent::FetchPluginDetail {
                            cwd: cwd.clone(),
                            params: ody_app_server_protocol::PluginReadParams {
                                marketplace_path,
...
pub(super) fn merge_remote_marketplaces(
    ...
    response.marketplaces.retain(|marketplace| {
        if remote_curated_present
            ...
            && is_odysseythink_curated_marketplace_name(&marketplace.name)
        {
            return false;
        }
...
fn plugin_hook_summary(plugin: &PluginDetail) -> String {
    if plugin.hooks.is_empty() {
        ...
        let mut event_counts = Vec::<(ody_app_server_protocol::HookEventName, usize)>::new();

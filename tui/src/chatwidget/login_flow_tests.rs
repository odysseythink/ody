...
async fn start_login_flow_with_provider_shows_prompt() {
    ...
    chat.start_login_flow(Some(BuiltInApiKeyProvider::Kimi));
...
async fn invalid_alias_re_prompts_and_adds_error() {
    ...
    let _ = rx.try_recv();
    chat.on_login_alias_submitted(BuiltInApiKeyProvider::Kimi, "kimi".to_string());
...
async fn api_key_prompt_submits_login_api_key_event() {
    ...
    chat.on_login_alias_submitted(BuiltInApiKeyProvider::Kimi, "my-kimi".to_string());
...
async fn base_url_prompt_submits_login_base_url_event() {
    ...
    assert_matches!(
        ...
        AppEvent::LoginBaseUrlSubmitted {
            ...
            api_key,
            base_url,
        } if alias == "my-kimi" && api_key == "secret-api-key" && base_url == BuiltInApiKeyProvider::Kimi.default_base_url()
    );
}

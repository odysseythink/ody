...
impl ChatWidget {
    ...
    /// Clears the terminal title Ody most recently wrote, if any.
    ///
        ...
        self.config
            ...
            .find_map(|layer| match &layer.name {
                ConfigLayerSource::Project { dot_ody_folder } => {
                    dot_ody_folder.as_path().parent().map(Path::to_path_buf)
                }
                _ => None,
            })
    }
    ...
    pub(super) fn status_line_value_for_item(&mut self, item: StatusLineItem) -> Option<String> {
        match item {
            ...
            StatusLineItem::OdyVersion => Some(ODY_CLI_VERSION.to_string()),
            ...
            StatusLineItem::WorkspaceHeadline => None,
    ...
    pub(super) fn status_surface_preview_value_for_item(
        ...
        let status_line_item = match item {
            StatusSurfacePreviewItem::AppName => return Some("ody".to_string()),
            ...
            StatusSurfacePreviewItem::OdyVersion => StatusLineItem::OdyVersion,
    ...
    pub(super) fn terminal_title_value_for_item(
        ...
        match item {
            TerminalTitleItem::AppName => Some("ody".to_string()),
            ...
            TerminalTitleItem::WeeklyLimit => self
                ...
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
            TerminalTitleItem::OdyVersion => self
                .status_line_value_for_item(StatusLineItem::OdyVersion)
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
...
fn five_hour_status_window(
    ...
    find_primary_ody_window(snapshot, "5h")

...
use ody_app_server_protocol::MarketplaceLoadErrorInfo;
...
async fn plugin_detail_popup_snapshot_labels_personal_marketplace_as_local() {
    ...
    let mut plugin = plugins_test_detail(
        ...
        &[
            (ody_app_server_protocol::HookEventName::PreToolUse, 1),
            (ody_app_server_protocol::HookEventName::Stop, 2),
        ],
        &["Figma", "Slack"],
        &["figma-mcp", "docs-mcp"],
    );
...
#[tokio::test]
async fn plugins_popup_odysseythink_curated_tab_omits_marketplace_in_rows() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.set_feature_enabled(Feature::Plugins, /*enabled*/ true);
        ...
        Ok(ConnectorsSnapshot {
            connectors: vec![
                ...
                AppInfo {
                    ...
                    app_metadata: None,
                    labels: None,
                    install_url: Some("https://example.test/hidden-odysseythink".to_string()),
                    is_accessible: true,
                    is_enabled: true,
                    plugin_display_names: Vec::new(),
                },
...
async fn server_overloaded_error_does_not_switch_models() {
    ...
    handle_error(
        &mut chat,
        "server overloaded",
        Some(OdyErrorInfo::ServerOverloaded),
    );
...
async fn feedback_upload_consent_popup_snapshot() {
    ...
    chat.show_selection_view(crate::bottom_pane::feedback_upload_consent_params(
        ...
        &ody_feedback::FeedbackDiagnostics::new(vec![ody_feedback::FeedbackDiagnostic {

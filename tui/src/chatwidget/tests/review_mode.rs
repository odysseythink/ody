...
async fn item_completed_pops_pending_steer_with_local_image_and_text_elements() {
    let (mut chat, mut rx, mut op_rx) = make_chatwidget_manual(Some("k3")).await;
    chat.thread_id = Some(ThreadId::new());
    chat.on_task_started();
...
async fn custom_prompt_submit_sends_review_op() {
    ...
    // Expect AppEvent::OdyOp(Op::Review { .. }) with trimmed prompt
    let evt = rx.try_recv().expect("expected one app event");
    match evt {
        AppEvent::OdyOp(Op::Review { target }) => {
            assert_eq!(
                target,
                ReviewTarget::Custom {
...
async fn custom_prompt_enter_empty_does_not_send() {
    ...
    // No AppEvent::OdyOp should be sent
...
async fn interrupted_turn_after_goal_budget_limited_uses_budget_message_snapshot() {
    ...
    chat.handle_server_notification(
        ody_app_server_protocol::ServerNotification::TurnStarted(
            ody_app_server_protocol::TurnStartedNotification {
                thread_id: "thread-1".to_string(),
                turn: ody_app_server_protocol::Turn {
                    id: "turn-1".to_string(),
                    items_view: ody_app_server_protocol::TurnItemsView::Full,
                    items: Vec::new(),
                    status: ody_app_server_protocol::TurnStatus::InProgress,
                    error: None,
                    started_at: None,
                    completed_at: None,
                    duration_ms: None,
    ...
    chat.handle_server_notification(
        ody_app_server_protocol::ServerNotification::ThreadGoalUpdated(
            ody_app_server_protocol::ThreadGoalUpdatedNotification {
                ...
                goal: ody_app_server_protocol::ThreadGoal {
                    ...
                    status: ody_app_server_protocol::ThreadGoalStatus::BudgetLimited,
        ...
        ody_app_server_protocol::ServerNotification::TurnCompleted(
            ody_app_server_protocol::TurnCompletedNotification {
                ...
                turn: ody_app_server_protocol::Turn {
                    ...
                    items_view: ody_app_server_protocol::TurnItemsView::Full,
                    items: Vec::new(),
                    status: ody_app_server_protocol::TurnStatus::Interrupted,
                    error: None,
                    started_at: None,
                    completed_at: None,
                    duration_ms: None,

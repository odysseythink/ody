...
pub(crate) fn load_local_chatgpt_auth(
    ...
    if matches!(auth.auth_mode, Some(AuthMode::ApiKey)) || auth.odysseythink_api_key.is_some() {
...
mod tests {
    ...
    fn write_chatgpt_auth(ody_home: &Path, plan_type: &str) {
        ...
        let auth = AuthDotJson {
            ...
            tokens: Some(TokenData {
                id_token: ody_login::token_data::parse_chatgpt_jwt_claims(&id_token)
    ...
    fn loads_local_chatgpt_auth_from_managed_auth() {
        ...
        write_chatgpt_auth(ody_home.path(), "business");
    ...
    fn preserves_usage_based_plan_type_wire_name() {
        let ody_home = TempDir::new().expect("tempdir");
        write_chatgpt_auth(ody_home.path(), "self_serve_business_usage_based");

...
// The standalone `ody-tui` binary prints a short help message before the
...
use ody_state::log_db;
use ody_utils_absolute_path::AbsolutePathBuf;
use ody_utils_absolute_path::canonicalize_existing_preserving_symlinks;
use ody_utils_home_dir::find_ody_home;
use ody_utils_oss::ensure_oss_provider_ready;
use ody_utils_oss::get_default_model_for_oss_provider;
...
pub(crate) use ody_app_server_client::legacy_core;
...
const TUI_LOG_FILE_NAME: &str = "ody-tui.log";
...
async fn init_state_db_for_app_server_target(
    ...
    match app_server_target {
        AppServerTarget::Embedded => state_db::try_init(config).await.map(Some).map_err(|err| {
            let database_path = ody_state::runtime_db_path_for_corruption_error(&err)
                .unwrap_or_else(|| ody_state::state_db_path(config.sqlite_home.as_path()));
...
fn remove_legacy_tui_log_file(ody_home: &Path) {
    ...
    let _ = std::fs::remove_file(ody_home.join("log").join(TUI_LOG_FILE_NAME));
}
...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            let ody_home = find_ody_home().wrap_err("failed to resolve ODY_HOME")?;
            ody_app_server_client::app_server_control_socket_path(&ody_home)
...
#[cfg(unix)]
async fn maybe_probe_default_daemon_socket(ody_home: &Path) -> Option<AbsolutePathBuf> {
    let socket_path = ody_app_server_client::app_server_control_socket_path(ody_home).ok()?;
...
#[cfg(not(unix))]
async fn maybe_probe_default_daemon_socket(_ody_home: &Path) -> Option<AbsolutePathBuf> {
    None
}
...
pub async fn run_main(
    ...
    #[allow(clippy::print_stderr)]
    let ody_home = match find_ody_home() {
        Ok(ody_home) => ody_home.to_path_buf(),
        Err(err) => {
            eprintln!("Error finding ody home: {err}");
            std::process::exit(1);
        }
    };
    ...
    if let Some(profile_v2) = cli.config_profile_v2.as_ref() {
        let user_config_path = resolve_profile_v2_config_path(&ody_home, profile_v2);
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        maybe_probe_default_daemon_socket(&ody_home).await
    } else {
        None
    };
    ...
    let environment_manager =
        if should_load_configured_environments(&loader_overrides, &app_server_target) {
            EnvironmentManager::from_ody_home(ody_home.clone(), Some(local_runtime_paths)).await
        } else {
    ...
    remove_legacy_tui_log_file(config.ody_home.as_path());

    let otel_originator = originator().value;
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ody_app_server_client::build_otel_provider(
            &config,
    ...
    if let Some(metrics) = otel.as_ref().and_then(ody_otel::OtelProvider::metrics) {
        let _ = ody_otel::record_process_start_once(metrics, otel_originator.as_str());
        let telemetry =
            ody_rollout::sqlite_telemetry_recorder(metrics.clone(), otel_originator.as_str());
    ...
    match effective_toml.try_into() {
        Ok(config_toml) => {
            match ody_app_server_client::migrate_personality_if_needed(
                &config.ody_home,
                &config_toml,
                state_db.clone(),
            )
...
async fn run_ratatui_app(
    ...
    let login_status = if initial_config.model_provider.requires_odysseythink_auth {
    ...
    let config = if should_show_onboarding {
        ...
        if show_login_screen && !uses_remote_workspace {
            cloud_config_bundle = cloud_config_bundle_loader_for_storage(
                initial_config.ody_home.to_path_buf(),
                /*enable_ody_api_key_env*/ false,
    ...
    let mut missing_session_exit = |id_str: &str, action: &str| {
        ...
        Ok(AppExitInfo {
            ...
            exit_reason: ExitReason::Fatal(format!(
                "No saved session found with ID {id_str}. Run `ody {action}` without an ID to choose from existing sessions."
            )),
        })
    };
    ...
    if let Some(w) = crate::render::highlight::set_theme_override(
        ...
        find_ody_home().ok().map(AbsolutePathBuf::into_path_buf),
    ) {
    ...
    let required_elevated_sandbox_needs_setup = windows_sandbox_level
        ...
        && config
            ...
            .source
            .is_some()
        && !crate::windows_sandbox::sandbox_setup_is_complete(config.ody_home.as_path());
    #[cfg(target_os = "windows")]
...
async fn get_login_status(
    ...
    if !config.model_provider.requires_odysseythink_auth {
...
async fn load_bootstrap_config_or_exit(
    ...
    cli_kv_overrides: Vec<(String, ody_config::TomlValue)>,
    loader_overrides: LoaderOverrides,
    strict_config: bool,
    cloud_config_bundle: CloudConfigBundleLoader,
...
mod tests {
    ...
    fn write_session_rollout(
        ...
        let session_meta = ody_protocol::protocol::SessionMeta {
            session_id: thread_id.into(),
            id: thread_id,
            timestamp: meta_rfc3339.to_string(),
            cwd: cwd.to_path_buf(),
            originator: "ody".to_string(),
            cli_version: "0.0.0".to_string(),
            source: ody_protocol::protocol::SessionSource::Cli,
            model_provider: Some(model_provider.to_string()),
            ..Default::default()
        };
        let session_meta = serde_json::to_value(ody_protocol::protocol::SessionMetaLine {
            meta: session_meta,
            git: None,
        })?;
        let lines = [
    ...
    fn resolve_remote_addr_accepts_default_socket() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: ody_app_server_client::app_server_control_socket_path(&ody_home)?,
            }
        );
        Ok(())
    }
    ...
    fn resolve_remote_addr_accepts_relative_socket_path() -> color_eyre::Result<()> {
        assert_eq!(
            resolve_remote_addr("unix://ody.sock")?,
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: AbsolutePathBuf::relative_to_current_dir("ody.sock")?,
            }
        );
        Ok(())
    }
    ...
    fn resolve_remote_addr_accepts_absolute_socket_path() -> color_eyre::Result<()> {
        let temp_dir = TempDir::new()?;
        let socket_path = temp_dir.path().join("ody.sock");
        assert_eq!(
    ...
    async fn default_daemon_auto_connect_skips_missing_socket() -> color_eyre::Result<()> {
        ...
        assert!(
            maybe_probe_default_daemon_socket(ody_home.path())
                .await
                .is_none()
        );
        Ok(())
    ...
    async fn default_daemon_auto_connect_probes_socket_only() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            maybe_probe_default_daemon_socket(ody_home.path()).await,
            Some(socket_path)
        );
        Ok(())
    }
    ...
    {
        let socket_path = AbsolutePathBuf::relative_to_current_dir("ody.sock")?;
    ...
    async fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        ...
        let mut app_server = AppServerSession::new(
            ody_app_server_client::AppServerClient::InProcess(
    ...
    async fn lookup_session_target_by_name_uses_backend_title_search() -> color_eyre::Result<()> {
        Box::pin(async {
            ...
            let state_runtime = ody_state::StateRuntime::init(
    ...
    async fn embedded_state_db_failure_is_typed_for_cli_recovery() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            ody_state::state_db_path(occupied_sqlite_home.as_path()).as_path()
        );
        assert!(
            startup_error
                .detail()
    ...
    -> color_eyre::Result<()> {
        ...
        let logs_db_path = ody_state::logs_db_path(&sqlite_home);
        ...
        assert!(
            ody_state::sqlite_error_detail_is_corruption(startup_error.detail()),
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let ody_home = temp_dir.path().to_path_buf();

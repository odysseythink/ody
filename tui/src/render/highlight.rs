...
//! | `ODY_HOME` | `OnceLock<Option<PathBuf>>` | Root for custom `.tmTheme` discovery |
//!
...
static ODY_HOME: OnceLock<Option<PathBuf>> = OnceLock::new();
...
/// Set the user-configured syntax theme override and ody home path.
///
/// Call this with the **final resolved config** (after onboarding, resume, and
/// fork reloads complete). The first call persists `name` and `ody_home` in
...
pub(crate) fn set_theme_override(
    ...
    let warning = validate_theme_name(name.as_deref(), ody_home.as_deref());
    let override_set_ok = THEME_OVERRIDE.set(name.clone()).is_ok();
    let ody_home_set_ok = ODY_HOME.set(ody_home.clone()).is_ok();
    ...
    if !override_set_ok || !ody_home_set_ok {
...
pub(crate) fn validate_theme_name(name: Option<&str>, ody_home: Option<&Path>) -> Option<String> {
    let name = name?;
    let custom_theme_path_display = ody_home
        .map(|home| custom_theme_path(name, home).display().to_string())
        .unwrap_or_else(|| format!("$ODY_HOME/themes/{name}.tmTheme"));
...
fn custom_theme_path(name: &str, ody_home: &Path) -> PathBuf {
    ody_home.join("themes").join(format!("{name}.tmTheme"))
}

/// Try to load a custom `.tmTheme` file from `{ody_home}/themes/{name}.tmTheme`.
fn load_custom_theme(name: &str, ody_home: &Path) -> Option<Theme> {
    ThemeSet::get_theme(custom_theme_path(name, ody_home)).ok()
}
...
fn resolve_theme_with_override(name: Option<&str>, ody_home: Option<&Path>) -> Theme {
    ...
    if let Some(name) = name {
        ...
        // 2. Try loading {ODY_HOME}/themes/{name}.tmTheme from disk.
        if let Some(home) = ody_home
...
pub(crate) fn configured_theme_name() -> String {
    ...
    if let Some(Some(name)) = THEME_OVERRIDE.get() {
        ...
        if let Some(Some(home)) = ODY_HOME.get()
...
pub(crate) fn resolve_theme_by_name(name: &str, ody_home: Option<&Path>) -> Option<Theme> {
    let ts = two_face::theme::extra();
    // Bundled theme?
...
/// `.tmTheme` file under `{ODY_HOME}/themes/`.
...
/// found in `{ody_home}/themes/`.
pub(crate) fn list_available_themes(ody_home: Option<&Path>) -> Vec<ThemeEntry> {
...
<key>background</key><string>{deleted_background}</string>
    ...
    fn unique_foreground_colors_for_theme(theme_name: &str) -> Vec<String> {
        let theme = resolve_theme_by_name(theme_name, /*ody_home*/ None)
    ...
    fn bundled_theme_can_provide_diff_scope_backgrounds() {
        let theme = resolve_theme_by_name("github", /*ody_home*/ None)
    ...
    fn validate_theme_name_none_for_bundled() {
        ...
        assert!(validate_theme_name(Some("dracula"), /*ody_home*/ None).is_none());
        assert!(validate_theme_name(Some("nord"), Some(Path::new("/nonexistent"))).is_none());
    }

    #[test]
    fn validate_theme_name_none_when_no_override() {
        assert!(validate_theme_name(/*name*/ None, /*ody_home*/ None).is_none());
    }

            ...
            ThreadItem::UserMessage { content, .. } => Some(TranscriptPreviewLine {
                ...
                text: content
                    ...
                    .filter_map(|input| match input {
                        ody_app_server_protocol::UserInput::Text { text, .. } => {
                            Some(text.as_str())
                        }
                        _ => None,
                    })
...
impl PickerState {
    ...
    async fn persist_density(&self) -> Result<()> {
        ...
        ConfigEditsBuilder::new(&persistence.ody_home)
...
mod tests {
    ...
    fn row_search_matches_metadata_fields() {
        ...
        let row = Row {
            ...
            created_at: None,
            updated_at: None,
            cwd: Some(PathBuf::from("/tmp/ody-session-picker")),
    ...
    fn expanded_session_details_include_metadata() {
        ...
        let row = Row {
            ...
            cwd: Some(PathBuf::from("/Users/felipe.coury/code/ody")),
            git_branch: Some(String::from("ody/raw-scrollback-mode")),
        };
        ...
        assert!(rendered.contains("Branch:      ody/raw-scrollback-mode"));
    ...
    fn footer_prioritizes_active_sort_timestamp() {
        ...
        assert_metadata_order(&updated[0], "⌁ tmp/ody", " main");
        assert_metadata_order(&created[0], "⌁ tmp/ody", " main");
    }
    ...
    fn footer_marks_missing_branch() {
        ...
        assert!(rendered.contains("⌁ /tmp/ody"));
        assert!(rendered.contains(" no branch"));
        assert_metadata_order(&footer[0], "⌁ /tmp/ody", " no branch");
    }
    ...
    fn footer_cwd_truncates_to_responsive_column() {
        ...
        assert!(footer.contains("⌁ ~/code/ody."));
...
"#
    ...
    async fn ctrl_o_keeps_toggled_density_when_persistence_fails() {
        let tmp = tempdir().expect("tmpdir");
        let ody_home_file = tmp.path().join("ody-home-file");
        std::fs::write(&ody_home_file, "not a directory").expect("write ody home file");
        let loader = page_only_loader(|_| {});
        let mut state = PickerState::new(
            FrameRequester::test_dummy(),
            loader,

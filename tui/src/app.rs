...
fn resume_hint_for_resumable_thread(
    ...
    ody_utils_cli::resume_hint(thread.thread_name.as_deref(), Some(thread.thread_id))
}
...
pub(crate) struct App {
    ...
    /// Whether the active thread has older paginated history that can refill
    /// underfilled terminal scrollback. Set when initial resume replay finishes
    /// and after each older page is applied.
...
fn active_turn_not_steerable_turn_error(error: &TypedRequestError) -> Option<AppServerTurnError> {
    ...
    matches!(
        turn_error.ody_error_info,
        Some(AppServerOdyErrorInfo::ActiveTurnNotSteerable { .. })
    )
...
fn archived_session_guidance(err: &color_eyre::eyre::Report) -> Option<String> {
    ...
    if !message.contains(" is archived. Run `ody unarchive ") {
        return None;
    }
    let message = message
        .split_once(" (code ")
...
impl App {
    ...
    pub async fn run(
        ...
        {
            session_telemetry.counter("ody.status_line", /*inc*/ 1, &[]);
        }
...
See the Ody keymap documentation for supported actions and examples."
            )
        })?;
        #[cfg(not(debug_assertions))]
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.ody_home.clone();

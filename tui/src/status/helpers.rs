...
mod tests {
    ...
    async fn test_config(ody_home: &TempDir, cwd: &TempDir) -> Config {
    ...
    async fn compose_agents_summary_includes_global_agents_path() {
        let ody_home = TempDir::new().expect("temp ody home");
        let cwd = TempDir::new().expect("temp cwd");
        let global_agents_path = ody_home.path().join("global.md");
        let config = test_config(&ody_home, &cwd).await;
    ...
    async fn compose_agents_summary_names_global_agents_override() {
        let ody_home = TempDir::new().expect("temp ody home");
        let cwd = TempDir::new().expect("temp cwd");
        let override_path = ody_home.path().join("override.md");
        let config = test_config(&ody_home, &cwd).await;

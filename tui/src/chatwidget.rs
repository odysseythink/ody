...
use ody_app_server_protocol::ThreadGoal as AppThreadGoal;
use ody_app_server_protocol::ThreadGoalStatus as AppThreadGoalStatus;
...
#[cfg(test)]
use ody_git_utils::CommitLogEntry;
...
use ody_protocol::approvals::GuardianAssessmentStatus;
...
use ody_protocol::plan_tool::PlanItemArg as UpdatePlanItemArg;
use ody_protocol::plan_tool::StepStatus as UpdatePlanItemStatus;
...
use ody_utils_cli::resume_hint;
use ody_utils_path_uri::PathUri;
use ody_utils_plugins::mention_syntax::PLUGIN_TEXT_MENTION_SIGIL;
use ody_utils_plugins::mention_syntax::TOOL_MENTION_SIGIL;
use rand::Rng;
...
use ody_utils_approval_presets::builtin_approval_presets;
...
const DEFAULT_OPENAI_BASE_URL: &str = "https://api.odysseythink.com/v1";
...
/// and by sending requests back to ody-core.
///
...
fn request_permissions_from_params(
    params: ody_app_server_protocol::PermissionsRequestApprovalParams,
...
impl ChatWidget {
                ...
                .filter_map(|item| match item {
                    ...
                    UserInput::Mention { name, path } => {
                        ...
                        let mention = if let Some(plugin_id) = plugin_id {
                            ...
                            ody_connectors::metadata::connector_mention_slug_from_name(name)
                        } else {
                            name.clone()
                        };
    ...
    /// Forward a command directly to ody.
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.ody_op_target {
            OdyOpTarget::Direct(ody_op_tx) => {
                crate::session_log::log_outbound_op(&op);
                if let Err(e) = ody_op_tx.send(op) {
                    tracing::error!("failed to submit op: {e}");
                    return false;
                }
            }
            OdyOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::OdyOp(op));
            }
        }
        true
    }

...
//! in-memory `Pet`: built-in catalog ids, custom pet ids under ODY_HOME,
...
impl Pet {
    ...
    /// has already been materialized into ODY_HOME; if callers skip the
    /// asset-fetch step, they will get a missing-spritesheet error here on
    /// first use.
    pub(super) fn load_with_ody_home(value: &str, ody_home: Option<&Path>) -> Result<Self> {
        if path_like(value) {
            return load_pet_path(value);
        }

        if let Some(custom_id) = value.strip_prefix(CUSTOM_PET_PREFIX) {
            return load_custom_pet(custom_id, ody_home);
        }

        if let Some(builtin) = catalog::builtin_pet(value) {
            return load_builtin_pet(builtin, ody_home);
        }

        load_custom_pet(value, ody_home)
    }
...
fn load_builtin_pet(pet: catalog::BuiltinPet, ody_home: Option<&Path>) -> Result<Pet> {
    let ody_home = ody_home.context("ODY_HOME is not available")?;
    let spritesheet_path = super::builtin_spritesheet_path(ody_home, pet.spritesheet_file);
...
fn load_custom_pet(value: &str, ody_home: Option<&Path>) -> Result<Pet> {
    let ody_home = ody_home.context("ODY_HOME is not available")?;
    let pet_dir = ody_home.join("pets").join(value);
    if pet_dir.join("pet.json").is_file() {
        return load_pet_manifest(&pet_dir, "pet.json", value, &custom_pet_cache_id(value));
    }

    let avatar_dir = ody_home.join("avatars").join(value);
...
mod tests {
    ...
    fn load_pet_from_dir(dir: &tempfile::TempDir) -> Pet {
        Pet::load_with_ody_home(dir.path().to_str().unwrap(), /*ody_home*/ None).unwrap()
    }

    fn load_pet_error_from_dir(dir: &tempfile::TempDir) -> anyhow::Error {
        Pet::load_with_ody_home(dir.path().to_str().unwrap(), /*ody_home*/ None).unwrap_err()
    }
    ...
    fn load_builtin_pet_uses_app_catalog_storage() {
        ...
        assert_eq!(
            ...
            super::super::builtin_spritesheet_path(ody_home.path(), "dewey-spritesheet-v4.webp")
        );
    ...
    fn load_pet_json_path_uses_containing_directory() {
        ...
        let pet = Pet::load_with_ody_home(
    ...
    #[test]
    fn custom_pet_selector_loads_ody_home_pet_manifest() {
        let dir = write_minimal_pet();
        let ody_home = tempfile::tempdir().unwrap();
        let pet_dir = ody_home.path().join("pets").join("chefito");
    ...
    fn custom_pet_selector_falls_back_to_legacy_avatar_manifest() {
        let dir = write_minimal_pet();
        let ody_home = tempfile::tempdir().unwrap();
        let avatar_dir = ody_home.path().join("avatars").join("legacy");
    ...
    fn custom_pet_rejects_spritesheet_path_escape() {
        let ody_home = tempfile::tempdir().unwrap();
        let pet_dir = ody_home.path().join("pets").join("escape");
        fs::create_dir_all(&pet_dir).unwrap();
        fs::write(
        ...
        let err = Pet::load_with_ody_home(

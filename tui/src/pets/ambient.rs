//! Ambient terminal rendering for the Ody companion.
//!
...
impl AmbientPet {
    ...
    /// ODY_HOME cache, and records the terminal protocol support snapshot used
    ...
    pub(crate) fn load(
        ...
        let pet = Pet::load_with_ody_home(
            ...
            /*ody_home*/ Some(ody_home),
        )
        .with_context(|| "load ambient pet")?;
        let cache_dir = ody_home
            .join("cache")
            .join("tui-pets")
            .join("frame-cache")
            .join(&pet.id)
...
mod tests {
    ...
    #[test]
    fn notification_labels_match_ody_app_vocabulary() {

...
#[ignore = "requires tmux and a locally built ody binary; run with --ignored for manual resize smoke"]
async fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let session_name = format!("ody-resize-reflow-smoke-{}", std::process::id());
    ...
    let start_output = checked_output(
        Command::new("tmux")
            ...
            .arg("--")
            .arg("env")
            .arg(format!("ODY_HOME={}", ody_home.path().display()))
    ...
    let ody_pane = stdout_text(&start_output).trim().to_string();
    anyhow::ensure!(!ody_pane.is_empty(), "tmux did not report a pane id");
    ...
    let baseline_capture =
        wait_for_capture_contains(&ody_pane, draft, Duration::from_secs(/*secs*/ 15))?;
    ...
    let final_capture = capture_pane(&ody_pane)?;
    let final_row =
...
async fn tmux_width_resize_restore_keeps_visible_content_anchored() -> Result<()> {
    ...
    let session_name = format!("ody-resize-width-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    sleep(Duration::from_millis(/*millis*/ 1_000));
    let restored_capture = capture_pane(&ody_pane)?;
    let restored_row =
...
async fn run_repeated_resize_smoke() -> Result<()> {
    ...
    let session_name = format!("ody-resize-repeat-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
...
fn ody_binary(repo_root: &Path) -> Result<PathBuf> {
...
"#
    ...
    std::fs::write(ody_home.join("config.toml"), config)?;
    Ok(())
}

fn write_auth(ody_home: &Path) -> Result<()> {
    std::fs::write(

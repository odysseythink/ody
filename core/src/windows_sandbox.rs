...
pub fn elevated_setup_failure_details(err: &anyhow::Error) -> Option<(String, String)> {
    ...
    let message = ody_windows_sandbox::sanitize_setup_metric_tag_value(&failure.message);
    Some((code, message))
}
...
async fn run_windows_sandbox_setup_and_persist(
    ...
    let ody_home = request.ody_home;
    let setup_ody_home = ody_home.clone();

    let setup_result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
        match mode {
            WindowsSandboxSetupMode::Elevated => {
                if !sandbox_setup_is_complete(setup_ody_home.as_path()) {
                    run_elevated_setup(
                        &permission_profile,
                        workspace_roots.as_slice(),
                        command_cwd.as_path(),
                        &env_map,
                        setup_ody_home.as_path(),
                    )?;
                }
            }
    ...
    ConfigEditsBuilder::new(ody_home.as_path())
...
fn emit_windows_sandbox_setup_success_metrics(
    ...
    let _ = metrics.record_duration(
        "ody.windows_sandbox.setup_duration_ms",
        duration,
        &[
            ("result", "success"),
    ...
    let _ = metrics.counter(
        "ody.windows_sandbox.setup_success",
        /*inc*/ 1,
...
fn emit_windows_sandbox_setup_failure_metrics(
    ...
    let _ = metrics.counter(
        "ody.windows_sandbox.setup_failure",
        /*inc*/ 1,
        &[("originator", originator_tag), ("mode", mode_tag)],
    );

...
fn strip_hidden_assistant_markup_and_parse_memory_citation(
    ...
    String,
    Option<ody_protocol::memory_citation::MemoryCitation>,
) {
        ...
        && role == "assistant"
    {
        let combined = content
            ...
            .filter_map(|ci| match ci {
                ody_protocol::models::ContentItem::OutputText { text } => Some(text.as_str()),
                _ => None,
            })
            .collect::<String>();
...
async fn save_image_generation_result(
    ...
    let bytes = BASE64_STANDARD
        ...
        .map_err(|err| {
            OdyErr::InvalidRequest(format!("invalid image generation payload: {err}"))
        })?;
    let path = image_generation_artifact_path(ody_home, session_id, call_id);
    ...
    .await
    {
        ...
        Err(err) => {
            ...
            let output_dir = output_path
                .parent()
                .unwrap_or_else(|| turn_context.config.ody_home.clone());
            tracing::warn!(
...
async fn record_image_generation_instructions(
    ...
    let image_output_path =
        image_generation_artifact_path(&turn_context.config.ody_home, &session_id, "<image_id>");
...
pub(crate) async fn finalize_turn_item(
    ...
    if let TurnItem::AgentMessage(agent_message) = &mut *turn_item {
        ...
        agent_message.content =
            vec![ody_protocol::items::AgentMessageContent::Text { text: stripped }];

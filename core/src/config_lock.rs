...
pub(crate) struct ConfigLockReplayOptions {
    pub allow_ody_version_mismatch: bool,
}
...
pub(crate) fn validate_config_lock_replay(
    ...
    if !options.allow_ody_version_mismatch && expected_lock.ody_version != actual_lock.ody_version {
        return Err(config_lock_error(format!(
            "config lock Ody version mismatch: lock was generated by {}, current version is {}; set debug.config_lockfile.allow_ody_version_mismatch=true to ignore this",
            expected_lock.ody_version, actual_lock.ody_version
        )));
    }
...
fn config_lock_for_comparison(
    ...
    if options.allow_ody_version_mismatch {
        lockfile.ody_version.clear();
    }
    lockfile
}

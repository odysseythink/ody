...
use ody_otel::TURN_MEMORY_METRIC;
use ody_otel::TURN_NETWORK_PROXY_METRIC;
use ody_otel::TURN_TOKEN_USAGE_METRIC;
use ody_otel::TURN_TOOL_CALL_METRIC;
...
const TASK_COMPACT_METRIC: &str = "ody.task.compact";

pub(crate) type SessionTaskResult = OdyResult<Option<String>>;
...
///
/// Implementations encapsulate a specific Ody workflow (regular chat,
...
pub(crate) trait SessionTask: Send + Sync + 'static {
    ...
    /// [`OdyErr::TurnAborted`] completes the task through the aborted-turn
    /// lifecycle instead.
    fn run(
        self: Arc<Self>,
...
impl Session {
    ...
    pub(crate) async fn start_task<T: SessionTask>(
        ...
        let task_span = info_span!(
            ...
            ody.turn.reasoning_effort = %reasoning_effort,
            ody.turn.token_usage.input_tokens = field::Empty,
            ody.turn.token_usage.cached_input_tokens = field::Empty,
            ody.turn.token_usage.non_cached_input_tokens = field::Empty,
            ody.turn.token_usage.output_tokens = field::Empty,
            ody.turn.token_usage.reasoning_output_tokens = field::Empty,
            ody.turn.token_usage.total_tokens = field::Empty,
        );
                ...
                if let Err(err) = sess.flush_rollout().await {
                    ...
                    sess.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                            message: format!(
                                "Failed to save the conversation transcript; Ody will continue retrying. Error: {err}"
                            ),
                        }),
                    )
                    .await;
    ...
    pub async fn on_task_finished(
        ...
        let (last_agent_message, abort_reason) = match task_result {
            ...
            Err(OdyErr::TurnAborted) => (None, Some(TurnAbortReason::Interrupted)),
            Err(err) => {
        ...
        // Emit token usage metrics.
        {
            ...
            current_span.record(
                "ody.turn.token_usage.input_tokens",
                turn_token_usage.input_tokens,
            );
            current_span.record(
                "ody.turn.token_usage.cached_input_tokens",
                turn_token_usage.cached_input(),
            );
            current_span.record(
                "ody.turn.token_usage.non_cached_input_tokens",
                turn_token_usage.non_cached_input(),
            );
            current_span.record(
                "ody.turn.token_usage.output_tokens",
                turn_token_usage.output_tokens,
            );
            current_span.record(
                "ody.turn.token_usage.reasoning_output_tokens",
                turn_token_usage.reasoning_output_tokens,
            );
            current_span.record(
                "ody.turn.token_usage.total_tokens",

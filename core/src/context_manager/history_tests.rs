...
use image::Luma;
use image::Rgba;
use ody_api::chat::ChatCompletionsRequest;
use ody_api::chat::ChatVendor;
...
/// Reproduction test that exercises both `ContextManager` history normalization
/// and `ody_api::chat::ChatCompletionsRequest::to_wire`, emitting the added
/// tracing logs so they can be captured.
#[test]
fn repro_history_to_chat_messages_emits_logs() {
    ...
    history.record_items([&user, &assistant, &tool_call, &tool_output], policy);
    let items = history.for_prompt(&default_input_modalities());
    ...
    // After merging the assistant content message and the following
    // assistant tool_calls message, we expect:

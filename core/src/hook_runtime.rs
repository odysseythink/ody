...
use ody_hooks::PostToolUseOutcome;
use ody_hooks::PostToolUseRequest;
use ody_hooks::PreToolUseOutcome;
use ody_hooks::PreToolUseRequest;
use ody_hooks::SessionStartOutcome;
use ody_hooks::StartHookTarget;
use ody_hooks::StopHookTarget;
use ody_hooks::StopOutcome;
use ody_hooks::SubagentHookContext;
use ody_hooks::UserPromptSubmitOutcome;
use ody_hooks::UserPromptSubmitRequest;
use ody_otel::HOOK_RUN_DURATION_METRIC;
use ody_otel::HOOK_RUN_METRIC;
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = ody_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = ody_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_legacy_after_agent_hook(
    ...
    for hook_outcome in hooks
        .dispatch(ody_hooks::HookPayload {
            session_id: sess.session_id().into(),
            #[allow(deprecated)]
            cwd: turn_context.cwd.clone(),
            client: turn_context.app_server_client_name.clone(),
            triggered_at: chrono::Utc::now(),
            hook_event: ody_hooks::HookEvent::AfterAgent {
                event: ody_hooks::HookEventAfterAgent {
        ...
        .await
    {
        ...
        let (error, should_abort) = match hook_outcome.result {
            ody_hooks::HookResult::Success => continue,
            ody_hooks::HookResult::FailedContinue(error) => (error, false),
            ody_hooks::HookResult::FailedAbort(error) => (error, true),
        };
    ...
    let event = EventMsg::Error(ody_protocol::protocol::ErrorEvent {
        message,
...
fn hook_run_analytics_payload(
    ...
    completed: &HookCompletedEvent,
) -> (ody_analytics::TrackEventsContext, HookRunFact) {
    (
...
mod tests {
    ...
    use ody_protocol::protocol::HookEventName;
    use ody_protocol::protocol::HookExecutionMode;
    use ody_protocol::protocol::HookHandlerType;
    use ody_protocol::protocol::HookRunStatus;
    use ody_protocol::protocol::HookScope;
    use ody_protocol::protocol::HookSource;
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    ody_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {

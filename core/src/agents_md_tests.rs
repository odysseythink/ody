...
use ody_exec_server::ExecutorFileSystemFuture;
use ody_exec_server::FileMetadata;
use ody_exec_server::FileSystemReadStream;
use ody_exec_server::FileSystemSandboxContext;
use ody_exec_server::LOCAL_FS;
use ody_exec_server::ReadDirectoryEntry;
...
fn foreign_agents_md_uses_environment_native_paths() {
    let (cwd, rendered_cwd) = if cfg!(windows) {
        (
            PathUri::parse("file:///ody%20runtime").expect("POSIX cwd URI"),
            "/ody runtime",
        )
    } else {
        (
            PathUri::parse("file:///C:/ody%20runtime").expect("Windows cwd URI"),
            r"C:\ody runtime",
        )
    };
...
async fn make_config(root: &TempDir, limit: usize, instructions: Option<&str>) -> TestConfig {
    ...
    let user_instructions = instructions.map(|text| UserInstructions {
        text: text.to_owned(),
        source: config.ody_home.join(DEFAULT_AGENTS_MD_FILENAME),
    });
    TestConfig {
        config,
        user_instructions,
...
async fn project_root_markers_are_honored_for_agents_discovery() {
    ...
    fs::write(root.path().join(".ody-root"), "").unwrap();
...
async fn project_layers_do_not_override_project_root_markers() {
    ...
    config.cwd = nested.abs();
    let project_layer = |dot_ody_folder: AbsolutePathBuf, marker: &str| {
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![
            project_layer(root.path().join(".ody").abs(), ".ignored-root-marker"),
            project_layer(config.cwd.join(".ody"), ".ignored-nested-marker"),
        ],
...
async fn instruction_sources_include_global_before_agents_md_docs() {
    ...
    let global_agents = cfg.ody_home.join(DEFAULT_AGENTS_MD_FILENAME);
    fs::create_dir_all(&cfg.ody_home).unwrap();
...
fn create_skill(ody_home: PathBuf, name: &str, description: &str) {
    let skill_dir = ody_home.join(format!("skills/{name}"));

...
use ody_config::permissions_toml::NetworkDomainPermissionToml;
use ody_config::permissions_toml::NetworkDomainPermissionsToml;
use ody_config::permissions_toml::NetworkMitmActionToml;
use ody_config::permissions_toml::NetworkMitmHookToml;
use ody_config::permissions_toml::NetworkMitmToml;
...
use ody_config::types::FeedbackConfigToml;
use ody_config::types::HistoryPersistence;
use ody_config::types::McpServerEnvVar;
use ody_config::types::McpServerOAuthConfig;
use ody_config::types::McpServerToolConfig;
use ody_config::types::McpServerTransportConfig;
use ody_config::types::MemoriesConfig;
use ody_config::types::MemoriesToml;
use ody_config::types::ModelAvailabilityNuxConfig;
use ody_config::types::Notice;
use ody_config::types::NotificationCondition;
use ody_config::types::NotificationMethod;
use ody_config::types::Notifications;
use ody_config::types::OtelConfigToml;
...
use ody_config::types::Tui;
use ody_config::types::TuiKeymap;
use ody_config::types::TuiNotificationSettings;
use ody_config::types::TuiPetAnchor;
use ody_config::types::WindowsSandboxModeToml;
use ody_config::types::WindowsToml;
...
use ody_protocol::protocol::NetworkAccess;
...
[features.code_mode]
enabled = true
excluded_tool_namespaces = ["mcp__ody_apps", "multi_agent_v1"]
...
"#,
    ...
    assert_eq!(
        ...
        vec!["mcp__ody_apps".to_string(), "multi_agent_v1".to_string()]
    );
    assert_eq!(
...
#[tokio::test]
async fn load_config_uses_ody_code_providers_and_default_model() -> std::io::Result<()> {
...
"#,
    ...
    .expect("ody-code config should deserialize");
...
#[tokio::test]
async fn load_config_ody_code_default_model_without_slash() -> std::io::Result<()> {
    let cfg = toml::from_str::<ConfigToml>(
        r#"
...
"odysseythink.com" = "allow"

[permissions.dev.network.mitm.hooks.github_write]
host = "api.github.com"
methods = ["POST", "PUT"]
path_prefixes = ["/repos/odysseythink/"]
action = ["strip_auth"]
                ...
                PermissionProfileToml {
                    ...
                    workspace_roots: Some(WorkspaceRootsToml {
                        entries: BTreeMap::from([
                            ...
                            ("~/code/odysseythink".to_string(), true),
                        ]),
                    }),
                        ...
                        mitm: Some(NetworkMitmToml {
                            hooks: Some(IndexMap::from([(
                                ...
                                NetworkMitmHookToml {
                                    ...
                                    path_prefixes: vec!["/repos/odysseythink/".to_string()],
                                    query: BTreeMap::new(),
...
"#;
    ...
    assert_eq!(
        ...
        vec!["/repos/odysseythink/".to_string()]
    );
    assert_eq!(
...
-> std::io::Result<()> {
    ...
    let memories_root = ody_home.path().join("memories").abs();
    assert!(
        !config
            .permissions
            .file_system_sandbox_policy()
...
-> std::io::Result<()> {
    ...
    let ody_home = temp_dir.path().join("ody-home");
    ...
    for root in [&cwd, &runtime_root, &profile_root] {
        ...
        std::fs::create_dir_all(root.join(".ody"))?;
    }
                        ...
                        filesystem: Some(FilesystemPermissionsToml {
                            ...
                            entries: BTreeMap::from([(
                                ...
                                FilesystemPermissionToml::Scoped(BTreeMap::from([
                                    ...
                                    (".ody".to_string(), FileSystemAccessMode::Read),
                                ])),
                            )]),
                        }),
                        network: None,
    ...
    for root in [cwd_abs, runtime_root_abs, profile_root_abs.clone()] {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&root.join(".ody"), cwd.as_path()),
            "expected .ody carveout under {root:?}, policy: {policy:?}"
        );
    }
    assert_eq!(
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&cwd.path().join(".ody"), cwd.path()),
...
async fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of Ody and will be ignored."
        )),
        "{:?}",
...
-> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of Ody and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `dev` does not define any recognized filesystem entries for this version of Ody."
        )),
        "{:?}",
        config.startup_warnings
    );
...
fn filter_plugin_mcp_servers_by_allowlist_enforces_plugin_and_identity_rules() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            ody_config::PluginRequirementsToml {
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    let ody_home = TempDir::new()?;
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, ody_home.path());
    let project_dot_ody =
        AbsolutePathBuf::resolve_path_against_base("project/.ody", ody_home.path());
    ...
    let requirements_toml = ody_config::ConfigRequirementsToml {
        mcp_servers: Some(mcp_requirements.clone()),
        ..Default::default()
    };
    let requirements = ody_config::ConfigRequirements {
    ...
    let refreshed_layer_stack = ConfigLayerStack::new(
        vec![
            ConfigLayerEntry::new(
                ody_app_server_protocol::ConfigLayerSource::User {
            ...
            ConfigLayerEntry::new(
                ody_app_server_protocol::ConfigLayerSource::Project {
                    dot_ody_folder: project_dot_ody.clone(),
                },
                toml::toml! {
            ...
            ConfigLayerEntry::new(
                ody_app_server_protocol::ConfigLayerSource::LegacyManagedConfigTomlFromMdm,
                toml::toml! {
                    [mcp_servers.managed_overrides_session]
    ...
    let thread_layer_stack = ConfigLayerStack::new(
        vec![
            ...
            ConfigLayerEntry::new(
                ody_app_server_protocol::ConfigLayerSource::Project {
                    dot_ody_folder: project_dot_ody,
                },
                toml::toml! {
                    [mcp_servers.fresh_project]
            ...
            ConfigLayerEntry::new(
                ody_app_server_protocol::ConfigLayerSource::SessionFlags,
                toml::toml! {
                    [mcp_servers.session_overrides_user]
...
"#,
    ...
    assert_eq!(
        ...
        Some((
            ody_mcp::McpServerSource::SelectedPlugin(McpPluginAttribution::new(
                "selected-root".to_string(),
                "Selected Plugin".to_string(),
            )),
            selected,
...
async fn default_zsh_path_sets_runtime_zsh_path() -> std::io::Result<()> {
    let ody_home = TempDir::new()?;
    let default_zsh_path = ody_home.path().join("packaged-zsh");
...
#[tokio::test]
async fn sqlite_home_defaults_to_ody_home_for_workspace_write() -> std::io::Result<()> {
    let ody_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite_home, ody_home.path().to_path_buf());
...
-> std::io::Result<()> {
    let ody_home = TempDir::new()?;
    let memories_root = ody_home.path().join("memories");
    let writable_root = ody_home.path().join("writable").abs();
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            sandbox_workspace_write: Some(SandboxWorkspaceWrite {
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    ...
    let config_layer_stack = load_config_layers_state(
        ...
        &Vec::new(),
        overrides,
        &ody_config::NoopThreadConfigLoader,
    )
    .await?;
    let cfg =
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(ody_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    let ody_home = TempDir::new()?;
    let expected_cwd = LegacyAppPathString::from_path(ody_home.path());
    ...
    let loaded = load_global_mcp_servers(ody_home.path()).await?;
...
"#,
    ...
    let err = load_global_mcp_servers(ody_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/ody-mcp");
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/ody-mcp""#),
...
async fn replace_mcp_servers_streamable_http_serializes_oauth_resource() -> anyhow::Result<()> {
    ...
    let servers = BTreeMap::from([(
        ...
        McpServerConfig {
            ...
            oauth: Some(McpServerOAuthConfig {
                client_id: Some("eci-prd-pub-ody-123".to_string()),
            }),
    ...
    assert!(serialized.contains(r#"client_id = "eci-prd-pub-ody-123""#));
    ...
    assert_eq!(docs.oauth_client_id(), Some("eci-prd-pub-ody-123"));
...
fn profile_v2_config_path_resolves_validated_names() -> anyhow::Result<()> {
    ...
    assert_eq!(
        resolve_profile_v2_config_path(ody_home.path(), &profile_name),
        ody_home.path().join("work.config.toml").abs()
    );
    Ok(())
}
...
impl PrecedenceTestFixture {
    ...
    fn ody_home(&self) -> AbsolutePathBuf {
        self.ody_home.abs()
    }
}
...
async fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let ody_home = TempDir::new()?;
    let workspace = ody_home.path().join("workspace");
    ...
    let config =
        Config::load_from_base_config_with_overrides(cfg, overrides, ody_home.abs()).await?;
...
async fn load_config_uses_requirements_guardian_policy_config() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        ody_config::ConfigRequirementsToml {
...
async fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let ody_home = TempDir::new()?;
    let missing_path = ody_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let ody_home = TempDir::new()?;
    let role_config_path = ody_home.path().join("agents").join("researcher.toml");
...
"#,
    ...
    let config_layer_stack = ody_config::ConfigLayerStack::new(
        vec![ody_config::ConfigLayerEntry::new(
            ody_app_server_protocol::ConfigLayerSource::User {
                file: ody_home.path().join(CONFIG_TOML_FILE).abs(),
                profile: None,
            },
            layer_config,
        )],
        Default::default(),
        ody_config::ConfigRequirementsToml::default(),
    )
    .map_err(std::io::Error::other)?;
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".ody").join("agents");
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let ody_home = TempDir::new()?;
    let researcher_path = ody_home.path().join("agents").join("researcher.toml");
    let reviewer_path = ody_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = ody_home.path().join("agents");
...
async fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let ody_home = TempDir::new()?;
    let catalog_path = ody_home.path().join("catalog.json");
...
"#;
    ...
    let ody_home_temp_dir = TempDir::new().unwrap();

    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        ody_home: ody_home_temp_dir,
        cfg,
    })
}
...
async fn trace_exporter_defaults_to_none_when_log_exporter_is_set() -> std::io::Result<()> {
    ...
    cfg.otel = Some(OtelConfigToml {
        exporter: Some(OtelExporterKind::OtlpHttp {
            ...
            headers: HashMap::new(),
            protocol: ody_config::types::OtelHttpProtocol::Binary,
            tls: None,
        }),
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirement_source = ody_config::RequirementSource::Unknown;
    ...
    let requirements = ody_config::ConfigRequirements {
        web_search_mode: ody_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        ody_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
projects = { "/Users/mbolin/code/ody4" = { trust_level = "trusted", foo = "bar" } , "/Users/mbolin/code/ody3" = { trust_level = "trusted" } }
model = "foo""#;
    let mut doc = initial.parse::<DocumentMut>()?;

    // Approve a new directory
    let new_project = Path::new("/Users/mbolin/code/ody2");
...
[projects."/Users/mbolin/code/ody4"]
trust_level = "trusted"
foo = "bar"

[projects."/Users/mbolin/code/ody3"]
trust_level = "trusted"
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(ody_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(ody_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
...
"#,
    ...
    assert_eq!(
        ...
        Some(ody_config::types::WindowsSandboxModeToml::Elevated)
    );
    assert!(
...
"#,
    ...
    assert_eq!(
        err.to_string(),
        "`approval_policy = \"never\"` cannot be used because requirements do not allow `sandbox_mode = \"danger-full-access\"`; Ody would fall back to read-only permissions with approvals disabled. Choose an `approval_policy` based on what you need, such as `on-request`, or choose an allowed sandbox mode."
    );
    Ok(())
}
...
async fn permission_profile_override_preserves_split_write_roots() -> std::io::Result<()> {
    let ody_home = TempDir::new()?;
    let cwd = ody_home.path().join("workspace");
    let outside_root = ody_home.path().join("outside-write");
...
export_dir = "locks"
allow_ody_version_mismatch = true
...
"#,
    ...
    assert!(config.config_lock_allow_ody_version_mismatch);
...
async fn debug_config_lockfile_load_path_loads_lock_from_nested_table() -> std::io::Result<()> {
    let ody_home = TempDir::new()?;
    let lock_path = ody_home.path().join("session.config.lock.toml");
    std::fs::write(
        &lock_path,
        format!(
            r#"version = {}
ody_version = "older-version"
...
-> std::io::Result<()> {
    let ody_home = TempDir::new()?;
    let selected_config = ody_home.path().join("default.config.toml");
    std::fs::write(
        &selected_config,
        r#"approvals_reviewer = "user"
"#,
...
discoverables = [
  ...
  { type = "plugin", id = "plugin_alpha@odysseythink-curated" },
  { type = "connector", id = "   " }
]
"#,
    )
        ...
        Some(ToolSuggestConfig {
            discoverables: vec![
                ...
                ToolSuggestDiscoverable {
                    ...
                    id: "plugin_alpha@odysseythink-curated".to_string(),
                },
                ToolSuggestDiscoverable {
                    kind: ToolSuggestDiscoverableType::Connector,
                    id: "   ".to_string(),

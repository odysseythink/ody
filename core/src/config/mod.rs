...
use ody_config::ConstrainedWithSource;
use ody_config::FeatureRequirementsToml;
use ody_config::McpServerIdentity;
use ody_config::McpServerRequirement;
use ody_config::PluginRequirementsToml;
...
use ody_config::types::History;
use ody_config::types::McpServerConfig;
use ody_config::types::McpServerDisabledReason;
...
use ody_config::types::ToolSuggestConfig;
...
use ody_config::types::UriBasedFileOpener;
use ody_config::types::WindowsSandboxModeToml;
use ody_core_plugins::PluginsConfigInput;
use ody_exec_server::ExecutorFileSystem;
use ody_exec_server::LOCAL_FS;
use ody_features::CodeModeConfigToml;
use ody_features::CurrentTimeReminderConfigToml;
use ody_features::CurrentTimeSource;
use ody_features::Feature;
use ody_features::FeatureConfigSource;
use ody_features::FeatureOverrides;
use ody_features::FeatureToml;
use ody_features::Features;
use ody_features::FeaturesToml;
use ody_features::MultiAgentV2ConfigToml;
use ody_features::NetworkProxyConfigToml;
...
use crate::config::permissions::get_readable_roots_required_for_ody_runtime;
...
pub use ody_config::ConstraintResult;
pub use ody_config::LoaderOverrides;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<PathBuf> {
    let raw = std::env::var(ody_state::SQLITE_HOME_ENV).ok()?;
...
impl Permissions {
    ...
    /// selection. Internal Ody-only writable roots are intentionally excluded.
...
// when Ody is still responsible for the network policy. `Disabled` means no
...
fn build_network_proxy_spec(
    ...
    network_requirements: Option<Sourced<ody_config::NetworkConstraints>>,
    permission_profile: &PermissionProfile,
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, Ody will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Ody
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.ody-code/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Ody"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Ody '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Ody unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/odysseythink/ody/blob/main/ody-rs/rmcp-client/src/oauth.rs#L2
    /// file: ODY_HOME/.credentials.json
    ///       This file will be readable to Ody and other applications running as the same user.
    ...
    /// Directory containing all Ody state (defaults to `~/.ody-code` but can be
    /// overridden by the `ODY_HOME` environment variable).
    pub ody_home: AbsolutePathBuf,

    /// Directory where Ody stores the SQLite state DB.
    pub sqlite_home: PathBuf,

    /// Directory where Ody writes log files (defaults to `$ODY_HOME/log`).
    pub log_dir: PathBuf,

    /// Directory where Ody writes effective session config lock files.
    pub config_lock_export_dir: Option<AbsolutePathBuf>,

    /// Whether config lock replay ignores Ody version drift between the
    /// lock metadata and the regenerated lock.
    pub config_lock_allow_ody_version_mismatch: bool,
    ...
    /// Path to the current Ody executable. This cannot be set in the config
...
impl ConfigBuilder {
    pub fn ody_home(mut self, ody_home: PathBuf) -> Self {
        self.ody_home = Some(ody_home);
        self
    }
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let ody_home = match ody_home {
            Some(ody_home) => AbsolutePathBuf::from_absolute_path(ody_home)?,
            None => find_ody_home()?,
        };
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&ody_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) = ody_config::first_layer_config_error::<ConfigToml>(
                    &config_layer_stack,
                    ody_config::CONFIG_TOML_FILE,
                )
                .await
                {
                    return Err(ody_config::io_error_from_config_error(
        ...
        {
            let allow_ody_version_mismatch = config_lock_settings
                .and_then(|config_lock| config_lock.allow_ody_version_mismatch)
                .unwrap_or(false);
            ...
            config.config_lock_allow_ody_version_mismatch = allow_ody_version_mismatch;
...
impl Config {
    ...
    pub async fn load_default_with_cli_overrides_for_ody_home(
        ody_home: PathBuf,
        cli_overrides: Vec<(String, TomlValue)>,
    ) -> std::io::Result<Self> {
        ...
        let cli_layer = ody_config::build_cli_overrides_layer(&cli_overrides);
        ody_config::merge_toml_values(&mut merged, &cli_layer);
        let ody_home = AbsolutePathBuf::from_absolute_path_checked(ody_home)?;
        let config_toml = deserialize_config_toml_with_base(merged, &ody_home)?;
        Self::load_config_with_layer_stack(
            LOCAL_FS.as_ref(),
            config_toml,
    ...
    /// ignores user settings. For example, the `ody exec` subcommand is
...
pub async fn load_config_as_toml_with_cli_and_loader_overrides(
    ...
    load_config_as_toml_with_cli_and_load_options(ody_home, cwd, cli_overrides, loader_overrides)
        .await
}
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    load_config_toml_with_layer_stack(ody_home, cwd, cli_overrides, options)
        .await
...
pub async fn load_config_toml_with_layer_stack(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, ody_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .ody/ folders.
...
/// Patch `ODY_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &ody_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            ody_protocol::permissions::FileSystemSandboxEntry {
                path: ody_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: ody_protocol::permissions::FileSystemAccessMode::Deny,
            }
        } else {
            ...
            ody_protocol::permissions::FileSystemSandboxEntry {
                path: ody_protocol::permissions::FileSystemPath::Path { path },
                access: ody_protocol::permissions::FileSystemAccessMode::Deny,
            }
        };
...
fn resolve_orchestrator_feature_enabled(
    feature: Option<&ody_config::config_toml::OrchestratorFeatureToml>,
) -> bool {
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let memories_root = memory_root(&ody_home);
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| ody_home.join("log").to_path_buf());
        let sqlite_home = cfg
            .sqlite_home
            .as_ref()
            .map(AbsolutePathBuf::to_path_buf)
            .or_else(|| resolve_sqlite_home_env(&resolved_cwd))
            .unwrap_or_else(|| ody_home.to_path_buf());
        ...
        let mut helper_readable_roots = get_readable_roots_required_for_ody_runtime(
            &ody_home,
            zsh_path.as_ref(),
        ...
        let config = Self {
            ...
            config_lock_allow_ody_version_mismatch: cfg
                .debug
                .as_ref()
                .and_then(|debug| debug.config_lockfile.as_ref())
                .and_then(|config_lock| config_lock.allow_ody_version_mismatch)
...
/// specified by the `ODY_HOME` environment variable. If not set, defaults to
/// `~/.ody-code`.
///
/// - If `ODY_HOME` is set, the value must exist and be a directory. The
...
pub fn find_ody_home() -> std::io::Result<AbsolutePathBuf> {
    ody_utils_home_dir::find_ody_home()
}

/// Returns the path to the folder where Ody logs are stored. Does not verify

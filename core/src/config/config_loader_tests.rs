...
use ody_config::ConfigRequirementsWithSources;
use ody_config::FilesystemDenyReadPattern;
use ody_config::LoaderOverrides;
use ody_config::RequirementSource;
use ody_config::RequirementsLayerEntry;
use ody_config::SessionThreadConfig;
use ody_config::StaticThreadConfigLoader;
use ody_config::ThreadConfigSource;
use ody_config::compose_requirements;
use ody_config::config_error_from_ignored_toml_fields;
use ody_config::config_error_from_toml;
...
use ody_config::loader::load_requirements_toml;
...
async fn write_project_hook_config(
    ...
    tokio::fs::create_dir_all(dot_ody_folder).await?;
    let foo = foo
        .map(|value| format!("foo = \"{value}\"\n\n"))
        .unwrap_or_default();
    tokio::fs::write(
        dot_ody_folder.join(CONFIG_TOML_FILE),
        format!(
            r#"{foo}[hooks]
...
invalid = ["#,
    ...
    let user_layer = layers
        ...
        .expect("expected a user layer even when ODY_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let config_error = config_error_from_io(&err);
    let _guard = ody_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        ody_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
collaboration_modes = "true""#;
    ...
    let _guard = ody_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = ody_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_active_user_layer()
        .expect("expected a user layer even when ODY_HOME/config.toml does not exist");
...
"#,
    ...
    assert_eq!(
        ...
        Some(ody_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(ody_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
    ...
    assert_eq!(
        ...
        Err(ConstraintError::InvalidValue {
            ...
            requirement_source: RequirementSource::MdmManagedPreferences {
                domain: "com.odysseythink.ody".to_string(),
...
"#,
    ...
    assert_eq!(
        ...
        vec![
            ...
            ConfigLayerSource::User {
                file: AbsolutePathBuf::from_absolute_path(ody_home.join(CONFIG_TOML_FILE))?,
                profile: None,
            },
        ]
    );
...
#[tokio::test]
async fn load_config_layers_resolves_relative_bundle_requirements_paths_against_ody_home()
-> anyhow::Result<()> {
    let tmp = tempdir()?;
    let ody_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&ody_home).await?;
...
"#;
    ...
    assert_eq!(
        ...
        Some(vec![
            FilesystemDenyReadPattern::from_input(&format!("{}/secrets/**", ody_home.display()))
                .expect("bundle requirements path should resolve against ody_home")
        ])
    );
...
async fn strict_config_rejects_unknown_cloud_config_key() {
    let tmp = tempdir().expect("tempdir");
    let ody_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&ody_home)
        .await
        .expect("create ody home");
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            ody_config::SandboxModeRequirement::ReadOnly,
            ody_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
        ...
        ConfigLoadOptions {
            cloud_config_bundle: CloudConfigBundleLoader::new(async {
                Err(CloudConfigBundleLoadError::new(
                    ody_config::CloudConfigBundleLoadErrorCode::RequestFailed,
                    /*status_code*/ None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".ody")).await?;
    tokio::fs::create_dir_all(project_root.join(".ody")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".ody").join(CONFIG_TOML_FILE),
        r#"foo = "root"
"#,
    )
    .await?;
    tokio::fs::write(
        nested.join(".ody").join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            ConfigLayerSource::Project { dot_ody_folder } => Some(dot_ody_folder),
            _ => None,
        })
        .collect();
    assert_eq!(project_layers.len(), 2);
    assert_eq!(project_layers[0].as_path(), nested.join(".ody").as_path());
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".ody").as_path()
    );
...
-> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".ody")).await?;
    tokio::fs::create_dir_all(worktree_child.join(".ody")).await?;
    ...
    .await?;
    write_project_hook_config(
        &worktree_root.join(".ody"),
        Some("worktree-root"),
        "echo worktree root hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_child.join(".ody"),
        Some("worktree-child"),
...
async fn nested_project_root_markers_do_not_redirect_regular_repo_hooks() -> std::io::Result<()> {
    ...
    assert_eq!(
        project_layers[0].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(nested.join(".ody"))?)
    );
    assert_eq!(
        project_layers[1].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(
...
"#;
    tokio::fs::write(project_root.join(".ody").join(CONFIG_TOML_FILE), root_cfg).await?;
    tokio::fs::write(nested.join(".ody").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".ody").join("root.txt"),
        "root instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let ody_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&ody_home).await?;
    tokio::fs::write(ody_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_ody_exists_without_config_toml() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_ody_folder: AbsolutePathBuf::from_absolute_path(project_root.join(".ody"))?,
        },
        TomlValue::Table(toml::map::Map::new()),
    );
...
#[tokio::test]
async fn ody_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let ody_home = home_dir.join(".ody");
    tokio::fs::create_dir_all(&ody_home).await?;
    tokio::fs::write(
        ody_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
#[tokio::test]
async fn ody_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_ody = project_root.join(".ody");
    let nested_dot_ody = nested.join(".ody");

    tokio::fs::create_dir_all(&nested_dot_ody).await?;
    tokio::fs::create_dir_all(project_root.join(".git")).await?;
    tokio::fs::write(
        nested_dot_ody.join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;

    tokio::fs::create_dir_all(&project_dot_ody).await?;
    make_config_for_test(
        &project_dot_ody,
        &project_root,
        TrustLevel::Trusted,
    ...
    .await?;
    let user_config_path = project_dot_ody.join(CONFIG_TOML_FILE);
...
"#,
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_ody_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_ody)?,
        },
        child_config,
    );
    assert_eq!(vec![&expected_project_layer], project_layers);
...
"#,
    ...
    let ody_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&ody_home_untrusted).await?;
    make_config_for_test(
        &ody_home_untrusted,
        &project_root,
    ...
    .await?;
    let untrusted_config_path = ody_home_untrusted.join(CONFIG_TOML_FILE);
...
{untrusted_config_contents}"#
    ...
    let ody_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&ody_home_unknown).await?;
    tokio::fs::write(
        ody_home_unknown.join(CONFIG_TOML_FILE),
        r#"foo = "user"
"#,
    )
    .await?;
...
async fn project_layer_ignores_unsupported_config_keys() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(&dot_ody).await?;
    ...
    // `.ody` folder. The malformed profile value below would fail typed path
    // resolution if `profiles` were not stripped before that pass runs.
    tokio::fs::write(
        dot_ody.join(CONFIG_TOML_FILE),
        r#"
model = "project-model"
...
"#,
    ...
    let expected_startup_warnings = vec![format!(
        ...
        dot_ody.join(CONFIG_TOML_FILE).display(),
    ...
    // still use the project-local `.ody` folder as its relative-path base.
    assert_eq!(
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".ody")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".ody").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let ody_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&ody_home).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let ody_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&ody_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &ody_home,
...
mod requirements_exec_policy_tests {
    ...
    use ody_config::ConfigRequirementsWithSources;
    use ody_config::RequirementSource;
    use ody_config::RequirementsExecPolicyDecisionToml;
    use ody_config::RequirementsExecPolicyParseError;
    use ody_config::RequirementsExecPolicyPatternTokenToml;
    use ody_config::RequirementsExecPolicyPrefixRuleToml;
    use ody_config::RequirementsExecPolicyToml;
    use ody_execpolicy::Decision;
    use ody_execpolicy::Evaluation;
    use ody_execpolicy::RuleMatch;
    use ody_utils_absolute_path::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
    ...
    fn config_stack_for_dot_ody_folder_with_requirements(
        dot_ody_folder: &Path,
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_ody_folder_with_requirements(temp_dir.path(), requirements);

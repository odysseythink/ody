...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = ody_home.join("config-link.toml");
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let ody_home = tmp.path();
    let link_a = ody_home.join("a.toml");
    let link_b = ody_home.join("b.toml");
    let config_path = ody_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(ody_home.join(CONFIG_TOML_FILE), original).expect("seed config");
    ...
    let updated = std::fs::read_to_string(ody_home.join(CONFIG_TOML_FILE)).expect("read config");
...
fn blocking_replace_mcp_servers_serializes_tool_approval_overrides() {
    ...
    apply_blocking(ody_home, &[ConfigEdit::ReplaceMcpServers(servers)]).expect("persist");
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !ody_home.join(CONFIG_TOML_FILE).exists(),

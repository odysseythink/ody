...
async fn restricted_read_implicitly_allows_helper_executables() -> std::io::Result<()> {
    ...
    let ody_home = temp_dir.path().join(".ody");
    let zsh_path = temp_dir.path().join("runtime").join("zsh");
    let arg0_root = ody_home.join("tmp").join("arg0");
    let allowed_arg0_dir = arg0_root.join("ody-arg0-session");
    let sibling_arg0_dir = arg0_root.join("ody-arg0-other-session");
    let execve_wrapper = allowed_arg0_dir.join("ody-execve-wrapper");
    ...
    let config = Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(&ody_home)?,
    )
    .await?;
...
fn network_toml_ignores_legacy_network_list_keys() {
    let parsed = toml::from_str::<NetworkToml>(
        r#"
allowed_domains = ["odysseythink.com"]
"#,
    )
...
fn network_toml_overlays_unix_socket_permissions_by_path() {
    ...
    assert_eq!(
        ...
        Some(ody_network_proxy::NetworkUnixSocketPermissions {
            entries: BTreeMap::from([
                (
                    "/tmp/base.sock".to_string(),
...
fn profile_network_proxy_config_keeps_proxy_disabled_for_proxy_policy() {
    ...
    assert_eq!(
        ...
        Some(ody_network_proxy::NetworkDomainPermissions {
            entries: vec![ody_network_proxy::NetworkDomainPermissionEntry {
                pattern: "odysseythink.com".to_string(),
                permission: ody_network_proxy::NetworkDomainPermission::Allow,
            }],
        })
    );
}

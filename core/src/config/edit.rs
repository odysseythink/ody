...
pub fn apply_blocking(ody_home: &Path, edits: &[ConfigEdit]) -> anyhow::Result<()> {
...
///
pub async fn apply(ody_home: &Path, edits: Vec<ConfigEdit>) -> anyhow::Result<()> {
...
impl ConfigEditsBuilder {
    pub fn new(ody_home: &Path) -> Self {
        Self::for_config_path(&ody_home.join(CONFIG_TOML_FILE))
    }
    ...
    pub fn for_config(config: &crate::config::Config) -> Self {
        let config_path = config
            ...
            .map(ody_utils_absolute_path::AbsolutePathBuf::to_path_buf)
            .unwrap_or_else(|| config.ody_home.join(CONFIG_TOML_FILE).to_path_buf());

...
use ody_execpolicy::AmendError;
use ody_execpolicy::Decision;
use ody_execpolicy::Error as ExecPolicyRuleError;
...
use ody_execpolicy::PolicyParser;
use ody_execpolicy::RuleMatch;
use ody_execpolicy::blocking_append_allow_prefix_rule;
use ody_execpolicy::blocking_append_network_rule;
...
use ody_shell_command::is_dangerous_command::command_might_be_dangerous;
use ody_shell_command::is_safe_command::is_known_safe_command;
use thiserror::Error;
use tokio::fs;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            ...
            .into_iter()
            .filter_map(ody_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(ody_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &ody_execpolicy::Error) -> String {
...
pub(crate) fn render_decision_for_unmatched_command(
    ...
    let is_known_safe = match command_origin {
        ...
        ExecPolicyCommandOrigin::PowerShell => {
            ody_shell_command::is_safe_command::is_safe_powershell_words(command)
        }
    };
    ...
    let command_is_dangerous = match command_origin {
        ...
        #[cfg(windows)]
        ExecPolicyCommandOrigin::PowerShell => {
            ody_shell_command::is_dangerous_command::is_dangerous_powershell_words(command)
        }
    };
...
fn default_policy_path(ody_home: &Path) -> PathBuf {
    ody_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
fn commands_for_exec_policy(command: &[String]) -> ExecPolicyCommands {
    ...
    #[cfg(windows)]
    {
        if let Some(commands) =
            ody_shell_command::powershell::parse_powershell_command_into_plain_commands(command)
            && !commands.is_empty()
        {
            return ExecPolicyCommands {
                commands,
...
/// - Note: we only use this amendment when the command fails to run in sandbox and ody prompts the user to run outside the sandbox

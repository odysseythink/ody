...
/// Start an interactive sub-Ody thread and return IO channels.
///
...
pub(crate) async fn run_ody_thread_interactive(
    ...
    let OdySpawnOk { ody, .. } = Box::pin(Ody::spawn(OdySpawnArgs {
        config,
        user_instructions,
        ...
        parent_rollout_thread_trace: ody_rollout_trace::ThreadTraceContext::disabled(),
        parent_trace: None,
    ...
    let ody_for_events = Arc::clone(&ody);
    ...
    let ody_for_ops = Arc::clone(&ody);
    tokio::spawn(async move {
        forward_ops(ody_for_ops, rx_ops, cancel_token_ops).await;
    });

    Ok(Ody {
        tx_sub: tx_ops,
        rx_event: rx_sub,
        agent_status: ody.agent_status.clone(),
        session: Arc::clone(&ody.session),
        session_loop_termination: ody.session_loop_termination.clone(),
    })
}
...
async fn forward_events(
    ...
    loop {
        tokio::select! {
            _ = &mut cancelled => {
                shutdown_delegate(&ody).await;
                break;
            }
            event = ody.next_event() => {
                ...
                match event {
                    ...
                    other => {
                        if !forward_event_or_shutdown(&ody, &tx_sub, &cancel_token, other).await
                        {
                            break;
                        }
                    }
...
async fn shutdown_delegate(ody: &Ody) {
    let _ = ody.submit(Op::Interrupt).await;
    let _ = ody.submit(Op::Shutdown {}).await;

    let _ = timeout(Duration::from_millis(500), async {
        while let Ok(event) = ody.next_event().await {
            if matches!(
                event.msg,
...
async fn forward_ops(
    ...
    loop {
        ...
        let _ = ody.submit_with_id(submission).await;
    }
}
    ...
    let guardian_decision = if routes_approval_to_guardian(parent_ctx) {
        ...
        let patch = changes
            ...
            .map(|(path, change)| match change {
                ody_protocol::protocol::FileChange::Add { content } => {
                    format!("*** Add File: {}\n{}", path.display(), content)
                }
                ody_protocol::protocol::FileChange::Delete { content } => {
                    format!("*** Delete File: {}\n{}", path.display(), content)
                }
                ody_protocol::protocol::FileChange::Update {
                    unified_diff,
                    move_path,
                } => {
...
async fn handle_request_user_input(
    ...
    .await
    {
        let _ = ody.submit(Op::UserInputAnswer { id, response }).await;
        return;
    }
...
async fn maybe_auto_review_mcp_request_user_input(
    ...
    Some(RequestUserInputResponse {
        answers: HashMap::from([(
            question.id.clone(),
            ody_protocol::request_user_input::RequestUserInputAnswer {
...
) -> ody_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = ody_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            ...
            parent_session
                .notify_approval(approval_id, ody_protocol::protocol::ReviewDecision::Abort)
                .await;
            ody_protocol::protocol::ReviewDecision::Abort
        }
        decision = fut => {
            decision
        }

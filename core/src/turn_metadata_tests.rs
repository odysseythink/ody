...
fn test_responses_metadata_json(
    ...
    window_id: &str,
    request_kind: OdyResponsesRequestKind,
) -> String {
    state
...
fn test_turn_responses_metadata_json(state: &TurnMetadataState, window_id: &str) -> String {
    test_responses_metadata_json(state, window_id, OdyResponsesRequestKind::Turn)
}
...
fn test_compaction_responses_metadata_json(
    ...
    test_responses_metadata_json(
        state,
        window_id,
        OdyResponsesRequestKind::Compaction(compaction),
    )
}
...
fn turn_metadata_state_merges_client_metadata_without_replacing_reserved_fields() {
    ...
    state.set_responsesapi_client_metadata(HashMap::from([
        ...
        (
            "x-ody-installation-id".to_string(),
            "client-supplied".to_string(),
        ),
        (
            "x-ody-parent-thread-id".to_string(),
            "client-supplied".to_string(),
        ),
        (
    ...
    assert!(json.get("x-ody-installation-id").is_none());
    assert!(json.get("x-ody-parent-thread-id").is_none());
    assert!(json.get("x-odysseythink-subagent").is_none());
    assert_eq!(
        json["forked_from_thread_id"].as_str(),
        Some("44444444-4444-4444-8444-444444444444")
    );

...
async fn start_thread_seeds_extension_data_for_mcp_and_lifecycle_contributors() {
    ...
    impl ody_extension_api::ThreadLifecycleContributor<Config> for InitialDataRecorder {
    ...
    impl ody_extension_api::McpServerContributor<Config> for InitialDataRecorder {
        ...
        fn contribute<'a>(
            &'a self,
            context: ody_extension_api::McpServerContributionContext<'a, Config>,
        ) -> ody_extension_api::ExtensionFuture<'a, Vec<ody_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
                ...
                vec![ody_extension_api::McpServerContribution::SelectedPlugin {
    ...
    let mut extensions = ody_extension_api::ExtensionRegistryBuilder::new();
    ...
    let selected_root_init = |id: &str, environment_id: &str| {
        let mut init = ody_extension_api::ExtensionDataInit::new();
    ...
    let selected_servers = |config: &ody_mcp::McpConfig| {
        ody_mcp::configured_mcp_servers(config)
            .into_iter()
...
#[tokio::test]
async fn explicit_installation_id_skips_ody_home_file() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.ody_home = temp_dir.path().join("ody-home").abs();
    config.cwd = config.ody_home.abs();
    ...
    assert!(!config.ody_home.join(INSTALLATION_ID_FILENAME).exists());
    assert_eq!(thread.thread.ody.session.installation_id, installation_id);

...
use ody_code_mode::CellId;
use ody_code_mode::CodeModeNestedToolCall;
use ody_code_mode::CodeModeSession;
use ody_code_mode::CodeModeToolKind;
use ody_code_mode::RuntimeResponse;
...
use ody_utils_output_truncation::truncate_function_output_items_with_policy;
...
pub(crate) const PUBLIC_TOOL_NAME: &str = ody_code_mode::PUBLIC_TOOL_NAME;
pub(crate) const WAIT_TOOL_NAME: &str = ody_code_mode::WAIT_TOOL_NAME;
pub(crate) const DEFAULT_WAIT_YIELD_TIME_MS: u64 = ody_code_mode::DEFAULT_WAIT_YIELD_TIME_MS;
...
impl CodeModeService {
    pub(crate) fn new() -> Self {
        ...
        Self {
            session: Some(Arc::new(ody_code_mode::CodeModeService::with_delegate(
    ...
    pub(crate) async fn execute(
        &self,
        request: ody_code_mode::ExecuteRequest,
    ) -> Result<ody_code_mode::StartedCell, String> {
        self.session()?.execute(request).await
    }

    pub(crate) async fn wait(
        &self,
        request: ody_code_mode::WaitRequest,
    ) -> Result<ody_code_mode::WaitOutcome, String> {
    ...
    pub(crate) fn mark_cell_ready_for_dispatch(&self, cell_id: &ody_code_mode::CellId) {

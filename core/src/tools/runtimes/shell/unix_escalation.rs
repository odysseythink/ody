...
use ody_execpolicy::MatchOptions;
...
use ody_protocol::protocol::NetworkPolicyRuleAction;
...
use ody_shell_command::bash::parse_shell_lc_plain_commands;
use ody_shell_command::bash::parse_shell_lc_single_command_prefix;
use ody_shell_escalation::EscalateServer;
use ody_shell_escalation::EscalationDecision;
use ody_shell_escalation::EscalationExecution;
use ody_shell_escalation::EscalationPermissions;
use ody_shell_escalation::EscalationPolicy;
use ody_shell_escalation::EscalationPolicyFuture;
use ody_shell_escalation::EscalationSession;
use ody_shell_escalation::ExecParams;
use ody_shell_escalation::ExecResult;
use ody_shell_escalation::PreparedExec;
use ody_shell_escalation::ResolvedPermissionProfile;
use ody_shell_escalation::ShellCommandExecutor;
use ody_shell_escalation::ShellCommandExecutorFuture;
use ody_shell_escalation::Stopwatch;
...
pub(super) async fn try_run_zsh_fork(
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        sandbox_policy_cwd,
        windows_sandbox_workspace_roots,
        ody_linux_sandbox_exe: ctx.turn.config.ody_linux_sandbox_exe.clone(),
        ...
        Ok(stopwatch
            .pause_for(async move {
                ...
                let permission_request = PermissionRequestPayload::bash(
                    ody_shell_command::parse_command::shlex_join(&command),
                    /*description*/ None,
                );
...
struct CoreShellCommandExecutor {
    ...
    network: Option<ody_network_proxy::NetworkProxy>,
...
impl CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["ODY_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            ody_linux_sandbox_exe: self.ody_linux_sandbox_exe.as_deref(),
...
fn map_exec_result(
    ...
    if result.timed_out {
        return Err(ToolError::Ody(OdyErr::Sandbox(SandboxErr::Timeout {
            output: Box::new(output),
        })));
    }

    if is_likely_sandbox_denied(sandbox, &output) {
        return Err(ToolError::Ody(OdyErr::Sandbox(SandboxErr::Denied {
            output: Box::new(output),

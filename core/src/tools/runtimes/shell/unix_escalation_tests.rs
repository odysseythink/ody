...
async fn preapproved_additional_permissions_escalate_intercepted_exec() -> anyhow::Result<()> {
    ...
    let provider = CoreShellActionProvider {
        policy: Arc::new(RwLock::new(ody_execpolicy::Policy::empty())),
        ...
        stopwatch: ody_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = ody_shell_escalation::EscalationPolicy::determine_action(
        &provider,
    ...
    let expected = ody_shell_escalation::EscalationDecision::Escalate(
        EscalationExecution::Permissions(EscalationPermissions::ResolvedPermissionProfile(
            ResolvedPermissionProfile { permission_profile },
        )),
    );
...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    std::fs::create_dir_all(&turn_context.config.ody_home)
        .context("recreate ody home for hook fixtures")?;
    ...
    let expected_hook_command =
        ody_shell_command::parse_command::shlex_join(&["/usr/bin/touch".to_string(), target_str]);
    let provider = CoreShellActionProvider {
        policy: std::sync::Arc::new(RwLock::new(ody_execpolicy::Policy::empty())),
    ...
    let action = tokio::time::timeout(
        ...
        ody_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        ody_shell_escalation::EscalationDecision::Escalate(
            ody_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
...
"#
    ...
    assert_eq!(action, ody_shell_escalation::EscalationDecision::Run);
    Ok(())
}
...
async fn denied_reads_keep_granular_sandbox_rejection_for_escalation() -> anyhow::Result<()> {
    ...
    assert_eq!(
        action,
        ody_shell_escalation::EscalationDecision::Deny {

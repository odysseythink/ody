...
use ody_protocol::protocol::NetworkSandboxPolicy;
...
async fn install_role_with_model_override(turn: &mut TurnContext) -> String {
    ...
    tokio::fs::create_dir_all(&turn.config.ody_home)
        .await
        .expect("ody home should be created");
        ...
        | ResponseInputItem::CustomToolCallOutput { output, .. } => {
            let content = match output.body {
                ...
                FunctionCallOutputBody::ContentItems(items) => {
                    ody_protocol::models::function_call_output_content_items_to_text(&items)
                        .unwrap_or_default()
                }
            };
...
async fn multi_agent_v2_list_agents_returns_completed_status_without_encrypted_spawn_preview() {
    ...
    let child_thread = manager
        ...
        .await
        .expect("child thread should exist");
    let child_turn = child_thread.ody.session.new_default_turn().await;
    child_thread
        .ody
        .session
        .send_event(
...
async fn multi_agent_v2_followup_task_completion_notifies_parent_on_every_turn() {
    ...
    root.thread.ody.session.new_default_turn().await;
    session.services.agent_control = manager.agent_control();
    session.thread_id = root.thread_id;
    let session = Arc::new(session);
    let turn = Arc::new(turn);
    ...
    let first_turn = thread.ody.session.new_default_turn().await;
    thread
        .ody
        .session
        .send_event(
    ...
    let second_turn = thread.ody.session.new_default_turn().await;
    thread
        .ody
        .session
        .send_event(
...
async fn multi_agent_v2_interrupted_turn_does_not_notify_parent() {
    ...
    let aborted_turn = thread.ody.session.new_default_turn().await;
    thread
        .ody
        .session
        .send_event(
...
async fn multi_agent_v2_interrupt_agent_accepts_task_name_target() {
    ...
    let worker_session = worker_thread.ody.session.clone();
    SpawnAgentHandlerV2::default()
        .handle(invocation(
...
async fn tool_handlers_cascade_close_and_resume_and_keep_explicitly_closed_subtrees_closed() {
    ...
    let parent_session = parent.thread.ody.session.clone();
    ...
    let child_thread = manager
        ...
        .await
        .expect("child thread should exist");
    let child_session = child_thread.ody.session.clone();
    ...
    let operator_session = operator.thread.ody.session.clone();
    let _ = manager
        .agent_control()
    ...
    let parent_resume_output = ResumeAgentHandler
        .handle(invocation(
            operator_session,
            operator.thread.ody.session.new_default_turn().await,
            "resume_agent",

...
pub(crate) fn collab_spawn_error(err: OdyErr) -> FunctionCallError {
    match err {
        OdyErr::UnsupportedOperation(message) if message == "thread manager dropped" => {
            FunctionCallError::RespondToModel("collab manager unavailable".to_string())
        }
        OdyErr::UnsupportedOperation(message) => FunctionCallError::RespondToModel(message),
        err => FunctionCallError::RespondToModel(format!("collab spawn failed: {err}")),
    }
}

pub(crate) fn collab_agent_error(agent_id: ThreadId, err: OdyErr) -> FunctionCallError {
    match err {
        OdyErr::ThreadNotFound(id) => {
...
pub(crate) async fn apply_requested_spawn_agent_model_overrides(
    ...
    if let Some(requested_model) = requested_model {
        let models_manager = session.services.models_manager.load();

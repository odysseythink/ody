...
use crate::plugins::test_support::write_odysseythink_curated_marketplace;
...
use ody_core_plugins::startup_sync::curated_plugins_repo_path;
...
async fn verified_plugin_install_completed_requires_installed_plugin() {
    ...
    write_odysseythink_curated_marketplace(&curated_root, &["sample"]);
    write_curated_plugin_sha(ody_home.path());
    write_plugins_feature_config(ody_home.path());

    let config = load_plugins_config(ody_home.path()).await;
    let plugins_manager = PluginsManager::new(ody_home.path().to_path_buf());

    assert!(!verified_plugin_install_completed(
        "sample@odysseythink-curated",
        &config,
        &plugins_manager,
    ));
    ...
    let refreshed_config = load_plugins_config(ody_home.path()).await;
    assert!(verified_plugin_install_completed(
        "sample@odysseythink-curated",
        &refreshed_config,
        &plugins_manager,
...
fn remote_plugin_install_suggestions_skip_core_installed_verification() {
    assert!(is_remote_plugin_install_suggestion(
        "snowflake@odysseythink-curated-remote"
    ));
    assert!(!is_remote_plugin_install_suggestion(
        "snowflake@odysseythink-curated"
    ));
...
fn recommended_plugin_install_args_accept_legacy_tool_id() {
    let current: RecommendedPluginInstallArgs = serde_json::from_value(json!({
        "plugin_id": "google-drive@odysseythink-curated-remote",
    ...
    let legacy: RecommendedPluginInstallArgs = serde_json::from_value(json!({
        "tool_type": "plugin",
        "action_type": "install",
        "tool_id": "google-drive@odysseythink-curated-remote",
        "suggest_reason": "Use Google Drive for this request"
    }))
...
async fn persist_disabled_install_request_writes_connector_config() {
    ...
    persist_disabled_install_request(&ody_home.path().abs(), &tool)
        .await
        .expect("persist connector disable");

    let contents =
        std::fs::read_to_string(ody_home.path().join(CONFIG_TOML_FILE)).expect("read config");
...
async fn persist_disabled_install_request_writes_plugin_config() {
    ...
    assert_eq!(
        ...
        Some(ToolSuggestConfig {
            discoverables: Vec::new(),
            disabled_tools: vec![ToolSuggestDisabledTool::plugin("slack@odysseythink-curated")],
        })
    );
}
...
[tool_suggest]
discoverables = [
  { type = "plugin", id = "sample@odysseythink-curated" }
]
    ...
    assert_eq!(
        ...
        Some(ToolSuggestConfig {
            discoverables: vec![ToolSuggestDiscoverable {
                ...
                id: "sample@odysseythink-curated".to_string(),
            }],
            disabled_tools: vec![

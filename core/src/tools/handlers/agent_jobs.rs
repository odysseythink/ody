...
fn required_state_db(
    session: &Arc<Session>,
) -> Result<Arc<ody_state::StateRuntime>, FunctionCallError> {
        ...
        if !cancel_requested && active_items.len() < options.max_concurrency {
            ...
            let pending_items = db
                .list_agent_job_items(
                    job_id.as_str(),
                    Some(ody_state::AgentJobItemStatus::Pending),
                    Some(slots),
                )
                .await?;
...
async fn recover_running_items(
    ...
    let running_items = db
        .list_agent_job_items(
            job_id,
            Some(ody_state::AgentJobItemStatus::Running),
            /*limit*/ None,
        )
        .await?;
...
async fn finalize_finished_item(
    ...
    if matches!(item.status, ody_state::AgentJobItemStatus::Running) {
...
fn render_instruction_template(instruction: &str, row_json: &Value) -> String {
    const OPEN_BRACE_SENTINEL: &str = "__ODY_OPEN_BRACE__";
    const CLOSE_BRACE_SENTINEL: &str = "__ODY_CLOSE_BRACE__";
...
fn job_runtime_timeout(job: &ody_state::AgentJob) -> Duration {
    job.max_runtime_seconds
        .map(Duration::from_secs)
        .unwrap_or(DEFAULT_AGENT_JOB_ITEM_TIMEOUT)
}

fn started_at_from_item(item: &ody_state::AgentJobItem) -> Instant {
...
fn is_item_stale(item: &ody_state::AgentJobItem, runtime_timeout: Duration) -> bool {
    let now = chrono::Utc::now();

...
impl ToolEmitter {
    ...
    pub async fn finish(
        ...
        let (event, result) = match out {
            ...
            Err(ToolError::Ody(OdyErr::Sandbox(SandboxErr::Timeout { output }))) => {
                let response = self.format_exec_output_for_model(&output, ctx);
                let event = ToolEventStage::Failure(ToolEventFailure::Output(*output));
                let result = Err(FunctionCallError::RespondToModel(response));
                (event, result)
            }
            Err(ToolError::Ody(OdyErr::Sandbox(SandboxErr::Denied { output, .. }))) => {
                let response = self.format_exec_output_for_model(&output, ctx);
...
mod tests {
    ...
    async fn assert_failed_apply_patch_tracks_committed_delta(
        ...
        let delta = ody_apply_patch::apply_patch(

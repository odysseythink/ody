...
use ody_otel::ToolDecisionSource;
...
impl ToolOrchestrator {
        ...
        T: ToolRuntime<Rq, Out>,
    {
        ...
        match first_result {
            ...
            Err(ToolError::Ody(OdyErr::Sandbox(SandboxErr::Denied {
                output,
    ...
    async fn request_approval<Rq, Out, T>(
        ...
        otel: &ody_otel::SessionTelemetry,
...
fn sandbox_outcome_from_tool_error(err: &ToolError) -> Option<&'static str> {
    match err {
        ToolError::Ody(OdyErr::Sandbox(SandboxErr::Denied { .. })) => Some("denied"),
        ToolError::Ody(OdyErr::Sandbox(SandboxErr::Timeout { .. })) => Some("timed_out"),
        ToolError::Ody(OdyErr::Sandbox(SandboxErr::Signal(_))) => Some("signal"),
        ToolError::Rejected(_) | ToolError::Ody(_) => None,
    }
}

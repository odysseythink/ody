...
use ody_tools::can_request_original_image_detail;
use ody_tools::collect_code_mode_exec_prompt_tool_definitions;
use ody_tools::collect_request_plugin_install_entries;
use ody_tools::default_namespace_description;
use ody_tools::request_user_input_available_modes;
use ody_tools::shell_command_backend_for_features;
use ody_tools::shell_type_for_model_and_features;
...
fn spec_for_model_request(
    ...
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly)
        ...
        && ody_code_mode::is_code_mode_nested_tool(spec.name())
    {
        ody_tools::augment_tool_spec_for_code_mode(spec)
    } else {
        spec
    }
}
...
fn is_hidden_by_code_mode_only(
    ...
    tool_mode == ToolMode::CodeModeOnly
        && exposure != ToolExposure::DirectModelOnly
        && ody_code_mode::is_code_mode_nested_tool(&ody_tools::code_mode_name_for_tool_name(
            tool_name,
        ))
}
...
fn code_mode_namespace_descriptions(
    specs: &[ToolSpec],
) -> BTreeMap<String, ody_code_mode::ToolNamespaceDescription> {
    ...
    for spec in specs {
        ...
        let entry = namespace_descriptions
            ...
            .or_insert_with(|| ody_code_mode::ToolNamespaceDescription {
...
/// `apply_patch` is only usable by models that advertise the tool type: its spec
/// is a freeform (grammar) tool, and models without that capability are handed a
/// schema-less function tool they cannot call correctly. Registering it anyway
/// in read-only modes fails the turn on dispatch, so the capability gate holds
/// for every mode. Plan/Design still have a write channel without it:
/// `plan_mode_gate_for_exec` (safety.rs) whitelists shell writes into the plan
/// file and its `<stem>/` directory.
fn should_register_apply_patch(has_environment: bool, apply_patch_supported: bool) -> bool {
    has_environment && apply_patch_supported
}
...
fn compare_code_mode_tools(
    left: &ody_code_mode::ToolDefinition,
    right: &ody_code_mode::ToolDefinition,
...
fn code_mode_namespace_name<'a>(
    tool: &ody_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, ody_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()

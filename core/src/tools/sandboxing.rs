...
pub(crate) enum ExecApprovalRequirement {
    ...
    Skip {
        ...
        /// Only applies if the command fails to run in sandbox and ody prompts the user to run outside the sandbox.
...
pub(crate) struct SandboxAttempt<'a> {
    ...
    pub permissions: &'a ody_protocol::models::PermissionProfile,
    /// Canonical permissions before this host materializes workspace roots.
    pub exec_server_permissions: &'a ody_protocol::models::PermissionProfile,
...
impl<'a> SandboxAttempt<'a> {
    ...
    ) -> Result<crate::sandboxing::ExecRequest, OdyErr> {
        let request = self
            .manager

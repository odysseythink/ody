...
use crate::client::X_ODY_INSTALLATION_ID_HEADER;
use crate::client::X_ODY_PARENT_THREAD_ID_HEADER;
use crate::client::X_ODY_TURN_METADATA_HEADER;
use crate::client::X_ODY_WINDOW_ID_HEADER;
...
pub(crate) enum OdyResponsesRequestKind {
    Turn,
    Prewarm,
...
impl OdyResponsesRequestKind {
    fn metadata(self) -> (&'static str, Option<CompactionTurnMetadata>) {
        match self {
            OdyResponsesRequestKind::Turn => ("turn", None),
            OdyResponsesRequestKind::Prewarm => ("prewarm", None),
            OdyResponsesRequestKind::Compaction(metadata) => ("compaction", Some(metadata)),
            OdyResponsesRequestKind::Memory => ("memory", None),
        }
    }

    fn has_turn_identity(self) -> bool {
        !matches!(self, OdyResponsesRequestKind::Memory)
    }
}
...
/// Caller-owned snapshot of Ody metadata sent to ResponsesAPI.
///
/// The full Ody turn metadata blob is transported canonically as
/// `client_metadata["x-ody-turn-metadata"]`. Flat `client_metadata` keys and direct HTTP/ws
/// headers are generated compatibility projections of this snapshot, not separate sources of
/// truth.
#[derive(Clone, Debug)]
pub struct OdyResponsesMetadata {
    pub(crate) installation_id: String,
    pub(crate) session_id: String,
    pub(crate) thread_id: String,
    pub(crate) turn_id: Option<String>,
    pub(crate) window_id: String,
    pub(crate) request_kind: Option<OdyResponsesRequestKind>,
...
impl OdyResponsesMetadata {
    ...
    pub(crate) fn client_metadata(&self) -> HashMap<String, String> {
        ...
        if let Some(parent_thread_id) = self.parent_thread_id {
            client_metadata.insert(
                X_ODY_PARENT_THREAD_ID_HEADER.to_string(),
        ...
        {
            client_metadata.insert(X_ODY_TURN_METADATA_HEADER.to_string(), turn_metadata_json);
        }
        client_metadata
    }
    ...
    pub(crate) fn compatibility_headers(&self) -> ApiHeaderMap {
        ...
        insert_header(&mut headers, X_ODY_WINDOW_ID_HEADER, &self.window_id);
        // Direct x-ody-turn-metadata is compatibility output. New per-request consumers should
        // prefer client_metadata["x-ody-turn-metadata"], which is rendered from this same object.
        if self.has_turn_metadata()
            && let Some(turn_metadata_json) = self.turn_metadata_json()
        {
            insert_header(
    ...
    fn turn_metadata_payload(&self) -> OdyTurnMetadataPayload<'_> {
        ...
        OdyTurnMetadataPayload {
            ...
            compaction,
            // responsesapi_client_metadata enriches the Ody turn metadata blob, not literal
            // top-level Responses client_metadata. Reserved Ody-owned keys are filtered when
...
#[derive(Serialize)]
struct OdyTurnMetadataPayload<'a> {
    #[serde(default, skip_serializing_if = "Option::is_none")]

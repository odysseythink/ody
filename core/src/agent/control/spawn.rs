...
impl AgentControl {
    ...
    pub(crate) async fn ensure_v2_agent_loaded(
        ...
        if self.state.agent_metadata_for_thread(thread_id).is_none() {
            return Err(OdyErr::ThreadNotFound(thread_id));
        }
        ...
        let history = stored_thread
            .history
            .ok_or(OdyErr::ThreadNotFound(thread_id))?
            .items;
    ...
    async fn spawn_agent_internal(
        ...
        )) = notification_source.as_ref()
        {
            ...
            let thread_config = new_thread.thread.ody.thread_config_snapshot().await;
            ...
            emit_subagent_session_started(
                ...
                client_metadata,
                new_thread.thread.ody.session.session_id(),
    ...
    async fn resume_single_agent_from_rollout(
        ...
        thread_id: ThreadId,
        session_source: SessionSource,
    ) -> OdyResult<(ThreadId, MultiAgentVersion)> {
        let state = self.upgrade()?;
        ...
        let history = stored_thread
            .history
            .ok_or_else(|| OdyErr::ThreadNotFound(thread_id))?
            .items;
        let initial_history = InitialHistory::Resumed(ResumedHistory {
            conversation_id: thread_id,
            history,

...
use ody_thread_store::ArchiveThreadParams;
...
impl AgentControlHarness {
    ...
    async fn start_thread(&self) -> (ThreadId, Arc<OdyThread>) {
        let new_thread = self
            .manager
...
async fn wait_for_subagent_notification(parent_thread: &Arc<OdyThread>) -> bool {
    let wait = async {
        loop {
...
async fn persist_thread_for_tree_resume(thread: &Arc<OdyThread>, message: &str) {
    thread
...
async fn assert_thread_not_loaded(manager: &ThreadManager, thread_id: ThreadId) {
    match manager.get_thread(thread_id).await {
        Err(OdyErr::ThreadNotFound(id)) => assert_eq!(id, thread_id),
...
async fn send_input_errors_when_thread_missing() {
    ...
    assert_matches!(err, OdyErr::ThreadNotFound(id) if id == thread_id);
}
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            if thread
                ...
                .session
                .input_queue
                .has_pending_input(&thread.ody.session.active_turn)
                .await
            {
                break;
            }
...
async fn ensure_v2_agent_loaded_reloads_registered_unloaded_agent() {
    ...
    match harness.manager.get_thread(spawned_agent.thread_id).await {
        Err(OdyErr::ThreadNotFound(id)) => assert_eq!(id, spawned_agent.thread_id),
        Err(err) => panic!("expected ThreadNotFound, got {err:?}"),
...
async fn spawn_agent_can_fork_parent_thread_history_with_sanitized_items() {
    ...
    let turn_context = parent_thread.ody.session.new_default_turn().await;
    ...
    let history = child_thread.ody.session.clone_history().await;
    ...
    assert_eq!(
        serde_json::to_value(child_thread.ody.session.reference_context_item().await)
    ...
    let no_hint_history = no_hint_child_thread.ody.session.clone_history().await;
    assert!(
...
async fn spawn_agent_fork_last_n_turns_keeps_only_recent_turns() {
    ...
    let queued_turn_context = parent_thread.ody.session.new_default_turn().await;
    parent_thread
        .ody
        .session
        .record_conversation_items(
    ...
    let triggered_turn_context = parent_thread.ody.session.new_default_turn().await;
    parent_thread
        .ody
        .session
        .record_conversation_items(
    ...
    parent_thread
        ...
        .await;
    let spawn_turn_context = parent_thread.ody.session.new_default_turn().await;
...
async fn spawn_agent_fork_last_n_turns_drops_parent_startup_prefix_when_under_limit() {
    let harness = AgentControlHarness::new().await;
    let (parent_thread_id, parent_thread) = harness.start_thread().await;
    let startup_turn_context = parent_thread.ody.session.new_default_turn().await;
    parent_thread
        .ody
        .session
        .record_conversation_items(
...
async fn spawn_agent_respects_max_threads_limit() {
    ...
    let OdyErr::AgentLimitReached {
        max_threads: seen_max_threads,
    } = err
    else {
        panic!("expected OdyErr::AgentLimitReached");
    };
...
async fn spawn_agent_limit_shared_across_clones() {
    ...
    let OdyErr::AgentLimitReached { max_threads } = err else {
        panic!("expected OdyErr::AgentLimitReached");
    };
...
async fn multi_agent_v2_completion_ignores_dead_direct_parent() {
    ...
    let tester_turn = tester_thread.ody.session.new_default_turn().await;
    tester_thread
        .ody
        .session
        .send_event(

...
pub(crate) struct AgentControl {
    ...
    /// `ThreadManagerState -> OdyThread -> Session -> SessionServices -> ThreadManagerState`.
...
impl AgentControl {
    ...
    pub(crate) async fn interrupt_agent(&self, agent_id: ThreadId) -> OdyResult<String> {
        let state = self.upgrade()?;
    ...
    async fn handle_thread_request_result(
        ...
        agent_id: ThreadId,
        state: &Arc<ThreadManagerState>,
        result: OdyResult<String>,
    ) -> OdyResult<String> {
        if matches!(result, Err(OdyErr::InternalAgentDied)) {
    ...
    pub(crate) fn ensure_agent_known(&self, agent_id: ThreadId) -> OdyResult<AgentMetadata> {
        self.state
            .agent_metadata_for_thread(agent_id)
            .ok_or(OdyErr::ThreadNotFound(agent_id))
    }
    ...
    pub(crate) async fn resolve_agent_reference(
        ...
        Err(OdyErr::UnsupportedOperation(format!(
    ...
    pub(crate) async fn subscribe_status(
        &self,
        agent_id: ThreadId,
    ) -> OdyResult<watch::Receiver<AgentStatus>> {
        let state = self.upgrade()?;
    ...
    ) -> OdyResult<(SessionSource, AgentMetadata)> {
        if depth == 1 {
    ...
    fn upgrade(&self) -> OdyResult<Arc<ThreadManagerState>> {
        self.manager
            .upgrade()
            .ok_or_else(|| OdyErr::UnsupportedOperation("thread manager dropped".to_string()))
    }
    ...
    async fn inherited_exec_policy_for_source(
        ...
        let parent_thread = state.get_thread(*parent_thread_id).await.ok()?;
        let parent_config = parent_thread.ody.session.get_config().await;
    ...
    async fn open_thread_spawn_children(
        &self,
        parent_thread_id: ThreadId,
    ) -> OdyResult<Vec<(ThreadId, AgentMetadata)>> {
    ...
    async fn live_thread_spawn_children(
        &self,
    ) -> OdyResult<HashMap<ThreadId, Vec<(ThreadId, AgentMetadata)>>> {
        let state = self.upgrade()?;

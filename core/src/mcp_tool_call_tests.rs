...
use ody_config::types::AppToolConfig;
use ody_config::types::AppToolsConfig;
use ody_config::types::ApprovalsReviewer;
use ody_config::types::AppsConfigToml;
...
use ody_rollout_trace::ToolDispatchInvocation;
use ody_rollout_trace::ToolDispatchPayload;
...
fn write_sample_plugin_mcp(ody_home: &std::path::Path) {
    let plugin_root = ody_home.join("plugins/cache/test/sample/local");
    ...
    let dispatch_trace = session
        ...
        .start_tool_dispatch_trace(|| {
            Some(ToolDispatchInvocation {
                ...
                ody_turn_id: turn_context.sub_id.clone(),
...
fn install_mcp_permission_request_hook(
    ...
    std::fs::create_dir_all(&turn_context.config.ody_home)
        .expect("create ody home for MCP permission hook");
    let script = format!(
        r#"import json
...
"#,
    ...
    std::fs::write(
        turn_context.config.ody_home.join("hooks.json"),
        serde_json::json!({
            "hooks": {
                "PermissionRequest": [{
                    "matcher": matcher,
    ...
    let hook_list = ody_hooks::list_hooks(HooksConfig {
...
fn mcp_app_resource_uri_reads_known_tool_meta_keys() {
    ...
    let output_template = serde_json::json!({
        "odysseythink/outputTemplate": "ui://widget/output-template.html",
    });
    assert_eq!(
...
#[test]
fn odysseythink_file_params_are_only_honored_for_ody_apps() {
    let meta = serde_json::json!({
        "odysseythink/fileParams": ["file"],
    });
    let meta = meta.as_object();

    assert_eq!(
        odysseythink_file_input_params_for_server(ODY_APPS_MCP_SERVER_NAME, meta),
        Some(vec!["file".to_string()])
    );
    assert_eq!(
        odysseythink_file_input_params_for_server("minimaltest", meta),
        None
    );
}
...
async fn mcp_result_telemetry_records_allowlisted_span_fields() {
    ...
    assert!(
        logs.contains("ody.mcp.target.id=\"com.apple.reminders\"")
            && logs.contains("ody.mcp.server_user_flow.triggered=false"),
...
async fn mcp_result_telemetry_ignores_invalid_and_missing_values() {
    ...
    .await;
    assert!(
        !invalid_logs.contains("ody.mcp.target.id=")
            && !invalid_logs.contains("ody.mcp.server_user_flow.triggered="),
    ...
    .await;
    assert!(
        !missing_logs.contains("ody.mcp.target.id=")
            && !missing_logs.contains("ody.mcp.server_user_flow.triggered="),
    ...
    assert!(
        !no_meta_logs.contains("ody.mcp.target.id=")
            && !no_meta_logs.contains("ody.mcp.server_user_flow.triggered="),
...
async fn mcp_result_telemetry_truncates_long_target_id() {
    ...
    assert!(
        logs.contains(&format!("ody.mcp.target.id=\"{truncated}\"")) && !logs.contains("tail"),
...
async fn approval_elicitation_request_uses_message_override_and_preserves_tool_params_keys() {
    ...
    assert_eq!(
        ...
        McpServerElicitationRequestParams {
            ...
            server_name: ODY_APPS_MCP_SERVER_NAME.to_string(),
...
#[test]
fn ody_apps_tool_question_uses_fallback_app_label() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        ODY_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn trusted_ody_apps_tool_question_offers_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        ODY_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn ody_apps_tool_question_without_elicitation_omits_always_allow() {
    let session_key = McpToolApprovalKey {
        server: ODY_APPS_MCP_SERVER_NAME.to_string(),
...
#[test]
fn ody_apps_connectors_support_persistent_approval() {
    let invocation = McpInvocation {
        server: ODY_APPS_MCP_SERVER_NAME.to_string(),
...
async fn mcp_tool_call_request_meta_includes_turn_metadata_for_custom_server() {
    ...
    let turn_metadata = meta
        .get(crate::X_ODY_TURN_METADATA_HEADER)
    ...
    assert_eq!(
        meta,
        serde_json::json!({
            crate::X_ODY_TURN_METADATA_HEADER: expected_turn_metadata,
        })
    );
}
...
#[test]
fn mcp_tool_call_item_metadata_only_trusts_ody_apps_identity() {
    let mut metadata = approval_metadata(
    ...
    assert_eq!(
        McpToolCallItemMetadata::from_tool_metadata(ODY_APPS_MCP_SERVER_NAME, Some(&metadata),),
...
#[tokio::test]
async fn ody_apps_tool_call_request_meta_includes_turn_metadata_and_ody_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = turn_context
        .turn_metadata_state
        .current_meta_value_for_mcp_request(mcp_turn_metadata_context(&turn_context))
    ...
    let metadata = McpToolApprovalMetadata {
        ...
        ody_apps_meta: Some(
            ...
            .as_object()
            .cloned()
            .expect("_ody_apps metadata should be an object"),
        ),
        odysseythink_file_input_params: None,
    };
...
#[tokio::test]
async fn ody_apps_tool_call_request_meta_includes_call_id_without_existing_ody_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = turn_context
        .turn_metadata_state
        .current_meta_value_for_mcp_request(mcp_turn_metadata_context(&turn_context))
...
fn ody_apps_auth_failure_result() -> CallToolResult {
    CallToolResult {
        content: vec![serde_json::json!({
            "type": "text",
...
fn ody_apps_auth_failure_metadata() -> McpToolApprovalMetadata {
    approval_metadata(
...
#[tokio::test]
async fn ody_apps_auth_elicitation_feature_disabled_returns_original_result() {
    let (session, turn_context, rx_event) = make_session_and_context_with_rx().await;
    install_host_owned_ody_apps_manager(&session, &turn_context).await;
    let result = ody_apps_auth_failure_result();
    let metadata = ody_apps_auth_failure_metadata();

    let returned = maybe_request_ody_apps_auth_elicitation(
        &session,
        &turn_context,
        "call_123",
        ODY_APPS_MCP_SERVER_NAME,
...
#[tokio::test]
async fn ody_apps_auth_elicitation_non_host_owned_server_returns_original_result() {
...
#[tokio::test]
async fn ody_apps_auth_elicitation_disallowed_by_policy_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    install_host_owned_ody_apps_manager(&session, &turn_context).await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
...
#[tokio::test]
async fn ody_apps_auth_elicitation_granular_mcp_disabled_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    install_host_owned_ody_apps_manager(&session, &turn_context).await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
...
#[tokio::test]
async fn ody_apps_auth_elicitation_feature_enabled_requests_elicitation() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    install_host_owned_ody_apps_manager(&session, &turn_context).await;
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let request_task = tokio::spawn({
        ...
        async move {
            maybe_request_ody_apps_auth_elicitation(
                &session,
                &turn_context,
                "call_123",
                ODY_APPS_MCP_SERVER_NAME,
    ...
    assert_eq!(request.server_name, ODY_APPS_MCP_SERVER_NAME);
    assert_eq!(
        request.id,
        ody_protocol::mcp::RequestId::String("ody_apps_auth_call_123".to_string())
    );
    assert!(matches!(
        request.request,
        ody_protocol::approvals::ElicitationRequest::Url { .. }
    ));

    session
        .resolve_elicitation(
            ODY_APPS_MCP_SERVER_NAME.to_string(),
            rmcp::model::RequestId::String("ody_apps_auth_call_123".into()),
            ElicitationResponse {
...
async fn guardian_review_decision_maps_to_mcp_tool_decision() {
    ...
    session.services.guardian_rejections.lock().await.insert(
        ...
        crate::guardian::GuardianRejection {
            ...
            source: ody_protocol::protocol::GuardianAssessmentDecisionSource::Agent,
        },
    );
...
#[test]
fn approval_elicitation_meta_includes_connector_source_for_ody_apps() {
    assert_eq!(
        build_mcp_tool_approval_elicitation_meta(
            ODY_APPS_MCP_SERVER_NAME,
            Some(&approval_metadata(
...
#[tokio::test]
async fn persist_ody_app_tool_approval_writes_tool_override() {
    ...
    persist_ody_app_tool_approval(&config, "calendar", "calendar/list_events")
        .await
...
async fn custom_mcp_tool_approval_mode_uses_plugin_mcp_policy() {
    ...
    write_sample_plugin_mcp(ody_home.as_path());
    std::fs::write(
...
async fn maybe_persist_mcp_tool_approval_reloads_session_config_for_custom_server() {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .ody_home(ody_home.clone().to_path_buf())
        .build()
        .await
        .expect("load config");
    turn_context.config = Arc::new(config);
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    let project_ody_dir = project_dir.path().join(".ody");
    std::fs::create_dir_all(&project_ody_dir).expect("create project .ody dir");
    std::fs::write(
        project_ody_dir.join(CONFIG_TOML_FILE),
        "[mcp_servers.docs]\ncommand = \"docs-server\"\n",
    )
    ...
    ConfigEditsBuilder::new(&ody_home)
        .set_project_trust_level(
            project_dir.path(),
            ody_protocol::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
    ...
    let contents = std::fs::read_to_string(project_ody_dir.join(CONFIG_TOML_FILE))
...
async fn approve_mode_skips_guardian_in_every_permission_mode() {
    ...
    for approval_policy in [
        ...
        turn_context.auth_manager = Some(crate::test_support::auth_manager_from_auth(
            ody_login::OdyAuth::create_dummy_chatgpt_auth_for_testing(),
        ));
        turn_context
            .approval_policy
            .set(approval_policy)

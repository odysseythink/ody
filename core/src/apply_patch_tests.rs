...
fn design_artifact_at(path: &std::path::Path) -> crate::plan_artifact::PlanArtifact {
    ...
    let thread_id =
        ody_protocol::ThreadId::from_string("00000000-0000-0000-0000-000000000001").unwrap();
    crate::plan_artifact::PlanArtifact::new_design(ody_home, thread_id, "2026-07-04")
}

#[test]
fn apply_design_mode_patch_gate_denies_in_design_mode_strict() {
    let tmp = tempdir().expect("tmp");
    let path = tmp.path().join("a.txt");
    let path_uri = PathUri::from_host_native_path(&path).expect("absolute test path");
    let action = ApplyPatchAction::new_add_for_test(&path_uri, "hello".to_string());
    ...
    let result = apply_plan_mode_patch_gate(
        &collaboration_mode(ModeKind::Design),
        PlanEnforcement::Strict,
        action,
        None,
    );
...
#[test]
fn apply_design_mode_patch_gate_allows_whitelisted_design_file() {
    let tmp = tempdir().expect("tmp");
    let artifact = design_artifact_at(tmp.path());
    let design_path = artifact.path().unwrap();
    let path_uri = PathUri::from_host_native_path(&design_path).expect("absolute test path");
    let action = ApplyPatchAction::new_add_for_test(&path_uri, "# Design\n".to_string());
    ...
    assert!(
        result.is_none(),
        "whitelisted design file should proceed to normal safety assessment"
    );
}

...
use crate::mcp_odysseythink_file::rewrite_mcp_tool_arguments_for_odysseythink_files;
...
use ody_connectors::AppToolPolicy;
use ody_connectors::AppToolPolicyEvaluator;
...
use ody_protocol::items::McpToolCallItem;
use ody_protocol::items::McpToolCallStatus;
use ody_protocol::items::TurnItem;
use ody_protocol::mcp::CallToolResult;
use ody_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_TOOL_APPROVAL_KIND_KEY;
use ody_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_TOOL_APPROVAL_KIND_MCP_TOOL_CALL;
use ody_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_CONNECTOR_DESCRIPTION_KEY;
use ody_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_TOOL_APPROVAL_CONNECTOR_ID_KEY;
use ody_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY;
use ody_protocol::mcp_approval_meta::PERSIST_ALWAYS as MCP_TOOL_APPROVAL_PERSIST_ALWAYS;
use ody_protocol::mcp_approval_meta::PERSIST_KEY as MCP_TOOL_APPROVAL_PERSIST_KEY;
use ody_protocol::mcp_approval_meta::PERSIST_SESSION as MCP_TOOL_APPROVAL_PERSIST_SESSION;
use ody_protocol::mcp_approval_meta::SOURCE_CONNECTOR as MCP_TOOL_APPROVAL_SOURCE_CONNECTOR;
use ody_protocol::mcp_approval_meta::SOURCE_KEY as MCP_TOOL_APPROVAL_SOURCE_KEY;
use ody_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_TOOL_DESCRIPTION_KEY;
...
const MCP_CALL_COUNT_METRIC: &str = "ody.mcp.call";
const MCP_CALL_DURATION_METRIC: &str = "ody.mcp.call.duration_ms";
const MCP_RESULT_TELEMETRY_META_KEY: &str = "ody/telemetry";
...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let app_tool_policy = if server == ODY_APPS_MCP_SERVER_NAME {
    ...
    let approval_mode = if server == ODY_APPS_MCP_SERVER_NAME {
    ...
    if server == ODY_APPS_MCP_SERVER_NAME && !app_tool_policy.enabled {
...
impl McpToolCallItemMetadata {
    fn from_tool_metadata(server: &str, metadata: Option<&McpToolApprovalMetadata>) -> Self {
        let trusted_mcp_app_metadata = if server == ODY_APPS_MCP_SERVER_NAME {
            metadata
        } else {
            None
        };
...
async fn handle_approved_mcp_tool_call(
    ...
    let rewrite = rewrite_mcp_tool_arguments_for_odysseythink_files(
        sess,
        turn_context,
        arguments_value.clone(),
        metadata.and_then(|metadata| metadata.odysseythink_file_input_params.as_deref()),
    )
    .await;
    ...
    .await;
    maybe_track_ody_app_used(sess, turn_context, &server, &tool_name).await;
...
fn mcp_tool_call_span(
    ...
    let span = tracing::info_span!(
        ...
        server.port = Empty,
        ody.mcp.target.id = Empty,
        ody.mcp.server_user_flow.triggered = Empty,
    );
...
async fn maybe_request_ody_apps_auth_elicitation(
    sess: &Session,
    turn_context: &TurnContext,
    call_id: &str,
    server: &str,
    ...
    if !sess
        ...
        .mcp_connection_manager
        .load_full()
        .is_host_owned_ody_apps_server(server)
    {
        return result;
    }
    ...
    let install_url = connector_id.map(|connector_id| {
        ody_connectors::metadata::connector_install_url(
    ...
    refresh_ody_apps_after_connector_auth(sess, turn_context).await;
    auth_elicitation_completed_result(&plan.auth_failure, result.meta)
}

async fn refresh_ody_apps_after_connector_auth(sess: &Session, turn_context: &TurnContext) {
    let mcp_tools_result = {
        let manager = sess.services.mcp_connection_manager.load_full();
        manager.hard_refresh_ody_apps_tools_cache().await
    };
    ...
    match mcp_tools_result {
        ...
        Err(err) => {
            tracing::warn!("failed to refresh Ody Apps tools after connector auth: {err:#}");
        }
    }
}
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    let server_environment_id = mcp_connection_manager
        ...
        .unwrap_or(ody_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID);
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                ody_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
fn sandbox_cwd_for_mcp_server(turn_context: &TurnContext, environment_id: &str) -> Option<PathUri> {
    ...
    if environment_id == ody_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID {
        #[allow(deprecated)]
...
async fn maybe_track_ody_app_used(
    sess: &Session,
    turn_context: &TurnContext,
    server: &str,
    tool_name: &str,
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    mcp_app_resource_uri: Option<String>,
    ody_apps_meta: Option<serde_json::Map<String, serde_json::Value>>,
    odysseythink_file_input_params: Option<Vec<String>>,
}

const MCP_TOOL_OPENAI_OUTPUT_TEMPLATE_META_KEY: &str = "odysseythink/outputTemplate";
...
async fn custom_mcp_tool_approval_mode(
    ...
    let user_configured_mode = turn_context
        ...
        .cloned()
        .and_then(|value| {
            HashMap::<String, ody_config::types::McpServerConfig>::deserialize(value).ok()
        })
        .and_then(|servers| {
...
fn build_mcp_tool_call_request_meta(
    ...
    if server == ODY_APPS_MCP_SERVER_NAME {
        let mut ody_apps_meta = metadata
            .and_then(|metadata| metadata.ody_apps_meta.clone())
            .unwrap_or_default();
        ody_apps_meta.insert(
            "call_id".to_string(),
            serde_json::Value::String(call_id.to_string()),
        );
        request_meta.insert(
            MCP_TOOL_ODY_APPS_META_KEY.to_string(),
            serde_json::Value::Object(ody_apps_meta),
        );
    }
...
pub(crate) const MCP_TOOL_APPROVAL_DECLINE_SYNTHETIC: &str = "__ody_mcp_decline__";
...
fn session_mcp_tool_approval_key(
    ...
    if invocation.server == ODY_APPS_MCP_SERVER_NAME && connector_id.is_none() {
        return None;
    }
...
pub(crate) async fn lookup_mcp_tool_metadata(
    ...
    let connector_description = if server == ODY_APPS_MCP_SERVER_NAME {
    ...
    Some(McpToolApprovalMetadata {
        ...
        odysseythink_file_input_params: odysseythink_file_input_params_for_server(
            server,
...
fn odysseythink_file_input_params_for_server(
    server: &str,
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == ODY_APPS_MCP_SERVER_NAME {
        let Some(connector_id) = key.connector_id.clone() else {
            remember_mcp_tool_approval(sess, key).await;
            return;
        };
        persist_ody_app_tool_approval(&turn_context.config, &connector_id, &tool_name).await
    } else {
...
async fn persist_ody_app_tool_approval(
    config: &Config,
    connector_id: &str,
    tool_name: &str,
) -> anyhow::Result<()> {
...
fn user_mcp_server_is_configured(config: &Config, server: &str) -> anyhow::Result<bool> {
    ...
    let servers =
        HashMap::<String, ody_config::types::McpServerConfig>::deserialize(mcp_servers_toml)?;

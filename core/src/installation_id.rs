...
pub async fn resolve_installation_id(ody_home: &AbsolutePathBuf) -> Result<String> {
    let path = ody_home.join(INSTALLATION_ID_FILENAME);
    fs::create_dir_all(ody_home).await?;
...
mod tests {
    ...
    async fn resolve_installation_id_generates_and_persists_uuid() {
        let ody_home = TempDir::new().expect("create temp dir");
        let ody_home_abs = ody_home.path().abs();
        let persisted_path = ody_home.path().join(INSTALLATION_ID_FILENAME);

        let installation_id = resolve_installation_id(&ody_home_abs)
            .await
    ...
    async fn resolve_installation_id_reuses_existing_uuid() {
        ...
        std::fs::write(
            ody_home.path().join(INSTALLATION_ID_FILENAME),
            existing.clone(),
        )
        .expect("write installation id");

        let resolved = resolve_installation_id(&ody_home_abs)
            .await
            .expect("resolve installation id");
    ...
    async fn resolve_installation_id_rewrites_invalid_file_contents() {
        let ody_home = TempDir::new().expect("create temp dir");
        let ody_home_abs = ody_home.path().abs();
        std::fs::write(ody_home.path().join(INSTALLATION_ID_FILENAME), "not-a-uuid")
        ...
        assert_eq!(
            std::fs::read_to_string(ody_home.path().join(INSTALLATION_ID_FILENAME))

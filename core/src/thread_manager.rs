...
use ody_thread_store::InMemoryThreadStore;
use ody_thread_store::LocalThreadStore;
use ody_thread_store::LocalThreadStoreConfig;
use ody_thread_store::ReadThreadByRolloutPathParams;
use ody_thread_store::ReadThreadParams;
use ody_thread_store::StoredThread;
use ody_thread_store::ThreadMetadataPatch;
...
struct TempOdyHomeGuard {
    path: PathBuf,
}

impl Drop for TempOdyHomeGuard {
    fn drop(&mut self) {
        let _ = std::fs::remove_dir_all(&self.path);
    }
}

/// Represents a newly created Ody thread (formerly called a conversation), including the first event
/// (which is [`EventMsg::SessionConfigured`]).
pub struct NewThread {
    pub thread_id: ThreadId,
    pub thread: Arc<OdyThread>,
...
pub struct ThreadManager {
    ...
    _test_ody_home_guard: Option<TempOdyHomeGuard>,
}
...
pub struct StartThreadOptions {
    ...
    pub dynamic_tools: Vec<ody_protocol::dynamic_tools::DynamicToolSpec>,
...
pub fn thread_store_from_config(
    ...
    match &config.experimental_thread_store {
        ThreadStoreConfig::Local => {
            ...
            {
                ody_rollout::spawn_rollout_compression_worker(config.ody_home.to_path_buf());
            }
...
impl ThreadManager {
    ...
    pub fn new(
        ...
        let ody_home = config.ody_home.clone();
    ...
    pub(crate) fn with_models_provider_home_and_catalog_for_tests(
        ...
        let skills_ody_home = match AbsolutePathBuf::from_absolute_path_checked(&ody_home) {
            ...
            Err(err) => panic!("test ody_home should be absolute: {err}"),
        };
        let (thread_created_tx, _) = broadcast::channel(THREAD_CREATED_CHANNEL_CAPACITY);
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> OdyResult<Arc<OdyThread>> {
        self.state.get_thread(thread_id).await
    }

    /// Updates metadata for loaded and cold threads through one entrypoint.
    ///
    /// Loaded threads route through `OdyThread`/`LiveThread`, so metadata changes stay ordered
        ...
        if let Some(state_db_ctx) = self.state.state_db() {
            for status in [
                ...
                for descendant_id in state_db_ctx
                    ...
                    .await
                    .map_err(|err| {
                        OdyErr::Fatal(format!("failed to load thread-spawn descendants: {err}"))
                    })?
                {
    ...
    pub async fn start_thread(&self, config: Config) -> OdyResult<NewThread> {
    ...
    pub async fn start_thread_with_tools(
        &self,
        config: Config,
        dynamic_tools: Vec<ody_protocol::dynamic_tools::DynamicToolSpec>,
    ) -> OdyResult<NewThread> {
    ...
    /// as `Arc<OdyThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<OdyThread>> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(&self, thread_id: ThreadId) -> OdyResult<Arc<OdyThread>> {
        let threads = self.threads.read().await;
        match threads.get(&thread_id) {
            Some(thread) if !thread.session_source.is_internal() => Ok(thread.clone()),
            Some(_) | None => Err(OdyErr::ThreadNotFound(thread_id)),
        }
    }
    ...
    pub(crate) async fn read_stored_thread(
        ...
        self.thread_store
            ...
            .map_err(|err| match err {
                ...
                err => OdyErr::Fatal(format!("failed to read stored thread {thread_id}: {err}")),
            })
    }

    /// Send an operation to a thread by ID.
    pub(crate) async fn send_op(&self, thread_id: ThreadId, op: Op) -> OdyResult<String> {
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<OdyThread>> {
    ...
    async fn user_instructions_for_spawn(
        ...
        let instructions = match inherited_thread_id {
            ...
            Some(thread_id) => match self.get_thread(thread_id).await {
                Ok(thread) => thread.ody.session.user_instructions().await,
                Err(_) => None,
            },
            None => None,
        };
    ...
    pub(crate) async fn spawn_thread_with_source(
        ...
        .await?;
        let new_thread = self
            .finalize_thread_spawn(ody, thread_id, tracked_session_source)
            .await?;
    ...
    async fn finalize_thread_spawn(
        ...
        let event = ody.next_event().await?;
        let session_configured = match event {
            ...
            _ => {
                return Err(OdyErr::SessionConfiguredNotFirstEvent);
            }
        };

        {
            ...
            if let std::collections::hash_map::Entry::Vacant(e) = threads.entry(thread_id) {
                let thread = Arc::new(OdyThread::new(
                    ody,
        ...
        if let Err(err) = ody.shutdown_and_wait().await {
            warn!("failed to shut down duplicate thread {thread_id}: {err}");
        }
        Err(OdyErr::InvalidRequest(format!(
    ...
    async fn parent_rollout_thread_trace_for_source(
        ...
        session_source: &SessionSource,
        initial_history: &InitialHistory,
    ) -> ody_rollout_trace::ThreadTraceContext {
        ...
        let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            ...
            return ody_rollout_trace::ThreadTraceContext::disabled();
        };
        ...
        self.get_thread(*parent_thread_id)
            .await
            .ok()
            .map(|thread| thread.ody.session.services.rollout_thread_trace.clone())
            .unwrap_or_else(ody_rollout_trace::ThreadTraceContext::disabled)
    }
}
...
fn thread_store_rollout_read_error(err: ThreadStoreError) -> OdyErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => OdyErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => OdyErr::InvalidRequest(message),
        err => OdyErr::Fatal(format!("failed to read thread by rollout path: {err}")),
    }
}

fn thread_store_metadata_update_error(thread_id: ThreadId, err: ThreadStoreError) -> OdyErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => OdyErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => OdyErr::InvalidRequest(message),
        ThreadStoreError::Unsupported { operation } => OdyErr::UnsupportedOperation(format!(
            "thread metadata update is not supported by this store: {operation}"
        )),
        err => OdyErr::Fatal(format!(

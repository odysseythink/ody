...
use ody_protocol::protocol::ThreadSettingsAppliedEvent;
...
use ody_protocol::mcp::RequestId as ProtocolRequestId;
...
pub async fn resolve_elicitation(
    ...
    decision: ody_protocol::approvals::ElicitationAction,
    content: Option<Value>,
    meta: Option<Value>,
) {
    let action = match decision {
        ody_protocol::approvals::ElicitationAction::Accept => ElicitationAction::Accept,
        ody_protocol::approvals::ElicitationAction::Decline => ElicitationAction::Decline,
        ody_protocol::approvals::ElicitationAction::Cancel => ElicitationAction::Cancel,
    };
    ...
    if let Err(err) = sess.flush_rollout().await {
        sess.send_event(
            ...
            EventMsg::Warning(WarningEvent {
                message: format!(
                    "Rolled the thread back, but failed to save the rollback marker. Ody will continue retrying. Error: {err}"
                ),
            }),
        )
        .await;
...
async fn shutdown_session_runtime(sess: &Arc<Session>) {
    sess.input_queue.close_host_notifications().await;
    crate::tools::handlers::HumanAcceptanceHandler::stop_sites(sess.thread_id).await;
...
async fn emit_thread_stop_lifecycle(sess: &Session) {
    for contributor in sess.services.extensions.thread_lifecycle_contributors() {
        contributor
            .on_thread_stop(ody_extension_api::ThreadStopInput {
...
pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
    ...
    info!("Shutting down Ody instance");
    ...
    sess.services
        .rollout_thread_trace
        .record_ended(ody_rollout_trace::RolloutStatus::Completed);
    true
}

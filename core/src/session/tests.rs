...
use ody_config::permissions_toml::FilesystemPermissionsToml;
use ody_config::permissions_toml::NetworkToml;
...
use ody_execpolicy::NetworkRuleProtocol;
use ody_execpolicy::Policy;
use ody_network_proxy::NetworkProxyConfig;
use ody_otel::MetricsClient;
use ody_otel::MetricsConfig;
use ody_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use ody_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use ody_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use ody_otel::THREAD_SKILLS_TRUNCATED_METRIC;
...
use ody_protocol::protocol::TokenCountEvent;
use ody_protocol::protocol::TokenUsage;
use ody_protocol::protocol::TokenUsageInfo;
use ody_protocol::protocol::TurnAbortedEvent;
...
use ody_protocol::mcp::CallToolResult as McpCallToolResult;
...
async fn regular_turn_emits_turn_started_with_trace_id_without_waiting_for_startup_prewarm() {
    let _trace_test_context = install_test_tracing("ody-core-tests");
    ...
    let response = session
        .request_mcp_server_elicitation(
            ...
            McpServerElicitationRequestParams {
                ...
                server_name: "ody_apps".to_string(),
...
fn write_project_hooks(dot_ody: &Path) -> std::io::Result<()> {
    std::fs::create_dir_all(dot_ody)?;
    std::fs::write(
        dot_ody.join("hooks.json"),
        r#"{
  "hooks": {
    "SessionStart": [
      {
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        ody_home.join(ody_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
...
) -> std::io::Result<Vec<ody_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&ody_hooks::SessionStartRequest {
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn ody_network_proxy::NetworkPolicyDecider> = Arc::new({
        let decider_calls = Arc::clone(&decider_calls);
        move |_request| {
            decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            async { ody_network_proxy::NetworkDecision::ask("not_allowed") }
        }
    });
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> ody_sandboxing::SandboxablePreference {
            ody_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(ody_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: ody_tools::ToolName::plain("probe"),
    };
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let ody_home = session.ody_home().await;
    std::fs::create_dir_all(&ody_home).expect("create ody home");
    let config_toml_path = ody_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    let ody_home = session.ody_home().await;
    std::fs::create_dir_all(&ody_home).expect("create ody home");
    let base_config_path = ody_home.join(CONFIG_TOML_FILE);
    let profile_config_path = ody_home.join("work.config.toml");
    std::fs::write(
        &base_config_path,
    ...
    assert_eq!(
        config
            ...
            .map(ody_utils_absolute_path::AbsolutePathBuf::as_path),
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    let session = make_session_with_config(|config| {
        config
            .features
            .enable(Feature::OdyHooks)
            .expect("enable Ody hooks");
    })
    .await?;
    let ody_home = session.ody_home().await;
    std::fs::create_dir_all(&ody_home)?;
    let config_toml_path = ody_home.join(CONFIG_TOML_FILE);
    let user_config: ody_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let config = session.get_config().await;
    let hook_list = ody_hooks::list_hooks(ody_hooks::HooksConfig {
        feature_enabled: true,
        ...
        ..ody_hooks::HooksConfig::default()
    });
    assert_eq!(hook_list.hooks.len(), 1);
    assert_eq!(
        hook_list.hooks[0].trust_status,
        ody_protocol::protocol::HookTrustStatus::Untrusted
    );

    let trusted_user_config: ody_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: ody_config::MatcherGroup {
                matcher: None,
                hooks: vec![ody_config::HookHandlerConfig::Command {
        ...
        let identity = ody_config::TomlValue::try_from(identity)?;
        ody_config::version_for_toml(&identity)
    };
...
disabled_tools = [
  ...
  { type = "plugin", id = "slack@odysseythink-curated" },
]
"#,
    )
    .expect("write user config");
    ...
    assert_eq!(
        ...
        vec![
            ...
            ToolSuggestDisabledTool::plugin("slack@odysseythink-curated"),
        ]
    );
}
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl ody_extension_api::TokenUsageContributor for TokenUsageRecorder {
        fn on_token_usage<'a>(
            &'a self,
            session_store: &'a ody_extension_api::ExtensionData,
            thread_store: &'a ody_extension_api::ExtensionData,
            turn_store: &'a ody_extension_api::ExtensionData,
            token_usage: &'a TokenUsageInfo,
        ) -> ody_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
    ...
    let mut builder = ody_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
...
async fn turn_start_lifecycle_exposes_turn_metadata_and_token_baseline() {
    ...
    impl ody_extension_api::TurnLifecycleContributor for TurnStartRecorder {
        fn on_turn_start<'a>(
            &'a self,
            input: ody_extension_api::TurnStartInput<'a>,
        ) -> ody_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_error_lifecycle_exposes_error_and_stores() {
    ...
    impl ody_extension_api::TurnLifecycleContributor for TurnErrorRecorder {
        fn on_turn_error<'a>(
            &'a self,
            input: ody_extension_api::TurnErrorInput<'a>,
        ) -> ody_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
    ...
    let expected = RecordedTurnError {
        ...
        turn_level_id: turn_context.sub_id.clone(),
        turn_id: turn_context.sub_id.clone(),
        error: OdyErrorInfo::UsageLimitExceeded,
        saw_session_store: true,
        saw_thread_store: true,
    };

    session
        .emit_turn_error_lifecycle(&turn_context, OdyErrorInfo::UsageLimitExceeded)
        .await;
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl ody_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
        fn on_config_changed(
            &self,
            session_store: &ody_extension_api::ExtensionData,
            thread_store: &ody_extension_api::ExtensionData,
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: ody_protocol::protocol::NetworkAccess::Restricted,
    };
...
async fn session_permission_profile_rebinds_runtime_workspace_roots() -> anyhow::Result<()> {
    let ody_home = tempfile::TempDir::new()?;
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    initial.ody.ensure_rollout_materialized().await;
    initial
        .ody
        .flush_rollout()
        .await
    ...
    fork_config.permissions.approval_policy =
        ody_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "ody_core__ody_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let previous_context_item = TurnContextItem {
        ...
        summary: ody_protocol::config_types::ReasoningSummary::Auto,
    };
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            ody_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            ody_protocol::protocol::UserMessageEvent {
                client_id: None,
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            ody_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_thread_history() {
    ...
    assert_eq!(
        error_event.ody_error_info,
        Some(OdyErrorInfo::ThreadRollbackFailed)
    );
    ...
    loop {
        ...
        match evt.msg {
            EventMsg::Error(payload)
                if payload.ody_error_info == Some(OdyErrorInfo::ThreadRollbackFailed) =>
            {
                return payload;
            }
            _ => continue,
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile = ody_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        ody_utils_absolute_path::canonicalize_preserving_symlinks(external_write_dir.path())
...
async fn active_profile_update_rebuilds_network_proxy_config() -> std::io::Result<()> {
    ...
    std::fs::write(
        ody_home.path().join(ody_config::CONFIG_TOML_FILE),
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let ody_home = parent_config.ody_home.clone();
    let skill_dir = ody_home.join("skills").join("demo");
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&ody_exec_server::LOCAL_FS));
    ...
    let role_path = ody_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_packaged_zsh() {
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(ody_extension_api::ExtensionRegistryBuilder::new().build()),
        ody_extension_api::ExtensionDataInit::default(),
        /*supports_form_elicitation*/ false,
        AgentControl::default(),
        environment_manager,
        /*inherited_environments*/ None,
        /*analytics_events_client*/ None,
        Arc::new(ody_thread_store::LocalThreadStore::new(
            ody_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        ody_rollout_trace::ThreadTraceContext::disabled(),
        /*attestation_provider*/ None,
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: ody_rollout_trace::ThreadTraceContext::disabled(),
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(ody_extension_api::ExtensionRegistryBuilder::new().build()),
        session_extension_data: ody_extension_api::ExtensionData::new(
            agent_control.session_id().to_string(),
        ),
        thread_extension_data: ody_extension_api::ExtensionData::new(thread_id.to_string()),
        flow_runner: Arc::new(crate::flow::SessionFlowRunner::new()),
        mcp_thread_init: ody_extension_api::ExtensionDataInit::default(),
        supports_form_elicitation: std::sync::atomic::AtomicBool::new(false),
        agent_control,
        ...
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(ody_thread_store::LocalThreadStore::new(
            ody_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        attestation_provider: None,
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            ody_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(ody_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
    ...
    assert_eq!(
        session
            .granted_turn_permissions(ody_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
        None
    );
}
    ...
    *session.active_turn.lock().await = Some(current_active_turn);
    ...
    session
        .record_granted_request_permissions_for_turn(
            ...
            ody_exec_server::LOCAL_ENVIRONMENT_ID,
    ...
    assert_eq!(
        originating_turn_state
            .lock()
            .await
            .granted_permissions(ody_exec_server::LOCAL_ENVIRONMENT_ID),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = ody_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(ody_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            ...
            session
                .request_permissions_for_environment(
                    &turn_context,
                    call_id,
                    ody_protocol::request_permissions::RequestPermissionsArgs {
                        environment_id: None,
    ...
    assert_eq!(
        ...
        Some(ody_exec_server::LOCAL_ENVIRONMENT_ID)
    );
    #[allow(deprecated)]
    let turn_cwd = turn_context.cwd.clone();
        ...
        async move {
            handler
                .handle(ToolInvocation {
                    ...
                    tracker,
                    call_id,
                    tool_name: ody_tools::ToolName::plain("request_permissions"),
...
async fn request_permissions_response_materializes_session_cwd_grants_before_recording() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        session
            .granted_session_permissions(ody_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
...
async fn user_turn_updates_approvals_reviewer() {
    ...
    handlers::user_input_or_turn(
        ...
        Op::UserInput {
            ...
            thread_settings: ody_protocol::protocol::ThreadSettingsOverrides {
                ...
                approvals_reviewer: Some(ody_config::types::ApprovalsReviewer::AutoReview),
    ...
    assert_eq!(
        ...
        ody_config::types::ApprovalsReviewer::AutoReview
    );
}
...
async fn primary_environment_uses_first_turn_environment() {
    ...
    let second_cwd_uri = ody_utils_path_uri::PathUri::from_abs_path(&second_cwd);
    turn_context
        .environments
        .turn_environments
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        ody_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context = ody_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(ody_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn ody_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    assert_eq!(
        ody_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn submission_loop_channel_close_emits_thread_stop_lifecycle() {
    ...
    impl ody_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadStopRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: ody_extension_api::ThreadStopInput<'a>,
        ) -> ody_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    impl ody_extension_api::ThreadLifecycleContributor<crate::config::Config> for LifecycleRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: ody_extension_api::ThreadStopInput<'a>,
        ) -> ody_extension_api::ExtensionFuture<'a, ()> {
    ...
    impl ody_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        fn on_turn_abort<'a>(
            &'a self,
            input: ody_extension_api::TurnAbortInput<'a>,
        ) -> ody_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
async fn shutdown_and_wait_allows_multiple_waiters() {
    ...
    let waiter_1 = {
        let ody = Arc::clone(&ody);
        tokio::spawn(async move { ody.shutdown_and_wait().await })
    };
    let waiter_2 = {
        let ody = Arc::clone(&ody);
        tokio::spawn(async move { ody.shutdown_and_wait().await })
...
async fn shutdown_and_wait_shuts_down_tracked_ephemeral_guardian_review() {
    ...
    parent_session
        .guardian_review_session
        .register_ephemeral_for_test(child_ody)
        .await;
...
async fn record_context_update_items(
    ...
    session
        .record_context_updates_and_set_reference_context_item(previous_context, None)
        .await;
    let previous_len = session.clone_history().await.raw_items().len();

    session
        .record_context_updates_and_set_reference_context_item(current_context, None)
        .await;
    let history = session.clone_history().await;
...
impl ody_extension_api::ContextContributor for PromptExtensionTestContributor {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a ody_extension_api::ExtensionData,
        thread_store: &'a ody_extension_api::ExtensionData,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<ody_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
            thread_store
...
-> Arc<ody_extension_api::ExtensionRegistry<crate::config::Config>> {
...
impl ody_extension_api::ContextContributor for TurnContextExtensionTestContributor {
    fn contribute_turn_context<'a>(
        &'a self,
        input: ody_extension_api::TurnContextContributionInput<'a>,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<ody_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
...
async fn handle_output_item_done_records_image_save_history_message() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(ody_extension_api::ExtensionData::new(
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    let text_element = ody_protocol::user_input::TextElement::new(
        ody_protocol::user_input::ByteRange { start: 5, end: 12 },
...
async fn task_finish_emits_thread_idle_lifecycle_after_active_turn_clears() {
    ...
    impl ody_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            input: ody_extension_api::ThreadIdleInput<'a>,
        ) -> ody_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
async fn thread_idle_lifecycle_waits_for_trigger_turn_mailbox_work() {
    ...
    impl ody_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            _input: ody_extension_api::ThreadIdleInput<'a>,
        ) -> ody_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            })
...
#[tokio::test]
async fn try_start_turn_if_idle_rejects_design_mode_without_injecting() {
    let (sess, _tc, _rx) = make_session_and_context_with_rx().await;
    let mut collaboration_mode = sess.collaboration_mode().await;
    ...
    let err = sess
        .try_start_turn_if_idle(vec![item.clone()])
        .await
        .expect_err("design mode should reject automatic idle input");

    // Design reuses the PlanMode rejection reason to avoid protocol churn (D4 step 4).
    assert_eq!(TryStartTurnIfIdleRejectionReason::PlanMode, err.reason());
    assert_eq!(vec![item], err.into_input());
    assert!(sess.active_turn.lock().await.is_none());
    assert_eq!(
...
async fn tool_calls_reopen_mailbox_delivery_for_current_turn() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(ody_extension_api::ExtensionData::new(tc.sub_id.clone())),
...
async fn rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    use crate::turn_diff_tracker::TurnDiffTracker;
    use ody_protocol::protocol::AskForApproval;
    use ody_tools::ShellCommandBackendConfig;
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            call_id,
            tool_name: ody_tools::ToolName::plain(tool_name),
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let ody_home = temp.path().join("home");
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let root_dot_ody = project_root.join(".ody");
    let nested_dot_ody = nested.join(".ody");

    std::fs::create_dir_all(&ody_home)?;
    std::fs::create_dir_all(&nested_dot_ody)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&root_dot_ody)?;
    write_project_hooks(&nested_dot_ody)?;
    write_project_trust_config(&ody_home, &[(&nested, TrustLevel::Trusted)]).await?;
    ...
    let expected_source_path = ody_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
        nested_dot_ody.join("hooks.json"),
    )?;
    assert_eq!(
        hook_list
            .hooks
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let dot_ody = project_root.join(".ody");
    std::fs::create_dir_all(&nested)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&dot_ody)?;
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let ody_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&ody_home)?;
        write_project_trust_config(&ody_home, &trust_entries).await?;

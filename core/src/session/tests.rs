...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        /*auth_manager*/ None,
        thread_id,
        ModelProviderInfo::create_odysseythink_provider(/* base_url */ /*base_url*/ None),
...
async fn emit_subagent_session_started_includes_fork_lineage_from_session_configuration() {
    ...
    Mock::given(method("POST"))
        .and(path("/ody/analytics-events/events"))
    ...
    emit_subagent_session_started(
        ...
        AppServerClientMetadata {
            client_name: Some("ody-tui".to_string()),
            ...
            if let Some(requests) = server.received_requests().await {
                for request in requests {
                    ...
                    if let Some(event) = payload["events"].as_array().and_then(|events| {
                        events
                            .iter()
                            .find(|event| event["event_type"] == "ody_thread_initialized")
                    }) {
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let services = SessionServices {
        ...
        supports_odysseythink_form_elicitation: std::sync::atomic::AtomicBool::new(false),
        agent_control,

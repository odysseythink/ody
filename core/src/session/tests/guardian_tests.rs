    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if matches!(
                event.msg,
                ody_protocol::protocol::EventMsg::GuardianAssessment(_)
            ) {
                break;
            }
        }
...
async fn guardian_allows_shell_command_additional_permissions_requests_past_policy_validation() {
    ...
    let handler = crate::tools::handlers::ShellCommandHandler::from(
        ody_tools::ShellCommandBackendConfig::Classic,
    );
    #[allow(deprecated)]
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: ody_tools::ToolName::plain("shell_command"),
...
async fn guardian_subagent_does_not_inherit_parent_exec_policy_rules() {
    let ody_home = tempdir().expect("create ody home");
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_ody_folder: project_dir.path().abs(),
            },
    ...
    let thread_store = Arc::new(ody_thread_store::LocalThreadStore::new(
        ody_thread_store::LocalThreadStoreConfig::from_config(&config),
        /*state_db*/ None,
    ));

    let OdySpawnOk { ody, .. } = Ody::spawn(OdySpawnArgs {
        config,
        ...
        plugins_manager,
        mcp_manager,
        extensions: ody_extension_api::empty_extension_registry(),

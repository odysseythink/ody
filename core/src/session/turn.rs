...
pub(crate) async fn run_turn(
    ...
    turn_extension_data: Arc<ody_extension_api::ExtensionData>,
    input: Vec<TurnInput>,
    ...
    loop {
        ...
        let sampling_request_result: OdyResult<_> = async {
        ...
        match sampling_request_result {
            ...
            Err(err @ OdyErr::TurnAborted) => {
                return Err(err);
            }
            Err(ody_error @ OdyErr::InvalidImageRequest()) => {
                {
                ...
                sess.track_turn_ody_error(turn_context.as_ref(), &ody_error);
                let error = OdyErrorInfo::BadRequest;
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                let event = EventMsg::Error(ErrorEvent {
                    message: "Invalid image in your last message. Please remove it and try again."
                        .to_string(),
                    ody_error_info: Some(error),
                });
...
async fn build_skills_and_plugins(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = ody_connectors::merge::merge_plugin_connectors_with_accessible(
            loaded_plugins
                .effective_apps()
                .into_iter()
    ...
    let previous_model_turn_context = Arc::new(
        turn_context
            .with_model(
                ...
                &sess.services.models_manager.load(),
            )
            .await,
    );
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = ody_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
    turn_store: Arc<ody_extension_api::ExtensionData>,
    turn_diff_tracker: SharedTurnDiffTracker,
    client_session: &mut ModelClientSession,
    responses_metadata: &OdyResponsesMetadata,
    input: Vec<ResponseItem>,
    cancellation_token: CancellationToken,
) -> OdyResult<(SamplingRequestResult, Vec<ResponseItem>)> {
    ...
    loop {
        ...
        .await
        {
            ...
            Err(OdyErr::ContextWindowExceeded) => {
...
fn agent_message_text(item: &ody_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            ody_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: ody_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(ody_protocol::items::AgentMessageItem {
...
async fn handle_assistant_item_done_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    turn_store: &ody_extension_api::ExtensionData,
    item: &ResponseItem,
    state: &mut PlanModeStreamState,
    previously_active_item: Option<&TurnItem>,
...
async fn try_run_sampling_request(
    ...
    prompt: &Prompt,
    cancellation_token: CancellationToken,
) -> OdyResult<SamplingRequestResult> {
    feedback_tags!(
        model = turn_context.model_info.slug.clone(),
        approval_policy = turn_context.approval_policy.value(),
    ...
    let outcome: OdyResult<SamplingRequestResult> = loop {
        let handle_responses = trace_span!(
            ...
            from = field::Empty,
            ody.request.reasoning_effort = %reasoning_effort,
            gen_ai.usage.input_tokens = field::Empty,
            gen_ai.usage.cache_read.input_tokens = field::Empty,
            gen_ai.usage.output_tokens = field::Empty,
            ody.usage.reasoning_output_tokens = field::Empty,
            ody.usage.total_tokens = field::Empty,
        );
        ...
        {
            Ok(event) => event,
            Err(ody_async_utils::CancelErr::Cancelled) => break Err(OdyErr::TurnAborted),
        };

...
use ody_analytics::SubAgentThreadStartedInput;
use ody_analytics::TurnOdyErrorFact;
use ody_app_server_protocol::McpServerElicitationRequest;
...
use ody_extension_api::TurnContextContributionInput;
...
use ody_network_proxy::NetworkProxyAuditMetadata;
use ody_network_proxy::normalize_host;
use ody_otel::current_span_trace_id;
use ody_otel::current_span_w3c_trace_context;
use ody_otel::set_parent_from_w3c_trace_context;
use ody_protocol::SessionId;
use ody_protocol::ThreadId;
use ody_protocol::approvals::ElicitationRequestEvent;
use ody_protocol::approvals::ExecPolicyAmendment;
use ody_protocol::approvals::NetworkPolicyAmendment;
use ody_protocol::approvals::NetworkPolicyRuleAction;
...
use ody_protocol::protocol::HasLegacyEvent;
...
use ody_protocol::protocol::RawResponseItemEvent;
...
use ody_protocol::protocol::TurnContextNetworkItem;
...
use ody_protocol::request_permissions::RequestPermissionsEvent;
...
use ody_rollout_trace::AgentResultTracePayload;
use ody_rollout_trace::ThreadStartedTraceMetadata;
use ody_rollout_trace::ThreadTraceContext;
use ody_sandboxing::policy_transforms::intersect_permission_profiles;
use ody_shell_command::parse_command::parse_command;
use ody_terminal_detection::user_agent;
use ody_thread_store::CreateThreadParams;
use ody_thread_store::LiveThread;
use ody_thread_store::LiveThreadInitGuard;
use ody_thread_store::LocalThreadStore;
use ody_thread_store::ReadThreadParams;
use ody_thread_store::ResumeThreadParams;
...
use ody_mcp::effective_mcp_servers_from_configured;
use ody_mcp::host_owned_ody_apps_enabled;
use ody_otel::SessionTelemetry;
use ody_otel::THREAD_STARTED_METRIC;
...
use ody_protocol::protocol::McpServerRefreshConfig;
use ody_protocol::protocol::ModelRerouteEvent;
...
use ody_protocol::protocol::SessionNetworkProxyRuntime;
use ody_protocol::protocol::StreamErrorEvent;
use ody_protocol::protocol::Submission;
use ody_protocol::protocol::ThreadMemoryMode;
...
/// The high-level interface to the Ody system.
...
/// Wrapper returned by [`Ody::spawn`] containing the spawned [`Ody`] and
/// the unique session id.
pub struct OdySpawnOk {
    pub ody: Ody,
    pub thread_id: ThreadId,
}

pub(crate) struct OdySpawnArgs {
    ...
    pub(crate) extensions: Arc<ody_extension_api::ExtensionRegistry<crate::config::Config>>,
    ...
    pub(crate) supports_odysseythink_form_elicitation: bool,
...
impl Ody {
    /// Spawn a new [`Ody`] and initialize the session.
    pub(crate) async fn spawn(args: OdySpawnArgs) -> OdyResult<OdySpawnOk> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if ody_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
        ...
        Self::spawn_internal(OdySpawnArgs {
            parent_trace,
            ..args
        })
    ...
    async fn spawn_internal(args: OdySpawnArgs) -> OdyResult<OdySpawnOk> {
        let OdySpawnArgs {
            mut config,
            user_instructions,
            installation_id,
        ...
        let exec_policy = if crate::guardian::is_guardian_reviewer_source(&session_source) {
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| OdyErr::Fatal(format!("failed to load rules: {err}")))?,
            )
        };

        let config = Arc::new(config);
        let refresh_strategy = if session_source.is_non_root_agent() {
            ody_models_manager::manager::RefreshStrategy::Offline
        } else {
            ody_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
        ...
        config
            ...
            .map_err(|err| OdyErr::InvalidRequest(err.to_string()))?;
        ...
        // Generate a unique ID for the lifetime of this Ody session.
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.ody_home)
        })?;
        ...
        Ok(OdySpawnOk { ody, thread_id })
    }

    /// Submit the `op` wrapped in a `Submission` with a unique ID.
    pub async fn submit(&self, op: Op) -> OdyResult<String> {
    ...
    /// Use sparingly: prefer `submit()` so Ody is responsible for generating
    /// unique IDs for each submission.
    pub async fn submit_with_id(&self, mut sub: Submission) -> OdyResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| OdyErr::InternalAgentDied)?;
        Ok(())
    }
    ...
    pub async fn set_thread_memory_mode(
        &self,
        mode: ody_protocol::protocol::ThreadMemoryMode,
    ) -> anyhow::Result<()> {
        handlers::persist_thread_memory_mode_update(&self.session, mode).await
    }

    pub async fn shutdown_and_wait(&self) -> OdyResult<()> {
        let session_loop_termination = self.session_loop_termination.clone();
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(OdyErr::InternalAgentDied) => {}
    ...
    pub async fn next_event(&self) -> OdyResult<Event> {
        let event = self
            .rx_event
            .recv()
            .await
...
fn session_permission_profile_state_from_config(
    config: &Config,
) -> OdyResult<PermissionProfileState> {
...
impl Session {
    ...
    /// Builds the `x-ody-beta-features` header value for this session.
    ///
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &ody_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn ody_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn ody_network_proxy::BlockedRequestObserver>>,
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect Ody performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn user_instructions(&self) -> Option<ody_extension_api::UserInstructions> {
        let state = self.state.lock().await;
        state
            .session_configuration
            .loaded_agents_md
    ...
    /// Record a terminal OdyErr before the app-server completion notification is reduced.
    pub(crate) fn track_turn_ody_error(&self, turn_context: &TurnContext, error: &OdyErr) {
        self.services
            .analytics_events_client
            .track_turn_ody_error(TurnOdyErrorFact::from_ody_err(
    ...
    pub(crate) async fn send_event(&self, turn_context: &TurnContext, msg: EventMsg) {
        ...
        if let EventMsg::Error(error) = &legacy_source
            && error
                .ody_error_info
                .as_ref()
                .is_some_and(OdyErrorInfo::affects_turn_status)
        {
            turn_context
                .terminal_error
                .lock()
        ...
        self.services
            ...
            .record_ody_turn_event(&turn_context.sub_id, &legacy_source);
        self.services
            .rollout_thread_trace
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &ody_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| ody_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&ody_home, amendment)
            .await?;
    ...
    pub(crate) async fn request_permissions_for_environment(
        ...
        let Ok(native_environment_cwd) = environment.cwd.to_abs_path() else {
            warn!(
                cwd = %environment.cwd,
                "request_permissions requires a cwd native to the Ody host"
            );
            return Some(RequestPermissionsResponse {
                permissions: RequestPermissionProfile::default(),
                scope: PermissionGrantScope::Turn,
        ...
        if crate::guardian::routes_approval_to_guardian(turn_context.as_ref()) {
            ...
            let review_rx = crate::guardian::spawn_approval_request_review(
                ...
                request,
                /*retry_reason*/ None,
                ody_analytics::GuardianApprovalRequestSource::MainTurn,
    ...
    async fn maybe_warn_on_server_model_mismatch(
        ...
        let warning_message = format!(
            "Your account was flagged for potentially high-risk cyber activity and this request was routed to gpt-5.2 as a fallback. To regain access to gpt-5.3-ody, apply for trusted access: {CYBER_VERIFY_URL} or learn more: {CYBER_SAFETY_URL}"
        );
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        {
            let mut state = self.state.lock().await;
            state.queue_pending_session_start_source(ody_hooks::SessionStartSource::Compact);
        }
    }
    ...
    pub(crate) async fn notify_stream_error(
        ...
        let additional_details = ody_error.to_string();
        let ody_error_info = OdyErrorInfo::ResponseStreamDisconnected {
            http_status_code: ody_error.http_status_code_value(),
        };
...
async fn build_hooks_for_config(
    ...
    Hooks::new(HooksConfig {
        ...
        feature_enabled: config.features.enabled(Feature::OdyHooks),

...
use ody_sandboxing::SandboxCommand;
use ody_sandboxing::SandboxManager;
use ody_sandboxing::SandboxTransformRequest;
use ody_sandboxing::SandboxType;
use ody_sandboxing::SandboxablePreference;
use ody_sandboxing::WindowsSandboxFilesystemOverrides;
pub(crate) use ody_sandboxing::is_likely_sandbox_denied;
#[cfg(test)]
use ody_sandboxing::permission_profile_supports_windows_restricted_token_sandbox;
use ody_sandboxing::resolve_windows_elevated_filesystem_overrides;
use ody_sandboxing::resolve_windows_restricted_token_filesystem_overrides;
#[cfg(test)]
use ody_sandboxing::unsupported_windows_restricted_token_sandbox_reason;
use ody_sandboxing::windows_sandbox_uses_elevated_backend;
use ody_utils_absolute_path::AbsolutePathBuf;
use ody_utils_path_uri::PathUri;
use ody_utils_pty::DEFAULT_OUTPUT_BYTES_CAP;
use ody_utils_pty::process_group::kill_child_process_group;
...
fn network_proxy_environment_error(
    ...
    OdyErr::Io(io::Error::other(format!(
...
pub fn build_exec_request(
    ...
    let mut exec_req = manager
        .transform(SandboxTransformRequest {
            ...
            linux_sandbox_exe: linux_sandbox_exe.as_deref(),
            use_legacy_landlock,
            windows_sandbox_level,
            windows_sandbox_private_desktop,
        })
    ...
    .map_err(OdyErr::UnsupportedOperation)?;
    Ok(exec_req)
}
...
pub(crate) async fn execute_exec_request(
    ...
    let cwd = cwd
        .to_abs_path()
        .map_err(|err| OdyErr::InvalidRequest(format!("invalid exec cwd: {err}")))?;
    ...
    let windows_sandbox_policy_cwd = windows_sandbox_policy_cwd
        .to_abs_path()
        .map_err(|err| OdyErr::InvalidRequest(format!("invalid sandbox cwd: {err}")))?;
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        ody_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    };
    if let Some(metrics) = ody_otel::global() {
        let _ = metrics.counter(
            "ody.windows_sandbox.createprocessasuserw_failed",
            /*inc*/ 1,
            &[
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_ody_home;
    use ody_windows_sandbox::run_windows_sandbox_capture_for_permission_profile_elevated;
    use ody_windows_sandbox::run_windows_sandbox_capture_with_filesystem_overrides;
    ...
    let (cancellation, timeout_ms) = if capture_policy.uses_expiration() {
        let cancellation = expiration.cancellation_token().map(|token| {
            ody_windows_sandbox::WindowsSandboxCancellationToken::new(move || token.is_cancelled())
        });
    ...
    let ody_home = find_ody_home().map_err(|err| {
        OdyErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve ody_home: {err}"
        )))
    })?;
    ...
    let spawn_res = tokio::task::spawn_blocking(move || {
        if use_elevated {
            run_windows_sandbox_capture_for_permission_profile_elevated(
                ody_windows_sandbox::ElevatedSandboxProfileCaptureRequest {
                    permission_profile: &permission_profile,
                    workspace_roots: workspace_roots.as_slice(),
                    ody_home: ody_home.as_ref(),
                    command,
                    cwd: &cwd,
                    env_map: env,
                    timeout_ms,
    ...
    let capture = match spawn_res {
        ...
        Ok(Err(err)) => {
            ...
            return Err(OdyErr::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, OdyErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(OdyErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(OdyErr::Sandbox(SandboxErr::Timeout {
                    output: Box::new(exec_output),
                }));
            }

            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                return Err(OdyErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
...
/// ODY_SANDBOX_NETWORK_DISABLED=1 is added to the environment of the spawned
/// process.
///
            ...
            match outcome {
                ...
                Some(ExecExpirationOutcome::Cancelled) => {
                    ...
                    let should_escalate = if let Some(process_group_id) = process_group_id {
                        ody_utils_pty::process_group::terminate_process_group(process_group_id)?
                    } else {
                        false
                    };
                    ...
                    .await
                    {
                        Ok(status) => {
                            ...
                            {
                                ody_utils_pty::process_group::kill_process_group(
                                    process_group_id,
                                )?;
                            }
                        }

...
impl ShellSnapshot {
    ...
    async fn build_for_cwd(
        ...
        async {
            let timer = config
                .session_telemetry
                .start_timer("ody.shell_snapshot.duration_ms", &[]);
            ...
            config
                .session_telemetry
                .counter("ody.shell_snapshot", /*inc*/ 1, &counter_tags);
...
pub async fn cleanup_stale_snapshots(
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path =
            find_thread_path_by_id_str(ody_home, session_id, state_db.as_deref()).await?;

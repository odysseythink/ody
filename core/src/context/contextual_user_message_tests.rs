...
fn detects_internal_model_context_fragment() {
    ...
    assert_eq!(
        text,
        "<ody_internal_context source=\"extension\">\nInternal steering.\n</ody_internal_context>"
    );
    assert!(is_contextual_user_fragment(&ContentItem::InputText {
        text
    }));
...
fn detects_recommended_plugins_fragment() {
    assert!(is_contextual_user_fragment(&ContentItem::InputText {
        text: "<recommended_plugins>\n- Google Drive (google-drive@odysseythink-curated-remote)\n</recommended_plugins>"
            .to_string(),
    }));
}
...
fn rejects_invalid_internal_model_context_source() {
    assert!(!is_contextual_user_fragment(&ContentItem::InputText {
        text: "<ody_internal_context source=\"Extension\">\nbody\n</ody_internal_context>"
            .to_string(),
    }));
}

...
use ody_exec_server::ProcessSignal;
use ody_exec_server::ReadResponse;
use ody_exec_server::StartedExecProcess;
...
impl BlockingTerminateExecProcess {
    async fn read(&self) -> Result<ReadResponse, ody_exec_server::ExecServerError> {
        Ok(ReadResponse {
            chunks: Vec::new(),
            next_seq: 1,
            exited: false,
    ...
    async fn write(&self) -> Result<WriteResponse, ody_exec_server::ExecServerError> {
        Ok(WriteResponse {
            status: WriteStatus::Accepted,
        })
    }

    async fn terminate(&self) -> Result<(), ody_exec_server::ExecServerError> {
...
async fn unified_exec_persists_across_requests() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        "export ODY_INTERACTIVE_SHELL_VAR=ody\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        &session,
        process_id,
        "echo $ODY_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    assert!(
...
async fn unified_exec_timeouts() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        format!("export ODY_INTERACTIVE_SHELL_VAR={TEST_VAR_VALUE}\n").as_str(),
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        &session,
        process_id,
        "sleep 5 && echo $ODY_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 10,
    )
    .await?;
    assert!(
...
async fn completed_pipe_commands_preserve_exit_code() -> anyhow::Result<()> {
    ...
    let environment = ody_exec_server::Environment::default_for_tests();

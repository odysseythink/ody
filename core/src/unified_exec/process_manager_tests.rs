...
fn exec_server_params_use_path_uri_and_env_policy_overlay_contract() {
    let cwd: ody_utils_absolute_path::AbsolutePathBuf = std::env::current_dir()
        .expect("current dir")
        .try_into()
        .expect("absolute path");
    let file_system_sandbox_policy =
        ody_protocol::permissions::FileSystemSandboxPolicy::unrestricted();
    let network_sandbox_policy = ody_protocol::permissions::NetworkSandboxPolicy::Restricted;
    let permission_profile = ody_protocol::models::PermissionProfile::Disabled;
    let mut request = ExecRequest {
        ...
        exec_server_env_config: Some(ExecServerEnvConfig {
            policy: ody_exec_server::ExecEnvPolicy {
                inherit: ody_protocol::config_types::ShellEnvironmentPolicyInherit::Core,
        ...
        sandbox: ody_sandboxing::SandboxType::None,
    ...
    request.exec_server_sandbox = Some(
        ody_exec_server::FileSystemSandboxContext::from_permission_profile(permission_profile),
    );
    let first =
        exec_server_params_for_request(/*process_id*/ 123, &request, /*tty*/ true);
    let second =
...
async fn failed_initial_end_for_unstored_process_uses_fallback_output() {
    ...
    let request = ExecCommandRequest {
        ...
        shell_mode: ody_tools::UnifiedExecShellMode::Direct,
        network: None,
        tty: true,
    ...
    let ody_protocol::protocol::EventMsg::ExecCommandEnd(end_event) = event.msg else {
        panic!("expected ExecCommandEnd event");
    };
    assert_eq!(end_event.call_id, "call-unified-denied");
    assert_eq!(
        end_event.status,
        ody_protocol::protocol::ExecCommandStatus::Failed
    );

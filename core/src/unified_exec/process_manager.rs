...
const NETWORK_ACCESS_DENIED_MESSAGE: &str =
    "Network access was denied by the Ody sandbox network proxy.";
...
fn exec_server_env_for_request(
    ...
    Option<ody_exec_server::ExecEnvPolicy>,
    HashMap<String, String>,
) {
...
fn network_approval_error_message(err: ToolError) -> String {
    match err {
        ...
        ToolError::Ody(err) => err.to_string(),
    }
}
...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn open_session_with_exec_env(
        ...
        .map_err(|err| match err {
            UnifiedExecError::SandboxDenied { output, .. } => {
                ToolError::Ody(OdyErr::Sandbox(SandboxErr::Denied {
    ...
    pub(crate) async fn open_session_with_prepared_exec_env(
        ...
        if request.sandbox == ody_sandboxing::SandboxType::WindowsRestrictedToken {
            ...
            let ody_home = crate::config::find_ody_home().map_err(|err| {
            ...
            let spawned = match request.windows_sandbox_level {
                ody_protocol::config_types::WindowsSandboxLevel::Elevated => {
                    ody_windows_sandbox::spawn_windows_sandbox_session_elevated_for_permission_profile(
                ...
                ody_protocol::config_types::WindowsSandboxLevel::RestrictedToken
                | ody_protocol::config_types::WindowsSandboxLevel::Disabled => {
                    ody_windows_sandbox::spawn_windows_sandbox_session_legacy(
                        &request.permission_profile,
                        request.windows_sandbox_workspace_roots.as_slice(),
                        ody_home.as_ref(),
                        request.command.clone(),
        ...
        let spawn_result = if tty {
            ody_utils_pty::pty::spawn_process_with_inherited_fds(
                program,
                args,
                native_cwd.as_path(),
                &request.env,
                &request.arg0,
                ody_utils_pty::TerminalSize::default(),
                &inherited_fds,
            )
            .await
        } else {
            ody_utils_pty::pipe::spawn_process_no_stdin_with_inherited_fds(
                program,
                args,
                native_cwd.as_path(),
                &request.env,
    ...
    pub(super) async fn open_session_with_sandbox(
        ...
        env.insert(
            ODY_THREAD_ID_ENV_VAR.to_string(),
        ...
        orchestrator
            ...
            .map_err(|err| match err {
                ToolError::Ody(OdyErr::Sandbox(SandboxErr::Denied { output, .. })) => {
                    let output = *output;

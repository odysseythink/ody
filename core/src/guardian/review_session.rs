...
pub(crate) enum GuardianReviewSessionOutcome {
    ...
    SessionFailed {
        error: anyhow::Error,
        error_info: Option<OdyErrorInfo>,
    },
    TimedOut,
    Aborted,
}
...
impl GuardianReviewSession {
    async fn shutdown(&self) {
        ...
        let _ = self.ody.shutdown_and_wait().await;
    }
    ...
    async fn refresh_last_committed_fork_snapshot(&self) {
        match load_rollout_items_for_fork(&self.ody.session).await {
...
impl GuardianReviewSessionManager {
    ...
    pub(crate) async fn trunk_rollout_path(&self) -> Option<PathBuf> {
        ...
        trunk.ody.session.ensure_rollout_materialized().await;
        match trunk.ody.session.current_rollout_path().await {
            Ok(path) => path,
            Err(err) => {
    ...
    #[cfg(test)]
    pub(crate) async fn cache_for_test(&self, ody: Ody) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            ody.session.get_config().await.as_ref(),
            ody.session.user_instructions().await,
        );
    ...
    #[cfg(test)]
    pub(crate) async fn register_ephemeral_for_test(&self, ody: Ody) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            ody.session.get_config().await.as_ref(),
            ody.session.user_instructions().await,
        );
    ...
    pub(crate) async fn send_trunk_event_raw_for_test(&self, event: Event) {
        ...
        trunk.ody.session.send_event_raw(event).await;
    }
...
async fn run_review_on_session(
    ...
    let mut analytics_result =
        GuardianReviewAnalyticsResult::from_session(GuardianReviewSessionAnalyticsParams {
            guardian_thread_id: review_session.ody.session.thread_id.to_string(),
            guardian_session_kind,
        ...
        Box::pin(async {
            params
                ...
                .sync_session_approved_hosts_to(
                    &review_session.ody.session.services.network_approval,
                )
                .await;
    ...
    let submit_result = run_before_review_deadline(
        deadline,
        params.external_cancel.as_ref(),
        Box::pin(review_session.ody.submit(Op::UserInput {
            items: prompt_items.items,
            final_output_json_schema: Some(params.schema.clone()),
            responsesapi_client_metadata: None,
            additional_context: Default::default(),
            thread_settings: ody_protocol::protocol::ThreadSettingsOverrides {
                environments: Some(ody_protocol::protocol::TurnEnvironmentSelections::new(
    ...
    if matches!(outcome.0, GuardianReviewSessionOutcome::Completed(_)) {
        if outcome.2
            && let Some(total_token_usage) = review_session.ody.session.total_token_usage().await
        {
...
async fn wait_for_guardian_review(
    ...
    loop {
        tokio::select! {
            ...
            event = review_session.ody.next_event() => {
                match event {
                                ...
                                && let Some(error) = last_error
                            {
                                return (
                                    GuardianReviewSessionOutcome::SessionFailed {
                                        ...
                                        error_info: error.ody_error_info,
                                    },
                                    true,
                                    true,
                                );
...
async fn interrupt_and_drain_turn(ody: &Ody, expected_turn_id: &str) -> anyhow::Result<()> {
...
mod tests {
    ...
    async fn test_review_params() -> GuardianReviewSessionParams {
        ...
        GuardianReviewSessionParams {
            ...
            model,
            reasoning_effort,
            guardian_default_review_model_id: "ody-auto-review".to_string(),
    ...
    async fn guardian_review_session_config_disables_hooks() {
        ...
        assert!(!guardian_config.features.enabled(Feature::OdyHooks));
    }
    ...
    async fn wait_for_guardian_review_preserves_structured_session_error() {
        ...
        tx_event
            .send(Event {
                ...
                msg: EventMsg::Error(ErrorEvent {
                    ...
                    ody_error_info: Some(OdyErrorInfo::ServerOverloaded),
                }),
            })
            .await
        ...
        assert_eq!(error_info, Some(OdyErrorInfo::ServerOverloaded));
        assert!(keep_review_session);
        assert!(capture_token_usage);
    }
    ...
    async fn interrupt_and_drain_turn_ignores_prior_turn_completion() {
        ...
        interrupt_and_drain_turn(&review_session.ody, "current-turn")
            .await
            .expect("drain current turn");

        assert!(review_session.ody.rx_event.try_recv().is_err());
    }
}

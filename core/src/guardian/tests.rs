...
use ody_analytics::GuardianApprovalRequestSource;
...
// The memories extension depends on ody-core, so this probe verifies the nested Guardian config
...
impl ody_extension_api::ThreadLifecycleContributor<Config> for GuardianMemoryContextProbe {
    fn on_thread_start<'a>(
        &'a self,
        input: ody_extension_api::ThreadStartInput<'a, Config>,
...
impl ody_extension_api::ContextContributor for GuardianMemoryContextProbe {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a ody_extension_api::ExtensionData,
        thread_store: &'a ody_extension_api::ExtensionData,
    ) -> ody_extension_api::ExtensionFuture<'a, Vec<ody_extension_api::PromptFragment>> {
        Box::pin(async move {
            if thread_store
                .get::<GuardianMemoryContextEnabled>()
                .is_some_and(|enabled| enabled.0)
            {
                vec![ody_extension_api::PromptFragment::developer_policy(
...
fn guardian_shell_request(id: &str) -> GuardianApprovalRequest {
    GuardianApprovalRequest::Shell {
        ...
        cwd: test_path_buf("/repo/ody/core").abs(),
        ...
        .record_conversation_items(
            ...
            &[
                ...
                ResponseItem::FunctionCall {
                    ...
                    namespace: None,
                    arguments: "{\"repo\":\"odysseythink/ody\"}".to_string(),
                ...
                ResponseItem::FunctionCallOutput {
                    id: None,
                    call_id: "call-1".to_string(),
                    output: ody_protocol::models::FunctionCallOutputPayload::from_text(
...
fn normalize_guardian_snapshot_paths(text: String) -> String {
    let mut text = text;
    for canonical_path in ["/repo/ody/core", "/repo"] {
...
fn guardian_prompt_text(items: &[ody_protocol::user_input::UserInput]) -> String {
    items
        .iter()
        .map(|item| match item {
            ody_protocol::user_input::UserInput::Text { text, .. } => text.as_str(),
            _ => "",
        })
        .collect::<String>()
}
...
async fn build_guardian_prompt_full_mode_preserves_initial_review_format() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The Ody agent has requested the following action:\n"));
        ...
        &FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: ody_protocol::permissions::FileSystemSpecialPath::Root,
                },
...
async fn build_guardian_prompt_delta_mode_preserves_original_numbering() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The Ody agent has requested the following next action:\n"));
...
async fn build_guardian_prompt_items_explains_network_access_review_scope() -> anyhow::Result<()> {
    ...
    settings.bind(|| {
        assert_snapshot!(
            "ody_core__guardian__tests__network_access_guardian_prompt_layout",
...
async fn guardian_request_model_for_auto_review(
    ...
    String,
    String,
    ody_analytics::GuardianReviewAnalyticsResult,
)> {
...
-> anyhow::Result<()> {
    ...
    assert!(matches!(
        ...
        Some(ody_analytics::GuardianReviewSessionKind::TrunkNew)
    ));
    let request = request_log.single_request();
    ...
    settings.bind(|| {
        assert_snapshot!(
            "ody_core__guardian__tests__guardian_review_request_layout",
...
async fn build_guardian_prompt_items_includes_parent_session_id() -> anyhow::Result<()> {
    ...
    let prompt_text = prompt
        ...
        .into_iter()
        .map(|item| match item {
            ody_protocol::user_input::UserInput::Text { text, .. } => text,
            ody_protocol::user_input::UserInput::Image { .. } => String::new(),
            _ => String::new(),
        })
        .collect::<String>();
...
async fn guardian_reuses_prompt_cache_key_and_appends_prior_reviews() -> anyhow::Result<()> {
    ...
    assert!(matches!(
        ...
        Some(ody_analytics::GuardianReviewSessionKind::TrunkReused)
    ));
    assert!(matches!(
    ...
    settings.bind(|| {
        assert_snapshot!(
            "ody_core__guardian__tests__guardian_followup_review_request_layout",
            format!(

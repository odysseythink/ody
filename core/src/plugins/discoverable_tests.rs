...
use ody_core_plugins::remote::fetch_and_cache_global_remote_plugin_catalog;
...
async fn list_discoverable_plugins_with_auth(
    ...
    let plugins_manager = PluginsManager::new_with_options(
        ...
        Some(Product::Ody),
        auth.map(ody_login::OdyAuth::api_auth_mode),
    );
...
async fn list_tool_suggest_discoverable_plugins_includes_cached_remote_global_plugins() {
    ...
    write_file(
        &ody_home.path().join(crate::config::CONFIG_TOML_FILE),
        r#"[features]
plugins = true
remote_plugin = true
"#,
    ...
    assert!(
        discoverable_plugins
            .iter()
            .all(|plugin| plugin.id != "github@odysseythink-curated-remote")
    );
    ...
    assert_eq!(
        discoverable_plugins
            .iter()
            .filter(|plugin| plugin.id.ends_with("@odysseythink-curated-remote"))
            .map(|plugin| plugin.id.as_str())
            .collect::<Vec<_>>(),
        vec!["github@odysseythink-curated-remote"]
    );
    let remote_plugins = discoverable_plugins
        .into_iter()
        .filter(|plugin| plugin.id == "github@odysseythink-curated-remote")
        .collect::<Vec<_>>();
...
[tool_suggest]
disabled_tools = [
  { type = "plugin", id = "github@odysseythink-curated-remote" }
]
"#,
    );
    let mut config_with_disabled_remote_plugin = load_plugins_config(ody_home.path()).await;
...
[tool_suggest]
discoverables = [{ type = "plugin", id = "sample@odysseythink-curated" }]
"#,
    );

...
command transformation live in the ody-sandboxing crate; this module keeps
...
#[cfg(target_os = "macos")]
use crate::spawn::ODY_SANDBOX_ENV_VAR;
use crate::spawn::ODY_SANDBOX_NETWORK_DISABLED_ENV_VAR;
...
pub use ody_protocol::models::SandboxPermissions;
...
pub(crate) struct ExecServerEnvConfig {
    pub(crate) policy: ody_exec_server::ExecEnvPolicy,
...
impl ExecRequest {
    ...
    pub(crate) fn from_sandbox_exec_request(
        ...
        if !network_sandbox_policy.is_enabled() {
            env.insert(
                ODY_SANDBOX_NETWORK_DISABLED_ENV_VAR.to_string(),
                "1".to_string(),
            );
        }
        #[cfg(target_os = "macos")]
        if sandbox == SandboxType::MacosSeatbelt {
            env.insert(ODY_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
        }
        Self {
            command,
            cwd,
...
) -> ody_protocol::error::Result<ExecToolCallOutput> {

...
use ody_api::RealtimeEvent;
use ody_api::RealtimeEventParser;
use ody_api::RealtimeSessionConfig;
use ody_api::RealtimeSessionMode;
use ody_api::RealtimeWebsocketClient;
...
use ody_protocol::protocol::RealtimeConversationStartedEvent;
...
const STANDALONE_HANDOFF_ID: &str = "ody";
...
struct RealtimeHandoffState {
    ...
    ody_responses_as_items: bool,
    ody_response_item_prefix: Option<String>,
    ody_response_handoff_prefix: Option<String>,
...
impl RealtimeHandoffState {
    fn new(
        ...
        Self {
            ...
            ody_responses_as_items,
            ody_response_item_prefix,
            ody_response_handoff_prefix,
            session_kind,
        }
    }
}
...
impl RealtimeConversationManager {
    ...
    async fn start(&self, start: RealtimeStart) -> OdyResult<RealtimeStartOutput> {
        let previous_state = {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> OdyResult<RealtimeStartOutput> {
        let RealtimeStart {
            api_provider,
            extra_headers,
            client_managed_handoffs,
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> OdyResult<()> {
        let sender = {
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(OdyErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, mut params: ConversationTextParams) -> OdyResult<()> {
        let sender = {
            let guard = self.state.lock().await;
            guard
                .as_ref()
        ...
        sender
            .send(params)
            .await
            .map_err(|_| OdyErr::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }
    ...
    pub(crate) async fn handoff_out(
        ...
        let response_handoff_prefix = match phase {
            Some(MessagePhase::Commentary) => handoff.ody_response_handoff_prefix.clone(),
        ...
        let output = match active_handoff {
            Some(handoff_id) => {
                ...
                *handoff.last_output_text.lock().await = Some(output_text.clone());
                if handoff.ody_responses_as_items {
                    RealtimeOutbound::ConversationItem {
                        text: realtime_backend_item(
                            output_text,
                            handoff.ody_response_item_prefix.as_deref(),
                        ),
                    }
                } else if handoff.session_kind == RealtimeSessionKind::V1
                    && handoff.ody_response_handoff_prefix.is_some()
                {
    ...
    pub(crate) async fn append_speech(&self, text: String) -> OdyResult<()> {
    ...
    pub(crate) async fn handoff_complete(&self) -> OdyResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
        ...
        let output = if handoff.ody_responses_as_items {
    ...
    pub(crate) async fn shutdown(&self) -> OdyResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    ...
    Ok(PreparedRealtimeConversationStart {
        ...
        ody_responses_as_items: params.ody_responses_as_items,
        ody_response_item_prefix: params.ody_response_item_prefix,
        ody_response_handoff_prefix: params.ody_response_handoff_prefix,
        realtime_call_api_provider,
...
) -> OdyResult<RealtimeSessionConfig> {
    let config = sess.get_config().await;
...
fn validate_realtime_voice(version: RealtimeWsVersion, voice: RealtimeVoice) -> OdyResult<()> {
    let voices = RealtimeVoicesList::builtin();
...
fn realtime_request_headers(
    ...
    if let Some(api_key) = api_key {
        let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
            OdyErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
        })?;

...
async fn run_remote_compact_task_inner(
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped => {
            ...
            attempt
                .track(
                    sess.as_ref(),
                    ody_analytics::CompactionStatus::Interrupted,
                    Some(&error),
                    analytics_details,
                )
                .await;
    ...
    if result.is_ok() {
        ...
        if let PostCompactHookOutcome::Stopped = post_compact_outcome {
            attempt
                .track(sess.as_ref(), status, ody_error, analytics_details)
                .await;
    ...
    attempt
        .track(sess.as_ref(), status, ody_error, analytics_details)
        .await;
    if matches!(&result, Err(OdyErr::TurnAborted)) {
        return result;
    }
    if let Err(err) = result {
        sess.track_turn_ody_error(turn_context, &err);
...
) -> OdyResult<RemoteCompactionV2Output> {
...
mod tests {
    ...
    fn response_stream(events: Vec<OdyResult<ResponseEvent>>) -> ResponseStream {

...
async fn windows_exec_server_runs_with_native_shell_and_cwd() -> Result<()> {
    ...
    const PATCH_FILE: &str = "ody-apply-patch-smoke.txt";
    const COMMAND: &str = r#"if ((Get-Location).Path -ne 'C:\windows') { exit 1 }"#;
    const VERIFY_COMMAND: &str = r#"$path = Join-Path (Get-Location) 'ody-apply-patch-smoke.txt'; if (-not (Test-Path $path)) { exit 1 }; if ([IO.File]::ReadAllText($path) -ne "patched through unified exec`n") { exit 2 }; Remove-Item $path; Write-Output 'PATCH_VERIFIED'"#;
        ...
        .scope(|exec_server_url, _wine_prefix| async move {
            ...
            let environments = TurnEnvironmentSelections::new(
                ...
                vec![TurnEnvironmentSelection {
                    ...
                    cwd: PathUri::parse("file:///C:/ody-home")?,
                }],
            );
            ...
            loop {
                match wait_for_event(&test.ody, |_| true).await {
            ...
            assert!(
                patch_end
                    ...
                    .contains_key(&std::path::PathBuf::from(format!(
                        r"C:\ody-home\apply-patch-smoke\nested\{PATCH_FILE}"
                    ))),
...
async fn app_server_starts_thread_with_windows_environment_native_cwd() -> Result<()> {
    ...
    WineExecServer
        .scope(|exec_server_url, wine_prefix| async move {
            ...
            let host_cwd = ody_home.path().to_path_buf().abs();

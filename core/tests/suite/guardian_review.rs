...
async fn guardian_session_prewarms_and_is_reused_for_first_review() -> Result<()> {
    ...
    let guardian_prewarm = prewarm_requests
        .iter()
        .find(|request| {
            request["client_metadata"]["x-odysseythink-subagent"].as_str() == Some("guardian")
        })
    ...
    .body_json();
    assert_eq!(
        guardian_review["client_metadata"]["x-odysseythink-subagent"].as_str(),
        Some("guardian")
    );
    assert_eq!(
...
printf '%s\n' "${@: -1}" >> "${payload_path}""#,
    ...
    assert!(
        !notify_payload_raw.contains(
            "The following is the Ody agent history whose request action you are assessing."
        ),

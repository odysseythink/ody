...
fn turn_metadata(harness: &WebsocketTestHarness, turn_id: Option<&str>) -> OdyResponsesMetadata {
...
fn websocket_connection_metadata(harness: &WebsocketTestHarness) -> OdyResponsesMetadata {
    responses_metadata(
        harness,
        /*turn_id*/ None,
...
async fn responses_websocket_streams_request() {
    ...
    let stream_request_start_ms = body["client_metadata"]
        [X_ODY_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY]
        .as_str()
...
async fn responses_websocket_request_prewarm_reuses_connection() {
    ...
    let warmup_turn_metadata: serde_json::Value = serde_json::from_str(
        warmup["client_metadata"]["x-ody-turn-metadata"]
            .as_str()
...
async fn responses_websocket_request_prewarm_traces_logical_request() {
    ...
    writer
        .append(RawTraceEventPayload::OdyTurnStarted {
...
async fn responses_websocket_prewarm_uses_v2_when_provider_supports_websockets() {
    ...
    let handshake = server.single_handshake();
    let odysseythink_beta_header = handshake
        .header(OPENAI_BETA_HEADER)
        .expect("missing OpenAI-Beta header");
    assert!(
        odysseythink_beta_header
            .split(',')
            .map(str::trim)
...
async fn responses_websocket_preconnect_runs_when_only_v2_feature_enabled() {
    ...
    assert_eq!(
        server.single_handshake().header("x-ody-turn-metadata"),
        None
    );
...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let usage_limit_error = json!({
        ...
        "headers": {
            "x-ody-primary-used-percent": "100.0",
            "x-ody-secondary-used-percent": "87.5",
            "x-ody-primary-over-secondary-limit-percent": "95.0",
            "x-ody-primary-window-minutes": "15",
            "x-ody-secondary-window-minutes": "60"
        }
    });
    ...
    let token_event =
        wait_for_event(&test.ody, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&test.ody, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn responses_websocket_forwards_turn_metadata_on_initial_and_incremental_create() {
    ...
    let first_metadata: serde_json::Value = serde_json::from_str(
        first["client_metadata"]["x-ody-turn-metadata"]
            .as_str()
    ...
    let second_metadata: serde_json::Value = serde_json::from_str(
        second["client_metadata"]["x-ody-turn-metadata"]
            .as_str()
...
async fn responses_websocket_sends_canonical_turn_metadata() {
    ...
    let turn_metadata: serde_json::Value = serde_json::from_str(
        body["client_metadata"]["x-ody-turn-metadata"]
            .as_str()
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn responses_websocket_v2_sets_odysseythink_beta_header() {
    skip_if_no_network!();

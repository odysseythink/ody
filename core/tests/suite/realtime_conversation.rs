...
const REALTIME_BACKEND_PROMPT: &str = ody_prompts::BACKEND_PROMPT;
...
const REALTIME_CONVERSATION_TEST_SUBPROCESS_ENV_VAR: &str =
    "ODY_REALTIME_CONVERSATION_TEST_SUBPROCESS";
const SILENT_CONTEXT_PREFIX: &str = "[BACKEND] Silent Ody context. Do not speak, acknowledge, or summarize this item. Wait for an explicit speakable handoff or direct user request.";
...
async fn seed_recent_thread(
    ...
    let db = test.ody.state_db().context("state db enabled")?;
    ...
    let mut metadata_builder = ody_state::ThreadMetadataBuilder::new(
        thread_id,
        rollout_path,
        updated_at,
        SessionSource::Cli,
...
async fn conversation_start_audio_text_close_round_trip() -> Result<()> {
    ...
    let started = wait_for_event_match(&test.ody, |msg| match msg {
    ...
    let session_updated = wait_for_event_match(&test.ody, |msg| match msg {
    ...
    let audio_out = wait_for_event_match(&test.ody, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
    ...
    test.ody.submit(Op::RealtimeConversationClose).await?;
    let closed = wait_for_event_match(&test.ody, |msg| match msg {
...
async fn conversation_webrtc_start_posts_generated_session() -> Result<()> {
    ...
    let created = wait_for_event_match(&test.ody, |msg| match msg {
...
async fn conversation_webrtc_default_v1_rejects_explicit_v2_voice() -> Result<()> {
    ...
    let error = wait_for_event_match(&test.ody, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
...
async fn conversation_webrtc_close_while_sideband_connecting_drops_pending_join() -> Result<()> {
    ...
    let sdp = wait_for_event_match(&test.ody, |msg| match msg {
    ...
    let stale_event = timeout(Duration::from_millis(700), async {
        wait_for_event_match(&test.ody, |msg| match msg {
            EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
                payload: RealtimeEvent::Error(message),
...
async fn conversation_webrtc_sideband_connect_failure_closes_with_error() -> Result<()> {
    ...
    let err = wait_for_event_match(&test.ody, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn conversation_start_uses_odysseythink_env_key_fallback_with_chatgpt_auth() -> Result<()> {
    if std::env::var_os(REALTIME_CONVERSATION_TEST_SUBPROCESS_ENV_VAR).is_none() {
        return run_realtime_conversation_test_in_subprocess(
            "suite::realtime_conversation::conversation_start_uses_odysseythink_env_key_fallback_with_chatgpt_auth",
    ...
    test.ody.submit(Op::RealtimeConversationClose).await?;
    let _closed = wait_for_event_match(&test.ody, |msg| match msg {
        EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
        _ => None,
    })
    .await;
...
async fn conversation_audio_before_start_emits_error() -> Result<()> {
    ...
    .await;
    assert_eq!(err.ody_error_info, Some(OdyErrorInfo::BadRequest));
    assert_eq!(err.message, "conversation is not running");
...
async fn conversation_second_start_replaces_runtime() -> Result<()> {
    ...
    test.ody
        ...
        .await?;
    let _ = wait_for_event_match(&test.ody, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::AudioOut(frame),
...
async fn conversation_uses_explicit_start_voice() -> Result<()> {
    ...
    .await;
    let test = test_ody().build_with_websocket_server(&server).await?;
    assert!(
        server
            .wait_for_handshakes(/*expected*/ 1, Duration::from_secs(2))
            .await
...
async fn conversation_user_text_turn_is_not_sent_to_realtime() -> Result<()> {
    ...
    let turn_complete = wait_for_event_match(&test.ody, |event| match event {
...
async fn conversation_handoff_persists_across_item_done_until_turn_complete() -> Result<()> {
    ...
    test.ody
        .submit(Op::RealtimeConversationStart(ConversationStartParams {
            ...
            ody_responses_as_items: false,
            ody_response_item_prefix: None,
            ody_response_handoff_prefix: Some(SILENT_CONTEXT_PREFIX.to_string()),
            model: None,
            output_modality: RealtimeOutputModality::Audio,
            include_startup_context: true,
            prompt: Some(Some("backend prompt".to_string())),
...
async fn inbound_handoff_request_starts_turn() -> Result<()> {
    ...
    let turn_id = loop {
        let event = test.ody.next_event().await?;

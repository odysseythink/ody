...
const INHERITED_MODEL: &str = "kimi-for-coding";
...
fn write_home_skill(ody_home: &Path, dir: &str, name: &str, description: &str) -> Result<()> {
    let skill_dir = ody_home.join("skills").join(dir);
...
async fn wait_for_spawned_thread_id(test: &TestOdy) -> Result<String> {
    let deadline = Instant::now() + Duration::from_secs(2);
    loop {
...
async fn setup_turn_one_with_custom_spawned_child(
    ...
    let mut builder = configure_test(test_ody().with_config(|config| {
        config
            .features
...
async fn subagent_stop_replaces_stop_and_skips_internal_subagents() -> Result<()> {
    ...
    let stop_inputs = read_hook_log(test.ody_home_path(), "stop_hook_log.jsonl")?;
    assert!(
        stop_inputs
            .iter()
    ...
    let subagent_stop_inputs_after_internal =
        read_hook_log(test.ody_home_path(), "subagent_stop_hook_log.jsonl")?;
    assert_eq!(subagent_stop_inputs_after_internal, subagent_stop_inputs);

    let stop_inputs_after_internal = read_hook_log(test.ody_home_path(), "stop_hook_log.jsonl")?;
...
async fn encrypted_multi_agent_v2_spawn_sends_agent_message_to_child() -> Result<()> {
    ...
    let mut builder = test_ody().with_model("koffing").with_config(|config| {
        config
            .features
            .enable(Feature::Collab)
            .expect("test config should allow feature update");
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.ody_home.join("custom-role.toml");
                std::fs::write(
                    &role_path,
                    format!(
...
async fn spawn_agent_tool_description_mentions_role_locked_settings() -> Result<()> {
    ...
    assert_eq!(
        ...
        "custom: {\nCustom role\n- This role's model is set to `k3` and its reasoning effort is set to `high`. These settings cannot be changed.\n}"
    );

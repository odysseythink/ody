...
fn disabled_user_turn(test: &TestOdy, items: Vec<UserInput>, model: String) -> Op {
...
async fn create_workspace_directory(test: &TestOdy, rel_path: &str) -> anyhow::Result<PathBuf> {
...
async fn view_image_tool_attaches_local_image() -> anyhow::Result<()> {
    ...
    wait_for_event_with_timeout(
        ...
        |event| match event {
            EventMsg::ItemStarted(event) => {
                if matches!(&event.item, ody_protocol::items::TurnItem::ImageView(_)) {
    ...
    match item_started.expect("view image item started event emitted") {
        ody_protocol::items::TurnItem::ImageView(item) => {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn view_image_tool_can_preserve_original_resolution_when_requested_on_gpt5_3_ody()
-> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));

...
async fn fork_thread_twice_drops_to_first_message() {
    ...
    let base_path = ody.rollout_path().expect("rollout path");
    ...
    let fork1_path = ody_fork1.rollout_path().expect("rollout path");
    ...
    let fork2_path = ody_fork2.rollout_path().expect("rollout path");

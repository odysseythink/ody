...
async fn build_auto_compaction_disabled_ody(server: &MockServer) -> TestOdy {
    let mut model_provider = non_odysseythink_model_provider(server);
...
async fn submit_context_window_exceeded_turn(ody: &Arc<OdyThread>, text: &str) {
    ody
        .submit(Op::UserInput {
...
fn local_compaction_provider(server: &wiremock::MockServer) -> ModelProviderInfo {
    let mut provider = built_in_model_providers(/*odysseythink_base_url*/ None)["odysseythink"].clone();
...
async fn assert_compaction_uses_turn_lifecycle_id(ody: &std::sync::Arc<ody_core::OdyThread>) {
    ...
    while turn_completed_id.is_none() {
        let event = ody.next_event().await.expect("next event");
        match event.msg {
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn Ody.
    ...
    ody.submit(Op::Compact).await.unwrap();
    let warning_event = wait_for_event(&ody, |ev| matches!(ev, EventMsg::Warning(_))).await;
    ...
    // Shut down Ody to flush rollout entries before inspecting the file.
    ody.submit(Op::Shutdown).await.unwrap();
    wait_for_event(&ody, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
...
async fn manual_pre_compact_block_decision_does_not_block_compaction() {
    ...
    ody.submit(Op::Compact).await.expect("trigger compact");
    ...
    wait_for_event(&ody, |ev| matches!(ev, EventMsg::Warning(_))).await;
    wait_for_event(&ody, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    let hook_inputs = read_hook_inputs(&test.ody_home_path().join("pre_compact_block_log.jsonl"));
...
async fn compact_hooks_respect_matchers_and_post_runs_after_compaction() {
    ...
    let hook_inputs =
        read_hook_inputs(&test.ody_home_path().join("post_compact_manual_log.jsonl"));
    assert_eq!(hook_inputs.len(), 1);
    let input = &hook_inputs[0];
...
async fn manual_compact_emits_api_and_local_token_usage_events() {
    ...
    let ody = builder.build(&server).await.unwrap().ody;
    ...
    let first = wait_for_event_match(&ody, |ev| match ev {
    ...
    let last = wait_for_event_match(&ody, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn manual_compact_emits_context_compaction_items() {
    ...
    {
        let event = ody.next_event().await.unwrap();
        match event.msg {
            EventMsg::ItemStarted(ItemStartedEvent {
...
async fn multiple_auto_compact_per_task_runs_after_token_limit_hit() {
    ...
    let non_odysseythink_provider_name = non_odysseythink_model_provider(&server).name;
    let ody = test_ody()
        .with_config(move |config| {
            config.model_provider.name = non_odysseythink_provider_name;
        })
        .build(&server)
        .await
        .expect("build ody")
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        ...
        ody_protocol::models::ResponseItem::Compaction {
            id: None,
    ...
    let mut resume_builder = test_ody().with_config(move |config| {
        set_test_compact_prompt(config);
        config.model_auto_compact_token_limit = Some(limit);
        let _ = config.features.disable(Feature::RemoteCompactionV2);
    });
    ...
    wait_for_event(&resumed.ody, |event| {
...
async fn pre_sampling_compact_runs_on_switch_to_smaller_context_model() {
    ...
    let test = builder.build(&server).await.expect("build test ody");
    ...
    assert_compaction_uses_turn_lifecycle_id(&test.ody).await;
...
async fn pre_sampling_compact_runs_after_resume_and_switch_to_smaller_model() {
    ...
    assert_compaction_uses_turn_lifecycle_id(&resumed.ody).await;
...
async fn manual_compact_non_context_failure_retries_then_emits_task_error() {
    ...
    let reconnect_message = wait_for_event_match(&ody, |event| match event {
    ...
    let task_error_message = wait_for_event_match(&ody, |event| match event {
        EventMsg::Error(err) => Some(err.message.clone()),
        _ => None,
    })
    .await;
...
async fn manual_compact_twice_preserves_latest_user_messages() {
    ...
    assert_eq!(
        ...
        requests[1].header("x-ody-window-id").as_deref()
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        requests[2].header("x-ody-window-id").as_deref()
    );
    assert_ne!(
...
async fn auto_compaction_feature_disabled_skips_mid_turn_compaction() {
    ...
    .await;
    let test = build_auto_compaction_disabled_ody(&server).await;

    submit_context_window_exceeded_turn(&test.ody, FUNCTION_CALL_LIMIT_MSG).await;
...
async fn auto_compact_clamps_config_limit_to_context_window() {
    ...
    let ody = builder.build(&server).await.unwrap();

    ody.submit_turn("OVER_LIMIT_TURN").await.unwrap();
    ody.submit_turn("FOLLOW_UP_AFTER_CLAMP").await.unwrap();
...
async fn auto_compaction_feature_disabled_skips_pre_turn_compaction() {
    ...
    test.submit_turn("USER_ONE")
        .await
        .expect("submit first turn");
    submit_context_window_exceeded_turn(&test.ody, "USER_TWO").await;
...
async fn snapshot_request_shape_manual_compact_without_previous_user_messages() {
    ...
    ody.submit(Op::Compact).await.expect("run /compact");
    wait_for_event(&ody, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn manual_compaction_keeps_the_creation_time_global_instructions() -> Result<()> {
    ...
    assert_eq!(
        test.ody.instruction_sources().await,
...
-> Result<()> {
    ...
    assert_eq!(
        resumed.ody.instruction_sources().await,
        vec![PathUri::from_abs_path(&source)],

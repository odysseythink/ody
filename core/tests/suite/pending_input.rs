...
async fn wait_for_reasoning_item_started(ody: &OdyThread) {
    wait_for_event(ody, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(ody: &OdyThread, text: &str) {
...
async fn wait_for_turn_complete(ody: &OdyThread) {
    wait_for_event(ody, |event| matches!(event, EventMsg::TurnComplete(_))).await;
}

async fn wait_for_sleep_item_started(ody: &OdyThread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(ody, |event| {
        matches!(
            event,
...
async fn wait_for_sleep_item_completed(ody: &OdyThread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(ody, |event| {
        matches!(
            event,
...
async fn steer_interrupts_wait_agent_and_is_sent_in_follow_up_request() {
    ...
    let ody = test_ody()
        ...
        .build_with_streaming_server(&server)
        .await
        .expect("build Ody test session")
        .ody;

    submit_user_input(&ody, INITIAL_PROMPT).await;
    wait_for_event(&ody, |event| {
        matches!(event, EventMsg::CollabWaitingBegin(_))
    })
    .await;

    steer_user_input(&ody, STEER_PROMPT).await;
...
async fn any_new_input_interrupts_sleep() {
    ...
    submit_user_input(&ody, INITIAL_PROMPT).await;
    wait_for_sleep_item_started(&ody, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    steer_user_input(&ody, STEER_PROMPT).await;
    wait_for_sleep_item_completed(&ody, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_sleep_item_started(&ody, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    submit_queue_only_agent_mail(&ody, "new mailbox input").await;
    wait_for_sleep_item_completed(&ody, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_turn_complete(&ody).await;
    ...
    ody.submit(Op::Shutdown).await.expect("shutdown session");
    wait_for_event(&ody, |event| matches!(event, EventMsg::ShutdownComplete)).await;

    let rollout_path = ody.rollout_path().expect("rollout path");
...
async fn queued_inter_agent_mail_triggers_follow_up_after_reasoning_item() {
    ...
    let ody = build_ody(&server).await;

    submit_user_input(&ody, "first prompt").await;

    wait_for_reasoning_item_started(&ody).await;

    submit_queue_only_agent_mail(&ody, "queued child update").await;
...
async fn queued_inter_agent_mail_triggers_follow_up_after_commentary_message_item() {
    ...
    wait_for_agent_message(&ody, "first answer").await;
...
async fn user_input_does_not_preempt_after_reasoning_item() {
    ...
    steer_user_input(&ody, "second prompt").await;
...
async fn steered_user_input_waits_for_model_continuation_after_mid_turn_compact() {
    ...
    submit_user_input(&ody, "first prompt").await;
    submit_user_input(&ody, "second prompt").await;

    wait_for_agent_message(&ody, "resumed old task").await;
    wait_for_turn_complete(&ody).await;
...
async fn steered_user_input_follows_compact_when_only_the_steer_needs_follow_up() {
    ...
    wait_for_agent_message(&ody, "processed steered prompt").await;
    wait_for_turn_complete(&ody).await;
...
async fn steered_user_input_waits_when_tool_output_triggers_compact_before_next_request() {
    ...
    wait_for_event(&ody, |event| matches!(event, EventMsg::TurnStarted(_))).await;
    steer_user_input(&ody, "second prompt").await;
    let _ = gate_first_completed_tx.send(());

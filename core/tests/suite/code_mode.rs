...
const tool = ALL_TOOLS.find(
  ({ name }) => name === "mcp__ody_apps__calendar_timezone_option_99"
);
if (!tool) {
...
"#,
        visible_tool_name = "mcp__ody_apps__calendar_timezone_option_99",
...
async fn code_mode_can_call_hidden_dynamic_tools() -> Result<()> {
    ...
    let code = r#"
const tool = ALL_TOOLS.find(({ name }) => name === "ody_app__hidden_dynamic_tool");
const out = await tools.ody_app__hidden_dynamic_tool({ city: "Paris" });
text(
  JSON.stringify({
    name: tool?.name ?? null,
...
"#;
    ...
    let turn_id = wait_for_event_match(&test.ody, |event| match event {
    ...
    test.ody
        ...
        .await?;
    wait_for_event(&test.ody, |event| match event {
    ...
    assert_eq!(
        parsed.get("name"),
        Some(&Value::String("ody_app__hidden_dynamic_tool".to_string()))
    );
    assert_eq!(
        parsed.get("out"),
    ...
    assert!(
        parsed
            ...
            .is_some_and(|description| {
                description.contains("Ody app tools.")
                    && description.contains("A hidden dynamic tool.")
                    && description.contains("declare const tools:")
                    && description.contains("ody_app__hidden_dynamic_tool(args:")
            })
    );

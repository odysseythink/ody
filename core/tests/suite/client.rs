...
use ody_protocol::models::LocalShellAction;
use ody_protocol::models::LocalShellExecAction;
use ody_protocol::models::LocalShellStatus;
...
fn assert_ody_client_metadata(
    ...
    assert_eq!(
        client_metadata["x-ody-window-id"].as_str(),
        turn_metadata["window_id"].as_str()
    );
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn odysseythink_stateless_responses_requests_preserve_item_turn_metadata_across_turns() {
    ...
    .await;
    let test = test_ody().build(&server).await.unwrap();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn non_odysseythink_responses_requests_omit_item_passthrough_metadata() {
    let server = MockServer::start().await;
...
async fn response_item_ids_persist_across_resume_and_preserve_server_ids() -> anyhow::Result<()> {
    ...
    wait_for_event(&initial.ody, |event| {
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: ody_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path())
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = ody_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = ody_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![ody_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = ody_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Ody to resume from our file
...
async fn send_provider_auth_request(server: &MockServer, auth: ModelProviderAuthInfo) {
    ...
    let model = ody_core::test_support::get_model_offline(config.model.as_deref());
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let thread_manager = ThreadManager::new(
        ...
        Arc::new(ody_core::test_support::EmptyUserInstructionsProvider),
    ...
    let NewThread { thread: ody, .. } = thread_manager
...
async fn skills_append_to_developer_message() {
    ...
    let ody_home = Arc::new(TempDir::new().unwrap());
    let skill_dir = ody_home.path().join("skills/demo");
...
async fn skills_use_aliases_in_developer_message_under_budget_pressure() {
    ...
    let ody_home_parent = TempDir::new().unwrap();
    let long_home_parent = ody_home_parent
        .path()
        .join("ody-home-with-long-shared-prefix-for-skill-alias-budget-test");
    std::fs::create_dir_all(&long_home_parent).expect("create long home parent");
    let ody_home = Arc::new(TempDir::new_in(long_home_parent).unwrap());
    let skill_root = ody_home.path().join("skills");
    for index in 0..12 {
    ...
    let _ody_home_guard = ody_home;
    let _ody_home_parent_guard = ody_home_parent;
}
...
async fn azure_responses_request_includes_store_and_reasoning_ids() {
    ...
    let provider = ModelProviderInfo {
        name: "azure".into(),
        base_url: Some(format!("{}/odysseythink", server.uri())),
        env_key: None,
        env_key_instructions: None,
        experimental_bearer_token: None,
        auth: None,
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/odysseythink/responses");
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-ody-primary-used-percent", "12.5")
        .insert_header("x-ody-secondary-used-percent", "40.0")
        .insert_header("x-ody-primary-window-minutes", "10")
        .insert_header("x-ody-secondary-window-minutes", "60")
        .insert_header("x-ody-primary-reset-at", "1704069000")
        .insert_header("x-ody-secondary-reset-at", "1704074400")
    ...
    wait_for_event(&ody, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-ody-primary-used-percent", "100.0")
        .insert_header("x-ody-secondary-used-percent", "87.5")
        .insert_header("x-ody-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-ody-primary-window-minutes", "15")
        .insert_header("x-ody-secondary-window-minutes", "60")
        .set_body_json(json!({
            "error": {
    ...
    let mut builder = test_ody();
    let ody_fixture = builder.build(&server).await?;
    let ody = ody_fixture.ody.clone();
    ...
    let token_event = wait_for_event(&ody, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&ody, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&ody, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&ody, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = OdyErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
async fn azure_overrides_assign_properties_used_for_responses_url() {
    ...
    // Expect POST to /odysseythink/responses with api-version query param
    Mock::given(method("POST"))
        .and(path("/odysseythink/responses"))
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Ody sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();

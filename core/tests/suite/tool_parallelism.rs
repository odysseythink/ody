...
async fn run_turn(test: &TestOdy, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestOdy, prompt: &str) -> anyhow::Result<Duration> {
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_ody_with_test_tool(&server).await?;

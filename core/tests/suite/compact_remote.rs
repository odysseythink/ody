...
const TURN_STATE_HEADER: &str = "x-ody-turn-state";
...
fn test_ody() -> TestOdyBuilder {
    base_test_ody().with_config(|config| {
        let _ = config.features.disable(Feature::RemoteCompactionV2);
    })
}

fn remote_realtime_test_ody_builder(
...
async fn close_realtime_conversation(ody: &ody_core::OdyThread) -> Result<()> {
    ody.submit(Op::RealtimeConversationClose).await?;
    wait_for_event_match(ody, |msg| match msg {
...
async fn wait_for_turn_complete(ody: &ody_core::OdyThread) {
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    assert_eq!(
        compact_request.header("x-ody-installation-id").as_deref(),
    ...
    assert_eq!(
        ...
        compact_request.header("x-ody-window-id").as_deref()
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        follow_up_request.header("x-ody-window-id").as_deref()
    );
    assert_ne!(
...
async fn assert_remote_manual_compact_request_parity(
    ...
    let harness = TestOdyHarness::with_builder(builder).await?;
    let ody = harness.test().ody.clone();
    let image_url =
...
async fn remote_compact_v2_reuses_compaction_trigger_for_followups() -> Result<()> {
    ...
    assert_eq!(
        compact_request.body_json()["client_metadata"]["x-ody-window-id"].as_str(),
        compact_metadata["window_id"].as_str()
    );
    assert_eq!(
        compact_metadata["compaction"],
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let dynamic_tools = vec![DynamicToolSpec::Namespace(DynamicToolNamespaceSpec {
        name: "ody_app".to_string(),
        description: "Ody app tools.".to_string(),
        tools: vec![
    ...
    assert_eq!(
        namespace_child_tool_names(&first_response_body, "ody_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        namespace_child_tool_names(&compact_body, "ody_app"),
        vec![visible_tool.to_string()]
    );
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    baseline_ody
        ...
        .await?;
    wait_for_event(&baseline_ody, |event| {
    ...
    baseline_ody.submit(Op::Compact).await?;
    wait_for_event(&baseline_ody, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    override_ody
        ...
        .await?;
    wait_for_event(&override_ody, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    override_ody.submit(Op::Compact).await?;
    wait_for_event(&override_ody, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_compact_and_resume_refresh_stale_developer_instructions() -> Result<()> {
    ...
    initial.ody.submit(Op::Compact).await?;
    wait_for_event(&initial.ody, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.ody, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_ody_builder(&realtime_server).with_config(|config| {
    ...
    start_realtime_conversation(test.ody.as_ref()).await?;
    ...
    close_realtime_conversation(test.ody.as_ref()).await?;
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_ody_builder(&realtime_server).with_config({
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_ody_builder(&realtime_server);
    let test = builder.build(&server).await?;
...
async fn snapshot_request_shape_remote_compact_resume_restates_realtime_end() -> Result<()> {
    ...
    start_realtime_conversation(initial.ody.as_ref()).await?;
    ...
    close_realtime_conversation(initial.ody.as_ref()).await?;
...
async fn remote_mid_turn_compact_v2_sends_turn_state_over_websocket() -> Result<()> {
    ...
    test.ody
        ...
        .await?;
    wait_for_turn_complete(&test.ody).await;

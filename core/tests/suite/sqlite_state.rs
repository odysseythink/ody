...
async fn new_thread_is_recorded_in_state_db() -> Result<()> {
    ...
    let db_path = ody_state::state_db_path(test.config.sqlite_home.as_path());
...
async fn backfill_scans_existing_rollouts() -> Result<()> {
    ...
    let mut builder = test_ody()
        .with_pre_build_hook(move |ody_home| {
            let rollout_path = ody_home.join(&rollout_rel_path_for_hook);
    ...
    let db_path = ody_state::state_db_path(test.config.sqlite_home.as_path());
    let rollout_path = test.config.ody_home.join(&rollout_rel_path);
...
async fn mcp_call_marks_thread_memory_mode_polluted_when_configured() -> Result<()> {
    ...
    let test = builder.build(&server).await?;
    wait_for_mcp_server(&test.ody, server_name).await?;
    let db = test.ody.state_db().expect("state db enabled");
    let thread_id = test.session_configured.thread_id;
    ...
    .await;
    wait_for_event_match(&test.ody, |event| match event {
...
async fn tool_call_logs_include_thread_id() -> Result<()> {
    ...
    let log_db_layer = ody_state::log_db::start(db.clone());

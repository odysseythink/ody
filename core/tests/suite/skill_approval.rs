...
fn write_skill_metadata(home: &Path, name: &str, contents: &str) -> Result<()> {
    ...
    fs::write(metadata_dir.join("odysseythink.yaml"), contents)?;
    Ok(())
}
...
fn skill_script_command(test: &TestOdy, script_name: &str) -> Result<String> {
...
async fn wait_for_exec_approval_request(test: &TestOdy) -> Option<ExecApprovalRequestEvent> {
    wait_for_event_match(test.ody.as_ref(), |event| match event {
...
async fn wait_for_turn_complete(test: &TestOdy) {
    wait_for_event(test.ody.as_ref(), |event| {
...
async fn shell_zsh_fork_still_enforces_workspace_write_sandbox() -> Result<()> {
    ...
    let outside_path = "/tmp/ody-zsh-fork-workspace-write-deny.txt";
    let workspace_write_profile = restrictive_workspace_write_profile();

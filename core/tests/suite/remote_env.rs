...
async fn unified_exec_test(server: &wiremock::MockServer) -> Result<TestOdy> {
...
async fn submit_turn_with_approval_and_environments(
    ...
    let turn_environment_selections = ody_protocol::protocol::TurnEnvironmentSelections::new(
...
async fn wait_for_completion_without_patch_approval(test: &TestOdy) {
    let event = wait_for_event(&test.ody, |event| {
        matches!(
            event,
            EventMsg::ApplyPatchApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn remote_test_env_exposes_target_shell_to_model() -> Result<()> {
    ...
    .await;
    let test = test_ody().build_with_remote_env(&server).await?;
...
async fn explicit_remote_shell_runs_in_remote_cwd() -> Result<()> {
    ...
    let (shell, command) = match core_test_support::test_environment() {
        TestEnvironment::Docker { .. } => (
            "bash",
            r#"case "$PWD" in /tmp/ody-core-test-cwd-*) ;; *) echo "unexpected cwd: $PWD" >&2; exit 1 ;; esac"#,
        ),
        TestEnvironment::WineExec => (
            "powershell",
            r#"$cwd = (Get-Location).Path; if ($cwd -notlike 'C:\ody-core-test-cwd-*') { Write-Error "unexpected cwd: $cwd"; exit 1 }"#,
        ),
...
async fn remote_sandbox_denial_requests_approval_and_retries() -> Result<()> {
    ...
    let remote_cwd = PathBuf::from(format!("/tmp/ody-remote-denial-cwd-{nonce}")).abs();
    let target_path = PathBuf::from(format!("/tmp/ody-remote-denial-target-{nonce}")).abs();
...
async fn deferred_executor_updates_model_context_after_startup() -> Result<()> {
    ...
    test.ody
        ...
        .await?;
    let request = wait_for_event_match(&test.ody, |event| match event {
...
async fn remote_request_permissions_grant_unblocks_later_remote_exec() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/ody-remote-request-permissions-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_freeform_routes_to_selected_remote_environment() -> Result<()> {
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/ody-remote-apply-patch-freeform-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn apply_patch_approvals_are_remembered_per_environment() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/ody-remote-apply-patch-approval-cwd-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
    ...
    let target_path = PathBuf::from(format!(
        "/tmp/ody-apply-patch-approval-scope-{}.txt",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_intercepted_exec_command_routes_to_selected_remote_environment() -> Result<()>
{
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/ody-remote-apply-patch-exec-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn remote_test_env_sandboxed_read_allows_readable_root() -> Result<()> {
    ...
    let allowed_dir = PathBuf::from(format!("/tmp/ody-remote-readable-{}", std::process::id()));
...
async fn remote_test_env_sandboxed_read_rejects_symlink_parent_dotdot_escape() -> Result<()> {
    ...
    let root = PathBuf::from(format!("/tmp/ody-remote-dotdot-{}", std::process::id()));

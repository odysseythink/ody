...
fn ody_home_for_windows_sandbox_test(name: &str) -> anyhow::Result<TestOdyHome> {
    if let Some(test_tmpdir) = std::env::var_os("TEST_TMPDIR") {
        // The elevated backend provisions machine-local sandbox users. Bazel
        // retries run in the same Windows VM, so keep ODY_HOME stable within
        // the test temp root and let setup reconcile its persisted ACL state.
        let ody_home = PathBuf::from(test_tmpdir).join(name);
        std::fs::create_dir_all(&ody_home)
            .with_context(|| format!("create stable test ODY_HOME {}", ody_home.display()))?;
        return Ok(TestOdyHome::Persistent(ody_home));
    }

    Ok(TestOdyHome::Temporary(TempDir::new()?))
}
...
fn stage_windows_sandbox_helpers() -> anyhow::Result<()> {
    ...
    let resources_dir = test_exe_dir.join("ody-resources");
    ...
    for helper_name in ["ody-windows-sandbox-setup", "ody-command-runner"] {
        let helper = ody_utils_cargo_bin::cargo_bin(helper_name)?;
...
async fn windows_restricted_token_rejects_exact_and_glob_deny_read_policy() -> anyhow::Result<()> {
    let ody_home =
        ody_home_for_windows_sandbox_test("windows-restricted-token-deny-read-ody-home")?;
...
async fn windows_elevated_enforces_deny_read_and_protects_setup_marker() -> anyhow::Result<()> {
    let ody_home = ody_home_for_windows_sandbox_test("windows-elevated-deny-read-ody-home")?;
    let _ody_home_guard = EnvVarGuard::set("ODY_HOME", ody_home.path().as_os_str());
    ...
    let public = cwd.join("public.txt");
    let setup_marker = ody_home.path().join(".sandbox").join("setup_marker.json");
    ...
    assert!(
        sandbox_setup_is_complete(ody_home.path()),

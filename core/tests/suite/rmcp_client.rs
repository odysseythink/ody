...
fn read_only_user_turn(fixture: &TestOdy, text: impl Into<String>) -> Op {
...
fn auto_approved_user_turn(fixture: &TestOdy, text: impl Into<String>) -> Op {
...
fn remote_aware_environment_id() -> String {
    if test_environment().is_remote() {
        ...
        ody_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string()
    }
}
...
fn unique_remote_path(binary_name: &str) -> anyhow::Result<String> {
    ...
    Ok(format!(
        "/tmp/ody-remote-env/{binary_name}-{}-{unique_suffix}",
        std::process::id()
    ))
}
...
async fn odysseythink_form_capability_is_advertised_to_mcp_servers() -> anyhow::Result<()> {
    assert_odysseythink_form_capability_advertisement(/*expected*/ true).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn odysseythink_form_capability_is_not_advertised_by_default() -> anyhow::Result<()> {
    assert_odysseythink_form_capability_advertisement(/*expected*/ false).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn odysseythink_form_capability_updates_for_loaded_thread() -> anyhow::Result<()> {
    skip_if_wine_exec!(
        Ok(()),
...
async fn assert_odysseythink_form_capability_advertisement(expected: bool) -> anyhow::Result<()> {
    skip_if_wine_exec!(
        Ok(()),
        "requires a Windows test_stdio_server in the Wine-exec environment"
    );
...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.ody, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn shutdown_cancels_startup_prewarm_waiting_for_mcp_startup() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(Duration::from_secs(2), fixture.ody.shutdown_and_wait())
        .await
...
async fn stdio_mcp_parallel_tool_calls_default_false_runs_serially() -> anyhow::Result<()> {
    ...
    while call_events.len() < 4 {
        let event = wait_for_event(&fixture.ody, |ev| {
            matches!(
                ev,
...
const REMOTE_EXEC_SERVER_URL_ENV_VAR: &str = "ODY_TEST_REMOTE_EXEC_SERVER_URL";
...
impl StreamableHttpTestServer {
    /// Returns the MCP endpoint URL that Ody should connect to.
    fn url(&self) -> &str {
        &self.server_url
    }
...
/// What this tests: Ody can discover and call a Streamable HTTP MCP tool in
...
async fn streamable_http_tool_call_round_trip() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Ody will call the MCP echo tool
    ...
    // Phase 3: configure Ody with the Streamable HTTP MCP server and build a
    ...
    // Phase 5: assert Ody begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.ody, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
/// This test writes to a fallback credentials file in ODY_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire ODY_HOME through the code.
#[test]
#[serial(ody_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Ody will call the OAuth-backed
    ...
    // Phase 3: seed an isolated ODY_HOME with fallback OAuth tokens for this
    // server so the test does not share credentials with other suite cases.
    let temp_home = Arc::new(tempdir()?);
    let _ody_home_guard = EnvVarGuard::set("ODY_HOME", temp_home.path().as_os_str());
    ...
    // Phase 4: configure Ody with the OAuth-backed Streamable HTTP MCP
    ...
    // Phase 7: assert Ody begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.ody, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn start_remote_streamable_http_test_server(
    ...
    // remote HTTP before handing the URL to the Ody fixture.

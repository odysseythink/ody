...
//! request payload that Ody would send to the model and assert that the
...
fn network_disabled() -> bool {
    std::env::var(ODY_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (Arc<TempDir>, Config, Arc<ThreadManager>, Arc<OdyThread>) {
    ...
    (test.home, test.config, test.thread_manager, test.ody)
}

async fn user_turn(conversation: &Arc<OdyThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
...
async fn compact_conversation(conversation: &Arc<OdyThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<OdyThread>) -> std::path::PathBuf {
    conversation.rollout_path().expect("rollout path")
}

async fn shutdown_conversation(conversation: &Arc<OdyThread>) {
    conversation
        .shutdown_and_wait()
        .await

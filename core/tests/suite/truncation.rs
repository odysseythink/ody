    ...
    let mut builder = test_ody().with_config(move |config| {
        ...
        servers.insert(
            ...
            ody_config::types::McpServerConfig {
                transport: ody_config::types::McpServerTransportConfig::Stdio {
...
async fn mcp_image_output_preserves_image_and_no_text_summary() -> Result<()> {
    ...
    // 1x1 PNG data URL
    let odysseythink_png = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4nGP4z8DwHwAFAAH/iZk9HQAAAABJRU5ErkJggg==";
    ...
    assert_eq!(
        arr[1],
        json!({"type": "input_image", "image_url": odysseythink_png, "detail": "high"})
    );

...
async fn responses_api_parent_and_subagent_requests_include_identity_headers() -> Result<()> {
    ...
    let parent_mock = mount_sse_once_match(
        ...
        |req: &wiremock::Request| {
            request_body_contains(req, PARENT_PROMPT)
                && request_header(req, "x-odysseythink-subagent").is_none()
        },
        sse(vec![
    ...
    let child_mock = mount_sse_once_match(
        ...
        |req: &wiremock::Request| {
            request_body_contains(req, CHILD_PROMPT)
                ...
                && request_header(req, "x-odysseythink-subagent") == Some("collab_spawn")
        },
        sse(vec![
    ...
    let child = wait_for_matching_request(&child_mock, "child request", |request| {
        request.body_contains_text(CHILD_PROMPT)
            ...
            && request.header("x-odysseythink-subagent").as_deref() == Some("collab_spawn")
    })
    .await?;

    let parent_window_id = parent
        .header("x-ody-window-id")
        .ok_or_else(|| anyhow!("parent request missing x-ody-window-id"))?;
    let child_window_id = child
        .header("x-ody-window-id")
        .ok_or_else(|| anyhow!("child request missing x-ody-window-id"))?;
    ...
    assert_eq!(parent.header("x-odysseythink-subagent"), None);
    assert_eq!(
        child.header("x-odysseythink-subagent").as_deref(),
        Some("collab_spawn")
    );
    assert_eq!(
        child.header("x-ody-parent-thread-id").as_deref(),
        Some(parent_thread_id)
    );
    let child_turn_metadata: serde_json::Value = serde_json::from_str(
        &child
            .header("x-ody-turn-metadata")
            .ok_or_else(|| anyhow!("child request missing x-ody-turn-metadata"))?,
    )?;
...
async fn submit_turn_with_timeout(test: &TestOdy, prompt: &str) -> Result<()> {

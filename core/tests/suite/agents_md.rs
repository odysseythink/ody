...
use core_test_support::test_ody::RecordingUserInstructionsProvider;
...
async fn agents_instructions(mut builder: TestOdyBuilder) -> Result<String> {
...
async fn submit_thread_turn(thread: &Arc<ody_core::OdyThread>, prompt: &str) -> Result<()> {
    thread
        .submit(Op::UserInput {
...
async fn fresh_thread_composes_global_before_project_and_reports_sources() -> Result<()> {
    ...
    assert_eq!(test.ody.instruction_sources().await, creation_sources);
...
async fn invalid_utf8_global_instructions_are_lossy() -> Result<()> {
    ...
    let mut builder = test_ody().with_home(home);
    let test = builder.build(&server).await?;
...
async fn cold_resume_replays_rendered_instructions_but_reports_current_config_sources() -> Result<()>
{
    ...
    let mut initial_builder = test_ody().with_home(Arc::clone(&home));
    let initial = initial_builder.build(&server).await?;

    // Assert the pre-resume thread reports the source used to create its snapshot.
    assert_eq!(
        initial.ody.instruction_sources().await,
    ...
    let mut resume_builder = test_ody().with_home(Arc::clone(&home));
...
async fn fork_replays_rendered_instructions_from_shared_history() -> Result<()> {
    ...
    let mut builder = test_ody().with_home(Arc::clone(&home));
    let parent = builder.build(&server).await?;

    // Assert the parent reports the source used to create its snapshot.
    assert_eq!(
        parent.ody.instruction_sources().await,
        vec![PathUri::from_abs_path(&source)],
        "parent reports the creation-time global source"
    );
    parent.submit_turn("persist instructions").await?;
    parent.ody.ensure_rollout_materialized().await;
    parent.ody.flush_rollout().await?;
    let rollout_path = parent.ody.rollout_path().expect("rollout path");
    ...
    fork_config.ody_self_exe = parent.config.ody_self_exe.clone();
    let forked = parent
        .thread_manager
        .fork_thread(

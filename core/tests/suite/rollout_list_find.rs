...
use chrono::Utc;
use ody_core::RolloutRecorder;
use ody_core::RolloutRecorderParams;
use ody_core::config::ConfigBuilder;
use ody_core::find_archived_thread_path_by_id_str;
use ody_core::find_thread_meta_by_name_str;
use ody_core::find_thread_path_by_id_str;
...
fn write_minimal_rollout_with_id_in_subdir(ody_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = ody_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(ody_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(ody_home, "sessions", id)
}
...
async fn upsert_thread_metadata(
    ...
    let runtime = StateRuntime::init(ody_home.to_path_buf(), "test-provider".to_string())
        .await
        .unwrap();
    runtime
    ...
    builder.cwd = ody_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_ody_home_directory() {
    let repo = TempDir::new().unwrap();
    let ody_home = repo.path().join(".ody");
    std::fs::create_dir_all(&ody_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".ody/**\n").unwrap();
    let id = Uuid::new_v4();

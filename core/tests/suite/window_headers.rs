...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let initial_thread = Arc::clone(&initial.ody);
    ...
    let resumed = builder
        ...
        .await?;
    submit_user_turn(&resumed.ody, "after resume").await?;
    shutdown_thread(&resumed.ody).await?;
...
async fn submit_compact_turn(ody: &Arc<OdyThread>) -> Result<()> {
    ody.submit(Op::Compact).await?;
    let warning_event = wait_for_event(ody, |event| matches!(event, EventMsg::Warning(_))).await;
...
async fn shutdown_thread(ody: &Arc<OdyThread>) -> Result<()> {
...
fn window_id_parts(request: &ResponsesRequest) -> (String, u64) {
    let window_id = request
        ...
        .expect("missing x-ody-window-id header");

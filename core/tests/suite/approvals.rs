...
use ody_protocol::protocol::ExecPolicyAmendment;
...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestOdy) -> (PathBuf, String) {
        match self {
    ...
    async fn prepare(
        ...
        match self {
            ...
            ActionKind::ApplyPatchShell { target, content } => {
                ...
                // Bazel may need to launch the configured Ody helper binary
...
impl Expectation {
    fn verify(&self, test: &TestOdy, result: &CommandResult) -> Result<()> {
        match self {
...
async fn wait_for_completion_without_approval(test: &TestOdy) {
    let event = wait_for_event(&test.ody, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestOdy) {
...
async fn wait_for_spawned_thread(test: &TestOdy) -> Result<Arc<OdyThread>> {
...
async fn run_scenario(scenario: &ScenarioSpec) -> Result<()> {
    ...
    let model = model_override.unwrap_or("k3");
    let policy_src = scenario.action.policy_src();

    let mut builder = test_ody().with_model(model).with_config(move |config| {
...
async fn invalid_requested_prefix_rule_falls_back_for_compound_command() -> Result<()> {
    ...
    let command =
        "touch /tmp/ody-fallback-rule-test.txt && echo hello > /tmp/ody-fallback-rule-test.txt";
    let event = shell_event_with_prefix_rule(
        call_id,
        command,
        /*timeout_ms*/ 1_000,
...
"#,
    ...
    let fetch_command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://ody-network-test.invalid', timeout=30).read().decode(errors='replace'))""#
        .to_string();

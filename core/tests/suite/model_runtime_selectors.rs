...
async fn remote_tool_mode_selector_overrides_feature_flags() -> Result<()> {
    ...
    assert!(
        direct_tools
            .iter()
            .all(|name| name != ody_code_mode::PUBLIC_TOOL_NAME
                && name != ody_code_mode::WAIT_TOOL_NAME),
    ...
    assert!(
        tool_names(&unsupported_response.body)
            .iter()
            .any(|name| name == ody_code_mode::PUBLIC_TOOL_NAME)
    );
    assert_eq!(
...
async fn remote_multi_agent_selector_uses_model_selected_before_first_turn() -> Result<()> {
    ...
    .await?;
    assert_eq!(test.ody.multi_agent_version(), None);

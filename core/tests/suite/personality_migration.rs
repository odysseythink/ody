...
async fn read_config_toml(ody_home: &Path) -> io::Result<ConfigToml> {
    let contents = tokio::fs::read_to_string(ody_home.join("config.toml")).await?;
    toml::from_str(&contents).map_err(|err| io::Error::new(io::ErrorKind::InvalidData, err))
}

async fn write_session_with_user_event(ody_home: &Path) -> io::Result<()> {
...
async fn write_archived_session_with_user_event(ody_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = ody_home.join(ARCHIVED_SESSIONS_SUBDIR);
    write_rollout_with_user_event(&dir, thread_id).await
}

async fn write_session_with_meta_only(ody_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = ody_home
        .join(SESSIONS_SUBDIR)
        .join("2025")
...
async fn no_marker_sessions_preserves_existing_config_fields() -> io::Result<()> {
    let temp = TempDir::new()?;
    write_session_with_user_event(temp.path()).await?;
    tokio::fs::write(temp.path().join("config.toml"), "model = \"k3\"\n").await?;
    ...
    assert_eq!(status, PersonalityMigrationStatus::Applied);
    let persisted = read_config_toml(temp.path()).await?;
    assert_eq!(persisted.model, Some("k3".to_string()));
    assert_eq!(persisted.personality, Some(Personality::Pragmatic));
    Ok(())
}

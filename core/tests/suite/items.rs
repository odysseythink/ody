...
fn image_generation_artifact_path(ody_home: &Path, session_id: &str, call_id: &str) -> PathBuf {
...
async fn user_message_item_is_emitted() -> anyhow::Result<()> {
    ...
    let started_item = wait_for_event_match(&ody, |ev| match ev {
    ...
    .await;
    let completed_item = wait_for_event_match(&ody, |ev| match ev {
    ...
    let legacy_message = wait_for_event_match(&ody, |ev| match ev {
...
async fn assistant_message_item_is_emitted() -> anyhow::Result<()> {
    ...
    let Some(ody_protocol::items::AgentMessageContent::Text { text }) = completed.content.first()
    else {
...
async fn agent_message_content_delta_has_item_metadata() -> anyhow::Result<()> {
    ...
    let (started_turn_id, started_item) = wait_for_event_match(&ody, |ev| match ev {
        EventMsg::ItemStarted(ItemStartedEvent {
            turn_id,
            item: TurnItem::AgentMessage(item),
            ..
    ...
    let delta_event = wait_for_event_match(&ody, |ev| match ev {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn plan_mode_inline_proposed_plan_is_stripped_and_does_not_emit_plan_item()
-> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    assert!(
        !saw_plan_item,
        "inline <proposed_plan> must not emit a PlanItem"
    );
...
async fn plan_mode_strips_plan_from_agent_messages() -> anyhow::Result<()> {
    ...
    while agent_item.is_none() || !saw_plan_item {
        let ev = wait_for_event(&ody, |_| true).await;
        match ev {
            EventMsg::AgentMessageContentDelta(event) => {
                agent_deltas.push(event.delta);
    ...
    assert!(
        !saw_plan_item,
        "inline <proposed_plan> must not produce a PlanItem"
    );
    let agent_text = agent_deltas.concat();
    assert_eq!(agent_text, "Intro\nOutro");
...
async fn reasoning_content_delta_has_item_metadata() -> anyhow::Result<()> {
    ...
    let reasoning_item = wait_for_event_match(&ody, |ev| match ev {
        EventMsg::ItemStarted(ItemStartedEvent {
            item: TurnItem::Reasoning(item),
            ..
        }) => Some(item.clone()),

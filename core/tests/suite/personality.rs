...
async fn personality_does_not_mutate_base_instructions_without_template() {
    ...
    let model_info = ody_core::test_support::construct_model_info_offline("gpt-5.4", &config);
    assert_eq!(
...
async fn user_turn_personality_some_adds_update_message() -> anyhow::Result<()> {
    ...
    .await;
    let mut builder = test_ody()
        .with_model("exp-ody-personality")
        .with_config(|config| {
            config
                .features
                .enable(Feature::Personality)
...
async fn remote_model_friendly_personality_instructions_with_feature() -> anyhow::Result<()> {
    ...
    let remote_slug = "ody-remote-default-personality";
    ...
    let mut builder = test_ody()
        .with_auth(ody_login::OdyAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
            config
                .features
                .enable(Feature::Personality)
...
async fn user_turn_personality_remote_model_template_includes_update_message() -> anyhow::Result<()>
{
    ...
    let remote_slug = "ody-remote-personality";

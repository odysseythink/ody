...
const FIXED_CWD: &str = "/tmp/ody_remote_compaction_parity_workspace";
...
async fn run_manual_session(
    ...
    for (idx, step) in scenario.steps.iter().enumerate() {
        submit_user_input(&ody, user_input_for_step(scenario.name, idx, *step)).await?;
    }
...
async fn run_manual_hook_session(mode: Mode) -> Result<Value> {
    ...
    let home = harness.test().ody_home_path();
...
async fn build_auto_harness(mode: Mode) -> Result<TestOdyHarness> {
    build_harness_inner(
        mode,
        RunSettings::default(),
        /*hooks*/ false,
...
async fn build_harness(mode: Mode, settings: RunSettings, hooks: bool) -> Result<TestOdyHarness> {
...
fn rollout_path(harness: &TestOdyHarness) -> PathBuf {
    harness
        .test()
        .session_configured
        .rollout_path
...
fn normalize_tmp_prefix_before_marker(text: &mut String, marker: &str) {
    ...
    while let Some(relative_marker_index) = text[search_start..].find(marker) {
        ...
        if let Some(start_index) = start {
            text.replace_range(start_index..marker_index, "<ODY_HOME>");
            search_start = start_index + "<ODY_HOME>".len() + marker.len();
        } else {
...
fn normalize_string_rewrites_linux_temp_skill_paths() {
    ...
    assert_eq!(
        text,
        "file: <ODY_HOME>/skills/.system/imagegen/SKILL.md and \
         <ODY_HOME>/skills/custom/SKILL.md"
    );
}

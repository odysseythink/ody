...
use core_test_support::test_ody::ApplyPatchModelOutput;
...
pub async fn apply_patch_harness() -> Result<TestOdyHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestOdyBuilder) -> TestOdyBuilder,
) -> Result<TestOdyHarness> {
    let builder = configure(test_ody());
    // Box harness construction so apply_patch_cli tests do not inline the
    // full test-thread startup path into each test future.
    Box::pin(TestOdyHarness::with_remote_env_builder(builder)).await
}

async fn submit_without_wait(harness: &TestOdyHarness, prompt: &str) -> Result<()> {
...
async fn apply_patch_custom_tool_streaming_emits_updated_changes() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(&ody_protocol::protocol::FileChange::Add {
            content: String::new(),
        })
    );
    assert_eq!(

...
fn read_only_user_turn(test: &TestOdy, items: Vec<UserInput>, model: String) -> Op {
...
async fn model_switch_to_smaller_model_updates_token_context_window() -> Result<()> {
    ...
    let large_window_event = wait_for_event(&test.ody, |event| {
        matches!(
            event,
    ...
    let smaller_turn_started_event = wait_for_event(&test.ody, |event| {
        matches!(
            event,
    ...
    let smaller_window_event = wait_for_event(&test.ody, |event| {
        matches!(
            event,
            EventMsg::TokenCount(token_count)
                if token_count

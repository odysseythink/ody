...
async fn websocket_test_ody_shell_chain() -> Result<()> {
    ...
    let mut builder = test_ody().with_windows_cmd_shell();
...
async fn websocket_first_turn_uses_startup_prewarm_and_create() -> Result<()> {
    ...
    assert_eq!(
        ...
        warmup["client_metadata"]["x-ody-window-id"].as_str()
    );
    assert!(
        turn["tools"]
            .as_array()
    ...
    let turn_metadata: Value = serde_json::from_str(
        turn["client_metadata"]["x-ody-turn-metadata"]
            .as_str()
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn websocket_v2_test_ody_shell_chain() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let mut builder = test_ody().with_windows_cmd_shell().with_config(|config| {
        config
            .features
    ...
    assert_eq!(
        handshake.header("odysseythink-beta"),

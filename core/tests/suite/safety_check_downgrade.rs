...
fn disabled_text_turn(test: &TestOdy, text: &str) -> Op {
...
async fn odysseythink_model_header_mismatch_emits_warning_event() -> Result<()> {
    ...
    let reroute = wait_for_event(&test.ody, |event| {
    ...
    let warning = wait_for_event(&test.ody, |event| matches!(event, EventMsg::Warning(_))).await;
    ...
    let _ = wait_for_event(&test.ody, |event| {
...
async fn cyber_policy_response_emits_typed_error_without_retry() -> Result<()> {
    ...
    let error = wait_for_event(&test.ody, |event| matches!(event, EventMsg::Error(_))).await;
    let EventMsg::Error(error) = error else {
        panic!("expected error event");
    };
    assert_eq!(error.message, CYBER_POLICY_MESSAGE);
    assert_eq!(error.ody_error_info, Some(OdyErrorInfo::CyberPolicy));
...
async fn response_model_field_mismatch_emits_warning_when_header_matches_requested() -> Result<()> {
    ...
    let warning = wait_for_event(&test.ody, |event| {
        matches!(
            event,
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn odysseythink_model_header_mismatch_only_emits_one_warning_per_turn() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    loop {
        let event = wait_for_event(&test.ody, |_| true).await;
        match event {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn odysseythink_model_header_casing_only_mismatch_does_not_warn() -> Result<()> {
    skip_if_no_network!(Ok(()));

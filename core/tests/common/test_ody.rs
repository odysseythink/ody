...
use ody_core::TimeProvider;
use ody_core::config::Config;
use ody_core::resolve_installation_id;
use ody_core::shell::Shell;
use ody_core::shell::get_shell_by_model_provided_path;
...
use ody_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
...
type WorkspaceSetup = dyn FnOnce(AbsolutePathBuf, Arc<dyn ExecutorFileSystem>) -> BoxFuture<'static, Result<()>>
    + Send;
const TEST_MODEL_WITH_EXPERIMENTAL_TOOLS: &str = "test-kimi-for-coding";
...
pub fn local(cwd: AbsolutePathBuf) -> TurnEnvironmentSelection {
    TurnEnvironmentSelection {
        environment_id: ody_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
...
#[derive(Debug)]
pub struct TestEnv {
    environment: ody_exec_server::Environment,
...
impl TestEnv {
    pub async fn local() -> Result<Self> {
        ...
        let environment =
            ody_exec_server::Environment::create_for_tests(/*exec_server_url*/ None)?;
        Ok(Self {
            environment,
    ...
    pub fn environment(&self) -> &ody_exec_server::Environment {
        &self.environment
    }
...
pub async fn test_env() -> Result<TestEnv> {
    match get_remote_test_env() {
        Some(remote_env) => {
            ...
            let environment =
                ody_exec_server::Environment::create_for_tests(Some(websocket_url.clone()))?;
            let cwd = remote_env
...
impl TestOdyBuilder {
    ...
    pub async fn build(&mut self, server: &wiremock::MockServer) -> anyhow::Result<TestOdy> {
    ...
    async fn build_with_home_and_base_url(
        ...
        let environment_manager = Arc::new(if include_local_environment {
            ...
            ody_exec_server::EnvironmentManager::create_for_tests(
                exec_server_url,
    ...
    async fn build_from_config(
        ...
        let user_instructions_provider =
            self.user_instructions_provider.clone().unwrap_or_else(|| {
                Arc::new(OdyHomeUserInstructionsProvider::new(
        ...
        let new_conversation = match (resume_from, user_shell_override) {
            ...
            (None, Some(user_shell_override)) => {
                Box::pin(
                    ody_core::test_support::start_thread_with_user_shell_override(
                        thread_manager.as_ref(),
                        config.clone(),
                        user_shell_override,
                        self.supports_form_elicitation,
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = ody_utils_cargo_bin::cargo_bin("ody") {
            config.ody_self_exe = Some(path);
        } else if let Ok(path) = ody_utils_cargo_bin::cargo_bin("ody-exec") {
            // `ody-exec` also supports `--ody-run-as-apply-patch`, so use it
        ...
        {
            let ody = bin_dir.join("ody");
            let ody_exec = bin_dir.join("ody-exec");
            if ody.is_file() {
                config.ody_self_exe = Some(ody);
            } else if ody_exec.is_file() {
                config.ody_self_exe = Some(ody_exec);
            }
        }
...
fn ensure_test_model_catalog(config: &mut Config) -> Result<()> {
    ...
    let mut model = bundled_models
        .models
        .iter()
        .find(|candidate| candidate.slug == "kimi-k2.5")
        .cloned()
        .expect("missing bundled model kimi-k2.5");
...
impl TestOdy {
    ...
    pub fn ody_home_path(&self) -> &Path {
        self.config.ody_home.as_path()
    }
    ...
    async fn submit_turn_with_context(
        ...
        let turn_id = wait_for_event_match(&self.ody, |event| match event {
...
impl TestOdyHarness {
    pub async fn new() -> Result<Self> {
        Self::with_builder(test_ody()).await
    }

    pub async fn with_config(mutator: impl FnOnce(&mut Config) + Send + 'static) -> Result<Self> {
        Self::with_builder(test_ody().with_config(mutator)).await
    }

    pub async fn with_builder(mut builder: TestOdyBuilder) -> Result<Self> {
        let server = start_mock_server().await;
        let test = builder.build(&server).await?;
        Ok(Self { server, test })
    }

    pub async fn with_remote_env_builder(mut builder: TestOdyBuilder) -> Result<Self> {
        let server = start_mock_server().await;
...
pub fn test_ody() -> TestOdyBuilder {
    TestOdyBuilder {

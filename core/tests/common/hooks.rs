...
pub fn trust_hooks(config: &mut Config, hooks: Vec<HookListEntry>) {
    config.config_layer_stack =
        trusted_config_layer_stack(&config.config_layer_stack, &config.ody_home, hooks);
}
...
pub fn trusted_config_layer_stack(
    ...
    config_layer_stack.with_user_config(&ody_home.join(CONFIG_TOML_FILE), user_config)
}

...
pub use ody_core::test_support::TestOdyResponsesRequestKind;
pub use ody_core::test_support::responses_metadata;
use ody_utils_absolute_path::AbsolutePathBuf;
pub use ody_utils_absolute_path::test_support::PathBufExt;
pub use ody_utils_absolute_path::test_support::PathExt;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    ody_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    ody_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(ody_arg0::arg0_dispatch);
}
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\ody\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer’s real `~/.ody-code`.
pub async fn load_default_config_for_test(ody_home: &TempDir) -> Config {
    ...
    F: FnMut(&ody_protocol::protocol::EventMsg) -> bool,
{
    ...
    wait_for_event_with_timeout(ody, predicate, Duration::from_secs(1)).await
}

/// Waits for a configured MCP server to finish startup and requires it to be ready.
pub async fn wait_for_mcp_server(ody: &OdyThread, server_name: &str) -> anyhow::Result<()> {
...
pub async fn submit_thread_settings(
    ...
    let submission_id = ody.submit(Op::ThreadSettings { thread_settings }).await?;
    loop {
        let ev = timeout(Duration::from_secs(10), ody.next_event())
            .await
...
pub async fn wait_for_event_match<T, F>(ody: &OdyThread, matcher: F) -> T
where
    F: Fn(&ody_protocol::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(ody, |ev| matcher(ev).is_some()).await;
    ...
    F: FnMut(&ody_protocol::protocol::EventMsg) -> bool,
{
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(10)), ody.next_event())
            .await
...
pub fn sandbox_env_var() -> &'static str {
    ody_core::spawn::ODY_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    ody_core::spawn::ODY_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

pub fn format_with_current_shell(command: &str) -> Vec<String> {
    ody_core::shell::default_user_shell().derive_exec_args(command, /*use_login_shell*/ true)
}
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    ody_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
...
macro_rules! skip_if_no_network {
    () => {{
        if ::std::env::var($crate::sandbox_network_env_var()).is_ok() {
            println!(
                "Skipping test because it cannot execute when network is disabled in a Ody sandbox."
            );
            return;
        }
    }};
...
#[macro_export]
macro_rules! ody_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_ody_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("ody-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }

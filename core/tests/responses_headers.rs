...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    let request_recorder = responses::mount_sse_once_match(
        &server,
        header("x-odysseythink-subagent", "review"),
        response_body,
    )
    .await;
    ...
    assert_eq!(
        request.header("x-ody-window-id").as_deref(),
        Some(expected_window_id.as_str())
    );
    assert_eq!(request.header("x-ody-parent-thread-id"), None);
    assert_eq!(
        request.body_json()["client_metadata"]["x-ody-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    assert_eq!(
        request.body_json()["client_metadata"]["x-ody-window-id"].as_str(),
        Some(expected_window_id.as_str())
    );
    assert_eq!(request.header("x-ody-sandbox"), None);
}
...
async fn responses_stream_includes_subagent_header_on_other() {
    ...
    let request_recorder = responses::mount_sse_once_match(
        &server,
        header("x-odysseythink-subagent", "my-task"),
        response_body,
    )
    .await;
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    let test = test_ody().build(&server).await.expect("build test ody");
    ...
    let initial_header = first_request
        ...
        .expect("x-ody-turn-metadata header should be present");
    let initial_parsed: serde_json::Value =
        serde_json::from_str(&initial_header).expect("x-ody-turn-metadata should be valid JSON");
    ...
    assert!(
        ...
        "turn_id should not be empty in x-ody-turn-metadata"
    );

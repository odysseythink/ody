...
This crate implements the business logic for Ody. It is designed to be used by the various Ody UIs written in Rust.
...
Note that `ody-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
pointer file), the resolved `gitdir:` target, and `.ody` read-only.
...
`bwrap` is missing, it falls back to the bundled `ody-resources/bwrap`
binary shipped with Ody and Ody surfaces a startup warning through its
normal notification path instead of printing directly from the sandbox helper.
Ody also surfaces a startup warning when bubblewrap cannot create user
...
namespaces, so Ody rejects sandboxed shell commands that would enter the
...
Expects the binary containing `ody-core` to simulate the virtual
`apply_patch` CLI when `arg1` is `--ody-run-as-apply-patch`. See the
`ody-arg0` crate for details.

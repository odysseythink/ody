# Ody MCP Server Interface [experimental]

This document describes Ody's experimental MCP server interface: a JSON-RPC API that runs over the Model Context Protocol (MCP) transport to control a local Ody engine.

- Status: experimental and subject to change without notice
- Server binary: `ody mcp-server` (or `ody-mcp-server`)
...
Run Ody as an MCP server and connect an MCP client:

```bash
ody mcp-server | your_mcp_client
```

For a simple inspection UI, you can also try:

```bash
npx @modelcontextprotocol/inspector ody mcp-server
```

Use the separate `ody mcp` subcommand to manage configured MCP server launchers in `config.toml`.
...
Fetch the catalog of models available in the current Ody build with `model/list`. The request accepts optional pagination inputs:
...
- `ody/event` with the serialized Ody event payload. The shape matches `core/src/protocol.rs`'s `Event` and `EventMsg` types. Some notifications include a `_meta.requestId` to correlate with the originating request.
...
The `ody` and `ody-reply` tools return standard MCP `CallToolResult` payloads. For compatibility with MCP clients that prefer `structuredContent`, Ody mirrors the content blocks inside `structuredContent` alongside the `threadId`.

Example:

```json
{
  "content": [{ "type": "text", "text": "Hello from Ody" }],
  "structuredContent": {
    "threadId": "019bbed6-1e9e-7f31-984c-a05b65045719",
    "content": "Hello from Ody"
  }
}
```
...
When Ody needs approval to apply changes or run commands, the server issues JSON-RPC requests to the client:

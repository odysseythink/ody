...
Ody uses BuildBuddy for a shared Bazel cache and remoted builds and tests. To use it
to speed up your builds and tests you'll need to provide an API key and select a
configuration.

### BuildBuddy API key

If you're an OpenAI employee, log in to https://odysseythink.buildbuddy.io and use Google sign-in.
...
```bazelrc
common --config=buildbuddy-odysseythink-rbe
```

OpenAI employees who don't want remote execution can use `buildbuddy-odysseythink`. External users
...
| `bazel ... --config=buildbuddy-odysseythink` | Yes | `odysseythink.buildbuddy.io` | Local | Local |
| `bazel ... --config=buildbuddy-odysseythink-rbe` | Yes | `odysseythink.buildbuddy.io` | Remote | Remote |
...
| --- | --- | --- |
| Push to `main` in `odysseythink/ody` | Yes | Yes |
| `workflow_dispatch` in `odysseythink/ody` | Yes | Yes |
| Same-repository pull request in `odysseythink/ody` | Yes | Yes |
| Fork pull request into `odysseythink/ody` | No | No; local |
...
run in `odysseythink/ody`. A missing or malformed pull request event
...
2. Create a `BUILD.bazel` that calls `ody_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `ody_rust_crate` to configure it.

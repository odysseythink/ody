...
async fn add_and_remove_server_updates_global_config() -> Result<()> {
    ...
    let mut add_cmd = ody_command(ody_home.path())?;
    add_cmd
    ...
    let mut remove_cmd = ody_command(ody_home.path())?;
    remove_cmd
    ...
    let mut remove_again_cmd = ody_command(ody_home.path())?;
    remove_again_cmd
        .args(["mcp", "remove", "docs"])
        .assert()
        .success()
...
async fn add_streamable_http_without_manual_token() -> Result<()> {
    ...
    assert!(!ody_home.path().join(".credentials.json").exists());
    assert!(!ody_home.path().join(".env").exists());
...
async fn add_streamable_http_with_oauth_options() -> Result<()> {
    ...
    assert_eq!(oauth_server.oauth_client_id(), Some("eci-prd-pub-ody-123"));
    assert_eq!(

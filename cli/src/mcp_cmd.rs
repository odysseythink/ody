...
use ody_core::config::LoaderOverrides;
...
use ody_mcp::ResolvedMcpOAuthScopes;
...
use ody_rmcp_client::delete_oauth_tokens;
use ody_rmcp_client::perform_oauth_login;
use ody_utils_cli::CliConfigOverrides;
use ody_utils_cli::format_env_display;

/// Subcommands:
/// - `list`   — list configured servers (with `--json`)
/// - `get`    — show a single server (with `--json`)
/// - `add`    — add a server launcher entry to `~/.ody-code/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "ody mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn perform_oauth_login_retry_without_scopes(
    name: &str,
    url: &str,
    store_mode: ody_config::types::OAuthCredentialsStoreMode,
    keyring_backend_kind: ody_config::types::AuthKeyringBackendKind,
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let mut servers = load_global_mcp_servers(&ody_home)
        .await
        .with_context(|| format!("failed to load MCP servers from {}", ody_home.display()))?;
    ...
    ConfigEditsBuilder::new(&ody_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", ody_home.display()))?;
    ...
    match oauth_login_support(&transport).await {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `ody mcp login {name}` to login."
        ),
    }
...
async fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `ody mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config_overrides: &CliConfigOverrides, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: ody mcp remove {}", get_args.name);

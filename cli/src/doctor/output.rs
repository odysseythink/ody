...
fn write_footer(out: &mut String, options: HumanOutputOptions) {
    if options.show_details {
        ...
        let _ = writeln!(
            ...
            "{}",
            dim(
                "Run ody doctor without --summary for detailed diagnostics.",
                options
            )
        );
        let _ = writeln!(
...
fn header_suffix(report: &DoctorReport) -> String {
    let version = format!("v{}", report.ody_version);
    report
        .checks
        .iter()
...
fn update_note(check: &DoctorCheck, report: &DoctorReport) -> Option<DoctorNote> {
    ...
    let mut parenthetical = format!("current {}", report.ody_version);
...
mod tests {
    ...
    fn sample_report() -> DoctorReport {
        let checks = vec![
            ...
            .detail("terminal title project value: ody"),
            DoctorCheck::new(
                "state.paths",
                "state",
                CheckStatus::Ok,
...
Notes
   ...
   ✗ auth         token expired - Run `ody login`.
─────────────────────────────────────────────────────────────
...
Notes
   [!!] terminal     narrow terminal
   [XX] auth         token expired - Run `ody login`.
-------------------------------------------------------------
...
",
    ...
    fn render_human_report_promotes_notes_without_changing_statuses() {
        ...
        assert!(rendered.contains("5 ok · 1 idle · 4 notes · 1 warn · 0 fail degraded"));
    }
    ...
    fn detail_value_colors_inline_statuses_and_low_signal_values() {
        let rendered = detail_value(
            "npm: no · commit unknown · integrity ok · ~/code/ody/target/debug/ody · <redacted>",
        ...
        assert!(rendered.contains("\u{1b}[38;5;117m~/code/ody/target/debug/ody"));
    ...
    fn redact_detail_preserves_env_var_names() {
        assert_eq!(
            redact_detail("auth env vars present: OPENAI_API_KEY, ODY_API_KEY"),
            "auth env vars present: OPENAI_API_KEY, ODY_API_KEY"
        );
    }

...
pub(super) async fn background_server_check(config: &Config) -> DoctorCheck {
    ...
    let state_dir = config.ody_home.join(STATE_DIR_NAME);
    ...
    let socket_path = match ody_app_server::app_server_control_socket_path(&config.ody_home) {
    ...
    if status.check_status() == CheckStatus::Warning {
        check = check.remediation("Run ody app-server daemon version for more details.");
    }
    check
}
...
mod tests {
    ...
    async fn test_config(ody_home: PathBuf) -> Config {

...
const ODY_DMG_URL_ARM64: &str = "https://persistent.oaistatic.com/ody-app-prod/Ody.dmg";
const ODY_DMG_URL_X64: &str =
    "https://persistent.oaistatic.com/ody-app-prod/Ody-latest-x64.dmg";

pub async fn run_mac_app_open_or_install(
    workspace: PathBuf,
    download_url_override: Option<String>,
) -> anyhow::Result<()> {
    if let Some(app_path) = find_existing_ody_app_path() {
        eprintln!(
            "Opening Ody Desktop at {app_path}...",
            app_path = app_path.display()
        );
        open_ody_app(&app_path, &workspace).await?;
        return Ok(());
    }
    eprintln!("Ody Desktop not found; downloading installer...");
    ...
    let installed_app = download_and_install_ody_to_user_applications(&download_url)
        .await
        .context("failed to download/install Ody Desktop")?;
    eprintln!(
        "Launching Ody Desktop from {installed_app}...",
        installed_app = installed_app.display()
    );
    open_ody_app(&installed_app, &workspace).await?;
    Ok(())
}
...
fn find_existing_ody_app_path() -> Option<PathBuf> {
    candidate_ody_app_paths()
        .into_iter()
        .find(|candidate| candidate.is_dir())
}

fn candidate_ody_app_paths() -> Vec<PathBuf> {
    let mut paths = vec![PathBuf::from("/Applications/Ody.app")];
    if let Some(home) = std::env::var_os("HOME") {
        paths.push(PathBuf::from(home).join("Applications").join("Ody.app"));
    }
    paths
}

async fn open_ody_app(app_path: &Path, workspace: &Path) -> anyhow::Result<()> {
    eprintln!(
        "Opening workspace {workspace}...",
        workspace = workspace.display()
    );
    let url = ody_new_thread_url(workspace);
...
fn ody_new_thread_url(workspace: &Path) -> String {
    let workspace = workspace.as_os_str().to_string_lossy();
    let mut serializer = url::form_urlencoded::Serializer::new(String::new());
    serializer.append_pair("path", workspace.as_ref());
    let query = serializer.finish();
    format!("ody://threads/new?{query}")
}

async fn download_and_install_ody_to_user_applications(dmg_url: &str) -> anyhow::Result<PathBuf> {
    let temp_dir = Builder::new()
        .prefix("ody-app-installer-")
        .tempdir()
        .context("failed to create temp dir")?;
    let tmp_root = temp_dir.path().to_path_buf();
    let _temp_dir = temp_dir;

    let dmg_path = tmp_root.join("Ody.dmg");
    download_dmg(dmg_url, &dmg_path).await?;

    eprintln!("Mounting Ody Desktop installer...");
    ...
    let result = async {
        let app_in_volume = find_ody_app_in_mount(&mount_point)
            .context("failed to locate Ody.app in mounted dmg")?;
        install_ody_app_bundle(&app_in_volume).await
    }
    .await;
...
async fn install_ody_app_bundle(app_in_volume: &Path) -> anyhow::Result<PathBuf> {
    for applications_dir in candidate_applications_dirs()? {
        eprintln!(
            "Installing Ody Desktop into {applications_dir}...",
        ...
        let dest_app = applications_dir.join("Ody.app");
        if dest_app.is_dir() {
            return Ok(dest_app);
        }
        ...
        match copy_app_bundle(app_in_volume, &dest_app).await {
            ...
            Err(err) => {
                eprintln!(
                    "warning: failed to install Ody.app to {applications_dir}: {err}",
                    applications_dir = applications_dir.display()
                );
            }
        }
    ...
    anyhow::bail!("failed to install Ody.app to any applications directory");
}
...
fn find_ody_app_in_mount(mount_point: &Path) -> anyhow::Result<PathBuf> {
    let direct = mount_point.join("Ody.app");
    if direct.is_dir() {
        return Ok(direct);
    }
...
mod tests {
    ...
    fn parses_mount_point_from_tab_separated_hdiutil_output() {
        let output = "/dev/disk2s1\tApple_HFS\tOdy\t/Volumes/Ody\n";
        assert_eq!(
    ...
    fn parses_mount_point_with_spaces() {
        let output = "/dev/disk2s1\tApple_HFS\tOdy Installer\t/Volumes/Ody Installer\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/Ody Installer")
        );
    }

    #[test]
    fn ody_new_thread_url_encodes_workspace_path() {
        let url = url::Url::parse(&ody_new_thread_url(Path::new("/tmp/ody workspace/#1")))
        ...
        assert_eq!(
            ...
            (
                ...
                "/new".to_string(),
                vec![("path".to_string(), "/tmp/ody workspace/#1".to_string())],
            )
        );
    }
}

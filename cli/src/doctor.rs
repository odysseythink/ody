//! Implements the `ody doctor` diagnostic report.
//!
...
use ody_terminal_detection::TerminalInfo;
...
/// Options for building a local Ody diagnostic report.
///
...
async fn build_report(
            ...
            let (
                ...
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation("Fix the reported config error, then rerun ody doctor.")
                    })
                },
                async { run_sync_check("network", progress.clone(), network_check) },
                async {
    ...
    DoctorReport {
        ...
        overall_status,
        ody_version: env!("CARGO_PKG_VERSION").to_string(),
        checks,
    }
}
...
async fn load_config(
    ...
    ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load Ody config")
}
...
/// JSON support report emitted by `ody doctor --json`.
///
...
fn redacted_json_report(report: &DoctorReport) -> JsonDoctorReport {
    ...
    JsonDoctorReport {
        ...
        ody_version: report.ody_version.clone(),
        checks,
    }
}
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    details.push(format!(
        "managed by bun: {}",
        env::var_os("ODY_MANAGED_BY_BUN").is_some()
    ));
    push_env_path_detail(
        &mut details,
        "managed package root",
        "ODY_MANAGED_PACKAGE_ROOT",
    );

    let path_entries = ody_path_entries();
    let mut status = CheckStatus::Ok;
    let mut summary = "installation looks consistent".to_string();
    let mut remediation = None;

    if path_entries.len() > 1 {
        details.push(format!("PATH ody entries: {}", path_entries.len()));
    }
    if show_details || path_entries.len() > 1 {
        details.extend(
            path_entries
                .iter()
                .enumerate()
                .map(|(index, path)| format!("PATH ody #{}: {path}", index + 1)),
        );
    }
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::Mismatch {
                ...
                summary =
                    "npm install -g @odysseythink/ody would update a different install".to_string();
            ...
            NpmRootCheck::MissingPackageRoot => {
                ...
                remediation = Some(
                    "Reinstall or update Ody so the JS shim provides ODY_MANAGED_PACKAGE_ROOT."
                        .to_string(),
                );
            }
...
fn doctor_managed_by_npm(current_exe: Option<&Path>) -> bool {
    env::var_os("ODY_MANAGED_BY_NPM").is_some()
...
fn npm_global_root_check() -> NpmRootCheck {
    let Some(running_package_root) = env::var_os("ODY_MANAGED_PACKAGE_ROOT").map(PathBuf::from)
    else {
...
fn compare_npm_package_roots(running_package_root: &Path, npm_root: &Path) -> NpmRootCheck {
    let npm_package_root = npm_root.join("@odysseythink").join("ody");
...
fn ody_path_entries() -> Vec<String> {
    #[cfg(windows)]
    let result = run_command("where", ["ody"]);
    #[cfg(not(windows))]
    let result = run_command("which", ["-a", "ody"]);
...
fn config_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    details.push(format!("ODY_HOME: {}", config.ody_home.display()));
...
fn config_toml_details(config: &Config, details: &mut Vec<String>) {
    let config_path = config.ody_home.join(ody_config::CONFIG_TOML_FILE);
...
fn auth_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    let auth_path = config.ody_home.join("auth.json");
    details.push(format!(
        "auth storage mode: {:?}",
    ...
    match load_auth_dot_json(
        ...
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check =
                    check.remediation("Run ody login again or provide a supported auth env var.");
            }
            check
        }
        ...
        Ok(None) => DoctorCheck::new(
            ...
            "auth",
            CheckStatus::Fail,
            "no Ody credentials were found",
        )
        .details(details)
        .remediation("Run ody login or provide an API key through a supported auth env var."),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or run ody login again."),
    }
}
...
fn stored_auth_mode_value(auth: &AuthDotJson) -> ody_app_server_protocol::AuthMode {
    ...
    match stored_auth_mode_value(auth) {
        ody_app_server_protocol::AuthMode::ApiKey => {
            let stored_key_present = auth
                .odysseythink_api_key
                .as_deref()
                .is_some_and(|key| !key.trim().is_empty());
            let env_key_present =
                env_var_present(OPENAI_API_KEY_ENV_VAR) || env_var_present(ODY_API_KEY_ENV_VAR);
...
fn network_check() -> DoctorCheck {
    ...
    for name in ["ODY_CA_CERTIFICATE", "SSL_CERT_FILE"] {
...
async fn state_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "ODY_HOME", &config.ody_home);
    ...
    for db in ody_state::runtime_db_paths(&config.sqlite_home) {
    ...
    rollout_stats_details(&mut details, &config.ody_home);
...
async fn sqlite_integrity_detail(
    ...
    match ody_state::sqlite_integrity_check(path).await {
...
fn rollout_stats_details(details: &mut Vec<String>, ody_home: &Path) {
    let active = collect_rollout_stats(&ody_home.join("sessions"));
    let archived = collect_rollout_stats(&ody_home.join("archived_sessions"));
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match ody_home {
        Ok(path) => DoctorCheck::new(
            ...
            "state",
            CheckStatus::Ok,
            "ODY_HOME was resolved without config",
        )
        .detail(format!("ODY_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "ODY_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
    ...
    let endpoints = match mode {
        ProviderAuthReachabilityMode::ApiKey => vec![ReachabilityEndpoint {
            ...
            url: provider_base_url
                .unwrap_or("https://api.odysseythink.com/v1")
                .to_string(),
            required: true,
        ...
        match provider_route_probe_url(route_probe_url).await {
            ...
            RouteProbeOutcome::Fail(status) => {
                ...
                issues.push(
                    ...
                    .remedy("Set base_url to the provider API root, for example https://api.odysseythink.com/v1")
                    .field("route probe"),
                );
            }
...
mod tests {
    ...
    fn compare_npm_package_roots_detects_match() {
        let running = PathBuf::from("/prefix/lib/node_modules/@odysseythink/ody");
        let npm_root = PathBuf::from("/prefix/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Match {
                package_root: npm_root.join("@odysseythink").join("ody")
            }
        );
    }
    ...
    fn compare_npm_package_roots_detects_mismatch() {
        let running = PathBuf::from("/old/lib/node_modules/@odysseythink/ody");
        let npm_root = PathBuf::from("/new/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Mismatch {
                running_package_root: running,
                npm_package_root: npm_root.join("@odysseythink").join("ody"),
            }
        );
    }
    ...
    fn startup_warning_counts_group_known_sources() {
        let warnings = vec![
            ...
            "[features].ody_hooks is deprecated. Use [features].hooks instead.".to_string(),
    ...
    async fn mcp_check_ignores_disabled_servers() {
        let disabled_server: McpServerConfig = toml::from_str(
            r#"
                ...
                enabled = false
                required = true
                bearer_token_env_var = "ODY_DOCTOR_DISABLED_MCP_TOKEN"
            "#,
        )
        ...
        assert!(
            check
                .details
                .iter()
                .all(|detail| !detail.contains("ODY_DOCTOR_DISABLED_MCP_TOKEN"))
        );
        assert!(
            check
                .details
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running Ody.".to_string())
        );
    }
        ...
        assert_eq!(
            ...
            ReachabilityPlan {
                ...
                endpoints: vec![ReachabilityEndpoint {
                    ...
                    url: "https://example.odysseythink.azure.com/odysseythink/v1".to_string(),
                    required: true,
    ...
    #[test]
    fn provider_reachability_adds_models_route_probe_for_odysseythink_compatible_base_urls() {
    ...
    fn provider_reachability_api_key_uses_odysseythink_endpoint() {
        ...
        assert_eq!(
            plan.endpoints,
            vec![ReachabilityEndpoint {
                label: "odysseythink API".to_string(),
                url: "https://api.odysseythink.com/v1".to_string(),
                required: true,
    ...
    async fn mcp_check_fails_required_missing_stdio_command() {
        let required_server: McpServerConfig = toml::from_str(
            r#"
                command = "definitely-missing-ody-doctor-mcp"
                required = true
            "#,
        )
        .expect("should deserialize required MCP config");
        ...
        assert!(check.details.iter().any(|detail| {
            detail.contains(
                "required: stdio command \"definitely-missing-ody-doctor-mcp\" is not resolvable",
            )
        }));
    }
    ...
    async fn mcp_check_skips_host_path_checks_for_remote_stdio() {
        #[cfg(not(windows))]
        let cwd = r"C:\Users\odysseythink\share";
        #[cfg(windows)]
        let cwd = "/home/odysseythink/share";

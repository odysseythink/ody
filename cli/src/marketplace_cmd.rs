...
use ody_core_plugins::PluginMarketplaceUpgradeOutcome;
...
use ody_core_plugins::installed_marketplaces::resolve_configured_marketplace_root;
use ody_core_plugins::marketplace::marketplace_root_dir;
use ody_core_plugins::marketplace_add::MarketplaceAddOutcome;
use ody_core_plugins::marketplace_add::MarketplaceAddRequest;
use ody_core_plugins::marketplace_add::add_marketplace;
use ody_core_plugins::marketplace_remove::MarketplaceRemoveOutcome;
use ody_core_plugins::marketplace_remove::MarketplaceRemoveRequest;
...
#[derive(Debug, Parser)]
#[command(bin_name = "ody plugin marketplace")]
...
enum MarketplaceSubcommand {
    ...
    /// List plugin marketplaces Ody is currently considering and their roots.
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "ody plugin marketplace add",
    after_help = "Examples:\n  ody plugin marketplace add ./path/to/marketplace\n  ody plugin marketplace add owner/repo --ref main\n  ody plugin marketplace add https://github.com/owner/repo --sparse plugins/foo"
)]
...
#[derive(Debug, Parser)]
#[command(bin_name = "ody plugin marketplace list")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "ody plugin marketplace upgrade",
    after_help = "Examples:\n  ody plugin marketplace upgrade\n  ody plugin marketplace upgrade debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "ody plugin marketplace remove",
    after_help = "Example:\n  ody plugin marketplace remove debug"
)]
...
async fn run_list(overrides: Vec<(String, toml::Value)>, args: ListMarketplaceArgs) -> Result<()> {
    ...
    let manager = PluginsManager::new(config.ody_home.to_path_buf());
    ...
    if args.json {
        let marketplace_sources =
            configured_marketplace_sources_by_root(config.ody_home.as_path(), &plugins_input);
        let output =
...
impl JsonMarketplaceListOutput {
    fn from_marketplaces(
        marketplaces: Vec<ody_core_plugins::marketplace::Marketplace>,
...
fn configured_marketplace_sources_by_root(
    ...
    let default_install_root = marketplace_install_root(ody_home);
    marketplaces
        .iter()
...
async fn run_upgrade(
    ...
    let config = Config::load_with_cli_overrides(overrides)
        ...
        .context("failed to load configuration")?;
    let ody_home = find_ody_home().context("failed to resolve ODY_HOME")?;
    let manager = PluginsManager::new(ody_home.to_path_buf());
    let plugins_input = config.plugins_config_input();
    let outcome = manager

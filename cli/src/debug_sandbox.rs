...
#[cfg(target_os = "macos")]
use ody_core::spawn::ODY_SANDBOX_ENV_VAR;
...
use ody_sandboxing::landlock::allow_network_for_proxy;
use ody_sandboxing::landlock::create_linux_sandbox_command_args_for_permission_profile;
#[cfg(target_os = "macos")]
use ody_sandboxing::seatbelt::CreateSeatbeltCommandArgsParams;
#[cfg(target_os = "macos")]
use ody_sandboxing::seatbelt::create_seatbelt_command_args;
use ody_sandboxing::with_managed_mitm_ca_readable_root;
...
async fn run_command_under_sandbox(
    ...
    let network = network_proxy
        .as_ref()
        .map(ody_core::config::StartedNetworkProxy::proxy);
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Seatbelt => {
            ...
            spawn_debug_sandbox_child(
                ...
                env,
                |env_map| {
                    env_map.insert(ODY_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
...
async fn run_command_under_windows_session(
    ...
    use ody_windows_sandbox::WindowsSandboxSessionRequest;
    use ody_windows_sandbox::spawn_windows_sandbox_session_for_level;
    ...
    let spawned = spawn_windows_sandbox_session_for_level(WindowsSandboxSessionRequest {
        ...
        ody_home: config.ody_home.as_path(),
        command,
        cwd: cwd.as_path(),
        env_map: env,
    ...
    let exit_code = ody_windows_sandbox::forward_sandbox_session_stdio(spawned).await;
...
async fn spawn_debug_sandbox_child(
    ...
    if !network_sandbox_policy.is_enabled() {
        cmd.env(ODY_SANDBOX_NETWORK_DISABLED_ENV_VAR, "1");
    }
...
async fn load_debug_sandbox_config_with_ody_home(
    ...
    // For legacy configs, `ody sandbox` historically defaulted to read-only
...
async fn build_debug_sandbox_config_with_loader_overrides(
    ...
    if let Some(ody_home) = ody_home {
        builder = builder
            .ody_home(ody_home.clone())
            .fallback_cwd(Some(ody_home));
    }
    builder.build().await
}
...
mod tests {
    ...
    async fn debug_sandbox_honors_config_profile_loader_overrides() -> anyhow::Result<()> {
        ...
        let docs = sandbox_paths.path().join("docs");
        let private = docs.join("private");
        let profile_path = ody_home.path().join("work.config.toml");
    ...
    async fn debug_sandbox_honors_explicit_builtin_permission_profile() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_ody_home(
            ...
            Some(ody_home.path().to_path_buf()),
            /*strict_config*/ false,
        )
        .await?;

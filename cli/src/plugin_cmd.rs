...
use ody_core_plugins::ConfiguredMarketplace;
use ody_core_plugins::OPENAI_BUNDLED_MARKETPLACE_NAME;
use ody_core_plugins::PluginInstallOutcome;
...
use ody_core_plugins::marketplace::MarketplaceListError;
use ody_core_plugins::marketplace::MarketplacePluginAuthPolicy;
...
const OPENAI_BUNDLED_ALPHA_MARKETPLACE_NAME: &str = "odysseythink-bundled-alpha";
const OPENAI_PRIMARY_RUNTIME_MARKETPLACE_NAME: &str = "odysseythink-primary-runtime";

#[derive(Debug, Parser)]
#[command(bin_name = "ody plugin")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "ody plugin add",
    after_help = "Examples:\n  ody plugin add sample@debug\n  ody plugin add sample --marketplace debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "ody plugin list",
    after_help = "Examples:\n  ody plugin list\n  ody plugin list --marketplace debug\n  ody plugin list --json\n  ody plugin list --available --json"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "ody plugin remove",
    after_help = "Examples:\n  ody plugin remove sample@debug\n  ody plugin remove sample --marketplace debug"
)]
        ...
        for (index, marketplace) in marketplaces.into_iter().enumerate() {
            ...
            for plugin in &marketplace.plugins {
                ...
                let path = match &plugin.source {
                    ody_core_plugins::marketplace::MarketplacePluginSource::Local { path } => {
                        path.as_path().display().to_string()
                    }
                    ody_core_plugins::marketplace::MarketplacePluginSource::Git {
                        url,
                        path,
                        ref_name,
                        sha,
...
impl JsonPluginListOutput {
    fn from_marketplaces(
        marketplaces: Vec<ody_core_plugins::ConfiguredMarketplace>,
...
pub(crate) fn configured_marketplace_snapshot_issues(
    ...
    for (configured_name, marketplace) in configured_marketplaces {
        ...
        match find_marketplace_manifest_path(&root) {
            ...
            None => {
                if is_implicit_system_marketplace_root(configured_name, ody_home, &root) {
                    continue;
                }
                issues.push(ConfiguredMarketplaceSnapshotIssue {
                    marketplace_name: configured_name.clone(),

...
#[path = "../src/proto/ody.exec_server.relay.v1.rs"]
mod relay_proto;
...
use ody_exec_server::ExecParams;
use ody_exec_server::ExecResponse;
use ody_exec_server::ExecServerClient;
use ody_exec_server::ExecServerRuntimePaths;
use ody_exec_server::FsReadFileParams;
use ody_exec_server::NoiseChannelIdentity;
use ody_exec_server::NoiseChannelPublicKey;
use ody_exec_server::NoiseRendezvousConnectArgs;
use ody_exec_server::NoiseRendezvousConnectBundle;
use ody_exec_server::ProcessId;
use ody_exec_server::RemoteEnvironmentConfig;
...
fn static_registry_auth_provider() -> ody_api::SharedAuthProvider {
...
async fn remote_environment_routes_encrypted_exec_server_rpc() -> Result<()> {
    ...
    let (ody_exe, ody_linux_sandbox_exe) = common::current_test_binary_helper_paths()?;
    let runtime_paths = ExecServerRuntimePaths::new(ody_exe, ody_linux_sandbox_exe)?;
    let config = RemoteEnvironmentConfig::new(
        registry.uri(),
        ENVIRONMENT_ID.to_string(),
        static_registry_auth_provider(),
    )?;
    let remote_environment = tokio::spawn(ody_exec_server::run_remote_environment(
        config,
        runtime_paths,
    ));

...
use ody_exec_server::FsCloseParams;
use ody_exec_server::FsOpenParams;
use ody_exec_server::FsReadBlockParams;
use ody_exec_server::FsReadBlockResponse;
use ody_exec_server::RemoteExecServerConnectArgs;
...
async fn file_reads_reject_named_pipes() -> Result<()> {
    ...
    let read_path = format!(r"\\.\pipe\ody-fs-read-{}", Uuid::new_v4());
    ...
    let stream_path = format!(r"\\.\pipe\ody-fs-stream-{}", Uuid::new_v4());

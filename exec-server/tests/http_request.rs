...
use ody_exec_server::HttpRequestBodyDeltaNotification;
...
async fn exec_server_http_request_buffers_response_body() -> anyhow::Result<()> {
    ...
    assert_eq!(
        (
            ...
            captured.headers.get("x-ody-test").map(String::as_str),
...
async fn wait_for_error_response(
    server: &mut ExecServerHarness,
    request_id: RequestId,
) -> anyhow::Result<ody_app_server_protocol::JSONRPCErrorError> {
    let response = server
        .wait_for_event(|event| {
            matches!(
                event,

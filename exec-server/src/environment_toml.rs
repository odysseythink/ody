...
pub(crate) fn environment_provider_from_ody_home(
    ody_home: &Path,
) -> Result<Box<dyn EnvironmentProvider>, ExecServerError> {
    let path = ody_home.join(ENVIRONMENTS_TOML_FILE);
...
mod tests {
        ...
        let provider = TomlEnvironmentProvider::new(EnvironmentsToml {
            ...
            environments: vec![
                ...
                EnvironmentToml {
                    ...
                    args: Some(vec![
                        "dev".to_string(),
                        "ody exec-server --listen stdio".to_string(),
                    ]),
    ...
    fn load_environments_toml_reads_root_environment_list() {
        ...
        let path = ody_home.path().join(ENVIRONMENTS_TOML_FILE);
        std::fs::write(
            &path,
            r#"
default = "ssh-dev"
...
id = "ssh-dev"
program = "ssh"
args = ["dev", "ody exec-server --listen stdio"]
cwd = "/tmp"
[environments.env]
ODY_LOG = "debug"
"#,
...
"#,
        ...
        for (index, (contents, expected)) in cases.into_iter().enumerate() {
            let path = ody_home.path().join(format!("environments-{index}.toml"));
    ...
    #[tokio::test]
    async fn environment_provider_from_ody_home_uses_present_environments_file() {
        let ody_home = tempdir().expect("tempdir");
        std::fs::write(
            ody_home.path().join(ENVIRONMENTS_TOML_FILE),
            r#"
default = "none"
include_local = false
"#,
        ...
        let provider =
            environment_provider_from_ody_home(ody_home.path()).expect("environment provider");
    ...
    #[tokio::test]
    async fn environment_provider_from_ody_home_falls_back_when_file_is_missing() {
        let ody_home = tempdir().expect("tempdir");

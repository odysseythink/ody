...
use ody_sandboxing::SandboxDirectSpawnTransformRequest;
use ody_sandboxing::SandboxExecRequest;
...
impl FileSystemSandboxRunner {
    ...
    fn sandbox_exec_request(
        ...
        let helper = &self.runtime_paths.ody_self_exe;
        ...
        let command = SandboxCommand {
            ...
            args: vec![ODY_FS_HELPER_ARG1.to_string()],
            cwd: cwd.uri.clone(),
        ...
        sandbox_manager
            .transform_for_direct_spawn(SandboxDirectSpawnTransformRequest {
                ...
                transform: SandboxTransformRequest {
                    ...
                    linux_sandbox_exe: self.runtime_paths.ody_linux_sandbox_exe.as_deref(),
...
mod tests {
    ...
    fn helper_env_preserves_path_for_system_bwrap_discovery_without_leaking_secrets() {
        ...
        assert_eq!(
            ...
            HashMap::from([
                ...
                ("TMPDIR".to_string(), "/tmp/ody".to_string()),
    ...
    fn sandbox_cwd_rejects_cwd_dependent_profile_without_context_cwd() {
        ...
        let sandbox_context = ody_file_system::FileSystemSandboxContext::from_permission_profile(
    ...
    fn helper_permissions_include_linux_sandbox_alias_parent() {
        ...
        let cwd = AbsolutePathBuf::from_absolute_path(std::env::temp_dir().as_path())
            .expect("absolute cwd");
        let mut policy = restricted_policy(Vec::new());
        let ody_parent = AbsolutePathBuf::from_absolute_path(root.path().join("bin"))
            .expect("absolute ody parent");
        ...
        assert!(policy.can_read_path_with_cwd(ody_parent.as_path(), cwd.as_path()));
    ...
    fn sandbox_context_with_cwd(
        ...
        ody_file_system::FileSystemSandboxContext::from_permission_profile_with_cwd(
            PermissionProfile::from_runtime_permissions(policy, NetworkSandboxPolicy::Restricted),
            cwd,
        )
    }

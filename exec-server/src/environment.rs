...
use crate::environment_toml::environment_provider_from_ody_home;
...
use ody_shell_command::shell_detect::DetectedShell;
...
/// default constructor preserves the legacy `ODY_EXEC_SERVER_URL` behavior
/// while configured construction accepts a provider-supplied snapshot.
///
/// Setting `ODY_EXEC_SERVER_URL=none` disables environment access by leaving
...
impl EnvironmentManager {
    ...
    /// Builds a manager from `ODY_HOME` and local runtime paths used when
    /// creating local filesystem helpers.
    ///
    /// If `ODY_HOME/environments.toml` is present, it defines the configured
    /// environments. Otherwise this preserves the legacy
    /// `ODY_EXEC_SERVER_URL` behavior.
    pub async fn from_ody_home(
        ody_home: impl AsRef<std::path::Path>,
        local_runtime_paths: Option<ExecServerRuntimePaths>,
        ...
        let provider = environment_provider_from_ody_home(ody_home.as_ref())?;
    ...
    /// reading user config files from `ODY_HOME`.
        ...
        match (registry_url, environment_id, auth_token) {
            ...
            _ => {
                return Err(ExecServerError::EnvironmentRegistryConfig(format!(
                    "Noise environment requires {ODY_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR}, \
{ODY_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR}, and \
{ODY_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR}"
                )));
            }
        };
...
impl Environment {
    /// Builds an environment from the raw `ODY_EXEC_SERVER_URL` value.
    pub fn create(
        exec_server_url: Option<String>,
        local_runtime_paths: ExecServerRuntimePaths,
    ) -> Result<Self, ExecServerError> {
    ...
    /// Builds an environment from the raw `ODY_EXEC_SERVER_URL` value and
...
impl EnvironmentInfo {
    pub(crate) fn local() -> Self {
        Self {
            shell: ody_shell_command::shell_detect::default_user_shell().into(),
        }
    }
}
...
mod tests {
        ...
        let environment = Environment::remote_with_transport(
            ExecServerTransportParams::StdioCommand {
                command: StdioExecServerCommand {
                    program: "ody-missing-exec-server-for-test".to_string(),
                    args: Vec::new(),
                    env: HashMap::new(),
                    cwd: None,
                },
    ...
    async fn local_environment_passes_runtime_paths_to_exec_backend() {
        ...
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile_with_cwd(
            ody_protocol::models::PermissionProfile::workspace_write(),
            sandbox_cwd.clone(),
        );
    ...
    async fn test_environment_rejects_sandboxed_filesystem_without_runtime_paths() {
        let environment = Environment::default_for_tests();
        let path = ody_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
            std::env::current_exe().expect("current exe").as_path(),
        )
        .expect("absolute current exe");
        let path = ody_utils_path_uri::PathUri::from_abs_path(&path);
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile(
            ody_protocol::models::PermissionProfile::from_runtime_permissions(
                &ody_protocol::permissions::FileSystemSandboxPolicy::restricted(Vec::new()),

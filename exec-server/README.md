...
`ody-exec-server` is the library backing `ody exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`ody-utils-pty`.

It provides:

- a CLI entrypoint: `ody exec-server`
...
The server speaks the shared `ody-app-server-protocol` message envelope on
the wire.
...
must support it.
It uses the standard Ody ChatGPT sign-in state; run `ody login` first when
remote registration needs authentication. Containerized callers that receive an
Agent Identity JWT in `ODY_ACCESS_TOKEN` can opt into that auth path with
`--use-agent-identity-auth`; Ody then registers an Agent task and sends the
derived AgentAssertion headers on the registry request.

Alternatively, API users can instead use `ODY_API_KEY`;
Ody sends it as a bearer token on the registration request. For example:

```sh
ODY_API_KEY="$OPENAI_API_KEY" \
ody exec-server \
  --remote ... \
...
`ody.exec_server.relay.v1.RelayMessageFrame`; the checked-in schema is in
`src/proto/ody.exec_server.relay.v1.proto`. The relay frame carries stream
...
- `arg0`: optional argv0 override forwarded to `ody-utils-pty`.
...
hidden helper process launched from the top-level `ody` executable and
...
`ody exec-server` command builds these paths from the `ody` arg0 dispatch
state. `RemoteEnvironmentConfig::new(...)` also takes the auth provider that
remote registration should use; the CLI builds that provider from Ody auth

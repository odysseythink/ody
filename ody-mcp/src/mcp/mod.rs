...
use ody_protocol::mcp::ResourceTemplate;
...
/// MCP runtime settings derived from `ody_core::config::Config`.
///
/// This struct should contain only long-lived configuration values that the
/// `ody-mcp` crate needs to construct server transports, enforce MCP

...
fn test_mcp_config(ody_home: PathBuf) -> McpConfig {
    McpConfig {
...
fn tool_plugin_provenance_collects_app_and_mcp_sources() {
    ...
    catalog.register(McpServerRegistration::from_plugin(
        ...
        /*plugin_order*/ 0,
        ody_apps_mcp_server_config("https://alpha.example", /*apps_mcp_product_sku*/ None),
    ));
...
fn selected_mcp_attribution_does_not_join_an_unrelated_local_summary() {
    ...
    catalog.register(McpServerRegistration::from_selected_plugin(
        ...
        /*selection_order*/ 0,
        ody_apps_mcp_server_config("https://github.example", /*apps_mcp_product_sku*/ None),
    ));
    config.mcp_server_catalog = catalog.build();
...
#[test]
fn ody_apps_mcp_url_for_base_url_keeps_existing_paths() {
    assert_eq!(
        ody_apps_mcp_url_for_base_url("https://chatgpt.com/backend-api"),
        "https://chatgpt.com/backend-api/wham/apps"
    );
    assert_eq!(
        ody_apps_mcp_url_for_base_url("https://chat.odysseythink.com"),
        "https://chat.odysseythink.com/backend-api/wham/apps"
    );
    assert_eq!(
        ody_apps_mcp_url_for_base_url("http://localhost:8080/api/ody"),
        "http://localhost:8080/api/ody/apps"
    );
    assert_eq!(
        ody_apps_mcp_url_for_base_url("http://localhost:8080"),
        "http://localhost:8080/api/ody/apps"
    );
}

#[test]
fn ody_apps_server_config_uses_legacy_ody_apps_path() {
    let config =
        ody_apps_mcp_server_config("https://chatgpt.com", /*apps_mcp_product_sku*/ None);
    let url = match &config.transport {
        McpServerTransportConfig::StreamableHttp { url, .. } => url,
        _ => panic!("expected streamable http transport for ody apps"),
    };
...
#[test]
fn ody_apps_server_config_forwards_configured_product_sku_header() {
    let config = ody_apps_mcp_server_config("https://chatgpt.com", Some("tpp"));
...
async fn effective_mcp_servers_preserve_runtime_servers() {
    ...
    let ody_apps = effective
        .get(ODY_APPS_MCP_SERVER_NAME)
        .expect("ody apps server should exist");
    ...
    let docs = docs
        .configured_config()
        .expect("configured server should retain transport");
    let ody_apps = ody_apps
        .configured_config()
        .expect("ody apps should use configured transport");

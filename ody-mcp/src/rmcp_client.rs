...
use crate::ody_apps::normalize_ody_apps_callable_name;
...
use ody_exec_server::ReqwestHttpClient;
use ody_protocol::mcp::McpServerInfo;
use ody_protocol::protocol::Event;
use ody_rmcp_client::ExecutorStdioServerLauncher;
...
/// MCP server capability indicating that Ody should include [`SandboxState`]
/// in tool-call request `_meta` under this key.
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "ody/sandbox-state-meta";
pub const OPENAI_FORM_CAPABILITY: &str = "odysseythink/form";

pub(crate) const MCP_TOOLS_LIST_DURATION_METRIC: &str = "ody.mcp.tools.list.duration_ms";
pub(crate) const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "ody.mcp.tools.fetch_uncached.duration_ms";
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) ody_apps_tools_cache_context: Option<OdyAppsToolsCacheContext>,
}
...
impl ManagedClient {
    fn listed_tools(&self) -> Vec<ToolInfo> {
        ...
        if self.ody_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        let tool_filter = server
            ...
            .unwrap_or_default();
        let cached_tool_info_snapshot = load_startup_cached_ody_apps_tools_snapshot(
            &server_name,
            ody_apps_tools_cache_context.as_ref(),
        );
...
pub(crate) async fn list_tools_for_client_uncached(
    ...
    let tools = resp
        ...
        .map(|tool| {
            ...
            let callable_name = normalize_ody_apps_callable_name(
                server_name,
                &tool_def.name,
            ...
            let callable_namespace =
                normalize_ody_apps_callable_namespace(server_name, connector_name.as_deref());
            if let Some(title) = tool_def.title.as_deref() {
                let normalized_title =
                    normalize_ody_apps_tool_title(server_name, connector_name.as_deref(), title);
...
fn mcp_initialize_request_params(
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("ody-mcp-client", env!("CARGO_PKG_VERSION")).with_title("Ody"),
    )
...
async fn make_rmcp_client(
    ...
    match transport {
        McpServerTransportConfig::Stdio {
            ...
            let cwd = cwd.map(ody_utils_path_uri::LegacyAppPathString::into_string);
...
mod tests {
    ...
    fn custom_mcp_connector_metadata_is_stripped() {
        ...
        assert!(meta.0.contains_key("odysseythink/fileParams"));
        assert_eq!(
    ...
    #[test]
    fn ody_apps_connector_metadata_is_preserved() {
        let mut tool = tool_with_connector_meta();

...
//! Session orchestration stays in `ody-core`.

use ody_protocol::mcp::CallToolResult;
use serde::Serialize;

pub const MCP_TOOL_ODY_APPS_META_KEY: &str = "_ody_apps";
...
pub struct OdyAppsConnectorAuthFailure {
...
pub struct OdyAppsAuthElicitation {
...
#[derive(Debug, Clone, PartialEq)]
pub struct OdyAppsAuthElicitationPlan {
    pub auth_failure: OdyAppsConnectorAuthFailure,
    pub elicitation: OdyAppsAuthElicitation,
}

#[derive(Serialize)]
struct OdyAppsConnectorAuthFailureMeta<'a> {
...
pub fn connector_auth_failure_from_tool_result(
...
) -> Option<OdyAppsConnectorAuthFailure> {
    ...
    let auth_failure = result
        ...
        .as_ref()?
        .as_object()?
        .get(MCP_TOOL_ODY_APPS_META_KEY)?
        .as_object()?
    ...
    Some(OdyAppsConnectorAuthFailure {
...
pub fn build_auth_elicitation_plan(
    ...
    connector_name: Option<&str>,
    install_url: Option<String>,
) -> Option<OdyAppsAuthElicitationPlan> {
    let auth_failure =
    ...
    Some(OdyAppsAuthElicitationPlan {
        auth_failure,
        elicitation,
    })
}
...
pub fn build_auth_elicitation(
    call_id: &str,
    auth_failure: &OdyAppsConnectorAuthFailure,
) -> OdyAppsAuthElicitation {
    OdyAppsAuthElicitation {
        meta: serde_json::json!({
            MCP_TOOL_ODY_APPS_META_KEY: {
                CONNECTOR_AUTH_FAILURE_META_KEY: OdyAppsConnectorAuthFailureMeta {
...
fn auth_elicitation_message(auth_failure: &OdyAppsConnectorAuthFailure) -> String {
    match auth_failure.auth_reason.as_deref() {
        Some("oauth_upgrade_required") => format!(
            "Reconnect {} to grant the permissions needed for this request.",
            auth_failure.connector_name
        ),
        Some("reauthentication_required") => format!(
            "Reconnect {} to restore access for this request.",
            auth_failure.connector_name
        ),
...
mod tests {
    ...
    fn parses_auth_failure_from_trusted_connector_metadata() {
        assert_eq!(
            connector_auth_failure_from_tool_result(
                ...
                Some("https://example.com/apps/google-calendar/connector_calendar".to_string()),
            ),
            Some(OdyAppsConnectorAuthFailure {
                connector_id: "connector_calendar".to_string(),
                connector_name: "Google Calendar".to_string(),
                install_url: "https://example.com/apps/google-calendar/connector_calendar"
                    .to_string(),
    ...
    fn rejects_missing_or_mismatched_connector_ids() {
        ...
        assert_eq!(
            connector_auth_failure_from_tool_result(
                ...
                Some("Google Drive"),
                Some("https://example.com/apps/google-drive/connector_drive".to_string()),
            ),
            None
        );
    }
    ...
    fn builds_url_elicitation_payload() {
        ...
        assert_eq!(
            ...
            OdyAppsAuthElicitation {
                meta: serde_json::json!({
                    MCP_TOOL_ODY_APPS_META_KEY: {
                        CONNECTOR_AUTH_FAILURE_META_KEY: {
                            ...
                            "install_url":
                                "https://example.com/apps/google-calendar/connector_calendar",
                            CONNECTOR_AUTH_FAILURE_AUTH_REASON_KEY: "reauthentication_required",
                            CONNECTOR_AUTH_FAILURE_LINK_ID_KEY: "link_123",
                            CONNECTOR_AUTH_FAILURE_ERROR_CODE_KEY: "UNAUTHORIZED",
                            CONNECTOR_AUTH_FAILURE_ERROR_HTTP_STATUS_CODE_KEY: 401,
                ...
                message: "Reconnect Google Calendar to restore access for this request."
                    .to_string(),
                url: "https://example.com/apps/google-calendar/connector_calendar".to_string(),
                elicitation_id: "ody_apps_auth_call_123".to_string(),
            }
        );
    }
    ...
    fn builds_auth_elicitation_plan() {
        ...
        assert_eq!(plan.elicitation.elicitation_id, "ody_apps_auth_call_123");
    }
}

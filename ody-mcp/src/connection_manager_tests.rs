...
use crate::ody_apps::load_startup_cached_ody_apps_tools_snapshot;
...
#[test]
fn declared_odysseythink_file_fields_treat_names_literally() {
    let meta = serde_json::json!({
        "odysseythink/fileParams": ["file", "input_file", "attachments"]
    });
...
fn tool_with_model_visible_input_schema_masks_file_params() {
    let mut tool = create_test_tool(ODY_APPS_MCP_SERVER_NAME, "upload").tool;
    ...
    tool.meta = Some(Meta(
        serde_json::json!({
            "odysseythink/fileParams": ["file", "files"]
        })
        .as_object()
        .expect("object")
        .clone(),
...
#[test]
fn ody_apps_tools_cache_is_overwritten_by_last_write() {
    let ody_home = tempdir().expect("tempdir");
    let cache_context = create_ody_apps_tools_cache_context(
        ody_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let tools_gateway_1 = vec![create_test_tool(ODY_APPS_MCP_SERVER_NAME, "one")];
    let tools_gateway_2 = vec![create_test_tool(ODY_APPS_MCP_SERVER_NAME, "two")];

    write_cached_ody_apps_tools(&cache_context, &tools_gateway_1);
    let cached_gateway_1 =
        read_cached_ody_apps_tools(&cache_context).expect("cache entry exists for first write");
    assert_eq!(cached_gateway_1[0].callable_name, "one");

    write_cached_ody_apps_tools(&cache_context, &tools_gateway_2);
    let cached_gateway_2 =
        read_cached_ody_apps_tools(&cache_context).expect("cache entry exists for second write");
    assert_eq!(cached_gateway_2[0].callable_name, "two");
}

#[test]
fn ody_apps_tools_cache_is_scoped_per_user() {
    let ody_home = tempdir().expect("tempdir");
    let cache_context_user_1 = create_ody_apps_tools_cache_context(
        ody_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let cache_context_user_2 = create_ody_apps_tools_cache_context(
        ody_home.path().to_path_buf(),
        Some("account-two"),
        Some("user-two"),
    );
    let tools_user_1 = vec![create_test_tool(ODY_APPS_MCP_SERVER_NAME, "one")];
    let tools_user_2 = vec![create_test_tool(ODY_APPS_MCP_SERVER_NAME, "two")];

    write_cached_ody_apps_tools(&cache_context_user_1, &tools_user_1);
    write_cached_ody_apps_tools(&cache_context_user_2, &tools_user_2);

    let read_user_1 =
        read_cached_ody_apps_tools(&cache_context_user_1).expect("cache entry for user one");
    let read_user_2 =
        read_cached_ody_apps_tools(&cache_context_user_2).expect("cache entry for user two");
...
#[test]
fn ody_apps_tools_cache_preserves_formerly_disallowed_connectors() {
    let ody_home = tempdir().expect("tempdir");
    let cache_context = create_ody_apps_tools_cache_context(
        ody_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let cached = read_cached_ody_apps_tools(&cache_context).expect("cache entry exists for user");
...
#[test]
fn ody_apps_tools_cache_is_ignored_when_schema_version_mismatches() {
    let ody_home = tempdir().expect("tempdir");
    let cache_context = create_ody_apps_tools_cache_context(
        ody_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        "schema_version": ODY_APPS_TOOLS_CACHE_SCHEMA_VERSION + 1,
        "tools": [create_test_tool(ODY_APPS_MCP_SERVER_NAME, "one")],
    }))
    .expect("serialize");
    std::fs::write(cache_path, bytes).expect("write");

    assert!(read_cached_ody_apps_tools(&cache_context).is_none());
}

#[test]
fn ody_apps_tools_cache_is_ignored_when_json_is_invalid() {
    let ody_home = tempdir().expect("tempdir");
    let cache_context = create_ody_apps_tools_cache_context(
        ody_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn startup_cached_ody_apps_tools_loads_from_disk_cache() {
    let ody_home = tempdir().expect("tempdir");
    let cache_context = create_ody_apps_tools_cache_context(
        ody_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let server_info = create_test_server_info("Ody Apps");
    write_cached_ody_apps_tools_if_needed(
        ODY_APPS_MCP_SERVER_NAME,
        Some(&cache_context),
        &server_info,
        &cached_tools,
...
#[test]
fn startup_cached_ody_apps_tools_loads_without_server_info_cache() {
    let ody_home = tempdir().expect("tempdir");
    let cache_context = create_ody_apps_tools_cache_context(
        ody_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        ...
        "tools": [create_test_tool(ODY_APPS_MCP_SERVER_NAME, "calendar_search")],
    }))
    .expect("serialize");
    std::fs::write(cache_path, bytes).expect("write");
...
#[test]
fn ody_apps_server_info_cache_survives_legacy_tools_cache_write() {
    let ody_home = tempdir().expect("tempdir");
    let cache_context = create_ody_apps_tools_cache_context(
        ody_home.path().to_path_buf(),
        Some("account-one"),
    ...
    assert!(
        load_startup_cached_ody_apps_tools_snapshot(
            ODY_APPS_MCP_SERVER_NAME,
            Some(&cache_context),
        )
        .is_none()
...
async fn list_all_tools_uses_cached_tool_info_snapshot_while_client_is_pending() {
    ...
    let tool = tools
        ...
        .find(|tool| {
            tool.canonical_tool_name()
                == ToolName::namespaced("mcp__ody_apps", "calendar_create_event")
        })
        .expect("tool from startup cache");
    assert_eq!(tool.server_name, ODY_APPS_MCP_SERVER_NAME);
...
async fn list_available_server_infos_uses_cache_while_client_is_pending() {
    ...
    assert_eq!(
        server_infos.get(ODY_APPS_MCP_SERVER_NAME),
        Some(&server_info)
    );
}

//! Aggregates MCP server connections for Ody.
//!
...
use ody_protocol::protocol::McpStartupCompleteEvent;
use ody_protocol::protocol::McpStartupFailure;
use ody_protocol::protocol::McpStartupStatus;
use ody_protocol::protocol::McpStartupUpdateEvent;
...
impl McpConnectionManager {
    ...
    pub async fn new(
        ...
        ody_home: PathBuf,
        ody_apps_tools_cache_key: OdyAppsToolsCacheKey,
        host_owned_ody_apps_enabled: bool,
        prefix_mcp_tool_names: bool,
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            .await;
            let ody_apps_tools_cache_context = if server_name == ODY_APPS_MCP_SERVER_NAME {
                Some(OdyAppsToolsCacheContext {
                    ody_home: ody_home.clone(),
                    user_key: ody_apps_tools_cache_key.clone(),
                })
            } else {
                None
            };
            ...
            let runtime_auth_provider =
                if server_name == ODY_APPS_MCP_SERVER_NAME && !uses_env_bearer_token {
                    ody_apps_auth_provider.clone()
                } else {
                    None
                };
    ...
    pub fn is_host_owned_ody_apps_server(&self, server_name: &str) -> bool {
        self.host_owned_ody_apps_enabled && server_name == ODY_APPS_MCP_SERVER_NAME
    }
    ...
    /// Force-refresh ody apps tools by bypassing the in-process cache.
    ///
    ...
    pub async fn hard_refresh_ody_apps_tools_cache(&self) -> Result<Vec<ToolInfo>> {
        let managed_client = self
            .clients
            .get(ODY_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{ODY_APPS_MCP_SERVER_NAME}'"))?
            .client()
            .await
        ...
        .await
        .with_context(|| {
            format!("failed to refresh tools for MCP server '{ODY_APPS_MCP_SERVER_NAME}'")
        })?;
        emit_duration(
        ...
        write_cached_ody_apps_tools_if_needed(
            ODY_APPS_MCP_SERVER_NAME,
            managed_client.ody_apps_tools_cache_context.as_ref(),

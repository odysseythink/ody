...
const PACKAGE_METADATA_FILENAME: &str = "ody-package.json";
const PATH_DIRNAME: &str = "ody-path";
const RELEASES_DIRNAME: &str = "releases";
const RESOURCES_DIRNAME: &str = "ody-resources";
...
pub struct InstallContext {
    ...
    pub package_layout: Option<OdyPackageLayout>,
}
...
pub enum InstallMethod {
    Standalone {
        ...
        /// such as
        /// `~/.ody-code/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
        /// Package-layout installs use the package root that contains `bin/`,
        /// `ody-resources/`, and `ody-path/`.
    ...
    /// A Ody binary launched through the npm-managed `ody.js` shim.
    Npm,
    /// A Ody binary launched through the bun-managed `ody.js` shim.
    Bun,
    /// A Ody binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled Ody binaries, custom
    /// internal launchers, and tests that execute Ody from an arbitrary path.
    Other,
}
...
impl InstallContext {
    ...
    fn from_exe_with_ody_home(
        is_macos: bool,
        current_exe: Option<&Path>,
        managed_by_npm: bool,
        managed_by_bun: bool,
        ody_home: Option<&Path>,
    ) -> Self {
        let package_layout = current_exe.and_then(OdyPackageLayout::from_exe);
        let method = if managed_by_npm {
            InstallMethod::Npm
        } else if managed_by_bun {
            InstallMethod::Bun
        } else if let Some(exe_path) = current_exe {
            install_method_from_exe(exe_path, ody_home, package_layout.as_ref(), is_macos)
        } else {
            InstallMethod::Other
        };
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let managed_by_npm = std::env::var_os("ODY_MANAGED_BY_NPM").is_some();
            let managed_by_bun = std::env::var_os("ODY_MANAGED_BY_BUN").is_some();
            Self::from_exe(
                cfg!(target_os = "macos"),
...
fn standalone_install_method(
    ...
    let canonical_ody_home = canonical_absolute_path(ody_home?)?;
    ...
    let releases_root = canonical_ody_home
        .join("packages")
...
mod tests {
    ...
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        ...
        let context = InstallContext::from_exe_with_ody_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(&exe_path),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ false,
            /*ody_home*/ Some(ody_home.path()),
        );
        assert_eq!(
            context,
            InstallContext {
    ...
    fn detects_package_layout_independently_from_install_method() -> std::io::Result<()> {
        ...
        let exe_path = bin_dir.join(if cfg!(windows) { "ody.exe" } else { "ody" });
        fs::write(&exe_path, "")?;
        fs::write(resources_dir.join(TEST_RESOURCE_NAME), "")?;
        ...
        let package_layout = OdyPackageLayout {
    ...
    fn standalone_package_layout_keeps_standalone_install_method() -> std::io::Result<()> {
        ...
        assert_eq!(
            ...
            InstallContext {
                ...
                package_layout: Some(OdyPackageLayout {
                    package_dir: canonical_package_dir,
                    bin_dir: canonical_bin_dir,
                    resources_dir: Some(canonical_resources_dir.clone()),
                    path_dir: Some(canonical_path_dir.clone()),
    ...
    #[test]
    fn standalone_package_rg_falls_back_when_ody_path_is_missing() -> std::io::Result<()> {
        let package_dir = tempfile::tempdir()?;
        let bin_dir = package_dir.path().join(BIN_DIRNAME);
        fs::create_dir_all(&bin_dir)?;
        fs::write(package_dir.path().join(PACKAGE_METADATA_FILENAME), "{}")?;
    ...
    fn npm_and_bun_take_precedence() {
        let npm_context = InstallContext::from_exe_with_ody_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/ody")),
            /*managed_by_npm*/ true,
            /*managed_by_bun*/ false,
            /*ody_home*/ None,
        );
        ...
        let bun_context = InstallContext::from_exe_with_ody_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/ody")),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ true,
    ...
    fn brew_is_detected_on_macos_prefixes() {
        let context = InstallContext::from_exe_with_ody_home(
            /*is_macos*/ true,
            /*current_exe*/ Some(Path::new("/opt/homebrew/bin/ody")),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ false,
            /*ody_home*/ None,
        );

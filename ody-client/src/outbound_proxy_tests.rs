...
fn no_proxy_matches_exact_suffix_wildcard_and_port() {
    let origin = RequestOrigin {
        ...
        host: "auth.odysseythink.com".to_string(),
        port: 443,
    };
    assert!(no_proxy_matches_origin("auth.odysseythink.com", &origin));
...
fn system_proxy_cache_key_preserves_url_specific_pac_decisions() {
    let request_url = "https://auth.odysseythink.com/oauth/token?access_token=secret";
    let cache_key = system_proxy_cache_key(request_url);

    assert_ne!(
        cache_key,
        system_proxy_cache_key("https://auth.odysseythink.com/oauth/revoke")
    );

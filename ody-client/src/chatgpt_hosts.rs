/// Returns whether `host` is one of the ChatGPT hosts Ody is allowed to treat
/// as first-party ChatGPT traffic.
pub fn is_allowed_chatgpt_host(host: &str) -> bool {
    const EXACT_HOSTS: &[&str] = &["chatgpt.com", "chat.odysseythink.com", "chatgpt-staging.com"];
...
mod tests {
    ...
    fn recognizes_chatgpt_hosts_without_suffix_tricks() {
        ...
        for host in [
            ...
            "api.odysseythink.com",
            "foo.chat.odysseythink.com",
        ] {
            assert!(!is_allowed_chatgpt_host(host));
        }
    }

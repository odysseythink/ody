...
# Run a suite of smoke tests against the Windows sandbox via the Ody CLI
...
def _resolve_ody_cmd() -> List[str]:
    """Resolve the Ody CLI to invoke `ody sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run Ody.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "ody.exe",
        ws_root / "target" / "release" / "ody.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "ody.exe",
            cargo_base / "release" / "ody.exe",
        ])
    ...
    raise FileNotFoundError(
        "Ody CLI not found. Build it first, e.g.\n"
        "  cargo build -p ody-cli --release\n"
        "or for debug:\n"
        "  cargo build -p ody-cli\n"
    )

ODY_CMD = _resolve_ody_cmd()
print(ODY_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    # Map policy to ody CLI overrides.
    ...
    argv = [*ODY_CMD, "sandbox", "windows", *policy_flags, *overrides, "--", *cmd_argv]
    print(cmd_argv)
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"ODY_HOME": str(proxy_home)},
            )
    ...
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\ody_testpipe"], WS_ROOT)
    ...
    # 34. WS: policy tamper (.ody artifacts) denied
    ody_home = Path(os.environ["USERPROFILE"]) / ".ody"
    cap_sid_target = ody_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", f"echo tamper > \"{cap_sid_target}\""],
        WS_ROOT,
    )
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .ody\\policy.json"], WS_ROOT)
    add("WS: .ody cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .ody policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > ody_escape.txt"], fake_root)
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://ody-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(

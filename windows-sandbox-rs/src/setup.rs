...
pub const OFFLINE_USERNAME: &str = "OdySandboxOffline";
pub const ONLINE_USERNAME: &str = "OdySandboxOnline";
const ERROR_CANCELLED: u32 = 1223;
const SECURITY_BUILTIN_DOMAIN_RID: u32 = 0x0000_0020;
const DOMAIN_ALIAS_RID_ADMINS: u32 = 0x0000_0220;
const SETUP_EXE_FILENAME: &str = "ody-windows-sandbox-setup.exe";
...
pub fn sandbox_dir(ody_home: &Path) -> PathBuf {
    ody_home.join(".sandbox")
}

pub fn sandbox_bin_dir(ody_home: &Path) -> PathBuf {
    ody_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(ody_home: &Path) -> PathBuf {
    ody_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(ody_home: &Path) -> PathBuf {
    sandbox_dir(ody_home).join("setup_marker.json")
}

pub fn sandbox_users_path(ody_home: &Path) -> PathBuf {
    sandbox_secrets_dir(ody_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, &permissions, env_map, ody_home);
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        ody_home: request.ody_home.to_path_buf(),
    ...
    let sbx_dir = sandbox_dir(request.ody_home);
    let log_path = current_log_file_path(&sbx_dir);
    let cleared_report = match clear_setup_error_report(request.ody_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| request.ody_home.to_path_buf());
    log_note(
        &format!(
    ...
    if let Err(err) = clear_setup_error_report(request.ody_home) {
        log_note(
...
fn gather_helper_read_roots(ody_home: &Path) -> Vec<PathBuf> {
    let helper_dir = helper_bin_dir(ody_home);
...
pub(crate) fn effective_write_roots_for_permissions(
    ...
    filter_sensitive_write_roots(write_roots, ody_home)
}
...
struct ElevationPayload {
    ...
    otel: Option<ody_otel::StatsigMetricsSettings>,
    real_user: String,
    mode: SetupMode,
    #[serde(default)]
    refresh_only: bool,
...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "ODY_NETWORK_ALLOW_LOCAL_BINDING";
...
fn report_helper_failure(
    ...
    match read_setup_error_report(ody_home) {
...
fn verify_setup_completed(ody_home: &Path) -> Result<()> {
    if sandbox_setup_is_complete(ody_home) {
        Ok(())
    } else {
        Err(failure(
...
fn run_setup_exe(payload: &ElevationPayload, needs_elevation: bool, ody_home: &Path) -> Result<()> {
    ...
    let cleared_report = match clear_setup_error_report(ody_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
                &format!(
    ...
    if !needs_elevation {
        ...
        verify_setup_completed(ody_home)?;
        if let Err(err) = clear_setup_error_report(ody_home) {
            log_note(
                &format!(
...
pub fn run_elevated_setup(
    ...
    let payload = ElevationPayload {
        ...
        allow_local_binding: offline_proxy_settings.allow_local_binding,
        real_user: std::env::var("USERNAME").unwrap_or_else(|_| "Administrators".to_string()),
        otel: ody_otel::global_statsig_metrics_settings(),
        mode: SetupMode::Full,
        refresh_only: false,
    };
    ...
    run_setup_exe(&payload, needs_elevation, request.ody_home)
}

pub fn run_elevated_provisioning_setup(ody_home: &Path, real_user: &str) -> Result<()> {
    let sbx_dir = sandbox_dir(ody_home);
    std::fs::create_dir_all(&sbx_dir).map_err(|err| {
        failure(
            SetupErrorCode::OrchestratorSandboxDirCreateFailed,
            format!("failed to create sandbox dir {}: {err}", sbx_dir.display()),
    ...
    let payload = ElevationPayload {
        ...
        command_cwd: ody_home.to_path_buf(),
        read_roots: Vec::new(),
        write_roots: Vec::new(),
    ...
    run_setup_exe(&payload, /*needs_elevation*/ false, ody_home)
}
...
fn build_payload_roots(
    ...
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        let mut read_roots = gather_helper_read_roots(request.ody_home);
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, ody_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to ODY_HOME or anything under ODY_HOME/.sandbox,
    // ODY_HOME/.sandbox-bin, or ODY_HOME/.sandbox-secrets. These locations contain sandbox
    // control/state and helper binaries and must remain tamper-resistant.
    let ody_home_key = canonical_path_key(ody_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(ody_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(ody_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(ody_home));
...
mod tests {
    ...
    fn setup_completion_requires_ready_artifacts() {
        let ody_home = TempDir::new().expect("tempdir");
        let err = verify_setup_completed(ody_home.path())
    ...
    fn report_helper_failure_uses_setup_error_report_when_clear_succeeded() {
        ...
        let ody_home = tmp.path().join("ody-home");
    ...
    fn setup_exe_lookup_checks_package_resource_dir_for_bin_exe() {
        ...
        let exe = bin_dir.join("ody.exe");
        let setup_exe = resources_dir.join("ody-windows-sandbox-setup.exe");
        fs::write(&exe, b"ody").expect("write exe");
    ...
    fn offline_proxy_settings_ignore_proxy_env_when_online_identity_selected() {
        ...
        env.insert(
            "ODY_NETWORK_ALLOW_LOCAL_BINDING".to_string(),
            "1".to_string(),
        );
    ...
    #[test]
    fn expanded_write_roots_still_drop_protected_ody_home() {
        let tmp = TempDir::new().expect("tempdir");
        let user_profile = tmp.path().join("user-profile");
        let ody_home = user_profile.join("OdyHome");
        let documents = user_profile.join("Documents");
        fs::create_dir_all(&ody_home).expect("create ody home");
        fs::create_dir_all(&documents).expect("create documents");
        ...
        let roots = super::filter_sensitive_write_roots(roots, &ody_home);
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        ...
        let roots = gather_read_roots(&command_cwd, &permissions, &HashMap::new(), &ody_home);
        let expected =
            dunce::canonicalize(helper_bin_dir(&ody_home)).expect("canonical helper dir");
    ...
    fn effective_write_roots_match_payload_filtering_for_overrides() {
        ...
        let sandbox_root = super::sandbox_dir(&ody_home);
        fs::create_dir_all(&ody_home).expect("create ody home");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        ...
        let forbidden_ody_home = dunce::canonicalize(&ody_home).expect("canonical ody home");
        ...
        assert!(!effective_write_roots.contains(&forbidden_ody_home));
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_ody).expect("canonical extra .ody"),
                explicit_deny,
            ]
            .into_iter()

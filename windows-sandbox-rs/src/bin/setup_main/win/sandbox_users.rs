...
use ody_windows_sandbox::dpapi_protect;
...
pub const SANDBOX_USERS_GROUP: &str = "OdySandboxUsers";
const SANDBOX_USERS_GROUP_COMMENT: &str = "Ody sandbox internal group (managed)";
...
fn write_secrets(
    ...
    let secrets_dir = sandbox_secrets_dir(ody_home);
...
pub(super) fn prepare_setup_marker(ody_home: &Path, real_user: &str) -> Result<()> {
    let marker_path = sandbox_dir(ody_home).join("setup_marker.json");

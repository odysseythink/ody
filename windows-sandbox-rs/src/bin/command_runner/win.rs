...
use anyhow::Context;
use anyhow::Result;
use ody_windows_sandbox::ErrorPayload;
use ody_windows_sandbox::ExitPayload;
use ody_windows_sandbox::FramedMessage;
use ody_windows_sandbox::IPC_PROTOCOL_VERSION;
use ody_windows_sandbox::LocalSid;
use ody_windows_sandbox::Message;
use ody_windows_sandbox::OutputPayload;
use ody_windows_sandbox::OutputStream;
use ody_windows_sandbox::PipeSpawnHandles;
use ody_windows_sandbox::ResizePayload;
use ody_windows_sandbox::SpawnReady;
use ody_windows_sandbox::SpawnRequest;
use ody_windows_sandbox::StderrMode;
use ody_windows_sandbox::StdinMode;
use ody_windows_sandbox::WindowsSandboxTokenMode;
use ody_windows_sandbox::allow_null_device;
use ody_windows_sandbox::create_readonly_token_with_caps_and_user_from;
use ody_windows_sandbox::create_workspace_write_token_with_caps_and_user_from;
use ody_windows_sandbox::decode_bytes;
use ody_windows_sandbox::encode_bytes;
use ody_windows_sandbox::get_current_token_for_restriction;
use ody_windows_sandbox::hide_current_user_profile_dir;
use ody_windows_sandbox::log_note;
use ody_windows_sandbox::read_frame;
use ody_windows_sandbox::read_handle_loop;
use ody_windows_sandbox::spawn_process_with_pipes;
use ody_windows_sandbox::to_wide;
use ody_windows_sandbox::token_mode_for_permission_profile;
use ody_windows_sandbox::write_frame;
use std::ffi::OsStr;
use std::fs::File;
...
const READ_ACL_MUTEX_NAME: &str = "Local\\OdySandboxReadAcl";
...
struct IpcSpawnedProcess {
    ...
    conpty_owner: Option<ody_windows_sandbox::ConptyInstance>,
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    let log_dir = req.ody_home.clone();
    hide_current_user_profile_dir(req.ody_home.as_path());
    ...
    let (pi, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, mut conpty) = ody_windows_sandbox::spawn_conpty_process_as_user(
            h_token.raw(),
            &req.command,
            &effective_cwd,
            &req.env,

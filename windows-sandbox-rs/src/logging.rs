...
use ody_utils_string::take_bytes_at_char_boundary;
...
pub fn current_log_file_path_for_ody_home(ody_home: &Path) -> PathBuf {
    current_log_file_path(&crate::sandbox_dir(ody_home))
}
...
mod tests {
    ...
    #[test]
    fn current_log_file_path_for_ody_home_uses_sandbox_dir() {
        let ody_home = Path::new("ody-home");

        assert_eq!(
            current_log_file_path_for_ody_home(ody_home),
            current_log_file_path(&ody_home.join(".sandbox"))
        );
    }
}

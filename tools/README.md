...
`ody-tools` is the shared support crate for building, adapting, and executing
model-visible tools outside `ody-core`.
...
compatibility-sensitive orchestration in `ody-core` until the surrounding
...
- code-mode compatibility shims that do not depend on `ody-core`
- other narrowly scoped host utilities that multiple crates need

The corresponding non-goals are just as important:

- do not move `ody-core` orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in `ody-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `ody-core`.
3. Leave compatibility-sensitive adapters in `ody-core` while downstream
...
`ody-core`, that is a sign to revisit the extraction boundary before adding
more here.

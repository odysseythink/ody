...
use chrono::Utc;
use ody_protocol::auth::PlanType as AuthPlanType;
...
const AGENT_IDENTITY_JWT_AUDIENCE: &str = "ody-app-server";
const AGENT_IDENTITY_JWT_ISSUER: &str = "https://chatgpt.com/ody-backend/agent-identity";
const AGENT_REGISTRATION_TIMEOUT: Duration = Duration::from_secs(15);
const PROD_AGENT_IDENTITY_AUTHAPI_BASE_URL: &str = "https://auth.odysseythink.com/api/accounts";
const STAGING_AGENT_IDENTITY_AUTHAPI_BASE_URL: &str = "https://auth.api.odysseythink.org/api/accounts";
const AGENT_IDENTITY_KEY_SEED_BYTES: usize = 64;
const AGENT_IDENTITY_KEY_DERIVATION_CONTEXT: &[u8] = b"ody-agent-identity-ed25519-v1";
...
impl ChatGptEnvironment {
    pub fn from_chatgpt_base_url(chatgpt_base_url: &str) -> Result<Self> {
        match chatgpt_base_url.trim_end_matches('/') {
            ...
            | "https://chatgpt.com/ody"
            | "https://chatgpt.com/backend-api/ody"
            | "https://chat.odysseythink.com"
            | "https://chat.odysseythink.com/backend-api"
            | "https://chat.odysseythink.com/ody"
            | "https://chat.odysseythink.com/backend-api/ody" => Ok(Self::Production),
            "https://chatgpt-staging.com"
            | "https://chatgpt-staging.com/backend-api"
            | "https://chatgpt-staging.com/ody"
            | "https://chatgpt-staging.com/backend-api/ody" => Ok(Self::Staging),
            _ => anyhow::bail!(
...
/// single Ody run, while the agent runtime id and private key are the
...
pub fn build_abom(session_source: SessionSource) -> AgentBillOfMaterials {
    AgentBillOfMaterials {
        ...
        agent_harness_id: match &session_source {
            SessionSource::VSCode => "ody-app".to_string(),
            SessionSource::Cli
            | SessionSource::Exec
            | SessionSource::Mcp
            ...
            | SessionSource::Unknown => "ody-cli".to_string(),
        },
...
mod tests {
    ...
    use ody_protocol::auth::KnownPlan;
...
-----END PRIVATE KEY-----"#,
    ...
    fn chatgpt_environment_maps_known_urls_to_authapi() -> anyhow::Result<()> {
        assert_eq!(
            ChatGptEnvironment::from_chatgpt_base_url("https://chatgpt.com/backend-api/ody")?,
            ChatGptEnvironment::Production
        );
        assert_eq!(
            ChatGptEnvironment::Production.agent_identity_authapi_base_url(),
            "https://auth.odysseythink.com/api/accounts"
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            "https://auth.api.odysseythink.org/api/accounts"
        );
        Ok(())
    }
    ...
    fn agent_registration_url_appends_to_authapi_base_url() {
        assert_eq!(
            agent_registration_url("https://auth.odysseythink.com/api/accounts"),
            "https://auth.odysseythink.com/api/accounts/v1/agent/register"
        );
        assert_eq!(
    ...
    fn agent_task_registration_url_appends_to_authapi_base_url() {
        assert_eq!(
            agent_task_registration_url("https://auth.odysseythink.com/api/accounts", "agent-runtime-id"),
            "https://auth.odysseythink.com/api/accounts/v1/agent/agent-runtime-id/task/register"
        );
        assert_eq!(
    ...
    fn agent_identity_jwks_url_uses_jwt_issuer_base_url() {
        assert_eq!(
            agent_identity_jwks_url("http://localhost:8080/api/ody"),
            "http://localhost:8080/api/ody/agent-identities/jwks"
        );
        assert_eq!(
            agent_identity_jwks_url("http://localhost:8080/api/ody/"),
            "http://localhost:8080/api/ody/agent-identities/jwks"

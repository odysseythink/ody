...
async fn send_ody_exec_originator() -> anyhow::Result<()> {
    ...
    responses::mount_sse_once_match(&server, header("Originator", "ody_exec"), body).await;

    test.cmd_with_server(&server)
        .env_remove(ODY_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)

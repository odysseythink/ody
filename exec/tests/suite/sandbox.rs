#![cfg(unix)]
use ody_core::spawn::StdioPolicy;
...
async fn spawn_command_under_sandbox(
    ...
    use ody_core::exec::build_exec_request;
...
async fn spawn_command_under_sandbox(
    ...
    use ody_core::spawn_command_under_linux_sandbox;

    let ody_linux_sandbox_exe = core_test_support::find_ody_linux_sandbox_exe()
...
#[tokio::test]
async fn sandbox_blocks_first_time_dot_ody_creation() {
    core_test_support::skip_if_sandbox!();
    #[cfg(target_os = "linux")]
    let sandbox_env = match linux_sandbox_test_env().await {
        Some(env) => env,
    ...
    let mut child = spawn_command_under_sandbox(
        vec![
            "bash".to_string(),
            "-lc".to_string(),
            "mkdir -p .ody && echo 'sandbox_mode = \"danger-full-access\"' > .ody/config.toml"
                .to_string(),
        ],
        repo_root.clone(),
        &permission_profile,
    ...
    .await
    .expect("should spawn command creating .ody");

    let status = child.wait().await.expect("should wait for .ody command");
    assert!(
        !status.success(),
        "sandbox unexpectedly allowed first-time .ody creation: {status:?}"
    );
    let dot_ody_metadata = tokio::fs::symlink_metadata(&dot_ody).await;
    if let Ok(metadata) = dot_ody_metadata {
        assert!(
            !metadata.is_dir(),
            "{} should not be creatable as a directory",
            dot_ody.display()
        );
    } else if let Err(err) = &dot_ody_metadata {
        assert_eq!(
            err.kind(),

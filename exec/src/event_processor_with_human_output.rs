...
use crate::event_processor::OdyStatus;
    ...
    fn render_item_completed(&mut self, item: ThreadItem) {
        match item {
            ThreadItem::AgentMessage { text, .. } => {
                eprintln!("{}\n{}", "ody".style(self.italic).style(self.magenta), text);
...
impl EventProcessor for EventProcessorWithHumanOutput {
    fn print_config_summary(
        ...
        eprintln!("odysseythink ody v{VERSION}\n--------");
    ...
    fn process_server_notification(&mut self, notification: ServerNotification) -> OdyStatus {
        match notification {
            ...
            ServerNotification::TurnPlanUpdated(notification) => {
                ...
                for step in notification.plan {
                    match step.status {
                        ...
                        ody_app_server_protocol::TurnPlanStepStatus::InProgress => {
                            eprintln!("  {} {}", "→".style(self.cyan), step.step);
                        }
                        ody_app_server_protocol::TurnPlanStepStatus::Pending => {
                            eprintln!(
                                "  {} {}",
                                "•".style(self.dimmed),
            ...
            ServerNotification::TurnStarted(_) => OdyStatus::Running,
            _ => OdyStatus::Running,
        }
    }
    ...
    fn print_final_output(&mut self) {
        ...
        ) && let Some(message) = self.final_message.as_deref()
        {
            eprintln!(
                "{}\n{}",
                "ody".style(self.italic).style(self.magenta),
                message
            );
        }
    }

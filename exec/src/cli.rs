...
#[command(
    version,
    override_usage = "ody exec [OPTIONS] [PROMPT]\n       ody exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running Ody outside a Git repository.
    ...
    /// Do not load `$ODY_HOME/config.toml`; auth still uses `ODY_HOME`.
...
struct ResumeArgsRaw {
    ...
    // so "ody resume --last <prompt>" treats the positional as a prompt, not a session id.

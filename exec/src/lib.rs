...
use ody_app_server_client::EnvironmentManager;
use ody_app_server_client::ExecServerRuntimePaths;
use ody_app_server_client::InProcessAppServerClient;
use ody_app_server_client::InProcessClientStartArgs;
use ody_app_server_client::InProcessServerEvent;
...
use ody_app_server_protocol::ThreadItem as AppServerThreadItem;
...
enum StdinPromptBehavior {
    ...
    /// `ody exec` behavior for `ody exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `ody exec -`
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    if let Err(err) = set_default_originator("ody_exec".to_string()) {
        tracing::warn!(?err, "Failed to set ody exec originator override {err:?}");
    }
    ...
    let user_config_path = config_profile_v2
        .as_ref()
        .map(|profile_v2| resolve_profile_v2_config_path(&ody_home, profile_v2));
    ...
    if let Err(err) = enforce_login_restrictions(&AuthConfig {
        ...
        // plugin/Apps catalog have been removed; the corresponding `Config` fields these used
        // to be sourced from are gone too.
    ...
    ody_core::otel_init::record_process_start(otel.as_ref(), "ody_exec");
    ody_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "ody_exec");
    ...
    let in_process_start_args = InProcessClientStartArgs {
        ...
        client_name: "ody_exec".to_string(),
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = ody_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what Ody
    // is using.
    ...
    info!("Ody initialized with event: {session_configured:?}");
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(mut notification) => {
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        ody_app_server_protocol::TurnStatus::Failed
                            | ody_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
...
async fn resolve_resume_thread_id(
    ...
    if let Some(state_db) = state_db {
        ...
        if let Some((_, session_meta)) =
            find_thread_meta_by_name_str(&config.ody_home, session_id, Some(state_db.as_ref()))
                .await?

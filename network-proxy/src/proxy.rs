...
#[command(name = "ody-network-proxy", about = "Ody network sandbox proxy")]
pub struct Args {}
...
impl NetworkProxyBuilder {
    ...
    pub fn managed_by_ody(mut self, managed_by_ody: bool) -> Self {
        self.managed_by_ody = managed_by_ody;
        self
    }
...
pub const PROXY_ACTIVE_ENV_KEY: &str = "ODY_NETWORK_PROXY_ACTIVE";
pub const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "ODY_NETWORK_ALLOW_LOCAL_BINDING";
...
#[cfg(target_os = "macos")]
pub const ODY_PROXY_GIT_SSH_COMMAND_MARKER: &str = "ODY_PROXY_GIT_SSH_COMMAND=1 ";
#[cfg(target_os = "macos")]
const ODY_PROXY_GIT_SSH_COMMAND_PREFIX: &str =
    "ODY_PROXY_GIT_SSH_COMMAND=1 ssh -o ProxyCommand='nc -X 5 -x ";
#[cfg(target_os = "macos")]
const ODY_PROXY_GIT_SSH_COMMAND_SUFFIX: &str = " %h %p'";
...
#[cfg(target_os = "macos")]
fn ody_proxy_git_ssh_command(socks_addr: SocketAddr) -> String {
    format!("{ODY_PROXY_GIT_SSH_COMMAND_PREFIX}{socks_addr}{ODY_PROXY_GIT_SSH_COMMAND_SUFFIX}")
}

#[cfg(target_os = "macos")]
fn is_ody_proxy_git_ssh_command(command: &str) -> bool {
    command.starts_with(ODY_PROXY_GIT_SSH_COMMAND_PREFIX)
        && command.ends_with(ODY_PROXY_GIT_SSH_COMMAND_SUFFIX)
}
...
fn apply_proxy_env_overrides(
    ...
    if socks_enabled {
        ...
        // but refresh a previously injected Ody fallback so it cannot point
        // at a stale proxy port after the proxy is restarted.
        match env.get(GIT_SSH_COMMAND_ENV_KEY) {
            Some(command) if !is_ody_proxy_git_ssh_command(command) => {}
            _ => {
                env.insert(
                    GIT_SSH_COMMAND_ENV_KEY.to_string(),
                    ody_proxy_git_ssh_command(socks_addr),
                );
            }
        }
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn non_ody_managed_proxy_builder_uses_configured_ports() {
    ...
    fn apply_proxy_env_overrides_sets_common_tool_vars() {
        ...
        assert_eq!(
            ...
            Some(
                &"ODY_PROXY_GIT_SSH_COMMAND=1 ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
                    .to_string()
            )
        );
    ...
    fn apply_proxy_env_overrides_sets_mitm_ca_trust_bundle_vars() {
        let mut env = HashMap::new();
        let mitm_ca_trust_bundle_path = Path::new("/tmp/ody-proxy/ca-bundle.pem");
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn apply_proxy_env_overrides_refreshes_previous_ody_proxy_git_ssh_command() {
        let mut env = HashMap::new();
        env.insert(
            GIT_SSH_COMMAND_ENV_KEY.to_string(),
            ody_proxy_git_ssh_command(SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 8081)),
        );
        ...
        assert_eq!(
            env.get(GIT_SSH_COMMAND_ENV_KEY),
            Some(&ody_proxy_git_ssh_command(SocketAddr::new(

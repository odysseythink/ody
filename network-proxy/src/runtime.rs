...
const NETWORK_POLICY_VIOLATION_PREFIX: &str = "ODY_NETWORK_POLICY_VIOLATION";
...
impl NetworkProxyState {
    ...
    pub async fn current_cfg(&self) -> Result<NetworkProxyConfig> {
        ...
        // `config.toml` (including Ody-managed writes) take effect without a restart.
...
mod tests {
    ...
    fn blocked_request_violation_log_line_serializes_payload() {
        ...
        assert_eq!(
            ...
            r#"ODY_NETWORK_POLICY_VIOLATION {"host":"google.com","reason":"not_allowed","client":"127.0.0.1","method":"GET","mode":"full","protocol":"http","decision":"ask","source":"decider","port":80,"timestamp":1735689600}"#
        );
    }

    #[tokio::test]
    async fn host_blocked_subdomain_wildcards_exclude_apex() {
        let state = network_proxy_state_for_policy(network_settings(&["*.odysseythink.com"], &[]));

        assert_eq!(
            state
                .host_blocked("api.odysseythink.com", /*port*/ 80)
                .await
                .unwrap(),
            HostBlockDecision::Allowed
        );
    ...
    async fn host_blocked_global_wildcard_allowlist_allows_public_hosts_except_denylist() {
        ...
        assert_eq!(
            state
                .host_blocked("api.odysseythink.com", /*port*/ 443)
                .await
                .unwrap(),
            HostBlockDecision::Allowed
        );
    ...
    fn validate_policy_against_constraints_allows_expanding_allowed_domains_when_enabled() {
        ...
        let config = NetworkProxyConfig {
            network: {
                let mut network = network_settings(&["example.com", "api.odysseythink.com"], &[]);
                network.enabled = true;
                network
            },
        };
    ...
    fn compile_globset_excludes_apex_for_subdomain_patterns() {
        let patterns = vec!["*.odysseythink.com".to_string()];
        let set = compile_denylist_globset(&patterns).unwrap();
        assert!(set.is_match("api.odysseythink.com"));
        assert!(!set.is_match("odysseythink.com"));
        assert!(!set.is_match("evilodysseythink.com"));
    }

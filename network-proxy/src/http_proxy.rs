...
mod tests {
    ...
    async fn http_connect_accept_denies_denylisted_host() {
        let policy = {
            let mut policy = NetworkProxySettings::default();
            policy.set_allowed_domains(vec!["**.odysseythink.com".to_string()]);
            policy.set_denied_domains(vec!["api.odysseythink.com".to_string()]);
            policy
        };
        let state = Arc::new(network_proxy_state_for_policy(policy));
        ...
        let mut req = Request::builder()
            .method(Method::CONNECT)
            .uri("https://api.odysseythink.com:443")
            .header("host", "api.odysseythink.com:443")
            .body(Body::empty())
            .unwrap();
        req.extensions_mut().insert(state);
    ...
    async fn http_plain_proxy_rejects_absolute_uri_host_header_mismatch() {
        ...
        let mut req = Request::builder()
            .method(Method::GET)
            .uri("http://raw.githubusercontent.com/odysseythink/ody/main/README.md")

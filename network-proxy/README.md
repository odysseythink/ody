...
`ody-network-proxy` is Ody's local network policy enforcement proxy. It runs:
...
`ody-network-proxy` reads from Ody's merged `config.toml` (via `ody-core` config loading).
...
# CA cert/key are managed internally under $ODY_HOME/proxy/ (ca.pem + ca.key).
# When MITM is active, spawned commands receive CA bundle env vars pointing at
# immutable bundles under $ODY_HOME/proxy/ so common HTTPS clients trust the managed CA.
...
# Use exact hosts or scoped wildcards like `*.odysseythink.com` or `**.odysseythink.com`.
...
`ody-network-proxy` can be embedded as a library with a thin API:

```rust
use ody_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `ody-network-proxy` is embedded in managed Ody runtime, policy decisions emit structured
OTEL-compatible events with `target=ody_otel.network_proxy`.

Event name:

- `ody.network_proxy.policy_decision`
...
This section documents the protections implemented by `ody-network-proxy`, and the boundaries of

...
use ody_config::TomlValue;
use ody_config::version_for_toml;
...
fn config_toml_source_path(layer: &ConfigLayerEntry) -> AbsolutePathBuf {
    match &layer.name {
        ...
        ConfigLayerSource::Project { dot_ody_folder } => layer
            .hooks_config_folder()
            .unwrap_or_else(|| dot_ody_folder.clone())
...
fn append_matcher_groups(
    ...
    display_order: &mut i64,
    source: &HookHandlerSource<'_>,
    event_name: ody_protocol::protocol::HookEventName,
...
mod tests {
        ...
        let source = RequirementSource::Composite {
            sources: vec![
                RequirementSource::SystemRequirementsToml {
                    file: test_path_buf("/etc/ody/requirements.toml").abs(),
                },
    ...
    fn hook_metadata_for_config_layer_source_discards_source_details() {
        let config_file = test_path_buf("/tmp/.ody/config.toml").abs();
        let dot_ody_folder = test_path_buf("/tmp/worktree/.ody").abs();

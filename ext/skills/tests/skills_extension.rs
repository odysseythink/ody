...
use ody_core_skills::SKILLS_HOW_TO_USE_WITH_ABSOLUTE_PATHS;
...
use ody_skills_extension::catalog::SkillCatalog;
use ody_skills_extension::catalog::SkillCatalogEntry;
use ody_skills_extension::catalog::SkillPackageId;
use ody_skills_extension::catalog::SkillProviderError;
use ody_skills_extension::catalog::SkillReadResult;
use ody_skills_extension::catalog::SkillResourceId;
use ody_skills_extension::catalog::SkillSearchResult;
use ody_skills_extension::catalog::SkillSourceKind;
use ody_skills_extension::install;
use ody_skills_extension::install_with_providers;
use ody_skills_extension::provider::SkillListQuery;
use ody_skills_extension::provider::SkillProvider;
use ody_skills_extension::provider::SkillProviderFuture;
use ody_skills_extension::provider::SkillReadRequest;
use ody_skills_extension::provider::SkillSearchRequest;
use ody_utils_absolute_path::AbsolutePathBuf;
use pretty_assertions::assert_eq;

type TestResult = Result<(), Box<dyn std::error::Error>>;

static NEXT_ODY_HOME_ID: AtomicUsize = AtomicUsize::new(0);
...
async fn installed_extension_uses_host_service_snapshot() -> TestResult {
    ...
    let skill_path = ody_home.join("skills").join("demo").join("SKILL.md");
    ...
    std::fs::remove_dir_all(ody_home)?;
    Ok(())
}
...
fn test_ody_home() -> PathBuf {
    let id = NEXT_ODY_HOME_ID.fetch_add(1, Ordering::Relaxed);
    std::env::temp_dir().join(format!(
        "ody-skills-extension-test-{}-{id}",
        std::process::id(),
    ))
}

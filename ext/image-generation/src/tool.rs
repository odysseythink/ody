...
use ody_api::ImageBackground;
use ody_api::ImageEditRequest;
use ody_api::ImageGenerationRequest;
use ody_api::ImageQuality;
use ody_api::ImageUrl;
use ody_core::context::extension_image_generation_output_hint;
use ody_core::image_generation_artifact_path;
use ody_extension_api::ExtensionTurnItem;
use ody_extension_api::FunctionCallError;
use ody_extension_api::ToolCall;
use ody_extension_api::ToolEnvironment;
...
use crate::backend::OdyImagesBackend;
...
impl ToolExecutor<ToolCall> for ImageGenerationTool {
    ...
    fn handle(&self, call: ToolCall) -> ody_extension_api::ToolExecutorFuture<'_> {
...
impl ImageGenerationTool {
    async fn handle_call(&self, call: ToolCall) -> Result<Box<dyn ToolOutput>, FunctionCallError> {
        ...
        call.turn_item_emitter
            ...
            .await;
        let output_path =
            image_generation_artifact_path(&self.ody_home, &self.thread_id, &call.call_id);
        let output_dir = output_path
            .parent()
            .unwrap_or_else(|| self.ody_home.clone());
        let output_hint =

...
impl From<&Config> for ImageGenerationExtensionConfig {
    ...
    fn from(config: &Config) -> Self {
        Self {
            ...
            available: config.model_provider.is_odysseythink(),
...
impl ToolContributor for ImageGenerationExtension {
    ...
    fn tools(
        ...
        if !config.available || !self.auth_manager.current_auth_uses_ody_backend() {
            return Vec::new();
        }

        vec![Arc::new(ImageGenerationTool::new(
            OdyImagesBackend::new(create_model_provider(

...
async fn build_memory_tool_developer_instructions_renders_embedded_template() {
    ...
    let ody_home = AbsolutePathBuf::from_absolute_path(temp.path()).unwrap();
    let memories_dir = ody_home.join("memories");
    ...
    let instructions = build_memory_tool_developer_instructions(&ody_home)
        .await
        .unwrap();

...
use ody_extension_api::NoopTurnItemEmitter;
use ody_extension_api::ThreadResumeInput;
use ody_extension_api::ThreadStartInput;
use ody_extension_api::ThreadStopInput;
use ody_extension_api::ToolCall;
use ody_extension_api::ToolCallOutcome;
use ody_extension_api::ToolCallSource;
use ody_extension_api::ToolExecutor;
use ody_extension_api::ToolFinishInput;
use ody_extension_api::ToolPayload;
use ody_extension_api::TurnErrorInput;
use ody_extension_api::TurnStartInput;
use ody_extension_api::TurnStopInput;
use ody_goal_extension::GoalObjectiveUpdate;
use ody_goal_extension::GoalRuntimeHandle;
use ody_goal_extension::GoalService;
use ody_goal_extension::GoalSetRequest;
use ody_goal_extension::GoalTokenBudgetUpdate;
use ody_goal_extension::install_with_backend;
...
async fn budget_limited_goal_keeps_accruing_until_turn_stop() -> anyhow::Result<()> {
    ...
    assert_eq!(ody_state::ThreadGoalStatus::BudgetLimited, goal.status);
...
async fn turn_error_usage_limit_accounts_progress_and_clears_accounting() -> anyhow::Result<()> {
    ...
    harness
        ...
        .await;
    harness
        .notify_turn_error("turn-1", OdyErrorInfo::UsageLimitExceeded)
        .await;
    ...
    let goal = runtime
        ...
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(23, goal.tokens_used);
    assert_eq!(ody_state::ThreadGoalStatus::UsageLimited, goal.status);
    assert_eq!(
        vec![
            CapturedGoalEvent {
...
async fn turn_error_blocks_goal() -> anyhow::Result<()> {
    ...
    harness
        .notify_turn_error("turn-1", OdyErrorInfo::Other)
        .await;
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(ody_state::ThreadGoalStatus::Blocked, goal.status);
    Ok(())
}
...
async fn usage_limit_plan_turn_does_not_stop_goal() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(ody_state::ThreadGoalStatus::Active, goal.status);
...
async fn thread_resume_rehydrates_active_goal_idle_accounting() -> anyhow::Result<()> {
    ...
    runtime
        ...
        .replace_thread_goal(
            ...
            ody_state::ThreadGoalStatus::Active,
            /*token_budget*/ None,
        )
        .await?;
    let harness = GoalExtensionHarness::new(runtime.clone(), thread_id).await?;
...
async fn installed_tools(
    runtime: Arc<ody_state::StateRuntime>,
    thread_id: ThreadId,
...
struct GoalExtensionHarness {
    registry: ody_extension_api::ExtensionRegistry<()>,
...
impl GoalExtensionHarness {
    ...
    async fn notify_tool_finish(&self, turn_id: &str, call_id: &str, tool_name: &str) {
        let turn_store = ExtensionData::new(turn_id);
        let tool_name = ody_extension_api::ToolName::plain(tool_name);
    ...
    async fn notify_turn_error(&self, turn_id: &str, error: OdyErrorInfo) {
        let turn_store = ExtensionData::new(turn_id);
        for contributor in self.registry.turn_lifecycle_contributors() {
            contributor
...
fn tool_call(tool_name: &str, call_id: &str, arguments: serde_json::Value) -> ToolCall {
    ToolCall {
        ...
        tool_name: ody_extension_api::ToolName::plain(tool_name),
...
async fn test_runtime() -> anyhow::Result<Arc<ody_state::StateRuntime>> {
    let tempdir = TempDir::new()?;
    ody_state::StateRuntime::init(tempdir.keep(), "test-provider".to_string()).await
}
...
async fn seed_thread_metadata(
    runtime: &ody_state::StateRuntime,
    thread_id: ThreadId,
) -> anyhow::Result<()> {
    let builder = ody_state::ThreadMetadataBuilder::new(
        thread_id,
        runtime
            .ody_home()
...
fn protocol_status(status: ody_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
        ody_state::ThreadGoalStatus::Active => ThreadGoalStatus::Active,
        ody_state::ThreadGoalStatus::Paused => ThreadGoalStatus::Paused,
        ody_state::ThreadGoalStatus::Blocked => ThreadGoalStatus::Blocked,
        ody_state::ThreadGoalStatus::UsageLimited => ThreadGoalStatus::UsageLimited,
        ody_state::ThreadGoalStatus::BudgetLimited => ThreadGoalStatus::BudgetLimited,
        ody_state::ThreadGoalStatus::Complete => ThreadGoalStatus::Complete,
    }
}

...
use ody_protocol::protocol::ThreadGoal;
use ody_protocol::protocol::ThreadGoalStatus;
use ody_protocol::protocol::validate_thread_goal_objective;
...
pub(crate) struct GoalToolExecutor {
    kind: GoalToolKind,
    thread_id: ThreadId,
    state_db: Arc<ody_state::StateRuntime>,
...
impl ToolExecutor<ToolCall> for GoalToolExecutor {
    ...
    fn handle(&self, invocation: ToolCall) -> ody_extension_api::ToolExecutorFuture<'_> {
        Box::pin(async move {
            match self.kind {
...
impl GoalToolExecutor {
    ...
    async fn handle_update(
        ...
        self.account_active_goal_progress(
            match args.status {
                ThreadGoalStatus::Complete => ody_state::GoalAccountingMode::ActiveOrComplete,
                ThreadGoalStatus::Blocked => ody_state::GoalAccountingMode::ActiveOrStopped,
    ...
    async fn account_active_goal_progress(
        &self,
        mode: ody_state::GoalAccountingMode,
        event_id: &str,
        ...
        Ok(match outcome {
            ody_state::GoalAccountingOutcome::Updated(goal) => {
                self.metrics
                    .record_terminal_if_status_changed(previous_status, &goal);
                self.analytics
            ...
            ody_state::GoalAccountingOutcome::Unchanged(_) => None,
        })
    }
    ...
    ) -> Result<Option<ody_state::ThreadGoalStatus>, FunctionCallError> {
        let goal = self
            .state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
...
pub(crate) async fn fill_empty_thread_preview_if_possible(
    state_db: &ody_state::StateRuntime,
    thread_id: ThreadId,
    goal: &ody_state::ThreadGoal,
) {
...
pub(crate) fn protocol_goal_from_state(goal: ody_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn protocol_status_from_state(status: ody_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(crate) fn state_status_from_protocol(status: ThreadGoalStatus) -> ody_state::ThreadGoalStatus {
    match status {
        ...
        ThreadGoalStatus::Blocked => ody_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => ody_state::ThreadGoalStatus::UsageLimited,
        ThreadGoalStatus::BudgetLimited => ody_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => ody_state::ThreadGoalStatus::Complete,
    }
}

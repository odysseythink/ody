...
//!
//! This crate wraps [`ody_app_server::in_process`] behind a single async API
...
//! [`InProcessClientHandle`](ody_app_server::in_process::InProcessClientHandle),
...
pub use ody_app_server::app_server_control_socket_path;
pub use ody_app_server::in_process::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
pub use ody_app_server::in_process::InProcessServerEvent;
use ody_app_server::in_process::InProcessStartArgs;
use ody_app_server::in_process::LogDbLayer;
pub use ody_app_server::in_process::StateDbHandle;
...
use ody_app_server_protocol::Result as JsonRpcResult;
...
use ody_config::NoopThreadConfigLoader;
use ody_config::RemoteThreadConfigLoader;
...
/// module exists so clients can remove a direct `ody-core` dependency
/// while legacy startup/config paths are migrated to RPCs.
pub mod legacy_core {
    pub use ody_core::check_execpolicy_for_warnings;
    pub use ody_core::format_exec_policy_error_with_source;

    pub mod config {
        pub use ody_core::config::*;

        pub mod edit {
            pub use ody_core::config::edit::*;
        }
    }
}
...
pub async fn migrate_personality_if_needed(
    ...
    let status = maybe_migrate_personality(ody_home, config_toml, state_db).await?;
    match status {
...
/// - [`ody_app_server::in_process::InProcessClientHandle`], which speaks to
...
impl AppServerClient {
    pub fn ody_home(&self, local_ody_home: &AbsolutePathBuf) -> Option<AppServerPath> {
        match self {
            Self::InProcess(_) => Some(AppServerPath::from_app_server(
                local_ody_home.display().to_string(),
            )),
            Self::Remote(client) => client.ody_home().map(AppServerPath::from_app_server),
        }
    }
...
mod tests {
    ...
    async fn build_test_config_for_ody_home(ody_home: &Path) -> Config {
        ...
        {
            Ok(config) => config,
            Err(_) => Config::load_default_with_cli_overrides_for_ody_home(
    ...
    async fn start_test_client_with_capacity(
        ...
        let ody_home = TempDir::new().expect("temp dir");
        let config = Arc::new(build_test_config_for_ody_home(ody_home.path()).await);
        ...
        write_websocket_message(
            ...
            JSONRPCMessage::Response(JSONRPCResponse {
                ...
                result: serde_json::json!({
                    "userAgent": "ody_cli_rs/9.8.7-test (Test OS; x86_64) rust",
                    "odyHome": "/server/.ody",
                }),
            }),
        )
    ...
    fn command_execution_output_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::CommandExecutionOutputDelta(
            ody_app_server_protocol::CommandExecutionOutputDeltaNotification {
    ...
    fn agent_message_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::AgentMessageDelta(
            ody_app_server_protocol::AgentMessageDeltaNotification {
                thread_id: "thread".to_string(),
                turn_id: "turn".to_string(),
                item_id: "item".to_string(),
                delta: delta.to_string(),
    ...
    fn item_completed_notification(text: &str) -> ServerNotification {
        ServerNotification::ItemCompleted(ody_app_server_protocol::ItemCompletedNotification {
            ...
            turn_id: "turn".to_string(),
            completed_at_ms: 0,
            item: ody_app_server_protocol::ThreadItem::AgentMessage {
                id: "item".to_string(),
                text: text.to_string(),
                phase: None,
    ...
    fn turn_completed_notification() -> ServerNotification {
        ServerNotification::TurnCompleted(ody_app_server_protocol::TurnCompletedNotification {
            thread_id: "thread".to_string(),
            turn: ody_app_server_protocol::Turn {
                id: "turn".to_string(),
    ...
    async fn typed_request_reports_json_rpc_errors() {
        ...
        let err = client
            .request_typed::<ConfigRequirementsReadResponse>(ClientRequest::ThreadRead {
                ...
                params: ody_app_server_protocol::ThreadReadParams {
    ...
    async fn threads_started_via_app_server_are_visible_through_typed_requests() {
        ...
        let response: ThreadStartResponse = client
            ...
            .expect("thread/start should succeed");
        let read = client
            .request_typed::<ody_app_server_protocol::ThreadReadResponse>(
    ...
    async fn forward_in_process_event_preserves_transcript_notifications_under_backpressure() {
        ...
        assert!(matches!(
            ...
            InProcessServerEvent::ServerNotification(ServerNotification::ItemCompleted(
                ...
                &notification.item,
                ody_app_server_protocol::ThreadItem::AgentMessage { text, .. } if text == "hello"
            )
        ));
        assert!(matches!(
            &events[4],
            InProcessServerEvent::ServerNotification(ServerNotification::TurnCompleted(
                notification
            )) if notification.turn.status == ody_app_server_protocol::TurnStatus::Completed
        ));
    }
    ...
    async fn remote_typed_request_roundtrip_works() {
        ...
        assert_eq!(client.ody_home(), Some("/server/.ody"));
        let response: GetAccountResponse = client
            .request_typed(ClientRequest::GetAccount {
                request_id: RequestId::Integer(1),
                params: ody_app_server_protocol::GetAccountParams {
                    refresh_token: false,
                },
            })
            .await
    ...
    async fn remote_unix_socket_typed_request_roundtrip_works() {
        ...
        let socket_path = AbsolutePathBuf::from_absolute_path(socket_dir.path().join("ody.sock"))
        ...
        for event in &remaining_events {
            match event {
                ...
                AppServerEvent::ServerNotification(ServerNotification::TurnCompleted(
                    ...
                    == ody_app_server_protocol::TurnStatus::Completed =>
                {
    ...
    fn event_requires_delivery_marks_transcript_and_terminal_events() {
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                ody_app_server_protocol::ServerNotification::TurnCompleted(
                    ody_app_server_protocol::TurnCompletedNotification {
                        thread_id: "thread".to_string(),
                        turn: ody_app_server_protocol::Turn {
                            id: "turn".to_string(),
                            items_view: ody_app_server_protocol::TurnItemsView::Full,
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                ody_app_server_protocol::ServerNotification::AgentMessageDelta(
                    ody_app_server_protocol::AgentMessageDeltaNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item_id: "item".to_string(),
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                ody_app_server_protocol::ServerNotification::ItemCompleted(
                    ody_app_server_protocol::ItemCompletedNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        completed_at_ms: 0,
                        item: ody_app_server_protocol::ThreadItem::AgentMessage {
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                ody_app_server_protocol::ServerNotification::ExternalAgentConfigImportCompleted(
                    ody_app_server_protocol::ExternalAgentConfigImportCompletedNotification {
        ...
        assert!(!event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                ody_app_server_protocol::ServerNotification::CommandExecutionOutputDelta(
                    ody_app_server_protocol::CommandExecutionOutputDeltaNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item_id: "item".to_string(),
    ...
    async fn runtime_start_args_use_remote_thread_config_loader_when_configured() {
        ...
        assert_eq!(
            err.code(),
            ody_config::ThreadConfigLoadErrorCode::RequestFailed
        );
    }

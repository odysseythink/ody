...
impl RemoteAppServerClient {
    ...
    pub fn ody_home(&self) -> Option<&str> {
        self.ody_home.as_deref()
    }
        ...
        S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        ...
        let (pending_events, server_version, ody_home) = initialize_remote_connection(
            &mut stream,
            &endpoint,
            initialize_params,
            INITIALIZE_TIMEOUT,
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
                    ...
                    match message {
                        JSONRPCMessage::Response(response) if response.id == initialize_request_id => {
                            ...
                            ody_home = response
                                ...
                                .get("odyHome")
                                .and_then(serde_json::Value::as_str)
                                .filter(|ody_home| !ody_home.is_empty())
                                .map(str::to_string);
                            break Ok(());
                        }
    ...
    Ok((pending_events, server_version, ody_home))
}

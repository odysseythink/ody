    ...
    fn apply_event(&mut self, event: RawTraceEvent) -> Result<()> {
        ...
        match event.payload {
            ...
            RawTraceEventPayload::CompactionInstalled {
                ...
                let Some(ody_turn_id) = event.ody_turn_id else {
                    bail!(
                        "compaction installed event {compaction_id} did not include a ody turn id"
                    );
                };

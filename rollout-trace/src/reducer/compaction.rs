...
impl TraceReducer {
    ...
    pub(super) fn start_compaction_request(
        ...
        let Some(turn) = self.rollout.ody_turns.get(&started.ody_turn_id) else {
            bail!(
                "compaction request {} referenced unknown ody turn {}",
                started.compaction_request_id,
                started.ody_turn_id
            );
        };
        if turn.thread_id != started.thread_id {
            bail!(
                "compaction request {} used thread {}, but ody turn {} belongs to {}",
                started.compaction_request_id,
                started.thread_id,
                started.ody_turn_id,
                turn.thread_id
        ...
        self.rollout.compaction_requests.insert(
            ...
            CompactionRequest {
                ...
                ody_turn_id: started.ody_turn_id,
    ...
    pub(super) fn reduce_compaction_installed_event(
        ...
        if turn.thread_id != thread_id {
            bail!(
                "compaction install {compaction_id} used thread {thread_id}, but ody turn {ody_turn_id} belongs to {}",
                turn.thread_id
            );
        }

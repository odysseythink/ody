...
impl TraceReducer {
    ...
    /// Starts a Ody turn inside an existing thread.
    pub(super) fn start_ody_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if self.rollout.ody_turns.contains_key(&ody_turn_id) {
            bail!("duplicate ody turn start for {ody_turn_id}");
        }

        self.thread_mut(&thread_id)?;

        self.rollout.ody_turns.insert(
            ody_turn_id.clone(),
            OdyTurn {
                ody_turn_id,
                thread_id,
    ...
    /// Marks a Ody turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_ody_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if let Some(event_thread_id) = thread_id.as_deref()
            && let Some(turn) = self.rollout.ody_turns.get(&ody_turn_id)
            && turn.thread_id != event_thread_id
        {
            bail!(
                "ody turn end for {ody_turn_id} used thread {event_thread_id}, \
                 but the turn belongs to {}",
                turn.thread_id
            );
        }

        let Some(turn) = self.rollout.ody_turns.get_mut(&ody_turn_id) else {
            bail!("ody turn end referenced unknown turn {ody_turn_id}");
        };

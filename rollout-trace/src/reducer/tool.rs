...
impl TraceReducer {
    ...
    pub(super) fn start_tool_call(
        ...
        let thread_id = self.tool_thread_id(thread_id, ody_turn_id.as_deref())?;
        self.validate_tool_turn(&thread_id, ody_turn_id.as_deref())?;
        ...
        self.rollout.tool_calls.insert(
            ...
            ToolCall {
                ...
                thread_id,
                started_by_ody_turn_id: ody_turn_id,
    ...
    fn tool_thread_id(
        ...
        let Some(ody_turn_id) = ody_turn_id else {
            bail!("tool call start did not include thread or Ody turn context");
        };
        self.rollout
            .ody_turns
            .get(ody_turn_id)
    ...
    fn validate_tool_turn(&self, thread_id: &str, ody_turn_id: Option<&str>) -> Result<()> {
        ...
        if let Some(ody_turn_id) = ody_turn_id {
            let Some(turn) = self.rollout.ody_turns.get(ody_turn_id) else {
                bail!("tool call start referenced unknown Ody turn {ody_turn_id}");
            };
            if turn.thread_id != thread_id {
                bail!(
                    "tool call start used thread {thread_id}, but Ody turn {ody_turn_id} \
                     belongs to {}",
                    turn.thread_id
                );
            }

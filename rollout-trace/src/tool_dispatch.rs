...
use ody_protocol::models::SearchToolCallParams;
use serde::Serialize;
...
/// Core-facing request data for the canonical Ody tool boundary.
pub struct ToolDispatchInvocation {
    pub thread_id: AgentThreadId,
    pub ody_turn_id: OdyTurnId,
...
/// Raw invocation payload for the canonical Ody tool boundary.
#[derive(Serialize)]
...
impl ToolDispatchTraceContext {
    ...
    pub(crate) fn start(writer: Arc<TraceWriter>, invocation: ToolDispatchInvocation) -> Self {
        ...
        let context = EnabledToolDispatchTraceContext {
            ...
            ody_turn_id: invocation.ody_turn_id.clone(),
...
fn suppresses_tool_dispatch_trace(invocation: &ToolDispatchInvocation) -> bool {
    matches!(invocation.payload, ToolDispatchPayload::Custom { .. })
        ...
        && invocation.tool_name == ody_code_mode::PUBLIC_TOOL_NAME
}

//! Mapping from Ody protocol events into raw rollout-trace events.
//!
...
//! `ody-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `ody-rollout-trace`.
//!
...
use ody_protocol::protocol::PatchApplyStatus;
...
pub(crate) struct OdyTurnTraceEvent {
    pub context_turn_id: OdyTurnId,
    pub payload: RawTraceEventPayload,
}

pub(crate) fn ody_turn_trace_event(
    thread_id: AgentThreadId,
    default_turn_id: &str,
    event: &EventMsg,
) -> Option<OdyTurnTraceEvent> {
    match event {
        EventMsg::TurnStarted(event) => {
            let ody_turn_id = event.turn_id.clone();
            Some(OdyTurnTraceEvent {
                context_turn_id: ody_turn_id.clone(),
                payload: RawTraceEventPayload::OdyTurnStarted {
                    ody_turn_id,
                    thread_id,
                },
            })
        ...
        EventMsg::TurnComplete(event) => {
            ...
            Some(OdyTurnTraceEvent {
                context_turn_id: ody_turn_id.clone(),
                payload: RawTraceEventPayload::OdyTurnEnded {
                    ody_turn_id,
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a ody_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a ody_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(&'a ody_protocol::protocol::CollabAgentInteractionBeginEvent),
    CollabAgentInteractionEnd(&'a ody_protocol::protocol::CollabAgentInteractionEndEvent),
    CollabWaitingBegin(&'a ody_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a ody_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a ody_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a ody_protocol::protocol::CollabCloseEndEvent),
...
struct ExecCommandBeginTracePayload<'a> {
    ...
    command: &'a [String],
    cwd: String,
    parsed_cmd: &'a [ody_protocol::parse_command::ParsedCommand],

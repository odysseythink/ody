...
const INFERENCE_CALL_ID_HEADER: &str = "x-ody-inference-call-id";
...
///
/// A Ody turn can create multiple attempts when auth recovery retries the
...
impl InferenceTraceContext {
    ...
    /// Builds an enabled context for all upstream attempts made by one Ody turn.
    pub fn enabled(
        writer: Arc<TraceWriter>,
        thread_id: AgentThreadId,
        ody_turn_id: OdyTurnId,
...
impl InferenceTraceAttempt {
    ...
    pub fn record_started(&self, request: &impl Serialize) {
        ...
        append_with_context_best_effort(
            ...
            RawTraceEventPayload::InferenceStarted {
                ...
                ody_turn_id: attempt.context.ody_turn_id.clone(),
    ...
    /// trace-specific serialization rules. That keeps ody-core focused on
    ...
    /// Records a provider stream that Ody intentionally stopped consuming.
    ///
...
/// Ody received it, so this helper restores that content in the raw payload.
...
fn append_with_context_best_effort(
    ...
    let event_context = RawTraceEventContext {
        ...
        ody_turn_id: Some(context.ody_turn_id.clone()),
    };
...
mod tests {
    ...
    fn enabled_context_records_replayable_inference_attempt() -> anyhow::Result<()> {
        ...
        assert_eq!(inference.ody_turn_id, "turn-1");

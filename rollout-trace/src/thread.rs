...
use crate::protocol_event::ody_turn_trace_event;
...
pub const ODY_ROLLOUT_TRACE_ROOT_ENV: &str = "ODY_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `ODY_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    /// Trace startup is best-effort. A tracing failure must not make the Ody
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(ODY_ROLLOUT_TRACE_ROOT_ENV) else {
    ...
    /// Emits typed Ody turn lifecycle events from protocol lifecycle events.
    pub fn record_ody_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            ody_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    /// what Ody did while executing that boundary.
    pub fn record_tool_call_event(&self, ody_turn_id: impl Into<OdyTurnId>, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        ...
        context.append_with_context_best_effort(ody_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_ody_turn_id: impl Into<OdyTurnId>,
    ...
    pub fn record_ody_turn_started(&self, ody_turn_id: impl Into<OdyTurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let ody_turn_id = ody_turn_id.into();
        context.append_with_context_best_effort(
            ody_turn_id.clone(),
            RawTraceEventPayload::OdyTurnStarted {
                ody_turn_id,
                thread_id: context.thread_id.clone(),
            },
        );
    ...
    /// Builds reusable inference trace context for one Ody turn.
    ///

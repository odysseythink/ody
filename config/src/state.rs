...
use ody_app_server_protocol::ConfigLayer;
use ody_app_server_protocol::ConfigLayerMetadata;
...
impl LoaderOverrides {
    ...
    pub fn without_managed_config_for_tests() -> Self {
        let base = std::env::temp_dir().join("ody-config-tests");
        Self {
    ...
    pub fn user_config_path(&self, ody_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
impl ConfigLayerEntry {
    ...
    // Get the `.ody/` folder associated with this config layer, if any.
    pub fn config_folder(&self) -> Option<AbsolutePathBuf> {
        match &self.name {
            ...
            ConfigLayerSource::Project { dot_ody_folder } => Some(dot_ody_folder.clone()),
    ...
    /// Returns the `.ody/` folder that should be used for hook declarations.
    ///
...
pub struct ConfigLayerStack {
    ...
    /// the base `$ODY_HOME/config.toml` layer followed by the profile override
...
impl ConfigLayerStack {
    ...
    /// the base `$ODY_HOME/config.toml` layer because the active layer is the
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<Option<usize>> {
    ...
    let mut previous_project_dot_ody_folder: Option<&AbsolutePathBuf> = None;
    for (index, layer) in layers.iter().enumerate() {
        ...
        if let ConfigLayerSource::Project {
            dot_ody_folder: current_project_dot_ody_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_ody_folder {
                ...
                if previous == current_project_dot_ody_folder
                    || !current_project_dot_ody_folder
                        .as_path()
                        .ancestors()
            ...
            previous_project_dot_ody_folder = Some(current_project_dot_ody_folder);
        }
    }

...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/ody/config.toml";
...
///
/// - system    `/etc/ody/requirements.toml` (Unix) or
///   `%ProgramData%\OpenAI\Ody\requirements.toml` (Windows)
...
///
/// - admin:    managed preferences (*)
/// - system    `/etc/ody/config.toml` (Unix) or
///   `%ProgramData%\OpenAI\Ody\config.toml` (Windows)
/// - cloud     enterprise-managed cloud config bundle fragments
/// - user      `${ODY_HOME}/config.toml`
/// - profile   `${ODY_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.ody/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.ody/config.toml` (loaded but disabled when untrusted)
/// - runtime   e.g., --config flags, model selector in UI
///
/// (*) Only available on macOS via managed device profiles.
///
/// See https://developers.odysseythink.com/ody/security for details.
///
...
pub async fn load_config_layers_state(
    ...
    if !ignore_managed_requirements {
        if let Some(bundle) = cloud_config_bundle.get().await.map_err(io::Error::other)? {
            let cloud_config_base_dir = AbsolutePathBuf::from_absolute_path(ody_home)?;
    ...
    let loaded_config_layers =
        layer_io::load_config_layers_internal(fs, ody_home, overrides.clone(), strict_config)
            .await?;
    if !ignore_managed_requirements {
    ...
    // contain overrides.
    let active_user_file = overrides.user_config_path(ody_home)?;
    let base_user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, ody_home);
        ...
        && let Some(base_user_config) = base_user_layer.config.as_table()
    {
        ...
        if legacy_profile_is_selected || legacy_profile_table_exists {
            return Err(io::Error::new(
                ...
                format!(
                    "--profile `{active_user_profile}` cannot be used while {} contains legacy `profile = \"{active_user_profile}\"` or `[profiles.{active_user_profile}]` config; move those settings into {} and remove the legacy profile selector/table. See https://developers.odysseythink.com/ody/config-advanced#profiles for more information.",
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        let raw_toml_base_dir = AbsolutePathBuf::from_absolute_path(ody_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/ody/requirements.toml"))
}
...
#[cfg(windows)]
fn windows_ody_system_dir() -> PathBuf {
...
fn legacy_requirements_to_toml_value(legacy: LegacyManagedConfigToml) -> io::Result<TomlValue> {
    ...
    if let Some(sandbox_mode) = sandbox_mode {
        ...
        // Allowing read-only is a requirement for Ody to function correctly.
...
fn project_ignored_config_keys_warning(
    ...
    let config_path = dot_ody_folder.join(CONFIG_TOML_FILE);
...
async fn load_project_layers(
    ...
    let ody_home_abs = AbsolutePathBuf::from_absolute_path(ody_home)?;
    let ody_home_normalized =
        normalize_path(ody_home_abs.as_path()).unwrap_or_else(|_| ody_home_abs.to_path_buf());
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
    ...
    for dir in dirs {
        ...
        let dot_ody_normalized =
            normalize_path(dot_ody_abs.as_path()).unwrap_or_else(|_| dot_ody_abs.to_path_buf());
        if dot_ody_abs == ody_home_abs || dot_ody_normalized == ody_home_normalized {
            continue;
        }
        let config_file = dot_ody_abs.join(CONFIG_TOML_FILE);
...
/// from a file like `/etc/ody/managed_config.toml` that has the same
    ...
    fn windows_system_requirements_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_requirements_toml_file()
                .expect("requirements.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Ody").join("requirements.toml"))
        );
    }
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Ody").join("config.toml"))
        );
    }
}

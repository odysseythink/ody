...
/// OAuth client settings used when Ody launches an MCP OAuth flow.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema)]
#[schemars(deny_unknown_fields)]
...
pub struct McpServerConfig {
    ...
    /// Effective environment id for where Ody should start this MCP server.
    pub environment_id: String,

    /// When `false`, Ody skips initializing this MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,

    /// When `true`, `ody exec` exits with an error if this MCP server fails to initialize.

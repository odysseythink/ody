...
pub fn set_user_agent_suffix(suffix: &str) {
    if let Ok(mut guard) = ody_login::default_client::USER_AGENT_SUFFIX.lock() {
...
pub async fn build_chatgpt_headers() -> HeaderMap {
    ...
    set_user_agent_suffix("ody_cloud_tasks_tui");
    let ua = ody_login::default_client::get_ody_user_agent();
    let mut headers = HeaderMap::new();
    headers.insert(
        USER_AGENT,
        HeaderValue::from_str(&ua).unwrap_or(HeaderValue::from_static("ody-cli")),
    );
    if let Some(am) = load_auth_manager(/*chatgpt_base_url*/ None).await
        && let Some(auth) = am.auth().await
        && auth.uses_ody_backend()
    {
        headers.extend(ody_model_provider::auth_provider_from_auth(&auth).to_auth_headers());
    }
    headers
}
...
pub fn task_url(base_url: &str, task_id: &str) -> String {
    ...
    if let Some(root) = normalized.strip_suffix("/backend-api") {
        return format!("{root}/ody/tasks/{task_id}");
    }
    if let Some(root) = normalized.strip_suffix("/api/ody") {
        return format!("{root}/ody/tasks/{task_id}");
    }
    if normalized.ends_with("/ody") {
        return format!("{normalized}/tasks/{task_id}");
    }
    format!("{normalized}/ody/tasks/{task_id}")
}

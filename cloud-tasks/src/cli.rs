...
pub enum Command {
    /// Submit a new Ody Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Ody Cloud task.
    Status(StatusCommand),
    /// List Ody Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Ody Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Ody Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Ody Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `ody cloud` to browse).
    ...
    /// Git branch to run in Ody Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Ody Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Ody Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Ody Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,

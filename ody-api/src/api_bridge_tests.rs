...
fn map_api_error_maps_server_overloaded() {
    ...
    assert!(matches!(err, OdyErr::ServerOverloaded));
}
...
fn map_api_error_maps_cloudflare_blocked_response_to_user_message() {
    ...
    let OdyErr::UnexpectedStatus(err) = err else {
        panic!("expected OdyErr::UnexpectedStatus, got {err:?}");
    };
    assert_eq!(
...
fn map_api_error_maps_cyber_policy_from_400_body() {
    ...
    let OdyErr::CyberPolicy { message } = err else {
        panic!("expected OdyErr::CyberPolicy, got {err:?}");
    };
    assert_eq!(
        message,
...
fn map_api_error_keeps_unknown_400_errors_generic() {
    ...
    let OdyErr::InvalidRequest(message) = err else {
        panic!("expected OdyErr::InvalidRequest, got {err:?}");
    };
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    headers.insert(
        ACTIVE_LIMIT_HEADER,
        http::HeaderValue::from_static("ody_other"),
    );
    headers.insert(
        "x-ody-other-limit-name",
        http::HeaderValue::from_static("ody_other"),
    );
    let body = serde_json::json!({
        "error": {
    ...
    let OdyErr::UsageLimitReached(usage_limit) = err else {
        panic!("expected OdyErr::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits
...
fn map_api_error_ignores_unparseable_rate_limit_reached_type_headers() {
    ...
    for value in values {
        let mut headers = HeaderMap::new();
        headers.insert("x-ody-rate-limit-reached-type", value);
        let body = serde_json::json!({
            "error": {
                "type": "usage_limit_reached",
                "plan_type": "pro",

...
const X_ODY_TURN_STATE_HEADER: &str = "x-ody-turn-state";
const OPENAI_MODEL_HEADER: &str = "odysseythink-model";
...
impl ResponsesStreamEvent {
    ...
    pub fn response_model(&self) -> Option<String> {
        let response_headers_model = self
            ...
            .and_then(header_odysseythink_model_value_from_json);
    ...
    pub(crate) fn model_verifications(&self) -> Option<Vec<ModelVerification>> {
        ...
        self.metadata
            .as_ref()
            .and_then(|metadata| metadata.get("odysseythink_verification_recommendation"))
...
fn header_turn_state_value_from_json(value: &Value) -> Option<String> {
    let headers = value.as_object()?;
    headers.iter().find_map(|(name, value)| {
        if name.eq_ignore_ascii_case(X_ODY_TURN_STATE_HEADER) {
            json_value_as_string(value)
        } else {
            None
        }
...
mod tests {
    ...
    async fn error_when_error_event() {
        let raw_error = r#"{"type":"response.failed","sequence_number":3,"response":{"id":"resp_689bcf18d7f08194bf3440ba62fe05d803fee0cdac429894","object":"response","created_at":1755041560,"status":"failed","background":false,"error":{"code":"rate_limit_exceeded","message":"Rate limit reached for k3 in organization org-AAA on tokens per min (TPM): Limit 30000, Used 22999, Requested 12528. Please try again in 11.054s. Visit https://platform.odysseythink.com/account/rate-limits to learn more."}, "usage":null,"user":null,"metadata":{}}}"#;
        ...
        match &events[0] {
            Err(ApiError::Retryable { message, delay }) => {
                assert_eq!(
                    message,
                    "Rate limit reached for k3 in organization org-AAA on tokens per min (TPM): Limit 30000, Used 22999, Requested 12528. Please try again in 11.054s. Visit https://platform.odysseythink.com/account/rate-limits to learn more."
                );
    ...
    async fn spawn_response_stream_ignores_model_verification_header() {
        let mut headers = HeaderMap::new();
        headers.insert(
            "odysseythink-verification-recommendation",
    ...
    async fn process_sse_emits_model_verification_field() {
        let events = run_sse(vec![
            json!({
                ...
                "response_id": "resp-1",
                "metadata": {
                    "odysseythink_verification_recommendation": [TRUSTED_ACCESS_FOR_CYBER_VERIFICATION]
                }
            }),
            json!({
                "type": "response.completed",
    ...
    fn responses_stream_event_response_model_prefers_response_headers() {
        let ev: ResponsesStreamEvent = serde_json::from_value(json!({
            "type": "response.created",
            "headers": {
                "odysseythink-model": "top-level-model"
            },
            "response": {
                "id": "resp-1",
                "headers": {
    ...
    fn responses_stream_event_model_verification_ignores_unknown_field() {
        let event = json!({
            "type": "response.metadata",
            "metadata": {
                "odysseythink_verification_recommendation": ["unknown"]
            }
        });
        let event: ResponsesStreamEvent =
            serde_json::from_value(event).expect("expected event to deserialize");
    ...
    fn test_try_parse_retry_after() {
        let err = Error {
            r#type: None,
            message: Some("Rate limit reached for k3 in organization org- on tokens per min (TPM): Limit 1, Used 1, Requested 19304. Please try again in 28ms. Visit https://platform.odysseythink.com/account/rate-limits to learn more.".to_string()),
    ...
    fn test_try_parse_retry_after_no_delay() {
        let err = Error {
            r#type: None,
            message: Some("Rate limit reached for k3 in organization <ORG> on tokens per min (TPM): Limit 30000, Used 6899, Requested 24050. Please try again in 1.898s. Visit https://platform.odysseythink.com/account/rate-limits to learn more.".to_string()),
            code: Some("rate_limit_exceeded".to_string()),
            plan_type: None,
            resets_at: None,
        };
    ...
    const CYBER_RESTRICTED_MODEL_FOR_TESTS: &str = "k3-ody";
}

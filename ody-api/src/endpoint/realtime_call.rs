...
const MULTIPART_BOUNDARY: &str = "ody-realtime-call-boundary";
const MULTIPART_CONTENT_TYPE: &str = "multipart/form-data; boundary=ody-realtime-call-boundary";
...
impl<T: HttpTransport> RealtimeCallClient<T> {
    ...
    pub async fn create_with_session_and_headers(
        ...
        trace!(target: "ody_api::realtime_websocket::wire", "realtime call request SDP: {sdp}");
...
mod tests {
    ...
    async fn sends_api_session_call_as_multipart_body() {
        ...
        assert_eq!(
            request.url,
            "https://api.odysseythink.com/v1/realtime/calls?intent=quicksilver&architecture=avas"
        );
        assert_eq!(
            request.headers.get(CONTENT_TYPE).unwrap(),
            HeaderValue::from_static(MULTIPART_CONTENT_TYPE)

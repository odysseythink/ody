...
const REALTIME_WIRE_LOG_TARGET: &str = "ody_api::realtime_websocket::wire";
...
impl RealtimeWebsocketClient {
    ...
    async fn connect_realtime_websocket_url(
        ...
        // Realtime websocket TLS should honor the same custom-CA env vars as the rest of Ody's
...
mod tests {
    ...
    use ody_protocol::protocol::RealtimeResponseCancelled;
    use ody_protocol::protocol::RealtimeResponseCreated;
    use ody_protocol::protocol::RealtimeResponseDone;
    use ody_protocol::protocol::RealtimeTranscriptDelta;
    ...
    fn websocket_url_from_v1_base_appends_realtime_path() {
        ...
        assert_eq!(
            url.as_str(),
            "wss://api.odysseythink.com/v1/realtime?intent=quicksilver&model=snapshot"
        );
    }
    ...
    fn websocket_url_from_nested_v1_base_appends_realtime_path() {
        let url = websocket_url_from_api_url(
            "https://example.com/odysseythink/v1",
            /*query_params*/ None,
            Some("snapshot"),
            RealtimeEventParser::V1,
            RealtimeSessionMode::Conversational,
        ...
        assert_eq!(
            url.as_str(),
            "wss://example.com/odysseythink/v1/realtime?intent=quicksilver&model=snapshot"
        );
    }
    ...
    fn websocket_url_for_call_id_joins_existing_realtime_session() {
        ...
        assert_eq!(
            url.as_str(),
            "wss://api.odysseythink.com/v1/realtime?call_id=rtc_test"
        );
    }

...
pub fn map_api_error(err: ApiError) -> OdyErr {
    match err {
        ApiError::ContextWindowExceeded => OdyErr::ContextWindowExceeded,
        ApiError::QuotaExceeded => OdyErr::QuotaExceeded,
        ApiError::UsageNotIncluded => OdyErr::UsageNotIncluded,
        ApiError::Retryable { message, delay } => OdyErr::Stream(message, delay),
        ApiError::Stream(msg) => OdyErr::Stream(msg, None),
        ApiError::ServerOverloaded => OdyErr::ServerOverloaded,
        ...
        ApiError::InvalidRequest { message } => OdyErr::InvalidRequest(message),
        ApiError::CyberPolicy { message } => OdyErr::CyberPolicy { message },
        ApiError::Transport(transport) => match transport {
            TransportError::Http {
                ...
                {
                    return OdyErr::ServerOverloaded;
                }
            ...
            TransportError::RetryLimit => OdyErr::RetryLimit(RetryLimitReachedError {
...
const ACTIVE_LIMIT_HEADER: &str = "x-ody-active-limit";
const REQUEST_ID_HEADER: &str = "x-request-id";
const OAI_REQUEST_ID_HEADER: &str = "x-oai-request-id";
const CF_RAY_HEADER: &str = "cf-ray";
const X_OPENAI_AUTHORIZATION_ERROR_HEADER: &str = "x-odysseythink-authorization-error";

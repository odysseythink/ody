...
use crate::events::OdyHookRunEventRequest;
use crate::events::OdyOnboardingExternalAgentImportFailureEventRequest;
use crate::events::OdyOnboardingExternalAgentImportFailureMetadata;
use crate::events::OdyPluginEventRequest;
use crate::events::OdyPluginInstallFailedEventRequest;
use crate::events::OdyPluginInstallFailedMetadata;
use crate::events::OdyPluginUsedEventRequest;
use crate::events::OdyReviewEventParams;
use crate::events::OdyReviewEventRequest;
...
use crate::events::ody_hook_run_metadata;
use crate::events::ody_plugin_metadata;
use crate::events::ody_plugin_used_metadata;
...
use crate::facts::TurnOdyErrorFact;
...
use ody_app_server_protocol::GuardianApprovalReview;
use ody_app_server_protocol::GuardianApprovalReviewAction;
use ody_app_server_protocol::GuardianApprovalReviewStatus;
use ody_app_server_protocol::GuardianCommandSource as AppServerGuardianCommandSource;
...
use ody_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification;
...
use ody_app_server_protocol::RequestPermissionProfile;
...
use ody_app_server_protocol::SubAgentActivityKind;
...
use ody_app_server_protocol::ThreadSource as AppServerThreadSource;
...
use ody_app_server_protocol::TurnDiffUpdatedNotification;
use ody_app_server_protocol::TurnError as AppServerTurnError;
...
use ody_client::default_client::DEFAULT_ORIGINATOR;
...
use ody_protocol::models::PermissionProfile as CorePermissionProfile;
...
use ody_protocol::request_permissions::PermissionGrantScope as CorePermissionGrantScope;
use ody_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use ody_protocol::request_permissions::RequestPermissionsResponse as CoreRequestPermissionsResponse;
...
fn sample_app_server_client_metadata() -> OdyAppServerClientMetadata {
    OdyAppServerClientMetadata {
...
fn sample_runtime_metadata() -> OdyRuntimeMetadata {
    OdyRuntimeMetadata {
        ody_rs_version: "0.1.0".to_string(),
...
fn sample_turn_start_response(turn_id: &str) -> ClientResponsePayload {
    ClientResponsePayload::TurnStart(ody_app_server_protocol::TurnStartResponse {
        turn: Turn {
...
fn sample_turn_completed_notification(
    ...
    ody_error_info: Option<ody_app_server_protocol::OdyErrorInfo>,
) -> ServerNotification {
    ServerNotification::TurnCompleted(TurnCompletedNotification {
        ...
        turn: Turn {
            ...
            items: vec![],
            status,
            error: ody_error_info.map(|ody_error_info| AppServerTurnError {
                message: "turn failed".to_string(),
                ody_error_info: Some(ody_error_info),
    ...
    JSONRPCErrorError {
        ...
        data: Some(
            serde_json::to_value(AppServerTurnError {
                message: "cannot steer a review turn".to_string(),
                ody_error_info: Some(OdyErrorInfo::ActiveTurnNotSteerable {
            ...
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "ody-web".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "ody-web".to_string(),
...
fn sample_initialize_fact(connection_id: u64) -> AnalyticsFact {
    AnalyticsFact::Initialize {
        ...
        runtime: OdyRuntimeMetadata {
            ody_rs_version: "0.99.0".to_string(),
    ...
    ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: RequestPermissionProfile {
                network: Some(ody_app_server_protocol::AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: None,
            },
...
fn sample_guardian_review_completed(
    ...
    ServerNotification::ItemGuardianApprovalReviewCompleted(
        ItemGuardianApprovalReviewCompletedNotification {
            ...
            decision_source: ody_app_server_protocol::AutoReviewDecisionSource::Agent,
...
fn normalize_path_for_skill_id_repo_scoped_uses_relative_path() {
    ...
    let skill_path = PathBuf::from("/repo/root/.ody/skills/doc/SKILL.md");
    ...
    assert_eq!(path, ".ody/skills/doc/SKILL.md");
}

#[test]
fn normalize_path_for_skill_id_user_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/Users/abc/.ody/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_admin_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/etc/ody/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_repo_root_not_in_skill_path_uses_absolute_path() {
    let repo_root = PathBuf::from("/repo/root");
    let skill_path = PathBuf::from("/other/path/.ody/skills/doc/SKILL.md");
...
fn app_mentioned_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppMentioned(OdyAppMentionedEventRequest {
        event_type: "ody_app_mentioned",
        event_params: ody_app_metadata(
            &tracking,
            AppInvocation {
...
fn app_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppUsed(OdyAppUsedEventRequest {
        event_type: "ody_app_used",
        event_params: ody_app_metadata(
            &tracking,
            AppInvocation {
...
fn compaction_event_serializes_expected_shape() {
    let event = TrackEventRequest::Compaction(Box::new(OdyCompactionEventRequest {
        event_type: "ody_compaction_event",
        event_params: crate::events::ody_compaction_event_params(
            OdyCompactionEvent {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
                ...
                ody_error_kind: None,
                ody_error_http_status_code: None,
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "status": "completed",
                "ody_error_kind": null,
                "ody_error_http_status_code": null,
...
fn thread_initialized_event_serializes_expected_shape() {
    let event = TrackEventRequest::ThreadInitialized(ThreadInitializedEvent {
        event_type: "ody_thread_initialized",
        event_params: ThreadInitializedEventParams {
            thread_id: "thread-0".to_string(),
            session_id: "session-thread-0".to_string(),
            app_server_client: OdyAppServerClientMetadata {
                product_client_id: DEFAULT_ORIGINATOR.to_string(),
                client_name: Some("ody-tui".to_string()),
                client_version: Some("1.0.0".to_string()),
                rpc_transport: AppServerRpcTransport::Stdio,
...
fn command_execution_event_serializes_expected_shape() {
    let event = TrackEventRequest::CommandExecution(OdyCommandExecutionEventRequest {
        event_type: "ody_command_execution_event",
        event_params: OdyCommandExecutionEventParams {
            base: OdyToolItemEventBase {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
                item_id: "item-1".to_string(),
                app_server_client: OdyAppServerClientMetadata {
                    product_client_id: "ody_tui".to_string(),
                    client_name: Some("ody-tui".to_string()),
...
fn review_event_serializes_expected_shape() {
    let event = TrackEventRequest::ReviewEvent(OdyReviewEventRequest {
        event_type: "ody_review_event",
        event_params: OdyReviewEventParams {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            item_id: None,
...
async fn initialize_caches_client_and_thread_lifecycle_publishes_once_initialized() {
    ...
    assert_eq!(payload[0]["event_type"], "ody_thread_initialized");
    ...
    assert_eq!(
        payload[0]["event_params"]["runtime"]["ody_rs_version"],
        "0.99.0"
    );
...
async fn compaction_event_ingests_custom_fact() {
    ...
    let parent_thread_id =
        ody_protocol::ThreadId::from_string("22222222-2222-2222-2222-222222222222")
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::Compaction(Box::new(
                OdyCompactionEvent {
                    ...
                    ody_error_kind: Some(OdyErrKind::ContextWindowExceeded),
                    ody_error_http_status_code: None,
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "ody_compaction_event");
    assert_eq!(payload[0]["event_params"]["session_id"], "session-thread-1");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["turn_id"], "turn-compact");
    assert_eq!(
        payload[0]["event_params"]["ody_error_kind"],
        json!("context_window_exceeded")
    );
    assert_eq!(
        payload[0]["event_params"]["ody_error_http_status_code"],
        json!(null)
    );
    assert_eq!(
        payload[0]["event_params"]["app_server_client"]["product_client_id"],
...
async fn guardian_review_event_ingests_custom_fact_with_optional_target_item() {
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::GuardianReview(Box::new(
                GuardianReviewEventParams {
                    ...
                    guardian_default_review_model_id: Some("ody-auto-review".to_string()),
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "ody_guardian_review");
    assert_eq!(
...
async fn item_lifecycle_notifications_publish_command_execution_event() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "ody_command_execution_event");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
...
async fn command_execution_approval_response_publishes_user_review_event() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "ody_review_event");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["turn_id"], "turn-1");
    assert_eq!(payload[0]["event_params"]["item_id"], "item-1");
...
async fn guardian_completed_notification_publishes_review_event_with_thread_metadata() {
    ...
    assert_eq!(payload["event_type"], "ody_review_event");
...
fn subagent_thread_started_thread_spawn_serializes_thread_lineage() {
    let parent_thread_id =
        ody_protocol::ThreadId::from_string("11111111-1111-1111-1111-111111111111")
            .expect("valid thread id");
    let forked_from_thread_id =
        ody_protocol::ThreadId::from_string("22222222-2222-4222-8222-222222222222")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
fn subagent_thread_started_other_serializes_explicit_parent_thread_id() {
    let parent_thread_id =
        ody_protocol::ThreadId::from_string("33333333-3333-4333-8333-333333333333")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
async fn subagent_events_use_inherited_connection_unless_turn_connection_is_explicit() {
    ...
    let mut events = Vec::new();
    let parent_thread_id =
        ody_protocol::ThreadId::from_string("44444444-4444-4444-4444-444444444444")
            .expect("valid parent thread id");
    ...
    assert_eq!(params.runtime.ody_rs_version, "0.1.0");
...
fn plugin_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::PluginUsed(OdyPluginUsedEventRequest {
        event_type: "ody_plugin_used",
        event_params: ody_plugin_used_metadata(&tracking, sample_plugin_metadata()),
    });
...
fn plugin_management_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstalled(OdyPluginEventRequest {
        event_type: "ody_plugin_installed",
        event_params: ody_plugin_metadata(sample_plugin_metadata()),
    });
...
fn plugin_install_failed_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstallFailed(OdyPluginInstallFailedEventRequest {
        event_type: "ody_plugin_install_failed",
        event_params: OdyPluginInstallFailedMetadata {
            plugin: ody_plugin_metadata(sample_plugin_metadata()),
...
fn plugin_management_event_can_use_remote_plugin_id_override() {
    ...
    let event = TrackEventRequest::PluginInstalled(OdyPluginEventRequest {
        event_type: "ody_plugin_installed",
        event_params: ody_plugin_metadata(plugin),
    });
...
fn hook_run_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::HookRun(OdyHookRunEventRequest {
        event_type: "ody_hook_run",
        event_params: ody_hook_run_metadata(
            &tracking,
            HookRunFact {
...
fn hook_run_metadata_maps_sources_and_statuses() {
    ...
    let system = serde_json::to_value(ody_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let project = serde_json::to_value(ody_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let cloud_requirements = serde_json::to_value(ody_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
    ...
    let unknown = serde_json::to_value(ody_hook_run_metadata(
        &tracking,
        HookRunFact {
...
fn hook_run_metadata_maps_stopped_status() {
    ...
    let stopped = serde_json::to_value(ody_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
            hook_source: HookSource::User,
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::SkillInvoked(SkillInvokedInput {
                ...
                invocations: vec![SkillInvocation {
                    ...
                    skill_scope: ody_protocol::protocol::SkillScope::User,
                    skill_path,
                    plugin_id: None,
...
async fn reducer_ingests_hook_run_fact() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "ody_hook_run");
...
async fn reducer_ingests_app_and_plugin_facts() {
    ...
    assert_eq!(payload[0]["event_type"], "ody_app_mentioned");
    assert_eq!(payload[1]["event_type"], "ody_app_used");
    assert_eq!(payload[2]["event_type"], "ody_plugin_used");
}
...
async fn reducer_ingests_plugin_state_changed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "ody_plugin_disabled",
            "event_params": {
                "plugin_id": "sample@test",
                "plugin_name": "sample",
                "marketplace_name": "test",
...
async fn reducer_ingests_plugin_install_failed_fact_without_detail() {
    ...
    let plugin = PluginTelemetryMetadata {
        plugin_id: PluginId::parse("unknown@odysseythink-curated-remote").expect("valid plugin id"),
    ...
    assert_eq!(
        ...
        json!([{
            ...
            "event_params": {
                ...
                "plugin_name": "unknown",
                "marketplace_name": "odysseythink-curated-remote",
                "has_skills": null,
...
async fn reducer_ingests_external_agent_config_import_completed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "ody_onboarding_external_agent_import_complete",
            "event_params": {
                "import_id": "import-1",
                "source": "app_server",
                "type": "PLUGINS",
...
fn external_agent_config_import_failure_event_serializes_expected_shape() {
    let event = TrackEventRequest::ExternalAgentConfigImportFailure(
        OdyOnboardingExternalAgentImportFailureEventRequest {
            event_type: "ody_onboarding_external_agent_import_failure",
            event_params: OdyOnboardingExternalAgentImportFailureMetadata {
                import_id: "import-1".to_string(),
                source: "app_server".to_string(),
...
fn turn_event_serializes_expected_shape() {
    let event = TrackEventRequest::TurnEvent(Box::new(OdyTurnEventRequest {
        event_type: "ody_turn_event",
        event_params: crate::events::OdyTurnEventParams {
            thread_id: "thread-2".to_string(),
...
async fn accepted_turn_steer_emits_expected_event() {
    ...
    assert_eq!(payload["event_type"], json!("ody_turn_steer_event"));
    ...
    assert_eq!(
        payload["event_params"]["runtime"]["ody_rs_version"],
        json!("0.1.0")
    );
...
async fn turn_lifecycle_emits_turn_event() {
    ...
    assert_eq!(payload["event_type"], json!("ody_turn_event"));
    assert_eq!(payload["event_params"]["thread_id"], json!("thread-2"));
    assert_eq!(
        payload["event_params"]["session_id"],
        json!("session-thread-2")
...
async fn turn_lifecycle_emits_failed_turn_event() {
    ...
    reducer
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::TurnOdyError(Box::new(
                TurnOdyErrorFact::from_ody_err(
                    "thread-2".to_string(),
                    "turn-2".to_string(),
                    &OdyErr::InvalidRequest("unknown turn environment id `env-2`".to_string()),
                ),
            ))),
            &mut out,
        )
    ...
    reducer
        .ingest(
            AnalyticsFact::Notification(Box::new(sample_turn_completed_notification(
                ...
                Some(ody_app_server_protocol::OdyErrorInfo::BadRequest),
            ))),
            &mut out,
        )
        .await;
    ...
    assert_eq!(
        payload["event_params"]["ody_error_kind"],
        json!("invalid_request")
    );
    assert_eq!(
        payload["event_params"]["ody_error_http_status_code"],
        json!(null)
    );
}
...
async fn turn_lifecycle_emits_interrupted_turn_event_without_error() {
    ...
    assert_eq!(payload["event_params"]["ody_error_kind"], json!(null));
}

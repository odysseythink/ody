...
#[derive(Parser, Debug)]
#[command(about = "Generate TypeScript bindings and JSON Schemas for the Ody app-server protocol")]
struct Args {
...
fn main() -> Result<()> {
    ...
    ody_app_server_protocol::generate_ts_with_options(
        ...
        ody_app_server_protocol::GenerateTsOptions {
            experimental_api: args.experimental,
            ..ody_app_server_protocol::GenerateTsOptions::default()
        },
    )?;
    ody_app_server_protocol::generate_json_with_experimental(&args.out_dir, args.experimental)
}

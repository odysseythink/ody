use super::*;
use ody_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
use ody_protocol::config_types::MultiAgentMode;
use ody_protocol::items::AgentMessageContent;
use ody_protocol::items::AgentMessageItem;
use ody_protocol::items::FileChangeItem;
use ody_protocol::items::ImageViewItem;
use ody_protocol::items::McpToolCallItem;
use ody_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
...
use ody_protocol::permissions::FileSystemSpecialPath as CoreFileSystemSpecialPath;
use ody_protocol::protocol::AgentStatus as CoreAgentStatus;
use ody_protocol::protocol::AskForApproval as CoreAskForApproval;
use ody_protocol::protocol::ConversationTextRole;
use ody_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use ody_protocol::protocol::NetworkAccess as CoreNetworkAccess;
use ody_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use ody_protocol::user_input::UserInput as CoreUserInput;
...
fn thread_sources_round_trip_as_scalar_labels() {
    for (source, label) in [
        ...
        let core_source: ody_protocol::protocol::ThreadSource = source.clone().into();
...
fn sandbox_policy_round_trips_external_sandbox_network_access() {
    ...
    assert_eq!(
        core_policy,
        ody_protocol::protocol::SandboxPolicy::ExternalSandbox {
...
fn sandbox_policy_round_trips_read_only_network_access() {
    ...
    assert_eq!(
        core_policy,
        ody_protocol::protocol::SandboxPolicy::ReadOnly {
            network_access: true,
        }
    );
...
#[test]
fn mcp_server_elicitation_request_from_core_odysseythink_form_request() {
    let requested_schema = json!({
        "type": "object",
        "properties": {
            "template": {
                "type": "odysseythink/imagePicker",
                "title": "Template",
                "items": [{
                    "id": "monthly-review",
...
fn sandbox_policy_round_trips_workspace_write_access() {
    ...
    assert_eq!(
        core_policy,
        ody_protocol::protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: true,
            exclude_tmpdir_env_var: false,
            exclude_slash_tmp: false,
...
fn network_requirements_deserializes_legacy_fields() {
    let requirements: NetworkRequirements = serde_json::from_value(json!({
        "allowedDomains": ["api.odysseythink.com"],
    ...
    assert_eq!(
        ...
        NetworkRequirements {
            ...
            allowed_domains: Some(vec!["api.odysseythink.com".to_string()]),
    ...
    let user_item = TurnItem::UserMessage(UserMessageItem {
        ...
        content: vec![
            ...
            CoreUserInput::Skill {
                ...
                path: PathBuf::from("/repo/.ody/skills/skill-creator/SKILL.md"),
            },
...
fn plugin_source_serializes_local_git_and_remote_variants() {
    ...
    assert_eq!(
        serde_json::to_value(PluginSource::Git {
            url: "https://github.com/odysseythink/example.git".to_string(),
        ...
        json!({
            "type": "git",
            "url": "https://github.com/odysseythink/example.git",
...
fn plugin_marketplace_entry_serializes_remote_only_path_as_null() {
    assert_eq!(
        serde_json::to_value(PluginMarketplaceEntry {
            name: "odysseythink-curated-remote".to_string(),
            path: None,
            interface: None,
            plugins: Vec::new(),
        })
        .unwrap(),
        json!({
            "name": "odysseythink-curated-remote",
            "path": null,
            "interface": null,
            "plugins": [],
        }),
...
fn plugin_read_params_serialization_uses_install_source_fields() {
    ...
    assert_eq!(
        serde_json::from_value::<PluginReadParams>(json!({
            "remoteMarketplaceName": "odysseythink-curated-remote",
            "pluginName": "gmail",
        }))
        .unwrap(),
        PluginReadParams {
...
fn plugin_skill_read_params_serialization_uses_remote_plugin_id() {
    assert_eq!(
        serde_json::to_value(PluginSkillReadParams {
            remote_marketplace_name: "odysseythink-curated-remote".to_string(),
...
fn plugin_share_params_and_response_serialization_use_camel_case_fields() {
    ...
    assert_eq!(
        serde_json::to_value(PluginShareCheckoutResponse {
            remote_plugin_id: "plugins~Plugin_00000000000000000000000000000000".to_string(),
            plugin_id: "gmail@ody-curated".to_string(),
            plugin_name: "gmail".to_string(),
            plugin_path,
            marketplace_name: "ody-curated".to_string(),
            marketplace_path,
        ...
        json!({
            "remotePluginId": "plugins~Plugin_00000000000000000000000000000000",
            "pluginId": "gmail@ody-curated",
            "pluginName": "gmail",
            "pluginPath": plugin_path_json,
            "marketplaceName": "ody-curated",
            "marketplacePath": marketplace_path_json,
            "remoteVersion": "1.2.3",
        }),
    );
...
fn plugin_share_list_response_serializes_share_items() {
    assert_eq!(
        serde_json::to_value(PluginShareListResponse {
            data: vec![PluginShareListItem {
                plugin: PluginSummary {
                    id: "gmail@odysseythink-curated-remote".to_string(),
        ...
        json!({
            "data": [{
                "plugin": {
                    "id": "gmail@odysseythink-curated-remote",
                    "remotePluginId": "plugins~Plugin_00000000000000000000000000000000",
                    "localVersion": null,
                    "name": "gmail",
                    "shareContext": null,
...
fn plugin_uninstall_params_serialization_omits_force_remote_sync() {
    assert_eq!(
        serde_json::to_value(PluginUninstallParams {
            plugin_id: "gmail@odysseythink-curated".to_string(),
        })
        .unwrap(),
        json!({
            "pluginId": "gmail@odysseythink-curated",
        }),
    );
...
#[test]
fn ody_error_info_serializes_http_status_code_in_camel_case() {
    let value = OdyErrorInfo::ResponseTooManyFailedAttempts {
...
#[test]
fn ody_error_info_serializes_cyber_policy_in_camel_case() {
    assert_eq!(
        serde_json::to_value(OdyErrorInfo::CyberPolicy).unwrap(),
        json!("cyberPolicy")
    );
}

#[test]
fn ody_error_info_serializes_active_turn_not_steerable_turn_kind_in_camel_case() {
    let value = OdyErrorInfo::ActiveTurnNotSteerable {
...
fn turn_start_params_round_trip_multi_agent_mode() {
    ...
    assert_eq!(
        ...
        Some(ody_protocol::config_types::MultiAgentMode::Proactive)
    );
    assert_eq!(

...
use ody_protocol::protocol::RealtimeAudioFrame as CoreRealtimeAudioFrame;
...
pub struct ThreadRealtimeStartParams {
    ...
    /// Leaves Ody response handoffs to the client's explicit append calls instead of forwarding
    ...
    #[ts(optional = nullable)]
    pub ody_responses_as_items: Option<bool>,
    /// Optional prefix added to automatic Ody response items when `odyResponsesAsItems` is true.
    #[ts(optional = nullable)]
    pub ody_response_item_prefix: Option<String>,
    /// Optional prefix added to automatic V1 Ody commentary sent with
    /// `conversation.handoff.append` when `odyResponsesAsItems` is not true. Final answers are
    ...
    /// Set to false to start without Ody's startup context. Omitted or null includes it.
    #[ts(optional = nullable)]

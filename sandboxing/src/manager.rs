...
fn wrap_windows_sandbox_exec_request_for_direct_spawn(
    ...
    let helper = ody_windows_sandbox::resolve_exe_for_launch(source.as_path(), ody_home);
    *program = helper.to_string_lossy().into_owned();
    ...
    let mut wrapper_args =
        ody_windows_sandbox::create_windows_sandbox_command_args_for_permission_profile(
            inner_command,
            &native_cwd,
            workspace_roots,
            &request.env,

...
use ody_login::AuthCredentialsStoreMode;
use ody_login::AuthKeyringBackendKind;
use ody_login::AuthManager;
use ody_login::OdyAuth;
use ody_login::ExternalAuth;
use ody_login::ExternalAuthRefreshContext;
use ody_login::ExternalAuthTokens;
use ody_login::TokenData;
...
impl ExternalAuth for TestExternalApiKeyAuth {
    ...
    fn resolve(&self) -> ody_login::ExternalAuthFuture<'_, Option<ExternalAuthTokens>> {
        Box::pin(async {
...
impl ModelsEndpointClient for TestModelsEndpoint {
    ...
    fn uses_ody_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(async { self.uses_ody_backend })
    }
...
async fn chatgpt_auth_tokens_for_tests(ody_home: &Path) -> OdyAuth {
    let auth_dot_json = ody_login::AuthDotJson {
        auth_mode: Some(AuthMode::ChatgptAuthTokens),
        odysseythink_api_key: None,
        tokens: Some(TokenData {
            id_token: ody_login::token_data::parse_chatgpt_jwt_claims(
...
c2ln",
    ...
    std::fs::create_dir_all(ody_home).expect("ody home should be created");
    std::fs::write(
...
async fn get_model_info_matches_hyphenated_provider_namespace_suffix() {
    ...
    let namespaced_model = "odysseythink-ody/gpt-image".to_string();
...
async fn refresh_available_models_uses_cached_remote_only_catalog_for_chatgpt_auth() {
    ...
    let fetch_manager =
        odysseythink_manager_for_tests(ody_home.path().to_path_buf(), fetch_endpoint.clone());
    ...
    let cache_manager =
        odysseythink_manager_for_tests(ody_home.path().to_path_buf(), cache_endpoint.clone());
...
async fn get_model_info_uses_fallback_for_bundled_models_when_chatgpt_remote_is_authoritative() {
    ...
    let manager = odysseythink_manager_for_tests(ody_home.path().to_path_buf(), endpoint);
...
impl ModelsEndpointClient for TestAuthAwareModelsEndpoint {
    ...
    fn uses_ody_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(TestAuthAwareModelsEndpoint::uses_ody_backend(self))
    }
...
async fn refresh_available_models_fetches_with_chatgpt_auth_tokens() {
    ...
    let auth = chatgpt_auth_tokens_for_tests(ody_home.path()).await;
    let manager = odysseythink_manager_for_tests_with_auth(
        ody_home.path().to_path_buf(),
        endpoint.clone(),

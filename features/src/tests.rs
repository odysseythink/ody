...
#[test]
fn ody_hooks_is_legacy_alias_for_hooks() {
    assert_eq!(feature_for_key("hooks"), Some(Feature::OdyHooks));
    assert_eq!(feature_for_key("ody_hooks"), Some(Feature::OdyHooks));
}
...
#[test]
fn apps_require_feature_flag_and_app_auth() {
    let mut features = Features::with_defaults();
    assert!(!features.apps_enabled_for_auth(/*has_app_auth*/ false));

    features.enable(Feature::Apps);
    assert!(!features.apps_enabled_for_auth(/*has_app_auth*/ false));
    assert!(features.apps_enabled_for_auth(/*has_app_auth*/ true));
}

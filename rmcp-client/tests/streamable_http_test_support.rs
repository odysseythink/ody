...
fn streamable_http_server_bin() -> Result<PathBuf, CargoBinError> {
    ody_utils_cargo_bin::cargo_bin("test_streamable_http_server")
}
...
fn init_params() -> InitializeRequestParams {
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("ody-test", "0.0.0-test").with_title("Ody rmcp recovery test"),
    )
...
pub(crate) async fn spawn_exec_server() -> anyhow::Result<ExecServerProcess> {
    ...
    let mut child = Command::new(ody_utils_cargo_bin::cargo_bin("ody")?)
...
/// Reads the websocket URL printed by `ody exec-server --listen`.

...
const SANDBOX_STATE_META_CAPABILITY: &str = "ody/sandbox-state-meta";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of Ody TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Ody TUI):
    /// - Build this binary: `cargo build -p ody-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `ody mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Ody TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            uri_template: "memo://ody/{slug}".to_string(),
            name: "ody-memo".to_string(),
            title: Some("Ody Memo".to_string()),
            description: Some(
                "Template for memo://ody/{slug} resources used in tests.".to_string(),
            ),
            mime_type: Some("text/plain".to_string()),
            icons: None,
        };
...
/// Scenarios for `image_scenario`, intended to exercise Ody TUI handling of MCP image outputs.
///
    ...
    async fn initialize(
        ...
        self.supports_form_elicitation.store(
            request
                ...
                .extensions
                .as_ref()
                .is_some_and(|extensions| extensions.contains_key("odysseythink/form")),
            Ordering::Relaxed,
        );

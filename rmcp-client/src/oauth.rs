...
//!
//! If the keyring is not available or fails, we fall back to ODY_HOME/.credentials.json which is consistent with other coding CLI agents.
...
const KEYRING_SERVICE: &str = "Ody MCP Credentials";
...
fn fallback_file_path() -> Result<PathBuf> {
    Ok(find_ody_home()?.join(FALLBACK_FILENAME).to_path_buf())
}
...
mod tests {
    ...
    impl TempOdyHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create ODY_HOME temp dir");
            unsafe {
                std::env::set_var("ODY_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,

...
#[cfg(unix)]
use crate::managed_install::resolved_managed_ody_bin;
...
async fn update_once(
    ...
    let managed_ody_bin = resolved_managed_ody_bin(&daemon.managed_ody_bin).await?;
    let managed_identity = executable_identity(&managed_ody_bin).await?;
    ...
    loop {
        ...
        match daemon
            .try_restart_if_running(restart_mode, updater_refresh_mode, &managed_ody_bin)
            .await?
        {
...
#[cfg(unix)]
pub(crate) fn reexec_managed_updater(managed_ody_bin: &std::path::Path) -> Result<()> {
    let err = StdCommand::new(managed_ody_bin)
        .args(["app-server", "daemon", "pid-update-loop"])
        .exec();
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed Ody binary {}",
            managed_ody_bin.display()
        )
    })
}
...
async fn install_latest_standalone() -> Result<()> {
    let script = reqwest::get("https://chatgpt.com/ody/install.sh")
        .await
        .context("failed to fetch standalone Ody updater")?
        .error_for_status()
        .context("standalone Ody updater request failed")?
        .bytes()
        .await
        .context("failed to read standalone Ody updater")?;
    ...
    let mut child = Command::new("/bin/sh")
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone Ody updater")?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone Ody updater stdin was unavailable")?;
    stdin
        .write_all(&script)
        .await
        .context("failed to pass standalone Ody updater to shell")?;
    drop(stdin);
    let status = child
        .wait()
        .await
        .context("failed to wait for standalone Ody updater")?;

    if status.success() {
        Ok(())
    } else {
        anyhow::bail!("standalone Ody updater exited with status {status}")
    }
}

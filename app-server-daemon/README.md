# ody-app-server-daemon

> `ody-app-server-daemon` is experimental and its lifecycle contract may
> change while the remote-management flow is still being developed.

`ody-app-server-daemon` backs the machine-readable `ody app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for Ody instances launched over SSH, including fresh developer
...
```sh
ody app-server daemon start
ody app-server daemon restart
ody app-server daemon enable-remote-control
ody app-server daemon disable-remote-control
ody app-server daemon stop
ody app-server daemon version
ody app-server daemon bootstrap --remote-control
```
...
$HOME/.ody/packages/standalone/current/ody app-server daemon bootstrap --remote-control
```

`bootstrap` requires the standalone managed install. It records the daemon
settings under `ODY_HOME/app-server-daemon/`, starts app-server as a
pidfile-backed detached process, and launches a detached updater loop.

## Installation and update cases

The daemon assumes Ody is installed through `install.sh` and always launches
the standalone managed binary under `ODY_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `ODY_HOME/packages/standalone/current/ody` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `ODY_HOME/packages/standalone/current/ody` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
Top-level `ody remote-control` bootstraps with `--remote-control` when the
...
All mutating lifecycle commands are serialized per `ODY_HOME`, so a concurrent
...
The daemon stores its local state under `ODY_HOME/app-server-daemon/`:

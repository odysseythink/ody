//! Migration helpers for importing external-agent configuration into Ody.

use ody_hooks::HOOK_EVENT_NAMES;
use ody_hooks::HOOK_EVENT_NAMES_WITH_MATCHERS;
...
mod tests {
    ...
    fn migrated_quoted_hook_command(script_name: &str) -> String {
        let hook_path = Path::new("/repo/.ody")
...
"#
    ...
    #[test]
    fn command_skill_names_must_fit_ody_skill_loader_limit() {
        let root = source_path("commands");
    ...
    #[test]
    fn subagent_requires_minimum_ody_agent_fields() {
...
"#,
    ...
    fn hook_migration_ignores_unsupported_handlers() {
        ...
        let mut migration = serde_json::Map::new();
        append_convertible_hook_groups(&settings, &mut migration, Some(Path::new("/repo/.ody")));
    ...
    fn hook_command_paths_rewrite_to_target_hook_dir() {
        ...
        assert_eq!(
            rewrite_hook_command(&plugin_script_command, Some(Path::new("/repo/.ody")),),
    ...
    fn hook_script_copy_keeps_existing_target_scripts() {
        ...
        let source_hooks = source_external_agent_dir.join(EXTERNAL_AGENT_HOOKS_SUBDIR);
        let target_config_dir = root.path().join(".ody");
        let target_hooks = target_config_dir.join(EXTERNAL_AGENT_MIGRATED_HOOKS_SUBDIR);

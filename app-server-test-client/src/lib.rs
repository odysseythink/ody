...
use ody_app_server_protocol::SandboxPolicy;
...
const OTEL_SERVICE_NAME: &str = "ody-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $ODY_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the Ody app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Ody", version, about = "Bootstrap Ody app-server", long_about = None)]
struct Cli {
    /// Path to the `ody` CLI binary. When set, requests use stdio by
    /// spawning `ody app-server` as a child process.
    #[arg(long, env = "ODY_BIN", global = true)]
    ody_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--ody-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "ODY_APP_SERVER_URL", global = true)]
    url: Option<String>,

    /// Forwarded to the `ody` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `ody app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/ody-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `ody app-server --listen`.
    ...
    /// Send a user message through the Ody app-server.
    SendMessage {
        /// User message to send to Ody.
        user_message: String,
    },
    ...
    ModelList,
    /// List stored threads from the Ody app-server.
    ...
    PluginAnalyticsSmoke {
        /// Installed local plugin id, such as `linear@odysseythink-curated-remote`.
        #[arg(long)]
        plugin_id: String,
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let ody_bin = ody_bin.unwrap_or_else(|| PathBuf::from("ody"));
            serve(&ody_bin, &config_overrides, &listen, kill)
        }
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(ody_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(ody_bin, url, "thread-decrement-elicitation")?;
        ...
        CliCommand::PluginAnalyticsSmoke {
            ...
            if url.is_some() {
                bail!("plugin-analytics-smoke requires --ody-bin and does not support --url");
            }
            let ody_bin = ody_bin.context("plugin-analytics-smoke requires --ody-bin")?;
            plugin_analytics_smoke::run(&ody_bin, &config_overrides, &plugin_id, capture_file)
        }
        ...
        CliCommand::PluginRemoteUninstall {
            ...
            if url.is_some() {
                bail!("plugin-remote-uninstall requires --ody-bin and does not support --url");
            }
            let ody_bin = ody_bin.context("plugin-remote-uninstall requires --ody-bin")?;
...
fn resolve_endpoint(ody_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if ody_bin.is_some() && url.is_some() {
        bail!("--ody-bin and --url are mutually exclusive");
    }
    if let Some(ody_bin) = ody_bin {
        return Ok(Endpoint::SpawnOdy(ody_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if ody_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --ody-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(ody_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(ody_bin);
        if let Some(ody_bin_parent) = ody_bin.parent() {
            let mut path = OsString::from(ody_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", ody_bin.display()))?;
...
fn serve(ody_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/ody-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,ody_=trace {}",
        shell_quote(&ody_bin.display().to_string())
    );
    for override_kv in config_overrides {
    ...
    println!("started ody app-server");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnOdy(ody_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut OdyClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = OdyClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = OdyClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (ody_bin, url) {
        (Some(_), Some(_)) => bail!("--ody-bin and --url are mutually exclusive"),
        (Some(ody_bin), None) => {
            let server = BackgroundAppServer::spawn(&ody_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve ody-app-server-test-client binary path")?;
...
impl OdyClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnOdy(ody_bin) => Self::spawn_stdio(ody_bin, config_overrides),
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(ody_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        Self::spawn_stdio_with_env(ody_bin, config_overrides, &[])
    }
    ...
    fn spawn_stdio_with_env(
        ...
        let ody_bin_display = ody_bin.display();
        let mut cmd = Command::new(ody_bin);
        if let Some(ody_bin_parent) = ody_bin.parent() {
            let mut path = OsString::from(ody_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut ody_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{ody_bin_display}` app-server"))?;

        let stdin = ody_app_server
            .stdin
            .take()
            .context("ody app-server stdin unavailable")?;
        let stdout = ody_app_server
            .stdout
            .take()
            .context("ody app-server stdout unavailable")?;

        Ok(Self {
            transport: ClientTransport::Stdio {
                child: ody_app_server,
                stdin: Some(stdin),
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `ody-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "ody-toy-app-server".to_string(),
                    title: Some("Ody Toy App Server".to_string()),
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to ody app-server")?;
                    return Ok(());
                }
                bail!("ody app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from ody app-server")?;
                if bytes == 0 {
                    bail!("ody app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = ody_core::otel_init::build_provider(
            &config,
...
impl Drop for OdyClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[ody app-server exited: {status}]");
            return;
        }

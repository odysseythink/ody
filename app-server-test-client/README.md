...
Quickstart for running and hitting `ody app-server`.
...
cargo build -p ody-cli --bin ody

# 2) Start websocket app-server in background
cargo run -p ody-app-server-test-client -- \
  --ody-bin ./target/debug/ody \
  serve --listen ws://127.0.0.1:4222 --kill

# 3) Call app-server (defaults to ws://127.0.0.1:4222)
cargo run -p ody-app-server-test-client -- model-list
```

`send-message` and `send-message-v2` handle `request_user_input` server requests interactively.
When Ody asks a question, choose a numbered option (or `o` for a free-form answer when offered)
...
active Ody profile must be authenticated. On a fresh local cache, the command
retries ephemeral turns while the installed remote bundle finishes syncing.

```bash
# Build a debug Ody binary; analytics capture is unavailable in release builds.
cargo build -p ody-cli --bin ody

cargo run -p ody-app-server-test-client -- \
  --ody-bin ./target/debug/ody \
  plugin-analytics-smoke \
  --plugin-id linear@odysseythink-curated-remote
```

Use `--capture-file /tmp/plugin-analytics.jsonl` to select the output path.
The command validates one `ody_plugin_disabled`, `ody_plugin_enabled`, and
`ody_plugin_used` event with the expected local plugin identity and capability
...
installed, installs it, validates `ody_plugin_installed`, uninstalls it, and
...
Cleanup does not require analytics capture or a debug Ody binary. When the
...
```bash
cargo run -p ody-app-server-test-client -- watch
```

## Testing Thread Rejoin Behavior

Build and start an app server using commands above. The app-server log is written to `/tmp/ody-app-server-test-client/app-server.log`
...
```bash
cargo run -p ody-app-server-test-client -- send-message-v2 "seed thread for rejoin test"
cargo run -p ody-app-server-test-client -- thread-list --limit 5
```
...
```bash
cargo run --bin ody-app-server-test-client -- \
...
```bash
cargo run --bin ody-app-server-test-client -- thread-resume <THREAD_ID>
```

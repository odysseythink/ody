...
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: &str = "ody-execve-wrapper";
const LOCK_FILENAME: &str = ".lock";
const TOKIO_WORKER_STACK_SIZE_BYTES: usize = 16 * 1024 * 1024;

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct Arg0DispatchPaths {
    /// Stable path to the current Ody executable for child re-execs.
    ///
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime.block_on(
            ody_shell_escalation::run_shell_escalation_execve_wrapper(file, argv),
        );
        match exit_code {
    ...
    if argv1 == ODY_CORE_APPLY_PATCH_ARG1 {
        let patch_arg = args.next().and_then(|s| s.to_str().map(str::to_owned));
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                let mut stdout = std::io::stdout();
                let mut stderr = std::io::stderr();
                let cwd = match ody_utils_absolute_path::AbsolutePathBuf::current_dir() {
                    Ok(cwd) => cwd,
                    Err(_) => std::process::exit(1),
                };
                let runtime = match tokio::runtime::Builder::new_current_thread()
                ...
                let cwd = cwd.into();
                match runtime.block_on(ody_apply_patch::apply_patch(
                    &patch_arg,
                    &cwd,
                    &mut stdout,
                    &mut stderr,
                    ody_exec_server::LOCAL_FS.as_ref(),
                    /*sandbox*/ None,
                )) {
                    Ok(_) => 0,
                    Err(_) => 1,
            ...
            None => {
                eprintln!("Error: {ODY_CORE_APPLY_PATCH_ARG1} requires a UTF-8 PATCH argument.");
                1
            }
        };
    ...
    let (path_entry_guard, updated_path_env_var) = prepare_path_env_var_with_aliases(
        ...
        std::env::var_os("PATH"),
        prepare_path_entry_for_ody_aliases,
    );
...
fn prepare_path_env_var_with_aliases(
    ...
    match prepare_aliases(path_for_aliases) {
        ...
        Err(err) => {
            // It is possible that Ody will proceed successfully even if
...
/// While we want to deploy the Ody CLI as a single executable for simplicity,
...
const ILLEGAL_ENV_VAR_PREFIX: &str = "ODY_";

/// Load env vars from ~/.ody-code/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `ODY_`.
fn load_dotenv() {
    if let Ok(ody_home) = find_ody_home()
        && let Ok(iter) = dotenvy::from_path_iter(ody_home.join(".env"))
    {
        set_filtered(iter);
    }
}

/// Helper to set vars from a dotenvy iterator while filtering out `ODY_` keys.
...
///   with the hidden `--ody-run-as-apply-patch` flag.
///
...
/// user to install a separate executable, simplifying the deployment of Ody
/// CLI.
...
fn prepare_path_entry_for_ody_aliases(
    existing_path: Option<OsString>,
    ...
    #[cfg(not(debug_assertions))]
    {
        ...
        if ody_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (ody_home: {ody_home:?})"
                ),
            ));
        }
    }
    ...
    // Use a ODY_HOME-scoped temp root to avoid cluttering the top-level directory.
    let temp_root = ody_home.join("tmp").join("arg0");
...
mod tests {
    ...
    fn package_path_test_fixture() -> anyhow::Result<PackagePathTestFixture> {
        ...
        let path_dir = package_dir.join("ody-path");
    ...
    fn run_main_with_arg0_guard_keeps_aliases_alive_until_main_returns() -> anyhow::Result<()> {
        let temp_dir = TempDir::new()?;
        let alias_path = temp_dir.path().join("ody-helper-alias");

...
//!
//! The history is stored at `~/.ody-code/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.ody-code`.
...
impl HistoryConfig {
    pub fn new(ody_home: impl Into<PathBuf>, history: &History) -> Self {
        Self {
            ody_home: ody_home.into(),
...
fn history_filepath(config: &HistoryConfig) -> PathBuf {
    config.ody_home.join(HISTORY_FILENAME)
}
...
pub async fn append_entry(
    ...
    // Resolve `~/.ody-code/history.jsonl` and ensure the parent directory exists.

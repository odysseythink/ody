...
`ody-otel` is the OpenTelemetry integration crate for Ody. It provides:

- Provider wiring for log/trace/metric exporters (`ody_otel::OtelProvider`
  and `ody_otel::provider`).
- Session-scoped business event emission via `ody_otel::SessionTelemetry`.
- Low-level metrics APIs via `ody_otel::metrics`.
- Trace-context helpers via `ody_otel::trace_context` and crate-root re-exports.
...
```rust
use ody_otel::config::OtelExporter;
use ody_otel::config::OtelHttpProtocol;
use ody_otel::config::OtelSettings;
use ody_otel::OtelProvider;
use tracing_subscriber::prelude::*;

let settings = OtelSettings {
    environment: "dev".to_string(),
    service_name: "ody-cli".to_string(),
    service_version: env!("CARGO_PKG_VERSION").to_string(),
    ody_home: std::path::PathBuf::from("/tmp"),
...
that member. If propagated trace context already has the named member, Ody
...
Ody-specific session events. Rich session/business events should go through
...
`ody-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using Ody-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use ody_otel::config::{OtelExporter, OtelHttpProtocol};
...
metrics.counter("ody.session_started", 1, &[("source", "tui")])?;
metrics.histogram("ody.request_latency", 83, &[("route", "chat")])?;
```
...
metrics.counter("ody.turns", 1, &[("model", "gpt-5.1")])?;

...
impl SessionTelemetry {
    ...
    pub fn record_plugin_install_elicitation_sent(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "ody.plugin_install_elicitation_sent",
    ...
    pub fn record_plugin_install_suggestion(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "ody.plugin_install_suggestion",
                plugin_install.tool_type = tool_type,
                plugin_install.tool_id = tool_id,
                plugin_install.tool_name = tool_name,
    ...
    pub fn record_responses(&self, handle_responses_span: &Span, event: &ResponseEvent) {
        ...
        match event {
            ...
            ResponseEvent::Completed {
                ...
                handle_responses_span.record(
                    "ody.usage.reasoning_output_tokens",
                    token_usage.reasoning_output_tokens,
                );
                handle_responses_span.record("ody.usage.total_tokens", token_usage.total_tokens);
            }
            _ => {}
        }
    }
    ...
    pub fn conversation_starts(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "ody.conversation_starts",
                provider_name = %provider_name,
                auth.env_odysseythink_api_key_present = self.metadata.auth_env.odysseythink_api_key_env_present,
                auth.env_ody_api_key_present = self.metadata.auth_env.ody_api_key_env_present,
    ...
    pub fn record_websocket_connect(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "ody.websocket_connect",
                duration_ms = %duration.as_millis(),
                http.response.status_code = status,
                success = success_str,
                error.message = error,
    ...
    pub fn record_websocket_request(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "ody.websocket_request",
                duration_ms = %duration.as_millis(),
                success = success_str,
                error.message = error,
                auth.env_odysseythink_api_key_present = self.metadata.auth_env.odysseythink_api_key_env_present,
    ...
    pub fn sandbox_outcome(
        ...
        log_event!(
            self,
            event.name = "ody.sandbox_outcome",
            tool_name = %tool_name,
            call_id = %call_id,
            outcome = %outcome,

...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "ody-cli"), ("env", "prod")])?;
    ...
    metrics.gauge_with_description(
        "ody.active",
        "Number of active Ody operations.",
        /*value*/ 2,
        &[("component", "test")],
    )?;
    metrics.shutdown()?;
    ...
    let counter = find_metric(&resource_metrics, "ody.turns").expect("counter metric missing");
    assert_eq!(counter.description(), "Total number of Ody turns.");
    ...
    let (bounds, bucket_counts, sum, count) = histogram_data(&resource_metrics, "ody.tool_latency");
    ...
    let histogram_attrs = attributes_to_map(
        find_metric(&resource_metrics, "ody.tool_latency")
            ...
            .expect("ody.tool_latency histogram attributes should exist"),
    );
    ...
    let gauge = find_metric(&resource_metrics, "ody.active").expect("gauge metric missing");
    assert_eq!(gauge.description(), "Number of active Ody operations.");
...
fn send_merges_default_tags_per_line() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "ody-cli"), ("env", "prod"), ("region", "us")])?;
    ...
    let resource_metrics = latest_metrics(&exporter);
    let alpha_metric =
        find_metric(&resource_metrics, "ody.alpha").expect("ody.alpha metric missing");
    ...
    let beta_metric = find_metric(&resource_metrics, "ody.beta").expect("ody.beta metric missing");
...
fn shutdown_flushes_in_memory_exporter() -> Result<()> {
    ...
    metrics.counter("ody.turns", /*inc*/ 1, &[])?;
    metrics.shutdown()?;

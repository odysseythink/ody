...
use ody_aws_auth::AwsAuthContext;
use ody_aws_auth::AwsAuthError;
use ody_aws_auth::AwsRequestToSign;
...
pub(super) async fn resolve_auth_method(
    ...
    let context = AwsAuthContext::load(config)
        .await
        .map_err(aws_auth_error_to_ody_error)?;
...
fn aws_auth_error_to_ody_error(error: AwsAuthError) -> OdyErr {
    OdyErr::Fatal(format!("failed to resolve Amazon Bedrock auth: {error}"))
}
...
fn remove_headers_not_preserved_by_bedrock_mantle(headers: &mut HeaderMap) {
    ...
    // richer Ody agent requests fail even though raw Responses requests work.

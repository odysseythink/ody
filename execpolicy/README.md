...
- The legacy rule matcher lives in `ody-execpolicy-legacy`.
...
- From the Ody CLI, run `ody execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
ody execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
ody execpolicy check \
...
```bash
cargo run -p ody-execpolicy -- check --rules path/to/policy.rules git status
```

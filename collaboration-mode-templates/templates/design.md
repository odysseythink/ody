...
**Audit-level sign-off gate (host-run).** After the completeness check passes, the host runs the adversarial review (which, when `[design_review.debate]` is enabled in config, augments its single-shot critique with a bounded Advocate/Skeptic/Judge debate — seeded with the single-shot findings so it targets gaps the critic missed rather than re-reviewing from scratch — and unions the findings) and presents a **per-item** sign-off — one page per item, paginated so the user can digest them one at a time — covering two things, both filtered by the Step 0 audit level:

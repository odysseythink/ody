...
Prefer read-only tools. The only file writes allowed are: (1) the design index, persisted via the submit_design tool — the host names and atomically writes it; (2) split part .md files written with ordinary Write under the <stem>/ directory returned by submit_design. For a large goal, submit the roadmap through `submit_roadmap`; the host asks the user to confirm and writes the roadmap itself. Every other path is rejected by the write gate.
...
When the gate triggers, **do not begin detailed design for the whole goal.** First propose the phase split with `submit_roadmap`. The host presents the required confirmation popup, validates the phase rows, and persists the accepted roadmap at `.ody-code/roadmaps/YYYY-MM-DD-<topic>.md`; do not use `write_file` for this artifact. This is not production code or an implementation plan. Include:

* an overview and explicit outcome for the complete goal;
* a `## Phases` Markdown table whose first column is numeric phase ID and whose last column is `Status`; every phase has an ID, goal, scope boundary, dependencies, acceptance criteria, and recommended collaboration mode (`default`, `plan`, or `design`);
* a dependency/sequence section and cross-phase risks;
* only phase 1 as `active` in the table, with every later phase `pending`.

Use a filesystem-safe topic slug. After the user confirms the roadmap, checkpoint a short design index with `submit_design final: false` that links to the returned roadmap path and explicitly states that the current design scope is `Phase 1`. The host rejects any later design submission that does not cite its confirmed roadmap and active phase. Then run Steps 1–5 for phase 1 only. Do not design, plan, or implement later phases in the current phase's turns. Later phases start only after the active phase is completed and a new Design/Plan session selects the next roadmap phase.
...
**Audit-level sign-off gate (host-run).** After the completeness check passes, the host runs the adversarial review (which, when `[design_review.debate]` is enabled in config, augments its single-shot critique with a bounded Advocate/Skeptic/Judge debate — seeded with the single-shot findings so it targets gaps the critic missed rather than re-reviewing from scratch, and can optionally append a user/usability-lens pass on user-facing designs — and unions the findings) and presents a **per-item** sign-off — one page per item, paginated so the user can digest them one at a time — covering two things, both filtered by the Step 0 audit level:

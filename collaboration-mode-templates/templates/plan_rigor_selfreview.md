...
- [ ] 6. Test-the-risk and state-change impact surface: every task that changes persistent, shared, or cross-boundary observable state names the changed state; all applicable read/observation paths (API, TUI, cache, index, event consumers, audit, authorization); consistency and side effects; failure handling; and a behavioral test asserting the mutation. A plan with no such task records `State-change impact surface: N/A` and why. For each test assertion, trace the expected value through the implementation constants it depends on — a test that expects a "must-survive" input to pass a filter that would actually reject it is a HARD failure; fix the constant or the assertion before proceeding.

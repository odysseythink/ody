...
#[derive(Debug, Parser)]
#[command(name = "ody-state-logs")]
#[command(about = "Tail Ody logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to ODY_HOME. Defaults to $ODY_HOME or ~/.ody-code.
    #[arg(long, env = "ODY_HOME")]
    ody_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --ody-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn main() -> anyhow::Result<()> {
    ...
    let ody_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(ody_home, "logs-client".to_string()).await?;
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let ody_home = args.ody_home.clone().unwrap_or_else(default_ody_home);
    Ok(ody_state::logs_db_path(ody_home.as_path()))
}

fn default_ody_home() -> PathBuf {
    if let Some(home) = home_dir() {
        return home.join(".ody");
    }
    PathBuf::from(".ody")
}
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["ody-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["ody-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["ody-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["ody-state-logs", "--level", "WARN"])

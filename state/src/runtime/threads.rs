...
ON CONFLICT(id) DO NOTHING
        ...
        .bind(
            metadata
                .thread_source
                .as_ref()
                .map(ody_protocol::protocol::ThreadSource::as_str),
        )
...
mod tests {
    ...
    async fn upsert_thread_keeps_creation_memory_mode_for_existing_rows() {
        let ody_home = unique_temp_dir();
        let runtime = StateRuntime::init(ody_home.clone(), "test-provider".to_string())
            .await
            .expect("state db should initialize");
        let thread_id =
            ThreadId::from_string("00000000-0000-0000-0000-000000000123").expect("valid thread id");
        let mut metadata = test_thread_metadata(&ody_home, thread_id, ody_home.clone());
    ...
    async fn delete_thread_cleans_associated_state() -> Result<()> {
        let ody_home = unique_temp_dir();
        let runtime = StateRuntime::init(ody_home.clone(), "test-provider".to_string()).await?;
    ...
    async fn list_threads_filters_by_cwd() {
        ...
        let first_cwd = ody_home.join("first");
        let second_cwd = ody_home.join("second");
        let other_cwd = ody_home.join("other");
        ...
        for (thread_id, cwd, updated_at) in [
            ...
            let mut metadata = test_thread_metadata(&ody_home, thread_id, cwd);
            metadata.updated_at =
    ...
    async fn list_threads_uses_indexes_matching_cwd_filters() {
        let ody_home = unique_temp_dir();
        let runtime = StateRuntime::init(ody_home, "test-provider".to_string())
            .await
            .expect("state db should initialize");
    ...
    async fn apply_rollout_items_restores_memory_mode_from_session_meta() {
        ...
        let metadata = test_thread_metadata(&ody_home, thread_id, ody_home.clone());
    ...
    async fn apply_rollout_items_preserves_existing_git_branch_and_fills_missing_git_fields() {
        ...
        let items = vec![RolloutItem::SessionMeta(SessionMetaLine {
            ...
            git: Some(GitInfo {
                commit_hash: Some(ody_git_utils::GitSha::new("rollout-sha")),
                branch: Some("rollout-branch".to_string()),
                repository_url: Some("git@example.com:odysseythink/ody.git".to_string()),
            }),
        })];
        ...
        assert_eq!(
            ...
            Some("git@example.com:odysseythink/ody.git")
        );
    }
    ...
    async fn upsert_thread_preserves_existing_git_fields_atomically() {
        ...
        metadata.git_branch = Some("sqlite-branch".to_string());
        metadata.git_origin_url = Some("git@example.com:odysseythink/ody.git".to_string());
    ...
    async fn update_thread_git_info_preserves_newer_non_git_metadata() {
        ...
        let updated = runtime
            .update_thread_git_info(
                ...
                Some(Some("git@example.com:odysseythink/ody.git")),
            )
            .await
    ...
    async fn insert_thread_if_absent_preserves_existing_metadata() {
        ...
        let mut existing = test_thread_metadata(&ody_home, thread_id, ody_home.clone());
        ...
        let mut fallback = test_thread_metadata(&ody_home, thread_id, ody_home.clone());
    ...
    async fn thread_updated_at_uses_unique_epoch_millis_and_reads_legacy_seconds() {
        ...
        let mut first = test_thread_metadata(&ody_home, first_id, ody_home.clone());
        first.updated_at = updated_at;
        first.recency_at = updated_at;
        let mut second = test_thread_metadata(&ody_home, second_id, ody_home.clone());
        ...
        let mut older = test_thread_metadata(&ody_home, older_id, ody_home.clone());
    ...
    async fn apply_rollout_items_uses_override_updated_at_when_provided() {
        ...
        let items = vec![RolloutItem::EventMsg(EventMsg::TokenCount(
            ody_protocol::protocol::TokenCountEvent {
                info: Some(ody_protocol::protocol::TokenUsageInfo {
                    total_token_usage: ody_protocol::protocol::TokenUsage {
                        input_tokens: 0,

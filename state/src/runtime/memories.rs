...
mod tests {
    ...
    async fn stage1_claim_skips_when_up_to_date() {
        ...
        let metadata = test_thread_metadata(&ody_home, thread_id, ody_home.join("a"));
        runtime
    ...
    async fn stage1_running_stale_can_be_stolen_but_fresh_running_is_skipped() {
        ...
        let owner_a = ThreadId::from_string(&Uuid::new_v4().to_string()).expect("owner id");
        let owner_b = ThreadId::from_string(&Uuid::new_v4().to_string()).expect("owner id");
        let cwd = ody_home.join("workspace");
        runtime
            .upsert_thread(&test_thread_metadata(&ody_home, thread_id, cwd))
            .await
            .expect("upsert thread");
    ...
    async fn claim_stage1_jobs_filters_by_age_idle_and_current_thread() {
        ...
        let mut current =
            test_thread_metadata(&ody_home, current_thread_id, ody_home.join("current"));
        ...
        let mut old = test_thread_metadata(&ody_home, old_thread_id, ody_home.join("old"));
    ...
    async fn claim_stage1_jobs_skips_threads_with_disabled_memory_mode() {
        ...
        let mut disabled =
            test_thread_metadata(&ody_home, disabled_thread_id, ody_home.join("disabled"));
        ...
        let mut enabled =
            test_thread_metadata(&ody_home, enabled_thread_id, ody_home.join("enabled"));
...
WHERE kind = ? AND job_key = ?
    ...
    async fn list_stage1_outputs_for_global_returns_latest_outputs() {
        ...
        runtime
            ...
            .expect("upsert thread a");
        let mut metadata_b =
            test_thread_metadata(&ody_home, thread_id_b, ody_home.join("workspace-b"));
        ...
        assert_eq!(outputs[0].cwd, ody_home.join("workspace-b"));
        ...
        assert_eq!(outputs[1].cwd, ody_home.join("workspace-a"));
    ...
    async fn list_stage1_outputs_for_global_skips_empty_payloads() {
        ...
        runtime
            .upsert_thread(&test_thread_metadata(
                &ody_home,
                thread_id_non_empty,
                ody_home.join("workspace-non-empty"),
            ))
            .await
...
VALUES (?, ?, ?, ?, ?)
        ...
        assert_eq!(outputs[0].cwd, ody_home.join("workspace-non-empty"));
    ...
    async fn get_phase2_input_selection_returns_current_selected_rows() {
        ...
        assert_eq!(
            ...
            ody_home.join(format!("rollout-{thread_id_c}.jsonl"))
        );
        ...
        for (thread_id, updated_at) in [(thread_id_enabled, 100), (thread_id_polluted, 101)] {
            runtime
                .upsert_thread(&test_thread_metadata(
                    &ody_home,
                    thread_id,
                    ody_home.join(thread_id.to_string()),
                ))
                .await
                .expect("upsert thread");

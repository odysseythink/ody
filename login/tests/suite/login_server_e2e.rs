...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = ody_home.clone();

    let opts = ServerOptions {
        ody_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: None,
        client_id: ody_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
...
#[tokio::test]
async fn creates_missing_ody_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let ody_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let client = reqwest::Client::new();
    let url = format!(
        "http://127.0.0.1:{login_port}/auth/callback?state={state}&error=access_denied&error_description=missing_ody_entitlement"
    );
    let resp = client.get(&url).send().await?;
    assert!(resp.status().is_success());
    let body = resp.text().await?;
    assert!(
        body.contains("You do not have access to Ody"),
        "error body should clearly explain the Ody access denial"
    );
    assert!(
    ...
    assert!(
        !body.contains("missing_ody_entitlement"),
...
async fn oauth_access_denied_unknown_reason_uses_generic_error_page() -> Result<()> {
    ...
    assert!(
        body.contains("Return to Ody to retry"),
    ...
    assert!(
        !body.contains("get access to Ody"),
    ...
    let default_port_server_handle = {
        ...
        thread::spawn(move || {
            while let Ok(req) = server.recv() {
                let _ = req.respond(tiny_http::Response::from_string("not ody"));
            }
        })
    };
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_ody_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        ody_home: first_ody_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: None,
        client_id: ody_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
    ...
    let second_ody_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        ody_home: second_ody_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: None,
        client_id: ody_login::CLIENT_ID.to_string(),
        issuer,

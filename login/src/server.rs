...
const DEFAULT_ISSUER: &str = "https://auth.odysseythink.com";
const DEFAULT_PORT: u16 = 1455;
// Keep in sync with the Ody CLI Hydra redirect URI allow-list.
...
pub struct ServerOptions {
    ...
    pub ody_streamlined_login: bool,
    ...
    match path.as_str() {
                ...
                Ok(tokens) => {
                    ...
                    match tiny_http::Header::from_bytes(&b"Location"[..], success_url.as_bytes()) {
                        ...
                        Err(_) => login_error_response(
                            "Sign-in completed but redirecting back to Ody failed.",
                            io::ErrorKind::Other,
        ...
        "/success" => {
            let use_streamlined_success = parsed_url
                .query_pairs()
                .any(|(key, value)| key == "ody_streamlined_login" && value == "true");
...
fn build_authorize_url(
    ...
    let mut query = vec![
        ...
        ("ody_cli_simplified_flow".to_string(), "true".to_string()),
...
fn compose_success_url(
    ...
    let platform_url = if issuer == DEFAULT_ISSUER {
        "https://platform.odysseythink.com"
    } else {
        "https://platform.api.odysseythink.org"
    };
    ...
    if ody_streamlined_login {
        params.push(("ody_streamlined_login", "true".to_string()));
    }
    let qs = params
        .drain(..)
        .map(|(k, v)| format!("{}={}", k, urlencoding::encode(&v)))
...
fn jwt_auth_claims(jwt: &str) -> serde_json::Map<String, serde_json::Value> {
    ...
    match base64::engine::general_purpose::URL_SAFE_NO_PAD.decode(payload_b64) {
        Ok(bytes) => match serde_json::from_slice::<serde_json::Value>(&bytes) {
            Ok(mut v) => {
                if let Some(obj) = v
                    .get_mut("https://api.odysseythink.com/auth")
                    .and_then(|x| x.as_object_mut())
                {
                    return obj.clone();
                }
                eprintln!("JWT payload missing expected 'https://api.odysseythink.com/auth' object");
            }
            Err(e) => {
...
/// Returns true when the OAuth callback represents a missing Ody entitlement.
fn is_missing_ody_entitlement_error(error_code: &str, error_description: Option<&str>) -> bool {
    error_code == "access_denied"
        && error_description.is_some_and(|description| {
            description
                .to_ascii_lowercase()
                .contains("missing_ody_entitlement")
        })
}
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_ody_entitlement_error(error_code, error_description) {
        return "Ody is not enabled for your workspace. Contact your workspace administrator to request access to Ody.".to_string();
    }
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_ody_entitlement_error(code, error_description) {
            (
                "You do not have access to Ody".to_string(),
                "This account is not currently authorized to use Ody in this workspace."
                    .to_string(),
                "Contact your workspace administrator to request access to Ody.".to_string(),
                "Contact your workspace administrator to get access to Ody, then return to Ody and try again."
                    .to_string(),
            )
        } else {
            (
                ...
                "Return to Ody to retry, switch accounts, or contact your workspace admin if access is restricted."
                    .to_string(),
            )
        };
...
pub(crate) async fn obtain_api_key(
    ...
    let resp = client
        ...
        .body(format!(
            ...
            urlencoding::encode(client_id),
            urlencoding::encode("odysseythink-api-key"),
...
mod tests {
    ...
    use super::is_missing_ody_entitlement_error;
    ...
    fn redact_sensitive_url_parts_preserves_safe_url_shape() {
        let mut url = url::Url::parse(
            "https://user:pass@auth.odysseythink.com/oauth/token?code=abc123&redirect_uri=http%3A%2F%2Flocalhost%3A1455%2Fauth%2Fcallback#frag",
        )
        .expect("valid url");
        ...
        assert_eq!(
            url.as_str(),
            "https://auth.odysseythink.com/oauth/token?code=%3Credacted%3E&redirect_uri=http%3A%2F%2Flocalhost%3A1455%2Fauth%2Fcallback"
        );
    }
    ...
    fn compose_success_url_omits_streamlined_success_by_default() {
        ...
        assert_eq!(
            url.query_pairs()
                .find(|(key, _)| key == "ody_streamlined_login"),
            None
        );
    }
    ...
    fn render_login_error_page_uses_entitlement_copy() {
        let error_description = Some("missing_ody_entitlement");
        assert!(is_missing_ody_entitlement_error(
            "access_denied",
            error_description
        ));
        ...
        assert!(body.contains("You do not have access to Ody"));
        assert!(body.contains("Contact your workspace administrator"));
        assert!(!body.contains("missing_ody_entitlement"));
    }
}

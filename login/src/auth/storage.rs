...
/// Expected structure for $ODY_HOME/auth.json.
...
pub(super) fn get_auth_file(ody_home: &Path) -> PathBuf {
    ody_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(ody_home: &Path) -> std::io::Result<bool> {
...
impl FileAuthStorage {
    pub(super) fn new(ody_home: PathBuf) -> Self {
        Self { ody_home }
    }

    /// Attempt to read and parse the `auth.json` file in the given `ODY_HOME` directory.
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.ody_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.ody_home)
    }
}

static ODY_AUTH_SECRET_NAME: Lazy<SecretName> =
    Lazy::new(|| match SecretName::new("ODY_AUTH") {
        Ok(name) => name,
        Err(err) => unreachable!("ODY_AUTH should be a valid secret name: {err}"),
    });
const KEYRING_SERVICE: &str = "Ody Auth";

// turns ody_home path into a stable, short key string
fn compute_store_key(ody_home: &Path) -> std::io::Result<String> {
    let canonical = ody_home
        .canonicalize()
        .unwrap_or_else(|_| ody_home.to_path_buf());
...
impl DirectKeyringAuthStorage {
    fn new(ody_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            ody_home,
            keyring_store,
        }
...
impl AuthStorageBackend for DirectKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.ody_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.ody_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.ody_home)?;
...
impl SecretsKeyringAuthStorage {
    fn new(ody_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        let direct_storage =
            DirectKeyringAuthStorage::new(ody_home.clone(), Arc::clone(&keyring_store));
...
impl AuthStorageBackend for SecretsKeyringAuthStorage {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        let keyring_removed = self
            .secrets_manager
            .delete(&SecretScope::Global, &ODY_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
...
impl AutoAuthStorage {
    fn new(
        ...
        Self {
            ...
            file_storage: Arc::new(FileAuthStorage::new(ody_home)),
        }
    }
}
...
// A global in-memory store for mapping ody_home -> AuthDotJson.
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_store(ody_home, mode, keyring_store, keyring_backend_kind)
}
...
fn create_auth_storage_with_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(ody_home)),
        AuthCredentialsStoreMode::Keyring => {
            create_keyring_auth_storage(ody_home, keyring_store, keyring_backend_kind)
        }
        ...
        AuthCredentialsStoreMode::Ephemeral => Arc::new(EphemeralAuthStorage::new(ody_home)),
    }
}
...
fn create_keyring_auth_storage(
    ...
    match keyring_backend_kind {
        AuthKeyringBackendKind::Direct => {
            Arc::new(DirectKeyringAuthStorage::new(ody_home, keyring_store))
        }
        AuthKeyringBackendKind::Secrets => {
            Arc::new(SecretsKeyringAuthStorage::new(ody_home, keyring_store))
        }
    }
}

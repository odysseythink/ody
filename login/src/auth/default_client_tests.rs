...
#[test]
fn test_get_ody_user_agent() {
    let user_agent = get_ody_user_agent();
...
fn is_first_party_originator_matches_known_values() {
    ...
    assert_eq!(is_first_party_originator("ody-tui"), true);
    assert_eq!(is_first_party_originator("ody_vscode"), true);
    assert_eq!(is_first_party_originator("Ody Something Else"), true);
    assert_eq!(is_first_party_originator("ody_cli"), false);
...
async fn test_create_client_sets_default_headers() {
    ...
    // User-Agent matches the computed Ody UA for that originator
    let expected_ua = get_ody_user_agent();
...
fn test_invalid_suffix_is_sanitized() {
    let prefix = "ody_cli_rs/0.0.0";
    let suffix = "bad\rsuffix";

    assert_eq!(
        sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
        "ody_cli_rs/0.0.0 (bad_suffix)"
    );
}

...
use ody_protocol::auth::KnownPlan as InternalKnownPlan;
...
fn login_with_api_key_overwrites_existing_auth_json() {
    ...
    assert_eq!(auth.odysseythink_api_key.as_deref(), Some("sk-new"));
...
async fn login_with_access_token_writes_agent_identity_jwt() {
    ...
    assert!(auth.tokens.is_none(), "tokens should be cleared");
    assert!(auth.odysseythink_api_key.is_none(), "API key should be cleared");
    server.verify().await;
}
...
async fn stored_agent_identity_jwt_keeps_auth_json_unchanged() -> anyhow::Result<()> {
    ...
    let OdyAuth::AgentIdentity(agent_identity_auth) = auth else {
    ...
    let auth = storage
        .try_read_auth_json(&get_auth_file(ody_home.path()))
        .expect("auth.json should parse");
    assert_eq!(
        auth.agent_identity,
        Some(AgentIdentityStorage::Jwt(agent_identity))
...
async fn login_with_access_token_writes_only_personal_access_token() {
    ...
    Mock::given(method("GET"))
        ...
        .mount(&server)
        .await;
    let _authapi_guard = EnvVarGuard::set("ODY_AUTHAPI_BASE_URL", &server.uri());
...
async fn chatgpt_auth_task_registration_retry_exhaustion_is_fallback_eligible() -> anyhow::Result<()>
{
    ...
    let storage = FileAuthStorage::new(ody_home.path().to_path_buf());
    let auth_path = get_auth_file(ody_home.path());
...
async fn agent_identity_jwt_task_registration_retry_exhaustion_is_strict() -> anyhow::Result<()> {
    ...
    let err = OdyAuth::from_agent_identity_jwt_with_authapi_base_url(
        &agent_identity,
        Some(&chatgpt_base_url),
        &authapi_base_url,
        /*auth_route_config*/ None,
...
async fn refresh_failure_is_scoped_to_the_matching_auth_snapshot() {
    ...
    let updated_auth = OdyAuth::from_auth_dot_json(
        ody_home.path(),
...
fn write_auth_file(params: AuthFileParams, ody_home: &Path) -> std::io::Result<String> {
    let fake_jwt = fake_jwt_for_auth_file_params(&params)?;
    let auth_file = get_auth_file(ody_home);
    let auth_json_data = json!({
        "OPENAI_API_KEY": params.odysseythink_api_key,
        "tokens": {
            "id_token": fake_jwt,
...
fn fake_jwt_for_auth_file_params(params: &AuthFileParams) -> std::io::Result<String> {
    ...
    let payload = serde_json::json!({
        ...
        "email_verified": true,
        "https://api.odysseythink.com/auth": auth_payload,
    });
...
fn remove_access_token_env_var() -> EnvVarGuard {
    EnvVarGuard::remove(ODY_ACCESS_TOKEN_ENV_VAR)
}
...
async fn load_auth_reads_access_token_from_env() {
    ...
    let _access_token_guard = EnvVarGuard::set(ODY_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    ...
    let OdyAuth::AgentIdentity(agent_identity) = auth else {
    ...
    assert!(
        !get_auth_file(ody_home.path()).exists(),
...
async fn load_auth_reads_personal_access_token_from_env() {
    ...
    Mock::given(method("GET"))
        ...
        .await;
    let _authapi_guard = EnvVarGuard::set("ODY_AUTHAPI_BASE_URL", &server.uri());
    let _access_token_guard = EnvVarGuard::set(ODY_ACCESS_TOKEN_ENV_VAR, "at-env-test");
...
async fn auth_manager_rejects_env_personal_access_token_workspace_mismatch() {
    ...
    let _access_token_guard =
        EnvVarGuard::set(ODY_ACCESS_TOKEN_ENV_VAR, "at-env-workspace-mismatch");
...
async fn personal_access_token_does_not_offer_unauthorized_recovery() {
    ...
    let _authapi_guard = EnvVarGuard::set("ODY_AUTHAPI_BASE_URL", &server.uri());
    let _access_token_guard =
        EnvVarGuard::set(ODY_ACCESS_TOKEN_ENV_VAR, "at-no-unauthorized-recovery");
    let manager = Arc::new(
        AuthManager::new(
            ody_home.path().to_path_buf(),
            /*enable_ody_api_key_env*/ false,
...
#[tokio::test]
#[serial(ody_auth_env)]
async fn load_auth_keeps_ody_api_key_env_precedence() {
    let ody_home = tempdir().unwrap();
    let record = agent_identity_record(WORKSPACE_ID_ALLOWED);
    let agent_identity = fake_agent_identity_jwt(&record).expect("fake agent identity");
    let _access_token_guard = EnvVarGuard::set(ODY_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    let _api_key_guard = EnvVarGuard::set(ODY_API_KEY_ENV_VAR, "sk-env");
...
async fn enforce_login_restrictions_blocks_env_api_key_when_chatgpt_required() {
    let _guard = EnvVarGuard::set(ODY_API_KEY_ENV_VAR, "sk-env");
    let _access_token_guard = remove_access_token_env_var();
    let ody_home = tempdir().unwrap();
...
fn agent_identity_record(account_id: &str) -> AgentIdentityAuthRecord {
    let key_material =
        ody_agent_identity::generate_agent_key_material().expect("generate agent key material");
...
fn fake_agent_identity_jwt_with_plan_type(
    ...
    let payload = json!({
        "iss": "https://chatgpt.com/ody-backend/agent-identity",
        "aud": "ody-app-server",
...
async fn assert_agent_identity_plan_alias(
    ...
    let authapi_base_url = server.uri();
    let chatgpt_base_url = format!("{authapi_base_url}/backend-api");
    let auth = OdyAuth::from_agent_identity_jwt_with_authapi_base_url(
        &jwt,
        Some(&chatgpt_base_url),
        &authapi_base_url,
        /*auth_route_config*/ None,

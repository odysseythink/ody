    ...
    if !resp.status().is_success() {
        ...
        if status == StatusCode::NOT_FOUND {
            return Err(io::Error::new(
                ...
                "device code login is not enabled for this Ody server. Use the browser login or verify the server URL.",
            ));
        }
...
fn print_device_code_prompt(verification_url: &str, code: &str) {
    ...
    println!(
        "\nWelcome to Ody [v{ANSI_GRAY}{version}{ANSI_RESET}]\n{ANSI_GRAY}odysseythink's command-line coding agent{ANSI_RESET}\n\
...
pub async fn request_device_code(opts: &ServerOptions) -> std::io::Result<DeviceCode> {
    ...
    Ok(DeviceCode {
        verification_url: format!("{base_url}/ody/device"),

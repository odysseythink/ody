...
use ody_config::CloudConfigBundle;
...
fn auth_identity(auth: &OdyAuth) -> (Option<String>, Option<String>) {
    (auth.get_chatgpt_user_id(), auth.get_account_id())
}

fn cloud_config_eligible_auth(auth: &OdyAuth) -> bool {
    ...
    C: BundleClient + 'static,
{
    pub(crate) fn new(
        ...
        let ody_home = AbsolutePathBuf::resolve_path_against_base(ody_home, "/");
        Self {
            auth_manager,
            client,
            cache: CloudConfigBundleCache::new(ody_home.clone()),
            ody_home,
            timeout,
        }
    }
    ...
    pub(crate) async fn load_startup_bundle_with_timeout(
        ...
        let _timer =
            ody_otel::start_global_timer("ody.cloud_config_bundle.fetch.duration_ms", &[]);
    ...
    async fn load_valid_cached_bundle(
        ...
        match self.cache.load(chatgpt_user_id, account_id).await {
            Ok(signed_payload) => {
                if let Err(err) = validate_bundle(&signed_payload.bundle, &self.ody_home) {
                    tracing::warn!(
    ...
    async fn validate_and_cache_remote_bundle(
        ...
        if let Err(err) = validate_bundle(&bundle, &self.ody_home) {
            emit_fetch_final_metric(
                trigger,
                "error",
                "invalid_bundle",

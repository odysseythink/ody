...
use ody_backend_client::ConfigBundleResponse;
use ody_backend_client::DeliveredTomlFragment;
use ody_config::AbsolutePathBuf;
use ody_config::CloudConfigFragment;
use ody_config::CloudConfigTomlBundle;
use ody_config::CloudRequirementsFragment;
use ody_config::CloudRequirementsTomlBundle;
use ody_config::types::AuthCredentialsStoreMode;
use ody_login::AuthKeyringBackendKind;
use ody_login::auth::AgentIdentityAuth;
use ody_login::auth::AgentIdentityAuthRecord;
...
fn write_auth_json(ody_home: &Path, value: serde_json::Value) -> std::io::Result<()> {
    std::fs::write(ody_home.join("auth.json"), serde_json::to_string(&value)?)?;
    Ok(())
}

fn create_test_cache(ody_home: &Path) -> CloudConfigBundleCache {
    CloudConfigBundleCache::new(AbsolutePathBuf::resolve_path_against_base(ody_home, "/"))
}
...
async fn auth_manager_with_agent_identity_business_plan() -> Arc<AuthManager> {
    ...
    AuthManager::from_auth_for_testing(OdyAuth::AgentIdentity(
...
impl BundleClient for StaticBundleClient {
    async fn get_bundle(&self, _auth: &OdyAuth) -> Result<CloudConfigBundle, BundleRequestError> {
...
impl BundleClient for TokenBundleClient {
    async fn get_bundle(&self, auth: &OdyAuth) -> Result<CloudConfigBundle, BundleRequestError> {
        self.request_count.fetch_add(1, Ordering::SeqCst);
        if matches!(
...
async fn get_bundle_ignores_invalid_cache_and_refetches() {
    let ody_home = tempdir().expect("tempdir");
    let cache = create_test_cache(ody_home.path());
    cache
        .save(
...
fn bundle_response_conversion_preserves_fragment_order() {
    let response = ConfigBundleResponse {
        config_toml: Some(Some(Box::new(ody_backend_client::DeliveredConfigToml {
        ...
        requirements_toml: Some(Some(Box::new(
            ody_backend_client::DeliveredRequirementsToml {

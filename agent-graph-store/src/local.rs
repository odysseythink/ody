...
impl std::fmt::Debug for LocalAgentGraphStore {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("LocalAgentGraphStore")
            .field("ody_home", &self.state_db.ody_home())
...
fn to_state_status(status: ThreadSpawnEdgeStatus) -> ody_state::DirectionalThreadSpawnEdgeStatus {
    match status {
        ThreadSpawnEdgeStatus::Open => ody_state::DirectionalThreadSpawnEdgeStatus::Open,
        ThreadSpawnEdgeStatus::Closed => ody_state::DirectionalThreadSpawnEdgeStatus::Closed,
    }
}
...
mod tests {
    ...
    async fn state_runtime() -> TestRuntime {
        let ody_home = TempDir::new().expect("tempdir should be created");
        let state_db =

...
/// Filename used for the redacted `ody doctor --json` feedback attachment.
pub const DOCTOR_REPORT_ATTACHMENT_FILENAME: &str = "ody-doctor-report.json";
...
pub struct FeedbackUploadOptions<'a> {
    ...
    ///
    /// These are included after `ody-logs.log` and before path-backed rollout
...
impl FeedbackSnapshot {
    ...
    pub fn save_to_temp_file(&self) -> io::Result<PathBuf> {
        ...
        let filename = format!("ody-feedback-{}.log", self.thread_id);
    ...
    fn feedback_attachments(
        ...
        if include_logs {
            attachments.push(Attachment {
                ...
                filename: String::from("ody-logs.log"),
...
mod tests {
    ...
    fn ring_buffer_drops_front_when_full() {
        let fb = OdyFeedback::with_capacity(/*max_bytes*/ 8);
        {
    ...
    fn feedback_attachments_gate_connectivity_diagnostics() {
        let extra_filename = format!("ody-feedback-extra-{}.jsonl", ThreadId::new());
        ...
        let snapshot_with_diagnostics = OdyFeedback::new()
        ...
        let attachments_without_diagnostics = OdyFeedback::new()
            .snapshot(/*session_id*/ None)

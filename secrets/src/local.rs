...
const ODY_AUTH_SECRETS_FILENAME: &str = "ody_auth.age";
...
pub enum LocalSecretsNamespace {
    ...
    #[default]
    ManagedSecrets,
    /// Ody authentication credentials used by the CLI, TUI, app server, and other clients.
    OdyAuth,
...
impl LocalSecretsBackend {
    pub fn new(ody_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
    ...
    fn load_or_create_passphrase(&self) -> Result<SecretString> {
        let account = compute_keyring_account(&self.ody_home);
        let loaded = self
            .keyring_store
...
mod tests {
    ...
    fn load_file_rejects_newer_schema_versions() -> Result<()> {
        ...
        let backend = LocalSecretsBackend::new(ody_home.path().to_path_buf(), keyring);
    ...
    fn set_fails_when_keyring_is_unavailable() -> Result<()> {
        let ody_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let account = compute_keyring_account(ody_home.path());
        keyring.set_error(
            &account,
    ...
    fn local_namespaces_write_separate_files() -> Result<()> {
        let ody_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let ody_auth_backend = LocalSecretsBackend::new_with_namespace(
        ...
        ody_auth_backend.set(&scope, &name, "ody-auth-value")?;
        mcp_backend.set(&scope, &name, "mcp-value")?;

        assert_eq!(
            ody_auth_backend.get(&scope, &name)?,
            Some("ody-auth-value".to_string())
        );
        assert_eq!(
        ...
        assert!(!ody_home.path().join("secrets").join("local.age").exists());
        Ok(())
    }
}

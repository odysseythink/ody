...
/// The easiest way to iterate is to add unit tests and have Ody fix the implementation.
...
/// Tests are at the top to encourage using TDD + Ody to fix the implementation.
mod tests {
    use super::*;
    ...
    fn supports_grep_recursive_current_dir() {
        assert_parsed(
            &vec_str(&["grep", "-R", "ODY_SANDBOX_ENV_VAR", "-n", "."]),
            vec![ParsedCommand::Search {
                cmd: "grep -R ODY_SANDBOX_ENV_VAR -n .".to_string(),
                query: Some("ODY_SANDBOX_ENV_VAR".to_string()),
                path: Some(".".to_string()),
            }],
        );
    }
    ...
    fn supports_grep_recursive_specific_file() {
        assert_parsed(
            ...
            vec![ParsedCommand::Search {
                cmd: "grep -R ODY_SANDBOX_ENV_VAR -n core/src/spawn.rs".to_string(),
                query: Some("ODY_SANDBOX_ENV_VAR".to_string()),

...
use ody_app_server_protocol::ThreadRealtimeAudioChunk;
...
const STARTUP_CONTEXT_HEADER: &str = "Startup context from Ody.";
...
const RESPONSE_HANDOFF_PREFIX: &str =
    "Silent Ody context. Do not speak, acknowledge, or summarize this item.";
...
impl RealtimeE2eHarness {
    ...
    async fn start_webrtc_realtime_with_ody_response_items(
        &mut self,
        offer_sdp: &str,
        ...
        self.start_webrtc_realtime_with_ody_response_routing(
            ...
            /*ody_responses_as_items*/ Some(true),
            /*ody_response_handoff_prefix*/ None,
        )
        .await
    }

    async fn start_webrtc_realtime_with_ody_response_routing(
        &mut self,
        offer_sdp: &str,
        client_managed_handoffs: Option<bool>,
        ody_responses_as_items: Option<bool>,
        ody_response_handoff_prefix: Option<&str>,
    ) -> Result<StartedWebrtcRealtime> {
        ...
        let start_request_id = self
            ...
            .send_thread_realtime_start_request(ThreadRealtimeStartParams {
                ...
                ody_response_item_prefix: ody_responses_as_items
                    .unwrap_or(false)
                    .then(|| RESPONSE_ITEM_PREFIX.to_string()),
                ody_response_handoff_prefix: ody_response_handoff_prefix.map(str::to_string),
    ...
    async fn start_websocket_realtime(&mut self) -> Result<ThreadRealtimeStartedNotification> {
        self.start_websocket_realtime_with_ody_responses_as_items(
            /*ody_responses_as_items*/ None,
        )
        .await
    }

    async fn start_websocket_realtime_with_ody_response_items(
        &mut self,
    ...
    async fn start_websocket_realtime_with_ody_responses_as_items(
        &mut self,
        ody_responses_as_items: Option<bool>,
    ) -> Result<ThreadRealtimeStartedNotification> {
        let start_request_id = self
...
async fn realtime_webrtc_start_emits_sdp_notification() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some("multipart/form-data; boundary=ody-realtime-call-boundary")
    );
...
async fn webrtc_v1_client_managed_handoffs_disable_automatic_output() -> Result<()> {
    ...
    let started = harness
        .start_webrtc_realtime_with_ody_response_routing(
            "v=offer\r\n",
    ...
    assert!(
        ...
        "automatic Ody output should not reach realtime in client-managed handoff mode"
    );
...
async fn webrtc_v1_handoff_request_delegates_context_and_manual_append_speaks() -> Result<()> {
    ...
    let started = harness
        .start_webrtc_realtime_with_ody_response_items("v=offer\r\n")
        .await?;
    assert_eq!(started.started.version, RealtimeConversationVersion::V1);
    assert_call_create_multipart(
        harness.call_capture.single_request(),
...
async fn realtime_automatic_standalone_output_is_item_and_append_speaks() -> Result<()> {
    ...
    let started = harness
        .start_websocket_realtime_with_ody_response_items()
        .await?;
    assert_eq!(started.version, RealtimeConversationVersion::V2);
    assert_eq!(
...
async fn websocket_v2_background_agent_steering_ack_requests_response_create() -> Result<()> {
    ...
    // Phase 4: release the gated delegated turn. Ody should then continue

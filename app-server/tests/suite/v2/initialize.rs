...
async fn initialize_uses_client_info_name_as_originator() -> Result<()> {
    ...
    let expected_ody_home = AbsolutePathBuf::try_from(ody_home.path().canonicalize()?)?;
    ...
    let InitializeResponse {
        user_agent,
        ody_home: response_ody_home,
        platform_family,
        platform_os,
    } = to_response::<InitializeResponse>(response)?;

    assert!(user_agent.starts_with("ody_vscode/"));
    assert_eq!(response_ody_home, expected_ody_home);
...
async fn initialize_probe_does_not_override_originator() -> Result<()> {
    ...
    assert!(user_agent.starts_with("ody_cli_rs/"));
    Ok(())
}

#[tokio::test]
async fn initialize_ody_backend_does_not_override_originator() -> Result<()> {
    let responses = Vec::new();
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let ody_home = TempDir::new()?;
    create_config_toml(ody_home.path(), &server.uri(), "never")?;
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "ody-backend".to_string(),
            title: Some("Ody Backend".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
...
async fn initialize_respects_originator_override_env_var() -> Result<()> {
    ...
    let mut mcp = TestAppServer::new_with_env(
        ody_home.path(),
        &[(
            "ODY_INTERNAL_ORIGINATOR_OVERRIDE",
            Some("ody_originator_via_env_var"),
        )],
    )
    .await?;
    ...
    assert!(user_agent.starts_with("ody_originator_via_env_var/"));
    assert_eq!(response_ody_home, expected_ody_home);
    assert_eq!(platform_family, std::env::consts::FAMILY);
    assert_eq!(platform_os, std::env::consts::OS);
    Ok(())
...
async fn initialize_rejects_invalid_client_name() -> Result<()> {
    ...
    let mut mcp = TestAppServer::new_with_env(
        ody_home.path(),
        &[("ODY_INTERNAL_ORIGINATOR_OVERRIDE", None)],
    )
    .await?;
...
async fn turn_start_notify_payload_includes_initialize_client_name() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let ody_home = TempDir::new()?;
    let notify_file = ody_home.path().join("notify.json");
    let notify_capture = cargo_bin("ody-app-server-test-notify-capture")?;
...
fn create_config_toml(
    ...
    create_config_toml_with_extra(ody_home, server_uri, approval_policy, "")
}

...
const TOOL_NAMESPACE: &str = "mcp__ody_apps__calendar";
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn mcp_server_odysseythink_form_elicitation_round_trip() -> Result<()> {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn odysseythink_form_capability_follows_the_turn_starting_connection() -> Result<()> {
    ...
    send_request(
        ...
        Some(serde_json::to_value(InitializeParams {
            ...
            capabilities: Some(InitializeCapabilities {
                ...
                mcp_server_odysseythink_form_elicitation: supports_odysseythink_form_elicitation,
                ..Default::default()
            }),
        })?),
    )
        ...
        if matches!(self.scenario, ElicitationScenario::OpenAiForm) {
            assert_eq!(
                request
                    ...
                    .extensions
                    .as_ref()
                    .and_then(|extensions| extensions.get("odysseythink/form"))
                    .cloned()
                    .map(Value::Object),
                Some(json!({}))
            );

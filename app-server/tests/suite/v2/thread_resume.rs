...
use ody_protocol::protocol::SessionSource as RolloutSessionSource;
...
use ody_rollout::append_rollout_item_to_path;
use ody_rollout::read_session_meta_line;
...
const ODY_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are Ody, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
async fn goal_first_live_thread_appears_in_state_db_thread_list() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let ody_home = TempDir::new()?;
    let ody_home_path = normalized_existing_path(ody_home.path())?;
    create_config_toml(&ody_home_path, &server.uri())?;
    let config_path = ody_home_path.join("config.toml");
    let config = std::fs::read_to_string(&config_path)?;
    std::fs::write(
        &config_path,
        config.replace("personality = true\n", "personality = true\ngoals = true\n"),
    ...
    let sqlite_home = ody_home_path
        .as_path()
        .to_str()
        .expect("test ody home should be utf-8");
    let mut mcp = TestAppServer::new_without_managed_config_with_env(
        &ody_home_path,
        &[("ODY_SQLITE_HOME", Some(sqlite_home))],
    )
    .await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn thread_resume_rejects_archived_session_by_id() -> Result<()> {
    ...
    let active_rollout_path = rollout_path(ody_home.path(), filename_ts, &conversation_id);
    let archived_dir = ody_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
    ...
    assert!(
        message.contains(&format!("session {conversation_id} is archived"))
            && message.contains(&format!(
                "ody unarchive {conversation_id}` to unarchive it first"
            )),
...
async fn thread_goal_set_edits_objective_without_resetting_usage() -> Result<()> {
    ...
    let state_db =
        StateRuntime::init(ody_home.path().to_path_buf(), "mock_provider".into()).await?;
    ...
    state_db
        ...
        .account_thread_goal_usage(
            ...
            /*token_delta*/ 50,
            ody_state::GoalAccountingMode::ActiveOnly,
...
async fn thread_resume_token_usage_replay_ignores_stale_interrupted_tail_turn() -> Result<()> {
    ...
    let rollout_file_path = rollout_path(ody_home.path(), filename_ts, &conversation_id);
...
async fn thread_resume_prefers_persisted_git_metadata_for_local_threads() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let ody_home = TempDir::new()?;
    let config_toml = ody_home.path().join("config.toml");
    std::fs::write(
        &config_toml,
        format!(
            r#"
...
"#,
    ...
    let repo_path = ody_home.path().join("repo");
    ...
    let rollout_path = rollout_path(ody_home.path(), "2025-01-05T12-00-00", &thread_id);
...
async fn thread_resume_defers_updated_at_until_turn_start() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let ody_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(ody_home.path(), &server.uri()).await?;
    ...
    let resume_id = mcp
        .send_thread_resume_request(ThreadResumeParams {
            ...
            cwd: Some(ody_home.path().to_string_lossy().to_string()),
            ..Default::default()
        })
        .await?;
    let resume_resp: JSONRPCResponse = timeout(
    ...
    assert_eq!(cwd, AbsolutePathBuf::from_absolute_path(ody_home.path())?);
...
async fn thread_resume_keeps_in_flight_turn_streaming() -> Result<()> {
    ...
    let mut primary = TestAppServer::new(ody_home.path()).await?;
    ...
    let mut secondary = TestAppServer::new(ody_home.path()).await?;
...
async fn thread_resume_rejects_mismatched_path_for_running_thread_id() -> Result<()> {
    ...
    let stale_path = rollout_path(ody_home.path(), "2025-01-01T00-00-00", &stale_thread_id);
...
"#;
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    create_config_toml(&ody_home, &server.uri())?;

    let mut primary = TestAppServer::new(&ody_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, primary.initialize()).await??;
    ...
    let expected_file_change = ThreadItem::FileChange {
        ...
        changes: vec![ody_app_server_protocol::FileUpdateChange {
...
async fn thread_resume_with_overrides_defers_updated_at_until_turn_start() -> Result<()> {
    ...
    let RestartedThreadFixture {
        ...
        rollout_file_path,
        updated_at,
    } = start_materialized_thread_and_restart(ody_home.path(), "materialize").await?;
...
async fn thread_resume_fails_when_required_mcp_server_fails_to_initialize() -> Result<()> {
    ...
    let ody_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(ody_home.path(), &server.uri()).await?;
    create_config_toml_with_required_broken_mcp(ody_home.path(), &server.uri())?;
...
async fn thread_resume_supports_history_and_overrides() -> Result<()> {
    ...
    let RestartedThreadFixture {
        mut mcp, thread_id, ..
    } = start_materialized_thread_and_restart(ody_home.path(), "seed history").await?;
...
async fn start_materialized_thread_and_restart(
    ...
    let mut first_mcp = TestAppServer::new(ody_home).await?;
    ...
    let mut second_mcp = TestAppServer::new(ody_home).await?;
...
async fn thread_resume_accepts_personality_override() -> Result<()> {
    ...
    assert!(
        instructions_text.contains(ODY_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT),
...
command = "ody-definitely-not-a-real-binary"
required = true
"#
        ),
    )
...
async fn setup_rollout_fixture(ody_home: &Path, server_uri: &str) -> Result<RolloutFixture> {
    create_config_toml(ody_home, server_uri)?;

...
use ody_app_server_protocol::AppBranding;
...
use ody_app_server_protocol::AppReview;
use ody_app_server_protocol::AppScreenshot;
...
#[tokio::test]
async fn list_apps_returns_empty_when_workspace_ody_plugins_disabled() -> Result<()> {
    let connectors = vec![AppInfo {
        id: "beta".to_string(),
        name: "Beta".to_string(),
        description: Some("Beta connector".to_string()),
...
fn write_connectors_and_plugins_config(ody_home: &Path, base_url: &str) -> std::io::Result<()> {
    let config_toml = ody_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
...
fn write_plugin_app_fixture(ody_home: &Path, plugin_name: &str, app_id: &str) -> Result<()> {

...
fn external_agent_home(ody_home: &Path) -> PathBuf {
    ody_home.join(concat!(".", "cl", "aude"))
}
...
-> Result<()> {
    ...
    let home_dir = ody_home.path().display().to_string();
    let sqlite_home_dir = sqlite_home.path().display().to_string();
    let mut mcp = TestAppServer::new_with_env(
        ody_home.path(),
        &[
            ("HOME", Some(home_dir.as_str())),
            ("ODY_SQLITE_HOME", Some(sqlite_home_dir.as_str())),
        ],
    )
    .await?;
    ...
    let state_db =
        ody_state::StateRuntime::init(sqlite_home.path().to_path_buf(), "mock_provider".into())
            .await?;
...
async fn external_agent_config_import_reports_failed_sync_import_in_completion() -> Result<()> {
    ...
    std::fs::write(ody_home.path().join("config.toml"), "invalid = [")?;
    let home_dir = ody_home.path().display().to_string();
    let analytics_capture_file = ody_home.path().join("analytics-events.jsonl");
    ...
    let events = timeout(DEFAULT_TIMEOUT, async {
        loop {
            ...
            if captured_events.iter().any(|event| {
                event["event_type"] == "ody_onboarding_external_agent_import_complete"
    ...
    let event = events
        .iter()
        .find(|event| {
            event["event_type"] == "ody_onboarding_external_agent_import_failure"
...
async fn external_agent_config_import_completed_tracks_analytics_event() -> Result<()> {
    ...
    let missing_session_path =
        external_agent_home(ody_home.path()).join("projects/repo/missing.jsonl");
    let project_root = ody_home.path().join("repo");
    let home_dir = ody_home.path().display().to_string();
    let mut mcp =
        TestAppServer::new_with_env(ody_home.path(), &[("HOME", Some(home_dir.as_str()))])
            .await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
    ...
    let event = wait_for_analytics_event(
        &analytics_server,
        DEFAULT_TIMEOUT,
        "ody_onboarding_external_agent_import_complete",
    )
    .await?;
    let event_params = &event["event_params"];
    ...
    let event = wait_for_analytics_event(
        &analytics_server,
        DEFAULT_TIMEOUT,
        "ody_onboarding_external_agent_import_failure",
    )
    .await?;
    let event_params = &event["event_params"];
    assert_eq!(event_params["import_id"], serde_json::json!(import_id));
...
{
    let ody_home = TempDir::new()?;
    let marketplace_root = ody_home.path().join("marketplace");
...
async fn external_agent_config_import_creates_session_rollouts() -> Result<()> {
    ...
    let session_dir = external_agent_home(ody_home.path()).join("projects/repo");
...
async fn external_agent_config_import_does_not_initialize_required_mcp() -> Result<()> {
    ...
    let ody_home = TempDir::new()?;
    create_config_toml(ody_home.path(), &server.uri())?;
    let mut config = std::fs::read_to_string(ody_home.path().join("config.toml"))?;
    config.push_str(
        r#"
...
"#,
    ...
    std::fs::write(ody_home.path().join("config.toml"), config)?;
    let project_root = ody_home.path().join("repo");
    let recent_timestamp = chrono::Utc::now().to_rfc3339_opts(chrono::SecondsFormat::Secs, true);
    let session_dir = external_agent_home(ody_home.path()).join("projects/repo");
    let session_path = session_dir.join("session.jsonl");

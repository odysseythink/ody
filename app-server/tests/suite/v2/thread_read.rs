...
use ody_protocol::protocol::SessionSource as ProtocolSessionSource;
...
use ody_thread_store::AppendThreadItemsParams;
...
async fn thread_turns_list_can_page_backward_and_forward() -> Result<()> {
    ...
    let rollout_path = rollout_path(ody_home.path(), filename_ts, &conversation_id);
...
async fn thread_turns_list_reads_store_history_without_rollout_path() -> Result<()> {
    let ody_home = TempDir::new()?;
    let thread_id = ody_protocol::ThreadId::from_string("00000000-0000-4000-8000-000000000123")?;
    let store_id = Uuid::new_v4().to_string();
    create_config_toml_with_thread_store(ody_home.path(), &store_id)?;
...
async fn thread_read_loaded_include_turns_reads_store_history_without_rollout_path() -> Result<()> {
    ...
    let thread_id = ody_protocol::ThreadId::from_string(&thread.id)?;
    store
...
async fn thread_list_includes_store_thread_without_rollout_path() -> Result<()> {
    let ody_home = TempDir::new()?;
    let thread_id = ody_protocol::ThreadId::from_string("00000000-0000-4000-8000-000000000124")?;
    let store_id = Uuid::new_v4().to_string();
    create_config_toml_with_thread_store(ody_home.path(), &store_id)?;
    let store = InMemoryThreadStore::for_id(store_id.clone());
    let _in_memory_store = InMemoryThreadStoreId { store_id };
...
async fn thread_resume_initial_turns_page_matches_requested_turns_list_page() -> Result<()> {
    ...
    assert_eq!(
        initial_turns_page,
        Some(ody_app_server_protocol::TurnsPage::from(expected_page))
    );
...
) -> anyhow::Result<ody_app_server_protocol::Turn> {
    let read_id = mcp
        .send_thread_turns_list_request(ThreadTurnsListParams {
            thread_id: thread_id.to_string(),
            cursor: None,
...
fn turn_user_texts(turns: &[ody_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()
...
fn turn_agent_texts(turns: &[ody_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()
...
fn create_config_toml_with_thread_store(ody_home: &Path, store_id: &str) -> std::io::Result<()> {

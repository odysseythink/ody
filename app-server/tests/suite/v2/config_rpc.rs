...
use ody_app_server_protocol::AppConfig;
use ody_app_server_protocol::AppToolApproval;
use ody_app_server_protocol::ApprovalsReviewer;
use ody_app_server_protocol::AppsConfig;
use ody_app_server_protocol::AppsDefaultConfig;
...
use ody_app_server_protocol::ForcedChatgptWorkspaceIds;
...
use ody_app_server_protocol::ToolsV2;
...
fn write_config(ody_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
"#,
    ...
    let ody_home_path = ody_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(ody_home_path.join("config.toml"))?;
...
"#,
    ...
    assert_eq!(desktop.get("selected-avatar-id"), Some(&json!("ody")));
    assert_eq!(
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let ody_home = TempDir::new()?;
    write_config(&ody_home, r#"model = "gpt-user""#)?;
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let ody_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path = AbsolutePathBuf::resolve_path_against_base("config.toml", ody_home);
...
"#,
    ...
    let write_id = mcp
        .send_config_value_write_request(ConfigValueWriteParams {
            file_path: Some(ody_home.path().join("config.toml").display().to_string()),
            key_path: "model".to_string(),
            value: json!("gpt-new"),
            merge_strategy: MergeStrategy::Replace,
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let ody_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(ody_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
fn assert_layers_user_then_optional_system(
    layers: &[ody_app_server_protocol::ConfigLayer],

...
async fn selected_executor_plugin_exposes_its_stdio_mcp_only_to_that_thread() -> Result<()> {
    ...
    std::fs::write(
        ody_home.path().join("environments.toml"),
        format!(
            r#"
include_local = true
...
"#,
            toml::Value::String(
                ody_utils_cargo_bin::cargo_bin("ody")?
                    .to_string_lossy()
                    .into_owned()
            )
        ),
    ...
    std::fs::create_dir_all(plugin.path().join(".ody-plugin"))?;
    std::fs::write(
        plugin.path().join(".ody-plugin/plugin.json"),

...
use ody_app_server_protocol::RemoteControlClient;
use ody_app_server_protocol::RemoteControlClientsListOrder;
use ody_app_server_protocol::RemoteControlClientsListParams;
use ody_app_server_protocol::RemoteControlClientsListResponse;
use ody_app_server_protocol::RemoteControlClientsRevokeParams;
use ody_app_server_protocol::RemoteControlClientsRevokeResponse;
use ody_app_server_protocol::RemoteControlConnectionStatus;
use ody_app_server_protocol::RemoteControlDisableResponse;
use ody_app_server_protocol::RemoteControlEnableResponse;
use ody_app_server_protocol::RemoteControlPairingStartParams;
use ody_app_server_protocol::RemoteControlPairingStartResponse;
use ody_app_server_protocol::RemoteControlPairingStatusParams;
use ody_app_server_protocol::RemoteControlPairingStatusResponse;
...
async fn listen_off_honors_persisted_remote_control_enable() -> Result<()> {
    let ody_home = TempDir::new()?;
    let listener = configured_remote_control_listener(ody_home.path()).await?;
    ...
    let _app_server = TestAppServer::new_with_args(ody_home.path(), &["--listen", "off"]).await?;
...
async fn remote_control_disable_returns_disabled_status() -> Result<()> {
    let ody_home = TempDir::new()?;
    let _listener = configured_remote_control_listener(ody_home.path()).await?;
    let mut mcp = TestAppServer::new(ody_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn remote_control_enable_returns_connecting_status() -> Result<()> {
    let ody_home = TempDir::new()?;
    let mut backend = BlockingRemoteControlBackend::start(ody_home.path()).await?;
    let mut mcp = TestAppServer::new(ody_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn remote_control_pairing_start_returns_pairing_artifacts() -> Result<()> {
    let ody_home = TempDir::new()?;
    let mut backend = PairingRemoteControlBackend::start(ody_home.path()).await?;
    let mut mcp = TestAppServer::new(ody_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn remote_control_client_management_works_while_disabled() -> Result<()> {
    let ody_home = TempDir::new()?;
    let mut backend = ClientManagementRemoteControlBackend::start(ody_home.path()).await?;
    let mut mcp = TestAppServer::new(ody_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
impl ClientManagementRemoteControlBackend {
    async fn start(ody_home: &std::path::Path) -> Result<Self> {
        let listener = configured_remote_control_listener(ody_home).await?;

...
async fn standalone_image_generation_failure_emits_terminal_item() -> Result<()> {
    ...
    Mock::given(method("POST"))
        .and(path("/api/ody/images/generations"))
...
async fn standalone_image_edit_uses_attached_model_visible_image() -> Result<()> {
    let edit_request = run_image_edit_test(|ody_home| {
        let image_path = ody_home.join("attached.png");
...
async fn run_image_edit_test(
    ...
    let ody_home = TempDir::new()?;
    let (arguments, input) = input(ody_home.path())?;
    let response_mock = responses::mount_sse_sequence(
        &server,
        vec![
            responses::sse(vec![
    ...
    Ok(requests
        .iter()
        .find(|request| request.url.path() == "/api/ody/images/edits")

...
use ody_app_server_protocol::PluginShareDiscoverability;
...
const TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS: &str =
    "ODY_TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS";
...
async fn plugin_installed_prefers_remote_curated_conflicts_when_remote_plugin_enabled() -> Result<()>
{
    ...
    write_odysseythink_curated_marketplace(ody_home.path(), &["linear", "calendar"])?;
    write_installed_plugin(&ody_home, "odysseythink-curated", "linear")?;
    write_installed_plugin(&ody_home, "odysseythink-curated", "calendar")?;
    std::fs::write(
        ody_home.path().join("config.toml"),
        format!(
...
"#,
    ...
    let mut app_server = TestAppServer::new(ody_home.path()).await?;
    ...
    let local_marketplace = response
        .marketplaces
        .iter()
        .find(|marketplace| marketplace.name == "odysseythink-curated")
        .expect("expected odysseythink-curated marketplace entry");
    assert_eq!(
        local_marketplace
            .plugins
            .iter()
            ...
            .collect::<Vec<_>>(),
        vec!["calendar@odysseythink-curated".to_string()]
    );
    let remote_marketplace = response
        .marketplaces
        .iter()
        .find(|marketplace| marketplace.name == "odysseythink-curated-remote")
        .expect("expected odysseythink-curated-remote marketplace entry");
    assert_eq!(
        remote_marketplace
            .plugins
            .iter()
...
#[tokio::test]
async fn plugin_list_returns_empty_when_workspace_ody_plugins_disabled() -> Result<()> {
    let ody_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
...
#[tokio::test]
async fn plugin_list_reuses_cached_workspace_ody_plugins_setting() -> Result<()> {
    let ody_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
...
async fn app_server_startup_sync_downloads_remote_installed_plugin_bundles() -> Result<()> {
    ...
    let installed_path = ody_home
        .path()
        .join("plugins/cache/odysseythink-curated-remote/linear/1.2.3");
    ...
    wait_for_path_exists(&installed_path.join(".ody-plugin/plugin.json")).await?;
    let installed_plugin_manifest: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(installed_path.join(".ody-plugin/plugin.json"))?,
    )?;
    assert_eq!(
    ...
    assert!(!config.contains("linear@odysseythink-curated-remote"));
    Ok(())
}
...
async fn plugin_list_sync_upgrades_and_removes_remote_installed_plugin_bundles() -> Result<()> {
    ...
    write_installed_plugin_with_version(&ody_home, "odysseythink-curated-remote", "stale", "1.0.0")?;
    ...
    let old_path = ody_home
        .path()
        .join("plugins/cache/odysseythink-curated-remote/linear/1.0.0");
    let new_path = ody_home
        .path()
        .join("plugins/cache/odysseythink-curated-remote/linear/1.2.3");
    let stale_path = ody_home
        .path()
        .join("plugins/cache/odysseythink-curated-remote/stale");
    ...
    assert_eq!(
        remote_marketplace
            ...
            .collect::<Vec<_>>(),
        vec![("linear@odysseythink-curated-remote".to_string(), true, true)]
    );

    wait_for_path_exists(&new_path.join(".ody-plugin/plugin.json")).await?;
    let installed_plugin_manifest: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(new_path.join(".ody-plugin/plugin.json"))?,
    )?;
    assert_eq!(
        installed_plugin_manifest["version"],
        serde_json::json!("1.2.3")
...
async fn plugin_list_includes_remote_marketplaces_when_remote_plugin_enabled() -> Result<()> {
  ...
  "pagination": {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(
            ResponseTemplate::new(200).set_body_string(r#"["linear@odysseythink-curated-remote"]"#),
        )
        .mount(&server)
        .await;
    ...
    let remote_marketplace = response
        ...
        .into_iter()
        .find(|marketplace| marketplace.name == "odysseythink-curated-remote")
        .expect("expected odysseythink-curated remote marketplace");
    ...
    assert_eq!(
        ...
        ody_app_server_protocol::PluginAvailability::Available
    );
    assert_eq!(
    ...
    let cache_files = std::fs::read_dir(ody_home.path().join("cache/remote_plugin_catalog"))?
    ...
    assert_eq!(
        ...
        vec!["linear@odysseythink-curated-remote".to_string()]
    );
    assert!(
        !server
            .received_requests()
...
async fn plugin_list_uses_cached_global_remote_catalog_and_refreshes_it() -> Result<()> {
    ...
    wait_for_cached_remote_catalog_plugin_ids(ody_home.path(), &[cached_remote_plugin_id])
        .await?;
    ...
    wait_for_remote_plugin_request_count(&server, "/ps/plugins/list", /*expected_count*/ 1).await?;
    wait_for_cached_remote_catalog_plugin_ids(ody_home.path(), &[refreshed_remote_plugin_id])
        .await?;
...
#[tokio::test]
async fn plugin_list_includes_odysseythink_curated_remote_collection_when_requested() -> Result<()> {
    let ody_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_plugins_enabled_config_with_base_url(
        ody_home.path(),
  ...
  "pagination": {
    ...
    mount_odysseythink_curated_remote_collection_plugin_list(&server, collection_body).await;
    mount_remote_installed_plugins(&server, "GLOBAL", empty_remote_installed_plugins_body()).await;
    mount_remote_installed_plugins(&server, "WORKSPACE", empty_remote_installed_plugins_body())
        .await;
    mount_empty_user_installed_plugins(&server).await;
...
#[tokio::test]
async fn plugin_list_propagates_explicit_odysseythink_curated_remote_collection_errors() -> Result<()> {
    let ody_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_plugins_enabled_config_with_base_url(
        ody_home.path(),
...
#[tokio::test]
async fn plugin_list_skips_explicit_odysseythink_curated_remote_collection_for_api_auth() -> Result<()> {
    let ody_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_plugins_enabled_config_with_base_url(
        ody_home.path(),
...
-> Result<()> {
    ...
    write_odysseythink_api_curated_marketplace(ody_home.path(), &["api-plugin"])?;
    login_with_api_key(
        ody_home.path(),
        "sk-test-key",
        AuthCredentialsStoreMode::File,
    ...
    let api_curated_marketplace = response
        .marketplaces
        .iter()
        .find(|marketplace| marketplace.name == "odysseythink-api-curated")
    ...
    assert_eq!(
        ...
        "api-plugin@odysseythink-api-curated"
    );
    assert!(response.marketplace_load_errors.is_empty());
    wait_for_remote_plugin_request_count(&server, "/ps/plugins/list", /*expected_count*/ 0).await?;
    Ok(())
...
"#,
    ...
    wait_for_path_exists(
        &ody_home.path().join(
            "plugins/cache/created-by-me-remote/private-linear/1.2.3/.ody-plugin/plugin.json",
        ),
    )
    .await?;
...
"#,
    ...
    assert_eq!(response.marketplaces.len(), 1);
    assert_eq!(response.marketplaces[0].name, "odysseythink-curated-remote");
    assert_eq!(
        response.marketplaces[0]
            .plugins
            .iter()
    ...
    let installed_path = ody_home
        .path()
        .join("plugins/cache/odysseythink-curated-remote/linear/1.2.3/.ody-plugin/plugin.json");
...
async fn plugin_list_marks_remote_plugin_disabled_by_admin() -> Result<()> {
  ...
  "pagination": {
    ...
    assert_eq!(
        plugin.availability,
        ody_app_server_protocol::PluginAvailability::DisabledByAdmin
    );
    Ok(())
}
...
async fn plugin_list_fetches_featured_plugin_ids_without_chatgpt_auth() -> Result<()> {
    ...
    write_odysseythink_curated_marketplace(ody_home.path(), &["linear", "gmail"])?;

    Mock::given(method("GET"))
        .and(path("/backend-api/plugins/featured"))
        .and(query_param("platform", "ody"))
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"["linear@odysseythink-curated"]"#))
        .mount(&server)
        .await;
    ...
    assert_eq!(
        response.featured_plugin_ids,
        vec!["linear@odysseythink-curated".to_string()]
    );
    Ok(())
}
...
async fn plugin_list_uses_warmed_featured_plugin_ids_cache_on_first_request() -> Result<()> {
    ...
    let mut mcp = TestAppServer::new_with_plugin_startup_tasks(ody_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn wait_for_cached_remote_catalog_plugin_ids(
    ...
    timeout(DEFAULT_TIMEOUT, async {
        loop {
            let plugin_ids = cached_remote_catalog_plugin_ids(ody_home)?;
...
fn cached_remote_catalog_plugin_ids(ody_home: &std::path::Path) -> Result<Vec<String>> {
    let cache_dir = ody_home.join("cache/remote_plugin_catalog");
    if !cache_dir.exists() {
        return Ok(Vec::new());
    }
...
async fn mount_odysseythink_curated_remote_collection_plugin_list(server: &MockServer, body: &str) {
    Mock::given(method("GET"))
        .and(path("/backend-api/ps/plugins/list"))
        .and(query_param("scope", "GLOBAL"))
        .and(query_param("limit", "200"))
...
fn write_plugin_sync_config(ody_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        ody_home.join("config.toml"),
        format!(
            r#"
...
[plugins."gmail@odysseythink-curated"]
enabled = false
...
fn write_plugin_share_local_path_mapping(
    ...
    std::fs::create_dir_all(ody_home.join(".tmp"))?;
    std::fs::write(
        ody_home.join(".tmp/plugin-share-local-paths-v1.json"),
        format!("{contents}\n"),
    )
}

...
async fn turn_start_shell_zsh_fork_executes_command_v2() -> Result<()> {
    ...
    let mut mcp = create_zsh_test_mcp_process(&ody_home, &workspace, &zsh_path).await?;
    ...
    assert!(command.starts_with(&command_packaged_zsh_path(&ody_home).display().to_string()));
...
async fn turn_start_shell_zsh_fork_subcommand_decline_marks_parent_declined_v2() -> Result<()> {
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            cwd: Some(workspace.clone()),
            approval_policy: Some(ody_app_server_protocol::AskForApproval::UnlessTrusted),
    ...
    while target_decision_index < target_decisions.len() || !saw_parent_approval {
        ...
        let is_parent_approval = approval_command
            .contains(&command_packaged_zsh_path(&ody_home).display().to_string())
...
async fn create_zsh_test_mcp_process(
    ...
    let app_server = create_test_package_app_server(ody_home, zsh_path)?;
...
fn create_test_package_app_server(ody_home: &Path, zsh_path: &Path) -> Result<PathBuf> {
    let package_dir = ody_home.join("test-package");
    let bin_dir = package_dir.join("bin");
    let package_zsh_path = packaged_zsh_path(ody_home);
    ...
    std::fs::write(package_dir.join("ody-package.json"), "{}")?;

    let app_server = bin_dir.join("ody-app-server");
    copy_with_permissions(
        &ody_utils_cargo_bin::cargo_bin("ody-app-server")?,
        &app_server,
    )?;
...
fn packaged_zsh_path(ody_home: &Path) -> PathBuf {
    ody_home
        .join("test-package")
        .join("ody-resources")
        .join("zsh")
...
fn command_packaged_zsh_path(ody_home: &Path) -> PathBuf {
    let path = packaged_zsh_path(ody_home);

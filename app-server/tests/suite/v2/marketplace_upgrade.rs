...
fn init_marketplace_repo(root: &Path, marketplace_name: &str, marker: &str) -> Result<String> {
    run_git(root, &["init"])?;
    run_git(root, &["config", "user.email", "ody@example.com"])?;
    run_git(root, &["config", "user.name", "Ody Tests"])?;
...
fn disable_plugin_startup_tasks(ody_home: &Path) -> Result<()> {
...
fn marketplace_install_root(ody_home: &Path) -> std::path::PathBuf {
    ody_home.join(INSTALLED_MARKETPLACES_DIR)
}

fn expected_installed_root(ody_home: &Path, marketplace_name: &str) -> Result<AbsolutePathBuf> {
    AbsolutePathBuf::try_from(
        marketplace_install_root(&ody_home.canonicalize()?).join(marketplace_name),
    )
...
async fn marketplace_upgrade_all_configured_git_marketplaces() -> Result<()> {
    ...
    disable_plugin_startup_tasks(ody_home.path())?;

    let mut mcp = TestAppServer::new(ody_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;

    let debug_root = expected_installed_root(ody_home.path(), "debug")?;
    let tools_root = expected_installed_root(ody_home.path(), "tools")?;

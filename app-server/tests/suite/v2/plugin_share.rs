...
async fn plugin_share_checkout_adds_personal_marketplace_entry() -> Result<()> {
    ...
    assert_eq!(
        ...
        PluginShareCheckoutResponse {
            remote_plugin_id: "plugins_123".to_string(),
            plugin_id: "demo-plugin@ody-curated".to_string(),
    ...
    let request_id = mcp
        .send_plugin_list_request(PluginListParams {
            ...
            marketplace_kinds: Some(vec![
                ody_app_server_protocol::PluginListMarketplaceKind::Local,
            ]),
        })
        .await?;
    let response: JSONRPCResponse = timeout(
...
async fn plugin_share_update_targets_updates_share_targets() -> Result<()> {
    ...
    assert_eq!(
        ...
        PluginShareUpdateTargetsResponse {
            ...
            discoverability: ody_app_server_protocol::PluginShareDiscoverability::Unlisted,
        }
    );
    Ok(())
}
...
async fn plugin_share_delete_removes_created_workspace_plugin() -> Result<()> {
    ...
    write_plugin_share_local_path_mapping(ody_home.path(), "plugins_123", &local_plugin_path)?;
...
fn write_remote_plugin_config(ody_home: &Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
...
fn write_test_plugin(root: &Path, plugin_name: &str) -> std::io::Result<PathBuf> {
    ...
    write_file(
        &plugin_path.join(".ody-plugin/plugin.json"),
...
fn write_corrupt_plugin_share_local_path_mapping(ody_home: &Path) -> std::io::Result<()> {
    write_file(

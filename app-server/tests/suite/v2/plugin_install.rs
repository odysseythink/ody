...
async fn plugin_install_writes_remote_plugin_to_cloud_and_cache() -> Result<()> {
    ...
    .await;
    configure_remote_plugin_test(ody_home.path(), &server)?;
    ...
    mount_remote_plugin_install_after_cache_write(
        &server,
        REMOTE_PLUGIN_ID,
        installed_path.join(".ody-plugin/plugin.json"),
    )
    .await;
    ...
    assert!(
        !ody_home
            .path()
            .join(format!(
                "plugins/cache/odysseythink-curated-remote/{REMOTE_PLUGIN_ID}/1.2.3"
            ))
            .exists()
    );
    Ok(())
...
async fn plugin_install_uses_remote_apps_needing_auth_response() -> Result<()> {
    ...
    .await;
    configure_remote_plugin_with_apps_test(ody_home.path(), &server)?;
    mount_remote_plugin_detail_with_app_manifest(
        &server,
        REMOTE_PLUGIN_ID,
        "1.2.3",
...
async fn plugin_install_rejects_invalid_remote_plugin_name() -> Result<()> {
    let ody_home = TempDir::new()?;
    write_remote_plugin_catalog_config(ody_home.path(), "https://example.invalid/backend-api/")?;
    let mut mcp = TestAppServer::new(ody_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
#[tokio::test]
async fn plugin_install_rejects_when_workspace_ody_plugins_disabled() -> Result<()> {
    let ody_home = TempDir::new()?;
    ...
    assert!(
        err.error
            .message
            .contains("Ody plugins are disabled for this workspace")
    );
    Ok(())
}
...
async fn plugin_install_returns_invalid_request_for_missing_marketplace_file() -> Result<()> {
    ...
    let request_id = mcp
        .send_plugin_install_request(PluginInstallParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                ody_home.path().join("missing-marketplace.json"),
            )?),
            remote_marketplace_name: None,
...
async fn plugin_install_returns_invalid_request_for_disallowed_product_plugin() -> Result<()> {
  ...
  "plugins": [
    ...
    let mut mcp =
        TestAppServer::new_with_args(ody_home.path(), &["--session-source", "atlas"]).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn plugin_install_preserves_status_when_remote_bundle_error_body_is_too_large() -> Result<()>
{
    ...
    assert_eq!(event_params["plugin_id"], REMOTE_PLUGIN_ID);
    assert_eq!(event_params["marketplace_name"], "odysseythink-curated-remote");
...
async fn start_apps_server(
    ...
    let router = Router::new()
        ...
        .with_state(state)
        .nest_service("/api/ody/ps/mcp", mcp_service);
...
fn write_connectors_config(ody_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
...
async fn mount_backend_analytics_events(server: &MockServer) {
    Mock::given(method("POST"))
        .and(path("/backend-api/ody/analytics-events/events"))

...
fn command_hook_hash(
    ...
    let Ok(value) = ody_config::TomlValue::try_from(identity) else {
        unreachable!("normalized hook identity should serialize to TOML");
    };
    ody_config::version_for_toml(&value)
}

fn write_user_hook_config(ody_home: &std::path::Path) -> Result<()> {
    std::fs::write(
...
fn write_plugin_hook_config(ody_home: &std::path::Path, hooks_json: &str) -> Result<()> {
    let plugin_root = ody_home.join("plugins/cache/test/demo/local");
...
fn write_project_hook_config(dot_ody_folder: &std::path::Path, command: &str) -> Result<()> {
    std::fs::create_dir_all(dot_ody_folder)?;
    std::fs::write(
        dot_ody_folder.join("config.toml"),
        format!(
            r#"[features]
hooks = true
...
async fn hooks_list_shows_discovered_hook() -> Result<()> {
    ...
    let cwd = TempDir::new()?;
    write_user_hook_config(ody_home.path())?;
...
async fn hooks_list_shows_plugin_hook_load_warnings() -> Result<()> {
    let ody_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    write_plugin_hook_config(ody_home.path(), "{ not-json")?;
...
"#,
    ...
    set_project_trust_level(ody_home.path(), workspace.path(), TrustLevel::Trusted)?;
...
async fn hooks_list_uses_root_repo_hooks_for_linked_worktrees() -> Result<()> {
    ...
    write_project_hook_config(&repo_root.join(".ody"), "echo root hook")?;
    write_project_hook_config(&worktree_root.join(".ody"), "echo worktree hook")?;
    set_project_trust_level(ody_home.path(), &repo_root, TrustLevel::Trusted)?;
...
async fn config_batch_write_updates_hook_trust_for_loaded_session() -> Result<()> {
    ...
    let ody_home = TempDir::new()?;
    let hook_script_path = ody_home.path().join("user_prompt_submit_hook.py");
    let hook_log_path = ody_home.path().join("user_prompt_submit_hook_log.jsonl");
    std::fs::write(
        &hook_script_path,
        format!(
            r#"import json

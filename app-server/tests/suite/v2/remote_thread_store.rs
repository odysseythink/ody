...
//! the temporary `ody_home` must not contain rollout session files or sqlite
...
use ody_app_server::in_process::InProcessClientHandle;
use ody_app_server::in_process::InProcessServerEvent;
...
use ody_thread_store::CreateThreadParams as StoreCreateThreadParams;
...
async fn thread_delete_with_non_local_thread_store_does_not_create_local_persistence() -> Result<()>
{
    ...
    // Plugin startup warmups may create `.tmp` under ody_home. Disable them
    // here so this regression stays focused on thread persistence artifacts.
    create_config_toml_with_thread_store(ody_home.path(), &server.uri(), &store_id)?;

    let thread_store = InMemoryThreadStore::for_id(store_id.clone());
    let _in_memory_store = InMemoryThreadStoreId { store_id };

    let mut client = start_in_process_server(ody_home.path()).await?;
    ...
    assert_no_local_persistence_artifacts(ody_home.path())?;
...
async fn start_in_process_server(ody_home: &Path) -> Result<InProcessClientHandle> {
    let loader_overrides = LoaderOverrides::without_managed_config_for_tests();
    let config = Arc::new(
        ConfigBuilder::default()
            .ody_home(ody_home.to_path_buf())
            .fallback_cwd(Some(ody_home.to_path_buf()))
            .loader_overrides(loader_overrides.clone())
            .build()
            .await?,
    );
...
fn assert_no_local_persistence_artifacts(ody_home: &Path) -> Result<()> {
    ...
    let mut entries = ody_home_entries(ody_home)?;
    // Bazel test runs may initialize shell snapshot storage under ody_home.
    ...
    assert_eq!(
        ...
        "non-local thread persistence should not create unexpected files in ody_home"
    );

    Ok(())
}

fn ody_home_entries(ody_home: &Path) -> Result<BTreeSet<String>> {
    Ok(std::fs::read_dir(ody_home)?
        .filter_map(|entry| {
            let entry = entry.ok()?;

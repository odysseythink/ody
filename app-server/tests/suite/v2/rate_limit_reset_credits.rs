...
const RATE_LIMIT_RESET_REQUEST_TIMEOUT_ENV_VAR: &str =
    "ODY_TEST_RATE_LIMIT_RESET_REQUEST_TIMEOUT_MS";
...
async fn consume_rate_limit_reset_credit_requires_legacy_auth() -> Result<()> {
    ...
    assert_eq!(
        ...
        "ody account authentication required for rate limit reset credits"
    );
...
async fn initialized_app_server(ody_home: &Path) -> Result<TestAppServer> {

...
#[tokio::test]
async fn odysseythink_model_header_mismatch_emits_model_rerouted_notification_v2() -> Result<()> {
    ...
    let response = responses::sse_response(body).insert_header("odysseythink-model", SERVER_MODEL);
...
async fn cyber_policy_response_emits_typed_error_notification_v2() -> Result<()> {
    ...
    assert_eq!(
        error,
        ErrorNotification {
            error: ody_app_server_protocol::TurnError {
    ...
    loop {
        ...
        match notification.method.as_str() {
            "error" => {
                ...
                if payload.error.ody_error_info == Some(OdyErrorInfo::CyberPolicy) {
                    error = Some(payload);
                }
            }
            "model/rerouted" => {

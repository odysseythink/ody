...
use ody_app_server_protocol::ByteRange;
...
use ody_app_server_protocol::FileChangePatchUpdatedNotification;
...
use ody_app_server_protocol::TextElement;
...
use ody_core::personality_migration::PERSONALITY_MIGRATION_FILENAME;
...
const TEST_ORIGINATOR: &str = "ody_vscode";
...
async fn run_local_image_turn(detail: Option<ImageDetail>) -> Result<Vec<Value>> {
    // Two Ody turns hit the mock model (session start + turn/start).
    let responses = vec![
    ...
    let image_path = ody_home.path().join("image.png");
...
async fn turn_start_sends_originator_header() -> Result<()> {
    ...
    timeout(
        ...
        mcp.initialize_with_client_info(ClientInfo {
            ...
            title: Some("Ody VS Code Extension".to_string()),
...
async fn turn_start_emits_thread_scoped_warning_notification_for_trimmed_skills() -> Result<()> {
    ...
    let cache_path = ody_home.path().join("models_cache.json");
    ...
    write_test_skill(ody_home.path(), "alpha-skill")?;
    write_test_skill(ody_home.path(), "beta-skill")?;

    let isolated_home = ody_home.path().to_string_lossy();
...
async fn turn_start_tracks_turn_event_analytics() -> Result<()> {
    ...
    let event = wait_for_analytics_event(&server, DEFAULT_READ_TIMEOUT, "ody_turn_event").await?;
    ...
    let turn_req = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            environments: Some(vec![TurnEnvironmentParams {
                ...
                cwd: ody_utils_absolute_path::AbsolutePathBuf::try_from(
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    // Three Ody turns hit the mock model (session start + two turn/start calls).
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ...
    assert_eq!(
        started.turn.status,
        ody_app_server_protocol::TurnStatus::InProgress
    );
    assert_eq!(started.turn.id, turn.id);
...
async fn turn_start_accepts_personality_override_v2() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(ThreadStartParams {
            model: Some("exp-ody-personality".to_string()),
            ..Default::default()
        })
        .await?;
    let thread_resp: JSONRPCResponse = timeout(
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let mut mcp = TestAppServer::new(ody_home.as_path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn turn_start_updates_sandbox_and_cwd_between_turns_v2() -> Result<()> {
    ...
    let first_turn = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            sandbox_policy: Some(ody_app_server_protocol::SandboxPolicy::WorkspaceWrite {
...
async fn turn_start_permission_profile_rebinds_runtime_workspace_roots_between_turns() -> Result<()>
{
    ...
    let old_root = ody_utils_absolute_path::AbsolutePathBuf::from_absolute_path(old_root)?;
    let new_root = ody_utils_absolute_path::AbsolutePathBuf::from_absolute_path(new_root)?;
...
async fn turn_start_resolves_sticky_thread_local_environment_and_turn_overrides() -> Result<()> {
    ...
    let server = create_mock_responses_server_repeating_assistant("done").await;
    create_config_toml(&ody_home, &server.uri(), "never", &BTreeMap::default())?;
    std::fs::write(
        ody_home.join("environments.toml"),
        r#"
[[environments]]
id = "remote"
url = "ws://127.0.0.1:1"
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![ody_app_server_protocol::FileUpdateChange {
...
async fn turn_start_streams_apply_patch_change_updates_v2() -> Result<()> {
    ...
    write_models_cache(&ody_home)?;
    let cache_path = ody_home.join("models_cache.json");
    let mut cache: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&cache_path)?)?;
    let models = cache["models"]
        .as_array_mut()
...
async fn turn_start_emits_spawn_agent_item_with_effective_role_model_metadata_v2() -> Result<()> {
    ...
    std::fs::write(
        ody_home.path().join("custom-role.toml"),
...
async fn turn_start_with_elevated_override_does_not_persist_project_trust() -> Result<()> {
    ...
    let config_toml = std::fs::read_to_string(ody_home.path().join("config.toml"))?;
...
fn write_test_skill(ody_home: &Path, name: &str) -> std::io::Result<()> {
    let skill_dir = ody_home.join("skills").join(name);

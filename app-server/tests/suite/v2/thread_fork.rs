...
async fn thread_fork_inherits_explicit_source_name_from_session_index() -> Result<()> {
    ...
    append_thread_name(ody_home.path(), source_thread_id, source_name).await?;
...
#[tokio::test]
async fn pathless_ephemeral_thread_rejects_ody_home_path_after_reload() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let ody_home = TempDir::new()?;
    create_config_toml(ody_home.path(), &server.uri())?;

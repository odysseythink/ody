...
use ody_app_server_protocol::AppMetadata;
...
"#,
  ...
  "pagination": {
    ...
    assert_eq!(response.plugin.marketplace_name, "odysseythink-curated-remote");
    assert_eq!(
        response.plugin.summary.id,
        "example-plugin@odysseythink-curated-remote"
    );
    assert_eq!(
...
async fn plugin_read_includes_share_url_for_admin_disabled_remote_plugin() -> Result<()> {
  ...
  "name": "example-plugin",
  "scope": "GLOBAL",
  "share_url": "https://example.com/plugins/share/example-plugin",
  ...
  "pagination": {
    ...
    assert_eq!(
        ...
        Some("https://example.com/plugins/share/example-plugin")
    );
    assert_eq!(
...
async fn plugin_read_returns_share_context_for_shared_local_plugin() -> Result<()> {
    ...
    write_plugin_share_local_path_mapping(ody_home.path(), "plugins_123", &plugin_path)?;
    Mock::given(method("GET"))
...
async fn plugin_read_fails_on_malformed_share_mapping() -> Result<()> {
    ...
    write_plugin_source(repo_root.path(), "demo-plugin", &[])?;
    std::fs::create_dir_all(ody_home.path().join(".tmp"))?;
    std::fs::write(
        ody_home
            .path()
...
"#,
    ...
    std::fs::write(
        ...
        r#"---
name: example-only
description: Visible only for Legacy
---

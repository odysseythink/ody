...
fn write_cached_remote_plugin_with_skill(
    ...
    let plugin_root = ody_home.join("plugins/cache/odysseythink-curated-remote/linear/local");
    std::fs::create_dir_all(plugin_root.join(".ody-plugin"))?;
    std::fs::write(
        plugin_root.join(".ody-plugin/plugin.json"),
        r#"{"name":"linear"}"#,
...
async fn skills_list_loads_remote_installed_plugin_skills_from_cache() -> Result<()> {
    ...
    let expected_skill_path =
        std::fs::canonicalize(write_cached_remote_plugin_with_skill(ody_home.path())?)?;
...
#[tokio::test]
async fn skills_list_excludes_plugin_skills_when_workspace_ody_plugins_disabled() -> Result<()> {
    let ody_home = TempDir::new()?;
    ...
    assert!(
        ...
        "plugin skills should be hidden when workspace Ody plugins are disabled"
    );
    Ok(())
}

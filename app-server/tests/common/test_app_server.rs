...
use ody_app_server_protocol::FsCreateDirectoryParams;
use ody_app_server_protocol::FsGetMetadataParams;
use ody_app_server_protocol::FsReadDirectoryParams;
use ody_app_server_protocol::FsReadFileParams;
use ody_app_server_protocol::FsRemoveParams;
...
use ody_app_server_protocol::ModelProviderCapabilitiesReadParams;
...
use ody_app_server_protocol::ProcessKillParams;
use ody_app_server_protocol::ProcessResizePtyParams;
use ody_app_server_protocol::ProcessSpawnParams;
use ody_app_server_protocol::ProcessWriteStdinParams;
...
use ody_app_server_protocol::ThreadRealtimeListVoicesParams;
...
pub const DEFAULT_CLIENT_NAME: &str = "ody-app-server-tests";
pub const DISABLE_PLUGIN_STARTUP_TASKS_ARG: &str = "--disable-plugin-startup-tasks-for-tests";
const DISABLE_MANAGED_CONFIG_ENV_VAR: &str = "ODY_APP_SERVER_DISABLE_MANAGED_CONFIG";
...
impl TestAppServer {
    ...
    pub async fn new_without_managed_config(ody_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(ody_home, &[(DISABLE_MANAGED_CONFIG_ENV_VAR, Some("1"))]).await
    }
    ...
    pub async fn new_without_managed_config_with_env(
        ...
        Self::new_with_env(ody_home, &all_env_overrides).await
    }

    pub async fn new_with_plugin_startup_tasks(ody_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(ody_home, &[], &[]).await
    }
    ...
    pub async fn new_with_env_and_plugin_startup_tasks(
        ...
        Self::new_with_env_and_args(ody_home, env_overrides, &[]).await
    }

    pub async fn new_with_args(ody_home: &Path, args: &[&str]) -> anyhow::Result<Self> {
        let mut all_args = vec![DISABLE_PLUGIN_STARTUP_TASKS_ARG];
        all_args.extend_from_slice(args);
        Self::new_with_env_and_args(ody_home, &[], &all_args).await
    }
    ...
    async fn new_with_env_and_args(
        ...
        let program = ody_utils_cargo_bin::cargo_bin("ody-app-server")
            .context("should find binary for ody-app-server")?;
...
"#,
    ...
    async fn new_with_program_env_and_args(
        ...
        cmd.env(
            ...
            ody_home.join("managed_config.toml"),
        );
        cmd.env_remove(ODY_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        cmd.args(args);
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("ody-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: ody_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
    ...
    /// Send a `workspace/service/start` JSON-RPC request (v2, experimental).
    pub async fn send_workspace_service_start_request(
        &mut self,
        params: WorkspaceServiceStartParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
        self.send_request("workspace/service/start", params).await
    }

    /// Send a `workspace/service/stop` JSON-RPC request (v2, experimental).
    pub async fn send_workspace_service_stop_request(
        &mut self,
        params: WorkspaceServiceStopParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
        self.send_request("workspace/service/stop", params).await
    }

    /// Send a `workspace/service/list` JSON-RPC request (v2, experimental).
    pub async fn send_workspace_service_list_request(
        &mut self,
        params: WorkspaceServiceListParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
        self.send_request("workspace/service/list", params).await
    }

    /// Send a `workspace/service/logs` JSON-RPC request (v2, experimental).
    pub async fn send_workspace_service_logs_request(
        &mut self,
        params: WorkspaceServiceLogsParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
        self.send_request("workspace/service/logs", params).await
    }

    /// Send a `workspace/preview/check` JSON-RPC request (v2, experimental).
    pub async fn send_workspace_preview_check_request(
        &mut self,
    ...
    pub async fn send_workspace_audit_list_request(
        &mut self,
        params: WorkspaceAuditListParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
        self.send_request("workspace/audit/list", params).await
    }
...
impl Drop for TestAppServer {
    fn drop(&mut self) {
        // These tests spawn a `ody-app-server` child process.
        //

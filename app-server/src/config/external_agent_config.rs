...
use ody_core_plugins::marketplace_add::add_marketplace;
use ody_core_plugins::marketplace_add::is_local_marketplace_source;
use ody_external_agent_migration::build_mcp_config_from_external;
use ody_external_agent_migration::count_missing_commands;
use ody_external_agent_migration::count_missing_subagents;
use ody_external_agent_migration::hook_migration_event_names;
use ody_external_agent_migration::import_commands;
use ody_external_agent_migration::import_hooks;
use ody_external_agent_migration::import_subagents;
use ody_external_agent_migration::missing_command_names;
use ody_external_agent_migration::missing_subagent_names;
use ody_external_agent_sessions::ExternalAgentSessionMigration;
use ody_external_agent_sessions::detect_recent_sessions;
...
const EXTERNAL_AGENT_CONFIG_DETECT_METRIC: &str = "ody.external_agent_config.detect";
const EXTERNAL_AGENT_CONFIG_IMPORT_METRIC: &str = "ody.external_agent_config.import";
...
impl ExternalAgentConfigService {
    ...
    #[cfg(test)]
    fn new_for_test(ody_home: PathBuf, external_agent_home: PathBuf) -> Self {
        Self {
            ody_home,
            external_agent_home,
        }
    ...
    async fn detect_migrations(
        ...
        let target_config = repo_root.map_or_else(
            || self.ody_home.join("config.toml"),
            |repo_root| repo_root.join(".ody").join("config.toml"),
        );
        ...
        let target_hooks = repo_root.map_or_else(
            || self.ody_home.join("hooks.json"),
            |repo_root| repo_root.join(".ody").join("hooks.json"),
        );
        ...
        let target_subagents = repo_root.map_or_else(
            || self.ody_home.join("agents"),
            |repo_root| repo_root.join(".ody").join("agents"),
        );
        ...
        if let Some(settings) = settings.as_ref() {
            match ConfigBuilder::default()
                .ody_home(self.ody_home.clone())
                .fallback_cwd(Some(self.ody_home.clone()))
                .build()
                .await
            {
                Ok(config) => {
                    ...
                    let configured_marketplace_plugins = configured_marketplace_plugins(
                        &config,
                        &PluginsManager::new(self.ody_home.clone()),
                    )?;
        ...
        if repo_root.is_none() {
            let sessions = detect_recent_sessions(&self.external_agent_home, &self.ody_home)?;
    ...
    pub(crate) async fn import_plugins(
        ...
        let plugins_manager = PluginsManager::new(self.ody_home.clone());
        for plugin_group in plugins {
            ...
            let add_marketplace_outcome = add_marketplace(self.ody_home.clone(), request).await;
    ...
    fn import_subagents(&self, cwd: Option<&Path>) -> io::Result<Vec<String>> {
        let (source_agents, target_agents) = if let Some(repo_root) = find_repo_root(cwd)? {
            (
                ...
                repo_root.join(".ody").join("agents"),
            )
        } else if cwd.is_some_and(|cwd| !cwd.as_os_str().is_empty()) {
            return Ok(Vec::new());
        } else {
        ...
        let (source_external_agent_dir, target_hooks) =
            if let Some(repo_root) = find_repo_root(cwd)? {
                (
                    ...
                    repo_root.join(".ody").join("hooks.json"),
                )
            } else if cwd.is_some_and(|cwd| !cwd.as_os_str().is_empty()) {
                return Ok(Vec::new());
            } else {
        ...
        let plugins = marketplace
            ...
            .filter(|plugin| {
                plugin
                    ...
                    .products
                    .as_deref()
                    .is_none_or(|products| Product::Ody.matches_product_restriction(products))
            })
...
fn rewrite_external_agent_terms(content: &str) -> String {
    ...
    for from in [
        ...
        rewritten = replace_case_insensitive_with_boundaries(&rewritten, from, "Ody");
    }
    rewritten
}
...
fn emit_migration_metric(
    ...
    let Some(metrics) = ody_otel::global() else {
        return;
    };

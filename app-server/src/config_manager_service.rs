...
use ody_app_server_protocol::Config as ApiConfig;
...
use ody_app_server_protocol::OverriddenMetadata;
...
use ody_core::config::validate_feature_requirements_for_config_toml;
use ody_core::path_utils;
use ody_core::path_utils::SymlinkWritePaths;
use ody_core::path_utils::resolve_symlink_write_paths;
use ody_core::path_utils::write_atomically;
...
impl ConfigManager {
    ...
    async fn apply_edits(
        ...
        let user_config_toml =
            deserialize_config_toml_with_base(user_config.clone(), self.ody_home()).map_err(
                |err| {
                    ConfigManagerError::write(
                        ConfigWriteErrorCode::ConfigValidationError,
                        format!("Invalid configuration: {err}"),
    ...
    /// include any in-repo .ody/ folders because there is no cwd/project root
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project { dot_ody_folder } => format!(
            "Overridden by project config: {}/{CONFIG_TOML_FILE}",
            dot_ody_folder.display(),
        ),

...
use ody_app_server_protocol::ComputerUseRequirements;
...
use ody_app_server_protocol::ConfigRequirements;
use ody_app_server_protocol::ConfigRequirementsReadResponse;
use ody_app_server_protocol::ConfigValueWriteParams;
use ody_app_server_protocol::ConfigWriteErrorCode;
use ody_app_server_protocol::ConfigWriteResponse;
use ody_app_server_protocol::ConfiguredHookHandler;
use ody_app_server_protocol::ConfiguredHookMatcherGroup;
use ody_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use ody_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use ody_app_server_protocol::JSONRPCErrorError;
use ody_app_server_protocol::ManagedHooksRequirements;
use ody_app_server_protocol::ModelProviderCapabilitiesReadResponse;
use ody_app_server_protocol::NetworkDomainPermission;
use ody_app_server_protocol::NetworkRequirements;
use ody_app_server_protocol::NetworkUnixSocketPermission;
...
use ody_config::HookHandlerConfig as CoreHookHandlerConfig;
use ody_config::ManagedHooksRequirementsToml;
use ody_config::MatcherGroup as CoreMatcherGroup;
...
impl ConfigRequestProcessor {
    ...
    ) -> Result<ody_core::config::Config, JSONRPCErrorError> {
        self.config_manager
    ...
    async fn write_value(
        ...
        let pending_changes = ody_core_plugins::toggles::collect_plugin_enabled_candidates(
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(ody_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(ody_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
                ...
                .map(|implementations| {
                    implementations
                        ...
                        .map(|implementation| match implementation {
                            ody_config::types::WindowsSandboxModeToml::Elevated => {
                                WindowsSandboxSetupMode::Elevated
                            }
                            ody_config::types::WindowsSandboxModeToml::Unelevated => {
...
fn map_computer_use_requirements_to_api(
    computer_use: ody_config::ComputerUseRequirementsToml,
...
) -> ody_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => ody_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: ody_config::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(ody_config::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(ody_config::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(ody_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: ody_config::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        ody_config::NetworkDomainPermissionToml::Allow => NetworkDomainPermission::Allow,
        ody_config::NetworkDomainPermissionToml::Deny => NetworkDomainPermission::Deny,
    }
}

fn map_network_unix_socket_permission_to_api(
    permission: ody_config::NetworkUnixSocketPermissionToml,
) -> NetworkUnixSocketPermission {
    match permission {
        ody_config::NetworkUnixSocketPermissionToml::Allow => NetworkUnixSocketPermission::Allow,
        ody_config::NetworkUnixSocketPermissionToml::Deny => NetworkUnixSocketPermission::Deny,
    }
}
...
mod tests {
    ...
    use ody_app_server_protocol::WindowsSandboxSetupMode;
    use ody_config::ComputerUseRequirementsToml;
    use ody_config::ConfigRequirementsToml;
    use ody_config::WindowsRequirementsToml;

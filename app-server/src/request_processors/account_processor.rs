...
const LOGIN_ISSUER_OVERRIDE_ENV_VAR: &str = "ODY_APP_SERVER_LOGIN_ISSUER";
...
impl AccountRequestProcessor {
    ...
    async fn login_chatgpt_v2(
        ...
        let result = self.login_chatgpt_response(ody_streamlined_login).await;
        self.outgoing.send_result(request_id, result).await;
    }
    ...
    async fn login_chatgpt_response(
        ...
        let opts = self.login_chatgpt_common(ody_streamlined_login).await?;
    ...
    async fn login_chatgpt_device_code_response(
        ...
        let opts = self
            .login_chatgpt_common(/*ody_streamlined_login*/ false)
            .await?;
    ...
    async fn get_account_rate_limits_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "ody account authentication required to read rate limits",
            ));
        };
        ...
        let response = client
            ...
            .await
            .map_err(|err| internal_error(format!("failed to fetch ody rate limits: {err}")))?;
        if response.rate_limits.is_empty() {
            return Err(internal_error(
                "failed to fetch ody rate limits: no snapshots returned",
            ));
        }
    ...
    async fn get_account_token_usage_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "ody account authentication required to read token usage",
            ));
        };
    ...
    async fn get_workspace_messages_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "ody account authentication required to read workspace messages",
            ));
        };
    ...
    async fn send_add_credits_nudge_email_inner(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "ody account authentication required to notify workspace owner",
            ));
        };
...
mod tests {
    use super::*;
    use ody_backend_client::TokenUsageProfileDailyBucket;
    use ody_backend_client::TokenUsageProfileStats;
    ...
    fn workspace_messages_feature_disabled_only_for_not_found() {
        ...
        for (status, expected) in cases {
            let err = BackendRequestError::UnexpectedStatus {
                ...
                url: "https://example.test/api/ody/workspace-messages".to_string(),
                status,

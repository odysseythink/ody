    ...
    async fn upload_feedback_response(
                    ...
                    .await
                {
                    ...
                    Err(err) => {
                        ...
                        if let Some(state_db_ctx) = state_db_ctx.as_ref() {
                            for status in [
                                ody_state::DirectionalThreadSpawnEdgeStatus::Open,
                                ody_state::DirectionalThreadSpawnEdgeStatus::Closed,
                            ] {
                                match state_db_ctx
        ...
        if include_logs {
            ...
            if let Some(sandbox_log_attachment) =
                windows_sandbox_log_attachment(&self.config.ody_home)
...
mod tests {
    ...
    fn windows_sandbox_log_attachment_uses_current_log() {
        let ody_home = tempfile::tempdir().expect("create tempdir");
        let sandbox_dir = ody_windows_sandbox::sandbox_dir(ody_home.path());
        std::fs::create_dir_all(&sandbox_dir).expect("create sandbox dir");
        let sandbox_log_path =
            ody_windows_sandbox::current_log_file_path_for_ody_home(ody_home.path());
        std::fs::write(&sandbox_log_path, "sandbox log").expect("write sandbox log");

        let attachment = windows_sandbox_log_attachment(ody_home.path())

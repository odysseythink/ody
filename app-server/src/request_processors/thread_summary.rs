...
fn extract_conversation_summary(
    ...
    let preview = head
        ...
        .find_map(|item| match ody_core::parse_turn_item(&item) {
...
pub(super) fn with_thread_spawn_agent_metadata(
    source: ody_protocol::protocol::SessionSource,
    agent_nickname: Option<String>,
    agent_role: Option<String>,
) -> ody_protocol::protocol::SessionSource {
    if agent_nickname.is_none() && agent_role.is_none() {
        return source;
    }

    match source {
        ody_protocol::protocol::SessionSource::SubAgent(
            ody_protocol::protocol::SubAgentSource::ThreadSpawn {
                parent_thread_id,
                depth,
                agent_path,
...
) -> Option<ody_app_server_protocol::ActivePermissionProfile> {
    active_permission_profile.map(Into::into)
}

pub(crate) fn thread_response_sandbox_policy(
    permission_profile: &ody_protocol::models::PermissionProfile,
    cwd: &Path,
) -> ody_app_server_protocol::SandboxPolicy {
    let sandbox_policy = ody_sandboxing::compatibility_sandbox_policy_for_permission_profile(
        permission_profile,
        cwd,
    );
    sandbox_policy.into()

...
fn collect_resume_override_mismatches(
    ...
    if let Some(requested_review_policy) = request.approvals_reviewer.as_ref() {
        let active_review_policy: ody_app_server_protocol::ApprovalsReviewer =
    ...
    if let Some(requested_sandbox) = request.sandbox.as_ref() {
        ...
        let sandbox_matches = matches!(
            ...
            (
                ...
                ody_protocol::protocol::SandboxPolicy::DangerFullAccess
            ) | (
                SandboxMode::DangerFullAccess,
...
fn validate_dynamic_tools(tools: &[DynamicToolSpec]) -> Result<(), String> {
    ...
    fn validate_dynamic_tool<'a>(
        ...
        if let Err(err) = ody_tools::parse_tool_input_schema(&tool.input_schema) {
            return Err(format!(
...
impl ThreadRequestProcessor {
    ...
    async fn load_thread(
        &self,
        thread_id: &str,
    ) -> Result<(ThreadId, Arc<OdyThread>), JSONRPCErrorError> {
    ...
    fn listener_task_context(&self) -> ListenerTaskContext {
        ListenerTaskContext {
            ...
            ody_home: self.config.ody_home.to_path_buf(),
    ...
    async fn request_trace_context(
        &self,
        request_id: &ConnectionRequestId,
    ) -> Option<ody_protocol::protocol::W3cTraceContext> {
    ...
    async fn thread_start_task(
        ...
        session_start_source: Option<ody_app_server_protocol::ThreadStartSource>,
        thread_source: Option<ody_protocol::protocol::ThreadSource>,
            ...
            && (requested_permissions_trust_project || effective_permissions_trust_project)
        {
            ...
            let cli_overrides_for_reload = if let Err(err) =
                ody_core::config::set_project_trust_level(
                    &listener_task_context.ody_home,
        ...
        let NewThread {
            ...
            .start_thread_with_options(StartThreadOptions {
                ...
                initial_history: match session_start_source
                    .unwrap_or(ody_app_server_protocol::ThreadStartSource::Startup)
                {
                    ody_app_server_protocol::ThreadStartSource::Startup => InitialHistory::New,
                    ody_app_server_protocol::ThreadStartSource::Clear => InitialHistory::Cleared,
                },
                session_source: None,
                thread_source,
                dynamic_tools,
    ...
    fn build_thread_config_overrides(
        ...
        approval_policy: Option<ody_app_server_protocol::AskForApproval>,
        approvals_reviewer: Option<ody_app_server_protocol::ApprovalsReviewer>,
    ...
    ) -> Result<(ThreadSetNameResponse, Option<ThreadNameUpdatedNotification>), JSONRPCErrorError>
    {
        ...
        let thread_id = ThreadId::from_string(&thread_id)
            .map_err(|err| invalid_request(format!("invalid thread id: {err}")))?;
        let Some(name) = ody_core::util::normalize_thread_name(&name) else {
    ...
    async fn memory_reset_response_inner(&self) -> Result<MemoryResetResponse, JSONRPCErrorError> {
        ...
        clear_memory_roots_contents(&self.config.ody_home)
            .await
            .map_err(|err| {
                internal_error(format!(
                    "failed to clear memory directories under {}: {err}",
                    self.config.ody_home.display()
                ))
            })?;
    ...
    async fn thread_resume_inner(
            ...
            .await
        {
            Ok(NewThread {
                ...
                let instruction_sources = ody_thread.legacy_instruction_sources().await;
                ...
                thread.thread_source = ody_thread
                    .config_snapshot()
                    .await
                    .thread_source
                    .map(Into::into);
                ...
                let config_snapshot = ody_thread.config_snapshot().await;
                let sandbox = thread_response_sandbox_policy(
                    &config_snapshot.permission_profile,
                    config_snapshot.cwd().as_path(),
                );
                ...
                self.thread_goal_processor
                    .emit_resume_goal_snapshot_and_continue(thread_id, ody_thread.as_ref())
                    .await;
            }
            Err(err) => {
    ...
    async fn thread_fork_inner(
        ...
        let source_thread_name = source_thread
            .name
            .as_deref()
            .and_then(ody_core::util::normalize_thread_name);
        let history_items = source_thread
            .history
            .as_ref()
            .map(|history| history.items.clone())
...
pub(super) fn core_thread_write_error(operation: &str, err: OdyErr) -> JSONRPCErrorError {
    match err {
        OdyErr::ThreadNotFound(thread_id) => {
            invalid_request(format!("thread not found: {thread_id}"))
        }
        OdyErr::InvalidRequest(message) => invalid_request(message),
        OdyErr::UnsupportedOperation(message) => method_not_found(message),
...
) -> (Thread, Option<ody_thread_store::StoredThreadHistory>) {
...
fn summary_from_state_db_metadata(
    ...
    cli_version: String,
    source: String,
    _thread_source: Option<ody_protocol::protocol::ThreadSource>,
    ...
    let source = serde_json::from_str(&source)
        ...
        .unwrap_or(ody_protocol::protocol::SessionSource::Unknown);
...
fn preview_from_rollout_items(items: &[RolloutItem]) -> String {
    items
        ...
        .find_map(|item| match item {
            RolloutItem::ResponseItem(item) => match ody_core::parse_turn_item(item) {
                Some(ody_protocol::items::TurnItem::UserMessage(user)) => Some(user.message()),
                _ => None,
            },
            _ => None,
        })
...
fn requested_permissions_trust_project(overrides: &ConfigOverrides, cwd: &Path) -> bool {
    if matches!(
        ...
        Some(
            ody_protocol::config_types::SandboxMode::WorkspaceWrite
                | ody_protocol::config_types::SandboxMode::DangerFullAccess
        )
    ) {
        return true;
    }
...
fn permission_profile_trusts_project(
    profile: &ody_protocol::models::PermissionProfile,
    cwd: &Path,
) -> bool {
    match profile {
        ody_protocol::models::PermissionProfile::Disabled
        | ody_protocol::models::PermissionProfile::External { .. } => true,
        ody_protocol::models::PermissionProfile::Managed { .. } => profile

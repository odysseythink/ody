...
) -> Vec<ody_app_server_protocol::SkillMetadata> {
    skills
        .iter()
        .map(|skill| {
            let enabled = !disabled_paths.contains(&skill.path_to_skills_md);
            ody_app_server_protocol::SkillMetadata {
                ...
                dependencies: skill.dependencies.clone().map(|dependencies| {
                    ody_app_server_protocol::SkillDependencies {
                        tools: dependencies
                            .tools
                            .into_iter()
                            .map(|tool| ody_app_server_protocol::SkillToolDependency {
                                r#type: tool.r#type,
                                value: tool.value,
...
fn hooks_to_info(hooks: &[ody_hooks::HookListEntry]) -> Vec<HookMetadata> {
    hooks
        .iter()
...
fn errors_to_info(
    errors: &[ody_core::skills::SkillError],
) -> Vec<ody_app_server_protocol::SkillErrorInfo> {
    errors
        .iter()
        .map(|err| ody_app_server_protocol::SkillErrorInfo {
...
impl CatalogRequestProcessor {
                ...
                async move {
                    let (cwd_abs, config_layer_stack) = match self.resolve_cwd_config(&cwd).await {
                        ...
                        Err(message) => {
                            ...
                            return (
                                index,
                                ody_app_server_protocol::SkillsListEntry {
                                    cwd,
                                    skills: Vec::new(),
                                    errors: vec![ody_app_server_protocol::SkillErrorInfo {
                                        path: error_path,
                                        message,
                                    }],
                                },
                    ...
                    let effective_skill_roots = if workspace_ody_plugins_enabled {
                    ...
                    let skills_input = ody_core::skills::SkillsLoadInput::new(
                        cwd_abs.clone(),
    ...
    async fn hooks_list_response(
        ...
        for cwd in cwds {
                ...
                .await
            {
                ...
                Err(err) => {
                    let error_path = cwd.clone();
                    data.push(ody_app_server_protocol::HooksListEntry {
                        cwd,
                        hooks: Vec::new(),
                        warnings: Vec::new(),
                        errors: vec![ody_app_server_protocol::HookErrorInfo {
                            path: error_path,
            ...
            let plugin_hooks = if plugins_enabled {
                ...
                ody_core_plugins::PluginHookLoadOutcome {
                ...
                ody_core_plugins::PluginHookLoadOutcome::default()
            };
            let hooks = ody_hooks::list_hooks(ody_hooks::HooksConfig {
    ...
    async fn skills_config_write_response_inner(
        ...
        let edits = vec![edit];
        ConfigEditsBuilder::new(&self.config.ody_home)
            .with_edits(edits)
            .apply()
            .await
            .map(|()| {

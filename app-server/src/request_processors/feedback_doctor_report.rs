...
//! the configured Ody executable as a subprocess, accepts only valid JSON from
//! `ody doctor --json`, derives a small set of Sentry tags, and otherwise
...
use ody_feedback::FeedbackAttachment;
...
pub(crate) struct DoctorFeedbackReport {
    /// JSON support report to upload as `ody-doctor-report.json`.
    pub(crate) attachment: FeedbackAttachment,
    /// Low-cardinality Sentry tags derived from the report status and check ids.
    pub(crate) tags: BTreeMap<String, String>,
}

/// Runs `ody doctor --json` and returns a best-effort feedback attachment.
///
/// Failure to spawn Ody, finish before the timeout, or parse JSON means the

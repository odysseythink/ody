...
use ody_app_server_protocol::TextPosition as AppTextPosition;
use ody_app_server_protocol::TextRange as AppTextRange;
use ody_config::ConfigLoadError;
use ody_config::TextRange as CoreTextRange;
use ody_core::ExecPolicyError;
...
use ody_rollout::state_db as rollout_state_db;
...
const SQLITE_RECOVERY_CONFIG_WARNING_SUMMARY: &str = "Ody rebuilt its local database.";
...
const OTEL_SERVICE_NAME: &str = "ody-app-server";
#[cfg(debug_assertions)]
const TEST_USER_CONFIG_FILE_ENV_VAR: &str = "ODY_APP_SERVER_TEST_USER_CONFIG_FILE";
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
    ...
    for layer in config.config_layer_stack.get_layers(
        ...
        let ConfigLayerSource::Project { dot_ody_folder } = &layer.name else {
            continue;
        };
        ...
        disabled_folders.push((
            dot_ody_folder.as_path().display().to_string(),
...
pub async fn run_main_with_transport_options(
    ...
    let otel = ody_core::otel_init::build_provider(
        &config,
    ...
    ody_core::otel_init::record_process_start(otel.as_ref(), OTEL_SERVICE_NAME);
    ody_core::otel_init::install_sqlite_telemetry(otel.as_ref(), OTEL_SERVICE_NAME);
    let unix_socket_startup_lock = match &transport {
        AppServerTransport::UnixSocket { socket_path } => {
            let startup_lock_path = app_server_startup_lock_path(&ody_home)?;
    ...
    if should_run_personality_migration {
        ...
        match effective_toml.try_into() {
            Ok(config_toml) => {
                match ody_core::personality_migration::maybe_migrate_personality(
                    &config.ody_home,
                    &config_toml,
                    state_db.clone(),
                )
                .await
                {
                    Ok(ody_core::personality_migration::PersonalityMigrationStatus::Applied) => {
                    ...
                    Ok(
                        ody_core::personality_migration::PersonalityMigrationStatus::SkippedMarker
                        | ody_core::personality_migration::PersonalityMigrationStatus::SkippedExplicitPersonality
                        | ody_core::personality_migration::PersonalityMigrationStatus::SkippedNoSessions,
                    ) => {}
                    Err(err) => {
    ...
    if let Some(warning) =
        ody_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        config_warnings.push(ConfigWarningNotification {
            summary: warning,
            details: None,
    ...
    let (remote_control_accept_handle, remote_control_handle) = start_remote_control(
        RemoteControlStartConfig {
            // Remote control pairing is a distinct feature from the removed remote hosted
            // plugin/Apps catalog; it previously reused this same config field purely as a
            // default base URL, so keep using the historical default here.
            remote_control_url: "https://chatgpt.com/backend-api".to_string(),
...
async fn init_sqlite_state_db_with_fresh_start_on_corruption(
    ...
    loop {
        ...
        if !ody_state::is_sqlite_corruption_error(&err)
        ...
        emit_state_db_backup_warning(&format!(
            "Ody local database at {} appears damaged. Moving it into a backup folder so the app server can rebuild it from saved data.",
            database_path.display()
        ));
        let backups = ody_state::backup_runtime_db_for_fresh_start(database_path.as_path())
            .await
            .map_err(|backup_err| {
                anyhow::anyhow!(
        ...
        for backup in &backups {
            emit_state_db_backup_warning(&format!(
                "Moved damaged Ody local database file {} to {}",
...
mod tests {
    ...
    fn debug_test_user_config_file_overrides_loader_path() {
        let path = std::env::temp_dir().join("ody-app-server-test-config.toml");
        let loader_overrides = loader_overrides_with_test_user_config_file(

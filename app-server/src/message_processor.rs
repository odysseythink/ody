...
impl ConnectionSessionState {
    ...
    pub(crate) fn supports_odysseythink_form_elicitation(&self) -> bool {
        self.initialized
            .get()
            .is_some_and(|session| session.supports_odysseythink_form_elicitation)
    }
...
pub(crate) struct MessageProcessorArgs {
    ...
    pub(crate) feedback: OdyFeedback,
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let thread_store = ody_core::thread_store_from_config(config.as_ref(), state_db.clone());
        let environment_manager_for_requests = Arc::clone(&environment_manager);
        let environment_manager_for_extensions = Arc::clone(&environment_manager);
        let restriction_product = session_source.restriction_product();
        let executor_skill_provider: Arc<dyn ody_skills_extension::SkillProvider> = Arc::new(
            ody_skills_extension::ExecutorSkillProvider::new_with_restriction_product(
    ...
    pub(crate) async fn process_request(
        ...
        Self::run_request_with_context(
            ...
            async {
                let ody_request = deserialize_client_request(&request);
                let result = match ody_request {
                    Ok(ody_request) => {
    ...
    async fn handle_client_request(
        ...
        if let ClientRequest::Initialize { request_id, params } = ody_request {
    ...
    async fn dispatch_initialized_client_request(
        ...
        if let Some(reason) = ody_request.experimental_reason()
        ...
        let serialization_scope = ody_request.serialization_scope();
        let app_server_client_name = session.app_server_client_name().map(str::to_string);
        let client_version = session.client_version().map(str::to_string);
        let supports_odysseythink_form_elicitation = session.supports_odysseythink_form_elicitation();
    ...
    async fn handle_initialized_client_request(
        ...
        let request_id = ConnectionRequestId {
            connection_id,
            request_id: ody_request.id().clone(),
        };

        let result: Result<Option<ClientResponsePayload>, JSONRPCErrorError> = match ody_request {

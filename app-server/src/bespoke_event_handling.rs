...
use ody_app_server_protocol::DeprecationNoticeNotification;
...
use ody_app_server_protocol::ExecPolicyAmendment as V2ExecPolicyAmendment;
...
use ody_app_server_protocol::GrantedPermissionProfile as V2GrantedPermissionProfile;
use ody_app_server_protocol::GuardianWarningNotification;
use ody_app_server_protocol::HookCompletedNotification;
use ody_app_server_protocol::HookStartedNotification;
...
use ody_app_server_protocol::McpServerElicitationRequestResponse;
use ody_app_server_protocol::McpServerStartupState;
use ody_app_server_protocol::McpServerStatusUpdatedNotification;
use ody_app_server_protocol::ModelReroutedNotification;
use ody_app_server_protocol::ModelSafetyBufferingUpdatedNotification;
...
use ody_app_server_protocol::ThreadRealtimeClosedNotification;
use ody_app_server_protocol::ThreadRealtimeErrorNotification;
use ody_app_server_protocol::ThreadRealtimeItemAddedNotification;
use ody_app_server_protocol::ThreadRealtimeOutputAudioDeltaNotification;
use ody_app_server_protocol::ThreadRealtimeSdpNotification;
use ody_app_server_protocol::ThreadRealtimeStartedNotification;
use ody_app_server_protocol::ThreadRealtimeTranscriptDeltaNotification;
use ody_app_server_protocol::ThreadRealtimeTranscriptDoneNotification;
use ody_app_server_protocol::ThreadRollbackResponse;
use ody_app_server_protocol::ThreadSettingsUpdatedNotification;
use ody_app_server_protocol::ThreadStatus;
use ody_app_server_protocol::ThreadTokenUsage;
use ody_app_server_protocol::ThreadTokenUsageUpdatedNotification;
use ody_app_server_protocol::ToolRequestUserInputOption;
use ody_app_server_protocol::ToolRequestUserInputParams;
use ody_app_server_protocol::ToolRequestUserInputQuestion;
use ody_app_server_protocol::ToolRequestUserInputResponse;
...
use ody_app_server_protocol::TurnPlanStep;
...
use ody_app_server_protocol::build_item_from_guardian_event;
use ody_app_server_protocol::guardian_auto_approval_review_notification;
use ody_app_server_protocol::item_event_to_server_notification;
use ody_core::OdyThread;
use ody_core::ThreadManager;
use ody_core::review_format::format_review_findings_block;
use ody_core::review_prompts;
...
use ody_protocol::request_user_input::RequestUserInputAnswer as CoreRequestUserInputAnswer;
use ody_protocol::request_user_input::RequestUserInputResponse as CoreRequestUserInputResponse;
use ody_sandboxing::policy_transforms::intersect_permission_profiles;
use ody_shell_command::parse_command::shlex_join;
    ...
    match msg {
        ...
        EventMsg::McpStartupUpdate(update) => {
            let (status, error) = match update.status {
                ody_protocol::protocol::McpStartupStatus::Starting => {
                    (McpServerStartupState::Starting, None)
                }
                ody_protocol::protocol::McpStartupStatus::Ready => {
                    (McpServerStartupState::Ready, None)
                }
                ody_protocol::protocol::McpStartupStatus::Failed { error } => {
                    (McpServerStartupState::Failed, Some(error))
                }
                ody_protocol::protocol::McpStartupStatus::Cancelled => {
        ...
        EventMsg::GuardianAssessment(assessment) => {
            ...
            if assessment.status == ody_protocol::protocol::GuardianAssessmentStatus::InProgress
            ...
            let completion_status = match assessment.status {
                ody_protocol::protocol::GuardianAssessmentStatus::Denied
                | ody_protocol::protocol::GuardianAssessmentStatus::Aborted => {
                    Some(CommandExecutionStatus::Declined)
                }
                ody_protocol::protocol::GuardianAssessmentStatus::TimedOut => {
                    Some(CommandExecutionStatus::Failed)
                }
                ody_protocol::protocol::GuardianAssessmentStatus::InProgress
                | ody_protocol::protocol::GuardianAssessmentStatus::Approved => None,
            };
                ...
                Err(err) => {
                    ...
                    if let Err(err) = conversation
                        .submit(Op::ResolveElicitation {
                            ...
                            request_id: request.id,
                            decision: ody_protocol::approvals::ElicitationAction::Cancel,
                            content: None,
                            meta: None,
                        })
                        .await
        ...
        EventMsg::Error(ev) => {
            ...
            let ody_error_info = ev.ody_error_info.clone();
            ...
            let turn_error = TurnError {
                message: ev.message,
                ody_error_info: ev.ody_error_info.map(V2OdyErrorInfo::from),
        ...
        EventMsg::ExecCommandBegin(exec_command_begin_event) => {
            if matches!(
                ...
                ody_protocol::protocol::ExecCommandSource::UnifiedExecInteraction
            ) {
        ...
        EventMsg::ThreadSettingsApplied(thread_settings_event) => {
            ...
            let thread_settings =
                thread_settings_from_core_snapshot(thread_settings_event.thread_settings, &config.model_providers);
            let changed = {
                let mut state = thread_state.lock().await;
...
async fn maybe_emit_raw_response_item_completed(
    conversation_id: ThreadId,
    turn_id: &str,
    item: ody_protocol::models::ResponseItem,
    outgoing: &ThreadScopedOutgoingMessageSender,
) {
...
pub(crate) async fn maybe_emit_hook_prompt_item_completed(
    ...
    let ody_protocol::models::ResponseItem::Message {
        role, content, id, ..
    } = item
    else {
        return;
    ...
    let notification = ItemCompletedNotification {
        ...
        item: ThreadItem::HookPrompt {
            ...
            fragments: hook_prompt
                .fragments
                .into_iter()
                .map(ody_app_server_protocol::HookPromptFragment::from)
                .collect(),
        },
    };
    outgoing
...
fn thread_rollback_response_from_stored_thread(
    stored_thread: ody_thread_store::StoredThread,
    session_id: String,
...
async fn on_mcp_server_elicitation_response(
    server_name: String,
    request_id: ody_protocol::mcp::RequestId,
    pending_request_id: RequestId,
    receiver: oneshot::Receiver<ClientRequestResult>,
    conversation: Arc<OdyThread>,
    thread_state: Arc<Mutex<ThreadState>>,
...
fn request_permissions_response_from_client_result(
    ...
    let response = serde_json::from_value::<PermissionsRequestApprovalResponse>(value)
        .unwrap_or_else(|err| {
            ...
            PermissionsRequestApprovalResponse {
                ...
                scope: ody_app_server_protocol::PermissionGrantScope::Turn,
    ...
    if strict_auto_review
        && matches!(
            response.scope,
            ody_app_server_protocol::PermissionGrantScope::Session
        )
    {
...
mod tests {
    ...
    use ody_protocol::models::FileSystemPermissions as CoreFileSystemPermissions;
    ...
    use ody_thread_store::StoredThreadHistory;
    use ody_utils_absolute_path::AbsolutePathBuf;
    ...
    fn turn_aborted_event(turn_id: &str) -> TurnAbortedEvent {
        TurnAbortedEvent {
            ...
            reason: ody_protocol::protocol::TurnAbortReason::Interrupted,
            completed_at: Some(TEST_TURN_COMPLETED_AT),
            duration_ms: Some(TEST_TURN_DURATION_MS),
        }
    }
    ...
    fn guardian_command_assessment(
        ...
        let (risk_level, user_authorization, rationale) = match status {
            ...
            GuardianAssessmentStatus::Approved => (
                Some(ody_protocol::protocol::GuardianRiskLevel::Low),
                Some(ody_protocol::protocol::GuardianUserAuthorization::High),
                Some("looks safe".to_string()),
            ),
            GuardianAssessmentStatus::Denied => (
                Some(ody_protocol::protocol::GuardianRiskLevel::High),
                Some(ody_protocol::protocol::GuardianUserAuthorization::Low),
        ...
        GuardianAssessmentEvent {
            ...
            decision_source: if matches!(status, GuardianAssessmentStatus::InProgress) {
                ...
                Some(ody_protocol::protocol::GuardianAssessmentDecisionSource::Agent)
            },
    ...
    fn guardian_assessment_started_uses_event_turn_id_fallback() {
        ...
        let action = ody_protocol::protocol::GuardianAssessmentAction::Command {
            source: ody_protocol::protocol::GuardianCommandSource::Shell,
        ...
        let notification = guardian_auto_approval_review_notification(
            ...
            &GuardianAssessmentEvent {
                ...
                status: ody_protocol::protocol::GuardianAssessmentStatus::InProgress,
                risk_level: None,
    ...
    fn guardian_assessment_completed_emits_review_payload() {
        ...
        let notification = guardian_auto_approval_review_notification(
            ...
            &GuardianAssessmentEvent {
                ...
                status: ody_protocol::protocol::GuardianAssessmentStatus::Denied,
                risk_level: Some(ody_protocol::protocol::GuardianRiskLevel::High),
                user_authorization: Some(ody_protocol::protocol::GuardianUserAuthorization::Low),
                rationale: Some("too risky".to_string()),
                decision_source: Some(
                    ody_protocol::protocol::GuardianAssessmentDecisionSource::Agent,
                ),
                action: action.clone(),
            },
        );
        ...
        match notification {
            ServerNotification::ItemGuardianApprovalReviewCompleted(payload) => {
                ...
                assert_eq!(
                    ...
                    Some(ody_app_server_protocol::GuardianRiskLevel::High)
                );
                assert_eq!(
                    payload.review.user_authorization,
                    Some(ody_app_server_protocol::GuardianUserAuthorization::Low)
                );
    ...
    fn guardian_assessment_aborted_emits_completed_review_payload() {
        let conversation_id = ThreadId::new();
        let action = ody_protocol::protocol::GuardianAssessmentAction::NetworkAccess {
            target: "api.odysseythink.com:443".to_string(),
            host: "api.odysseythink.com".to_string(),
            protocol: ody_protocol::protocol::NetworkApprovalProtocol::Https,
            port: 443,
        };
        let notification = guardian_auto_approval_review_notification(
            &conversation_id,
            ...
            &GuardianAssessmentEvent {
                ...
                started_at_ms: 1_000,
                completed_at_ms: Some(1_042),
                status: ody_protocol::protocol::GuardianAssessmentStatus::Aborted,
                risk_level: None,
                user_authorization: None,
                rationale: None,
                decision_source: Some(
    ...
    async fn test_handle_error_records_message() -> Result<()> {
        ...
        handle_error(
            ...
            TurnError {
                ...
                ody_error_info: Some(V2OdyErrorInfo::InternalServerError),
                additional_details: None,
            },
            &thread_state,
        )
    ...
    async fn turn_started_omits_active_snapshot_items() -> Result<()> {
        ...
        let thread_state = new_thread_state();
        {
            ...
            state.track_current_turn_event(
                "turn-1",
                &EventMsg::TurnStarted(ody_protocol::protocol::TurnStartedEvent {
            ...
            state.track_current_turn_event(
                "turn-1",
                &EventMsg::UserMessage(ody_protocol::protocol::UserMessageEvent {
                    client_id: None,
        ...
        apply_bespoke_event_handling(
            Event {
                id: "turn-1".to_string(),
                msg: EventMsg::TurnStarted(ody_protocol::protocol::TurnStartedEvent {
                    turn_id: "turn-1".to_string(),
                    trace_id: None,
                    started_at: Some(42),
                    model_context_window: None,
        ...
        assert_eq!(
            ...
            ItemCompletedNotification {
                item: ThreadItem::SubAgentActivity {
                    ...
                    kind: ody_app_server_protocol::SubAgentActivityKind::Interrupted,
    ...
    async fn test_handle_turn_complete_emits_failed_with_error() -> Result<()> {
        ...
        handle_error(
            ...
            TurnError {
                ...
                ody_error_info: Some(V2OdyErrorInfo::Other),
                additional_details: None,
            },
            &thread_state,
        )
    ...
    async fn test_handle_turn_complete_emits_error_multiple_turns() -> Result<()> {
        ...
        handle_error(
            ...
            TurnError {
                ...
                ody_error_info: Some(V2OdyErrorInfo::BadRequest),
                additional_details: None,
            },
            &thread_state,
        )
            ...
            OutgoingMessage::AppServerNotification(ServerNotification::ItemCompleted(
                ...
                assert_eq!(
                    ...
                    ThreadItem::HookPrompt {
                        ...
                        fragments: vec![
                            ody_app_server_protocol::HookPromptFragment {

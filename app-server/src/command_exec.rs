...
use ody_app_server_protocol::CommandExecOutputDeltaNotification;
use ody_app_server_protocol::CommandExecOutputStream;
use ody_app_server_protocol::CommandExecResizeParams;
use ody_app_server_protocol::CommandExecResizeResponse;
use ody_app_server_protocol::CommandExecResponse;
use ody_app_server_protocol::CommandExecTerminalSize;
use ody_app_server_protocol::CommandExecTerminateParams;
use ody_app_server_protocol::CommandExecTerminateResponse;
use ody_app_server_protocol::CommandExecWriteParams;
use ody_app_server_protocol::CommandExecWriteResponse;
use ody_app_server_protocol::JSONRPCErrorError;
use ody_app_server_protocol::ServerNotification;
use ody_core::config::StartedNetworkProxy;
use ody_core::exec::ExecExpiration;
use ody_core::exec::ExecExpirationOutcome;
use ody_core::exec::IO_DRAIN_TIMEOUT_MS;
use ody_core::sandboxing::ExecRequest;
use ody_protocol::exec_output::bytes_to_string_smart;
use ody_sandboxing::SandboxType;
use ody_utils_pty::DEFAULT_OUTPUT_BYTES_CAP;
use ody_utils_pty::ProcessHandle;
use ody_utils_pty::SpawnedProcess;
use ody_utils_pty::TerminalSize;
...
impl CommandExecManager {
    pub(crate) async fn start(
        ...
        let spawned = if tty {
            ody_utils_pty::spawn_pty_process(
                program,
                args,
                cwd.as_path(),
                &env,
            ...
            ody_utils_pty::spawn_pipe_process(program, args, cwd.as_path(), &env, &arg0).await
        } else {
            ody_utils_pty::spawn_pipe_process_no_stdin(program, args, cwd.as_path(), &env, &arg0)
                .await
        };
...
mod tests {
    ...
    fn windows_sandbox_exec_request() -> ExecRequest {
        ...
        ExecRequest::new(
            ...
            ody_core::exec::ExecCapturePolicy::ShellTool,
        ...
        let err = manager
            .start(StartCommandExecParams {
                ...
                request_id: ConnectionRequestId {
                    ...
                    request_id: ody_app_server_protocol::RequestId::Integer(42),
                },
    ...
    async fn windows_sandbox_non_streaming_exec_uses_execution_path() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: ody_app_server_protocol::RequestId::Integer(99),
        };
    ...
    async fn cancellation_expiration_keeps_process_alive_until_terminated() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: ody_app_server_protocol::RequestId::Integer(100),
        };
        let cwd = AbsolutePathBuf::current_dir().expect("current dir");
    ...
    async fn timeout_or_cancellation_reports_cancellation_without_timeout_exit_code() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: ody_app_server_protocol::RequestId::Integer(101),
        };
    ...
    async fn windows_sandbox_process_ids_reject_write_requests() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: ody_app_server_protocol::RequestId::Integer(1),
        };
    ...
    async fn windows_sandbox_process_ids_reject_terminate_requests() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: ody_app_server_protocol::RequestId::Integer(2),
        };
        let process_id = ConnectionProcessId {
            connection_id: request_id.connection_id,
    ...
    async fn dropped_control_request_is_reported_as_not_running() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: ody_app_server_protocol::RequestId::Integer(3),
        };

...
//!
//! # Relationship to `ody-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `ody-app-server-client`,
...
use ody_app_server_protocol::Result;
...
/// through `ody-app-server-client`, which adds worker-task buffering,
...
pub struct InProcessClientHandle {
    ...
    #[cfg(test)]
    _test_ody_home: Option<tempfile::TempDir>,
}
...
async fn start_uninitialized(args: InProcessStartArgs) -> IoResult<InProcessClientHandle> {
    ...
    let installation_id = resolve_installation_id(&args.config.ody_home).await?;
    let (client_tx, mut client_rx) = mpsc::channel::<InProcessClientMessage>(channel_capacity);
    let (event_tx, event_rx) = mpsc::channel::<InProcessServerEvent>(channel_capacity);

    let runtime_handle = tokio::spawn(async move {
        let (outgoing_tx, mut outgoing_rx) = mpsc::channel::<OutgoingEnvelope>(channel_capacity);
        let analytics_events_client = analytics_events_client_from_config(args.config.as_ref());
        ...
        let config_manager = ConfigManager::new(
            args.config.ody_home.to_path_buf(),
            args.cli_overrides,
...
mod tests {
    ...
    async fn start_test_client_with_capacity(
        ...
        let config = Arc::new(build_test_config(ody_home.path()).await);
        let state_db = ody_rollout::state_db::try_init(config.as_ref())
            .await
        ...
        client._test_ody_home = Some(ody_home);
        client
    }

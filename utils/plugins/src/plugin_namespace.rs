...
///
/// `.codex-plugin` keeps plugins from the openai/plugins curated marketplace loadable.
pub const DISCOVERABLE_PLUGIN_MANIFEST_PATHS: &[&str] =
    &[".ody-plugin/plugin.json", ".claude-plugin/plugin.json", ".codex-plugin/plugin.json"];
...
/// plugin manifest (same `name` rules as full manifest loading in ody-core).
...
mod tests {
    ...
    async fn uses_manifest_name() {
        ...
        fs::create_dir_all(plugin_root.join(".ody-plugin")).expect("mkdir manifest");
        fs::write(

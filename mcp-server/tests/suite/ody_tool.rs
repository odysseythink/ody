...
use ody_shell_command::parse_command;
...
async fn test_shell_command_approval_triggers_elicitation() {
    if env::var(ODY_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "ody" tool request, which should hit the responses endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let ody_request_id = mcp_process
        .send_ody_tool_call(OdyToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            ody_request_id.to_string(),
            params.ody_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    // Verify the original `ody` tool call completes and that the file was created.
    let ody_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(ody_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(ody_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    ...
    let ody_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        message: expected_message,
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        ody_elicitation: "exec-approval".to_string(),
        ody_mcp_tool_call_id,
        ody_event_id,
        ody_command: command,
        ody_cwd: workdir.to_path_buf(),
        ody_call_id: "call1234".to_string(),
        ody_parsed_cmd,
    })?;
    Ok(params_json)
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "ody" tool request that will trigger the apply_patch command
    let ody_request_id = mcp_process
        .send_ody_tool_call(OdyToolCallParam {
    ...
    // Verify the original `ody` tool call completes
    let ody_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(ody_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_ody_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    ody_tool_passes_base_instructions()
        .await
        .expect("ody tool should pass base instructions");
}

async fn ody_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `ody mcp` with a specific config.toml.
    let ody_home = TempDir::new()?;
    create_config_toml(ody_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(ody_home.path()).await?;
    ...
    .await??;
    assert_eq!(ody_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(ody_response.id, RequestId::Number(ody_request_id));
    assert_eq!(
        ody_response.result,
        json!({
            "content": [
            ...
            "structuredContent": {
                "threadId": ody_response
                    .result
                    .get("structuredContent")
                    .and_then(|v| v.get("threadId"))
                    .and_then(serde_json::Value::as_str)
                    .expect("ody tool response should include structuredContent.threadId"),
                "content": "Enjoy!"
            }
        })
    );
...
fn create_expected_patch_approval_elicitation_request_params(
    ...
    message_lines.push("Allow Ody to apply proposed code changes?".to_string());
    let params_json = serde_json::to_value(PatchApprovalElicitRequestParams {
        message: message_lines.join("\n"),
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        ody_elicitation: "patch-approval".to_string(),
        ody_mcp_tool_call_id,
        ody_event_id,
        ody_reason: reason,
        ody_grant_root: grant_root,
        ody_changes: changes,
        ody_call_id: "call1234".to_string(),
    })?;
...
/// Create a Ody config that uses the mock server as the model provider.

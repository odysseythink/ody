...
impl McpProcess {
    pub async fn new(ody_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(ody_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = ody_utils_cargo_bin::cargo_bin("ody-mcp-server")
            .context("should find binary for ody-mcp-server")?;
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = ody_client::default_client::originator().value;
    ...
    /// Method "ody/event" with params.msg.type == "task_complete".
        ...
        loop {
            ...
            match message {
                JsonRpcMessage::Notification(notification) => {
                    let is_match = if notification.notification.method == "ody/event" {
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `ody-mcp-server` child process.
        //

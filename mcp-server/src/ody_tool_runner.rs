//! Asynchronous worker that executes a **Ody** tool-call inside a spawned
...
use ody_core::config::Config as OdyConfig;
...
/// To adhere to MCP `tools/call` response format, include the Ody
...
/// Run a complete Ody session and stream events back to the client.
///
/// On completion (success or error) the function sends the appropriate
/// `tools/call` response so the LLM can continue the conversation.
pub async fn run_ody_tool_session(
    id: RequestId,
    ...
    // Use the original MCP request ID as the `sub_id` for the Ody submission so that
    ...
    if let Err(e) = thread.submit_with_id(submission).await {
        ...
        running_requests_id_to_ody_uuid.lock().await.remove(&id);
        return;
    }

    run_ody_tool_session_inner(
        thread_id,
        thread,
        outgoing,
        id,
...
pub async fn run_ody_tool_session_reply(
    thread_id: ThreadId,
...
async fn run_ody_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<OdyThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,

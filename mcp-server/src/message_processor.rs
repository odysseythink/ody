...
use crate::ody_tool_config::OdyToolCallParam;
use crate::ody_tool_config::OdyToolCallReplyParam;
use crate::ody_tool_config::create_tool_for_ody_tool_call_param;
use crate::ody_tool_config::create_tool_for_ody_tool_call_reply_param;
...
pub(crate) struct MessageProcessor {
    ...
    running_requests_id_to_ody_uuid: Arc<Mutex<HashMap<RequestId, ThreadId>>>,
}
...
impl MessageProcessor {
    ...
    pub(crate) async fn new(
        ...
        let thread_manager = Arc::new(ThreadManager::new(
            ...
            ody_core::thread_store_from_config(config.as_ref(), state_db.clone()),
            state_db.clone(),
            installation_id,
        ...
        Self {
            ...
            arg0_paths,
            thread_manager,
            running_requests_id_to_ody_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }
    ...
    async fn handle_initialize(
        ...
        let server_info =
            Implementation::new("ody-mcp-server", env!("CARGO_PKG_VERSION")).with_title("Ody");

        // Preserve Ody's existing non-spec `serverInfo.user_agent` field.
        ...
        if let serde_json::Value::Object(ref mut obj) = server_info_value {
            obj.insert("user_agent".to_string(), json!(get_ody_user_agent()));
        }
    ...
    async fn handle_list_tools(
        ...
        let result = rmcp::model::ListToolsResult {
            meta: None,
            tools: vec![
                create_tool_for_ody_tool_call_param(),
                create_tool_for_ody_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_tool_call_ody(
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<OdyToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg.into_config(self.arg0_paths.clone()).await {
                    ...
                    Err(e) => {
                        let result = CallToolResult::error(vec![rmcp::model::Content::text(
                            format!("Failed to load Ody configuration from overrides: {e}"),
                        )]);
                        self.outgoing.send_response(id, result).await;
                        return;
                    }
                ...
                Err(e) => {
                    let result = CallToolResult::error(vec![rmcp::model::Content::text(format!(
                        "Failed to parse configuration for Ody tool: {e}"
                    ))]);
                    self.outgoing.send_response(id, result).await;
                    return;
                }
            ...
            None => {
                let result = CallToolResult::error(vec![rmcp::model::Content::text(
                    "Missing arguments for ody tool-call; the `prompt` field is required.",
                )]);
                self.outgoing.send_response(id, result).await;
                return;
            }
        ...
        let running_requests_id_to_ody_uuid = self.running_requests_id_to_ody_uuid.clone();

        // Spawn an async task to handle the Ody session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the Ody session and stream events back to the client.
            crate::ody_tool_runner::run_ody_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
                thread_manager,
                running_requests_id_to_ody_uuid,
            )
            .await;
        });
    }

    async fn handle_tool_call_ody_session_reply(
        &self,
        ...
        // parse arguments
        let ody_tool_call_reply_param: OdyToolCallReplyParam = match arguments {
            Some(json_val) => match serde_json::from_value::<OdyToolCallReplyParam>(json_val) {
                Ok(params) => params,
                Err(e) => {
                    tracing::error!("Failed to parse Ody tool call reply parameters: {e}");
                    let result = CallToolResult::error(vec![rmcp::model::Content::text(format!(
                        "Failed to parse configuration for Ody tool: {e}"
                    ))]);
            ...
            None => {
                tracing::error!(
                    "Missing arguments for ody-reply tool-call; the `thread_id` and `prompt` fields are required."
                );
                let result = CallToolResult::error(vec![rmcp::model::Content::text(
                    "Missing arguments for ody-reply tool-call; the `thread_id` and `prompt` fields are required.",
                )]);
        ...
        let thread_id = match ody_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
        ...
        let ody = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                let result = crate::ody_tool_runner::create_call_tool_result_with_thread_id(
                    thread_id,
        ...
        let prompt = ody_tool_call_reply_param.prompt.clone();
        tokio::spawn({
            let outgoing = outgoing.clone();
            let running_requests_id_to_ody_uuid = running_requests_id_to_ody_uuid.clone();

            async move {
                crate::ody_tool_runner::run_ody_tool_session_reply(
                    thread_id,
                    ody,
                    outgoing,
                    request_id,
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        let thread_id = {
            let map_guard = self.running_requests_id_to_ody_uuid.lock().await;
        ...
        // Obtain the Ody thread from the server.
        let ody_arc = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                return;
        ...
        // Submit interrupt to Ody.
        if let Err(e) = ody_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: ody_protocol::protocol::Op::Interrupt,
                client_user_message_id: None,
                trace: None,
            })
            .await
        {
            tracing::error!("Failed to submit interrupt to Ody: {e}");
            return;
        }
        // unregister the id so we don't keep it in the map
        self.running_requests_id_to_ody_uuid
            .lock()
            .await
            .remove(&request_id);
    }

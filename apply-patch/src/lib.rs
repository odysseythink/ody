...
/// Special argv[1] flag used when the Ody executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `ody-apply-patch` (to avoid forcing
/// `ody-arg0` to depend on `ody-core`), it remains part of the "ody core"
/// process-invocation contract for the standalone `apply_patch` command
/// surface.
pub const ODY_CORE_APPLY_PATCH_ARG1: &str = "--ody-run-as-apply-patch";

...
async fn build_test_ody(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
) -> anyhow::Result<TestOdy> {
    build_test_ody_with_memories_config(server, home, startup_test_memories_config()).await
}

async fn build_test_ody_with_memories_config(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
    memories: MemoriesConfig,
) -> anyhow::Result<TestOdy> {
...
async fn init_state_db(home: &Arc<TempDir>) -> anyhow::Result<Arc<ody_state::StateRuntime>> {
    let db =
        ody_state::StateRuntime::init(home.path().to_path_buf(), "test-provider".into()).await?;
    db.mark_backfill_complete(/*last_watermark*/ None).await?;
    Ok(db)
}

async fn trigger_memories_startup(test: &TestOdy) {
    let config_snapshot = test.ody.config_snapshot().await;
...
) -> (Arc<MemoryStartupContext>, Arc<ody_core::config::Config>) {
    let config_snapshot = test.ody.config_snapshot().await;
    let mut config = test.config.clone();
    config
        .features
...
async fn seed_stage1_output(
    ...
    let mut metadata_builder = ody_state::ThreadMetadataBuilder::new(
        thread_id,
        ody_home.join(format!("rollout-{thread_id}.jsonl")),
        updated_at,
        SessionSource::Cli,
    );
    metadata_builder.cwd = ody_home.join(format!("workspace-{rollout_slug}"));
...
async fn seed_stage1_candidate(
    ...
    let thread_id = ThreadId::new();
    let rollout_path = ody_home.join(format!("rollout-{thread_id}.jsonl"));
...
) -> anyhow::Result<ody_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.ody.config_snapshot().await;
...
async fn seed_stage1_output_for_existing_thread(
    ...
    let ownership_token = match claim {
        ody_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
...
async fn shutdown_test_ody(test: &TestOdy) -> anyhow::Result<()> {

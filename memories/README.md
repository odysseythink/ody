...
- In `ody`, edit those undated template files in place.
- The dated snapshot-copy workflow is used in the separate `odysseythink/project/agent_memory/write` harness repo, not here.
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.ody-code/memories/.git` by `ody-git-utils`

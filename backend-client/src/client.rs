...
use crate::types::OdyWorkspaceMessagesResponse;
...
use ody_protocol::protocol::SpendControlLimitSnapshot;
...
impl Client {
    pub fn new(base_url: impl Into<String>) -> Result<Self> {
        ...
        Ok(Self {
            base_url,
            http,
            auth_provider: ody_model_provider::unauthenticated_auth_provider(),
            user_agent: None,
    ...
    pub fn from_auth(base_url: impl Into<String>, auth: &OdyAuth) -> Result<Self> {
        Ok(Self::new(base_url)?
            .with_user_agent(get_ody_user_agent())
            .with_auth_provider(ody_model_provider::auth_provider_from_auth(auth)))
    }
    ...
    fn headers(&self) -> HeaderMap {
        ...
        if let Some(ua) = &self.user_agent {
            ...
            h.insert(USER_AGENT, HeaderValue::from_static("ody-cli"));
        }
    ...
    pub async fn get_rate_limits(&self) -> Result<RateLimitSnapshot> {
        ...
        let preferred = snapshots
            .iter()
            .find(|snapshot| snapshot.limit_id.as_deref() == Some("ody"))
            .cloned();
    ...
    pub async fn get_accounts_check(&self) -> Result<AccountsCheckResponse> {
        let url = match self.path_style {
            PathStyle::OdyApi => format!("{}/api/ody/accounts/check", self.base_url),
    ...
    fn token_usage_profile_url(&self) -> String {
        match self.path_style {
            PathStyle::OdyApi => format!("{}/api/ody/profiles/me", self.base_url),
    ...
    pub async fn list_tasks(
        ...
        let url = match self.path_style {
            PathStyle::OdyApi => format!("{}/api/ody/tasks/list", self.base_url),
    ...
    pub async fn get_task_details_with_body(
        ...
        let url = match self.path_style {
            PathStyle::OdyApi => format!("{}/api/ody/tasks/{}", self.base_url, task_id),
    ...
    pub async fn list_sibling_turns(
        ...
        let url = match self.path_style {
            PathStyle::OdyApi => format!(
                "{}/api/ody/tasks/{}/turns/{}/sibling_turns",
    ...
    /// Fetch the selected cloud-managed config bundle from ody-backend.
    ///
    /// `GET /api/ody/config/bundle` (Ody API style) or
    ...
    pub async fn get_config_bundle(
        ...
        let url = match self.path_style {
            PathStyle::OdyApi => format!("{}/api/ody/config/bundle", self.base_url),
    ...
    pub async fn list_workspace_messages(
        &self,
    ) -> std::result::Result<OdyWorkspaceMessagesResponse, RequestError> {
        ...
        let (body, ct) = self.exec_request_detailed(req, "GET", &url).await?;
        self.decode_json::<OdyWorkspaceMessagesResponse>(&url, &ct, &body)
            .map_err(RequestError::from)
    }
    ...
    pub async fn create_task(&self, request_body: serde_json::Value) -> Result<String> {
        let url = match self.path_style {
            PathStyle::OdyApi => format!("{}/api/ody/tasks", self.base_url),
    ...
    fn send_add_credits_nudge_email_url(&self) -> String {
        match self.path_style {
            PathStyle::OdyApi => format!(
                "{}/api/ody/accounts/send_add_credits_nudge_email",
                self.base_url
            ),
            PathStyle::ChatGptApi => {
                format!(
    ...
    fn workspace_messages_url(&self) -> String {
        match self.path_style {
            PathStyle::OdyApi => format!("{}/api/ody/workspace-messages", self.base_url),
...
mod tests {
    use super::*;
    use ody_backend_openapi_models::models::AdditionalRateLimitDetails;
    use ody_backend_openapi_models::models::RateLimitReachedKind;
    use ody_backend_openapi_models::models::RateLimitReachedType as BackendRateLimitReachedType;
    ...
    fn usage_payload_maps_primary_and_additional_rate_limits() {
        let payload = RateLimitStatusPayload {
            ...
            additional_rate_limits: Some(Some(vec![AdditionalRateLimitDetails {
                limit_name: "ody_other".to_string(),
                metered_feature: "ody_other".to_string(),
                rate_limit: Some(Some(Box::new(crate::types::RateLimitStatusDetails {
                    primary_window: Some(Some(Box::new(crate::types::RateLimitWindowSnapshot {
                        used_percent: 70,
            ...
            spend_control: Some(Some(Box::new(
                ody_backend_openapi_models::models::SpendControlStatusDetails {
                    reached: false,
        ...
        assert_eq!(snapshots[0].limit_id.as_deref(), Some("ody"));
        ...
        assert_eq!(snapshots[1].limit_id.as_deref(), Some("ody_other"));
        assert_eq!(snapshots[1].limit_name.as_deref(), Some("ody_other"));
        assert_eq!(
    ...
    fn preferred_snapshot_selection_matches_get_rate_limits_behavior() {
        ...
        assert_eq!(preferred.limit_id.as_deref(), Some("ody"));
    }
    ...
    fn add_credits_nudge_email_uses_expected_paths_and_bodies() {
        let ody_client = test_client("https://example.test", PathStyle::OdyApi);
        assert_eq!(
            ody_client.send_add_credits_nudge_email_url(),
            "https://example.test/api/ody/accounts/send_add_credits_nudge_email"
        );
    ...
    fn token_usage_profile_uses_expected_paths() {
        let ody_client = test_client("https://example.test", PathStyle::OdyApi);
        assert_eq!(
            ody_client.token_usage_profile_url(),
            "https://example.test/api/ody/profiles/me"
        );
    ...
    fn workspace_messages_uses_expected_paths() {
        let ody_client = test_client("https://example.test", PathStyle::OdyApi);
        assert_eq!(
            ody_client.workspace_messages_url(),
            "https://example.test/api/ody/workspace-messages"
        );

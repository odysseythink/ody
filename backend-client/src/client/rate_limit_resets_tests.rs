...
fn rate_limit_reset_contract_uses_expected_paths_and_payloads() {
    assert_eq!(
        test_client("https://example.test", PathStyle::OdyApi).rate_limit_status_url(),
        "https://example.test/api/ody/usage"
    );
    assert_eq!(
        test_client("https://example.test", PathStyle::OdyApi)
            .consume_rate_limit_reset_credit_url(),
        "https://example.test/api/ody/rate-limit-reset-credits/consume"
    );
    assert_eq!(
